...
\tikzset{
  ...
  % Four named mark families share one construction (rule/arrow/fill/datum at
  % the same line weights and the same same-hue edge): hairline (no colour of
  % its own), focus (hhteal, "the thing to notice"), caution (hhamber, "the
  % thing to watch"), and danger (pderror, "the thing that broke"). Danger is
  % the only family wired to a figures/pd-palette.tex colour rather than an
  % hh* one -- every file that \input{figures/pd-figure-language} loads
  % figures/pd-palette first, so pderror is always already defined. Reserve it
  % for genuine breach/refusal/attack content; a plain data series with no
  % danger meaning should use ink, navy, or a neutral grey, not this family.
  pd danger rule/.style={draw=pderror,line width=1.05pt},
  pd danger arrow/.style={-{Stealth[length=2.0mm,width=1.25mm]},draw=pderror,line width=.88pt,dashed},
  ...
  % No fill: page grammar (this file's header, and textbook-craft) is content
  % kind signalled by typography and the margin, never a tinted rectangle, and
  % a direct label is no exception -- it used to carry fill=hhpaper as a
  % knockout patch so a crossing line or curve would not run through its text,
  % but that patch is the Wave-12 F2 mistake in miniature: it paints as a
  % cream box wherever the page under it is not exactly hhpaper (a pgfplots
  % axis interior, which defaults to white, is exactly that case), which is
  % the tinted-rectangle-behind-a-label the maintainers were shown and asked
  % to remove. inner xsep/ysep are kept asymmetric regardless: at 1.5pt the
  % descenders of a last line (q, y, p, g) reach the edge of the label's own
  % bounding box under Computer Modern, where they clear it under Palatino --
  % that governs the anchor footprint whether or not the box is ever painted.
  pd direct label/.style={font=\pdfiglabel,text=hhink,align=left,
    inner xsep=1.5pt,inner ysep=2.5pt,fill=none},
  ...
  pd danger datum/.style={diamond,fill=pderror,draw=hhpaper,line width=.55pt,inner sep=2.5pt},
  ...
  pd danger fill/.style={fill=pderror!26,draw=pderror!70,line width=.4pt,text=hhink},
