...
% margin column (Book) the float keeps only its bare "Figure N"/"Table N" tag
% (an ordinary \caption with an empty argument, labelsep suppressed so no
% stray colon prints before nothing) and the descriptive sentence prints in
% the margin instead, at \marginfont. Outside the margin column (standalone
% chapters) this is exactly \caption{text} -- no content is lost, so the same
% chapter source compiles both ways. Top-anchored like \pdmarginfigure, not
% foot-anchored like \pdgloss: a caption can run several lines, as long as a
% portrait's, and the doctrine here is height, not kind -- a block that tall
% belongs on the top anchor so a stale \pagetotal cannot lift it off the page.
\newsavebox{\pd@mcbox}
...
% rebuilt with cross-chapter references. Restoring labelsep sequentially,
% also ungrouped, keeps the same scoping discipline.
\newcommand{\pdmargincaption}[1]{%
  \ifpdmargincolumn
    \captionsetup{labelsep=none}%
    \caption{}%
    \captionsetup{labelsep=colon}%
    \sbox{\pd@mcbox}{\begin{minipage}[t]{\marginparwidth}\marginfont#1\end{minipage}}%
    \pd@placemargintop{\pd@mcbox}%
  \else
    \caption{#1}%
  \fi}
