...
% \pdsession asks which side of the leaf it is on (see \pd@sessionshift), so
% \checkoddpage and \ifoddpage have to exist even in the standalone chapter
% papers, where the question has the same answer on every page.
\@ifpackageloaded{changepage}{}{\usepackage{changepage}}
...
\newlength{\pd@sessionshift}
\newlength{\pd@sessionlwidth}
\lstnewenvironment{pdsession}[1][]{%
  \endgraf\addvspace{8pt plus 1pt}%
  \ifdefined\Needspace\Needspace{7\baselineskip}\fi
  % In the Book a transcript runs across the column and the margin (5.55 in,
  % about 77 monospace characters), the full-width block of the page grammar.
  ...
  %
  % The Book is twoside, so the margin the block runs into is the OUTER one,
  % which is to the right on a recto and to the left on a verso. Widening
  % rightward on both would run the block into the spine on every even leaf.
  % \pd@sessionshift is the left displacement that makes the widening go
  % outward: zero on a recto, the width of the margin column on a verso. It is
  % applied to the rules with \hspace* and to the listing body through
  % listings' own xleftmargin, because the nesting a changepage environment
  % would need is what the note above rules out. In the standalone chapter
  % papers \ifpdmargincolumn is false, the block is plain \linewidth, and the
  % shift stays zero.
  \ifpdmargincolumn
    \setlength{\pd@sessionwidth}{\dimexpr\linewidth+\marginparsep+\marginparwidth\relax}%
    \checkoddpage
    \ifoddpage
      \setlength{\pd@sessionshift}{\z@}%
    \else
      \setlength{\pd@sessionshift}{-\dimexpr\marginparsep+\marginparwidth\relax}%
    \fi
  \else
    \setlength{\pd@sessionwidth}{\linewidth}%
    \setlength{\pd@sessionshift}{\z@}%
  \fi
  \noindent\hspace*{\pd@sessionshift}\makebox[\linewidth][l]{\color{pdinkmuted}\rule{\pd@sessionwidth}{0.4pt}}\endgraf\nobreak\vspace{3pt}%
  \noindent\pd@marginhead{At the terminal}%
  \ifx\relax#1\relax\else{\itshape #1.}\fi\endgraf\nobreak
  % listings takes the width it wraps at as linewidth MINUS xleftmargin, so a
  % negative xleftmargin widens the block as well as moving it. Handing it
  % width+shift cancels that: the block wraps at \pd@sessionwidth wherever it
  % sits. Without this the verso transcripts wrapped 1.3 in wide and ran off
  % the paper.
  \setlength{\pd@sessionlwidth}{\dimexpr\pd@sessionwidth+\pd@sessionshift\relax}%
  \lstset{style=pdsession,linewidth=\pd@sessionlwidth,xleftmargin=\pd@sessionshift}%
}{%
  % A long transcript can break across a leaf, and then the closing rule is on
  % the OTHER side of the book from the opening one -- 5.55 in of rule widening
  % the wrong way, 0.35 in of it off the paper (p. 460 of the first two-sided
  % build). The shift is decided again here, for the leaf this rule is on.
  \endgraf\nobreak
  \ifpdmargincolumn
    \checkoddpage
    \ifoddpage
