...
\newcommand{\pdgloss}[2]{%
  {\bfseries #1}%
  \ifpdmargincolumn
    \marginnote{{\scshape #1}\par\nobreak\vspace{1pt plus 1pt}#2}%
  \else
    \space{\itshape (#2)}%
  \fi}

% Marginalia (Wave 12, 12.2): a duotone portrait or title page in the margin
% column at the point where the person's idea is load-bearing, with a
...
\newcommand{\pdmarginfigure}[2]{%
  \ifpdmargincolumn
    \IfFileExists{plates/marginalia/#1.jpg}{%
      \sbox{\pd@mfbox}{\begin{minipage}{\marginparwidth}
      ...
      % \marginpar is placed by the page builder and moved up when it would run
      % off the foot; it cannot be used inside a box, where marginnote stands in.
      % \marginpar aligns a box's reference point with the line; the box is
      % raised so its top sits at the line's top and its height is declared,
      % so the page builder measures it and moves it up only when it must.
      \ifinner\marginnote{\usebox{\pd@mfbox}}\else
        \marginpar{\raisebox{\dimexpr\ht\strutbox-\ht\pd@mfbox\relax}[\ht\strutbox][\dimexpr\ht\pd@mfbox+\dp\pd@mfbox-\ht\strutbox\relax]{\usebox{\pd@mfbox}}}\fi
    }{}%
  \fi}

% ---------------------------------------------------------------------------
% Margin citations and provenance sidenotes (Wave 16): \pdcite emits the
% normal \cite plus, in the Book, a margin note with a short form per key;
% \pdprov renders a run's script/seed/tag provenance in the margin instead of
% inline. The citation short-form table is GENERATED and \input immediately
% below, before any chapter body can call \pdcite.
%
%   figures/pd-cite-shortforms.tex   -- scripts/harbor-research/build_cite_shortforms.py
%
% Generated, committed, twinned the same way this file is; do not hand-edit.
% ---------------------------------------------------------------------------
...
\newcommand{\pdcite}[1]{%
  \cite{#1}%
  \ifpdmargincolumn
    \marginnote{\pd@citefirsttrue\@for\pd@citekey:=#1\do{%
      \ifpd@citefirst\pd@citefirstfalse\else; \fi
      \pd@citeshort@one{\pd@citekey}}}%
  \fi}
...
% changes outside the Book.
\newcommand{\pdprov}[3]{%
  \ifpdmargincolumn
    \marginnote{\texttt{\small #1} \textperiodcentered\ #2 \textperiodcentered\ \textsc{#3}}%
  \else
    [#3]%
  \fi}
...
\NewEnviron{pdrecitation}{%
  \ifpdmargincolumn
    ...
    % It is never raised above the text already on the page.
    \sbox{\pd@recallbox}{\begin{minipage}[b]{\marginparwidth}\raggedright
      ...
      \footnotesize\BODY\end{enumerate}\end{minipage}}%
    \setlength{\pd@recalloff}{\dimexpr\baselineskip-\ht\pd@recallbox-\dp\pd@recallbox\relax}%
    \ifdim\pd@recalloff<-\pagetotal \setlength{\pd@recalloff}{-\pagetotal}\fi
    \ifdim\pd@recalloff>0pt \setlength{\pd@recalloff}{0pt}\fi
    \marginnote{\usebox{\pd@recallbox}}[\pd@recalloff]%
  \else
...
\newcommand{\pdexercisepointer}[3]{%
  \ifpdmargincolumn
    \marginnote{\small\itshape Exercises #1--#2, p.~#3.}%
  \else
    \par\smallskip\noindent{\small\itshape Exercises #1--#2, page~#3.}\par
  \fi}
\newcommand{\pdexercisepointerone}[2]{%
  \ifpdmargincolumn
    \marginnote{\small\itshape Exercise #1, p.~#2.}%
  \else
