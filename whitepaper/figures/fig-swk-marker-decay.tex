...
\end{axis}
\end{tikzpicture}
\caption{Marker weight $w(t)=r^{t}$ for two decay rates against the prune
threshold of $0.05$ (dashed). At $r=0.9$ the marker survives $28$ ticks and is
pruned at tick $29$; at $r=0.95$ it lives until tick $59$, since
$\ln 0.05/\ln 0.95 = 58.4$. Both curves are nearly flat where they meet the
threshold, which is why moving the rate half the distance to one more than
doubles the lifetime: that is the whole content of the calibration question.
[internal; analytic]}
