...
\begin{tikzpicture}[pd figure,x=1cm,y=1cm]
  \tikzset{
    ...
    result/.style={text width=3.05cm,minimum height=1.35cm}
  }
  ...
  \node[operation] (AO) at (4.72,4.82)
    {set \texttt{formatOnSave}\\\texttt{false -> true}};
  \node[pd ready state,result] (AR) at (8.95,4.82)
  ...
  \node[operation] (BO) at (4.72,2.62)
    {replace \texttt{tsdk}\\by raw text splice};
  \node[pd breach state,result] (BR) at (8.95,2.62)
  ...
  \node[operation] (CO) at (4.72,0.42)
    {add \texttt{**/fixtures}\\to \texttt{search.exclude}};
  \node[pd warn state,result] (CR) at (8.95,0.42)
    {\textbf{delayed}\\valid JSON; tests 98\%\\settings path untested};
  \node[pd breach state,result] (CL) at (8.95,-1.48)
