...
and move the two forward-referencing entries — *sealed* (lines 76–78) and *clique* (lines 80–82) — down to §5, where they are first essential, leaving §3 with the five terms that stand on their own (inspection game, mixed strategy/equilibrium, incentive-compatible, bond/slash, amortization).
...
**Verdict:** a gap. Three of the paper's central structures are *comparisons* — keep-gain vs. confiscation, Model A vs. Model B, and the six boundary conditions with their measurement obligations — and all three are currently prose the reader has to hold in working memory. See C4 and C5.
...
`whitepaper/source/spawn-to-person.tex` (1940 lines) is **strongly related** — its §"The grading oracle's incentive-compatibility" (line 1449 ff.) poses exactly the conjecture paper 3 closes. Four items of real drift, all in the *sibling's* direction, but two of which paper 3 should absorb.
