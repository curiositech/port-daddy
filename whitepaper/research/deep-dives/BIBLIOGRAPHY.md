...
not from `whitepaper/research/tex/*.tex`. They therefore searched against
...
| M. Dastani, S. Sardina, V. Yazdanpanah. "Norm Enforcement as Supervisory Control." PRIMA 2017, LNAI 10621, 330–348. DOI 10.1007/978-3-319-69131-2_20. | `verified` — **full text obtained and read, 2026-08-27** | Found on `core.ac.uk` (download id `572201458`) after Unpaywall/Semantic Scholar/institutional-repo routes all closed empty — CORE aggregates from sources those two miss. Full text confirms the title's implication: **Theorem 1** proves a norm is "regimentable" in their sense iff it is controllable, proved by directly importing Ramadge–Wonham's Controllability Theorem (their ref [13], p. 145) — the same identification Paper 2 makes, in different vocabulary, in a different subfield. The paper does **not** claim this identification as its own contribution (§7/§8 place their novelty in the sanction- and repair-based extensions instead) and does not cite Basin et al. 2013. Net effect on flag 2: **verdict unchanged (NARROW)**, but Paper 2's regimentable/controllable identification now has *three* independent priors treating it as an unremarkable corollary of classical SCT (Ramadge–Wonham itself, Basin et al. 2013, and this paper) rather than one — cite alongside Basin 2013 in Related Work; see flag-2 `findings.md` amendment. |
...
`whitepaper/research/tex/`, case-insensitive, extended regex:
...
Three sources still block a full close after the routes below were exhausted.
**Update 2026-08-27:** the fourth, Dastani/Sardina/Yazdanpanah, is no longer one
of them — found via `core.ac.uk`, a route not tried in the original pass (it
aggregates from repositories Unpaywall/Semantic Scholar miss). Full result in
Part 1 above and flag-2's `findings.md`. For the remaining three: do not spend
more time probing; get institutional access or write to an author.
...
| Bach, *J. Symb. Comput.* 27(4):429–433, 1999 | Load-bearing in Paper 7 — cited to pre-empt a referee's category error. | **Content closed pending primary text.** DOI corrected to `10.1006/jsco.1998.0261`; bibliographic record `verified` on Crossref and DBLP, matching `paper7.tex` exactly. A Semantic Scholar TLDR is only a machine-generated discovery lead, not evidence for the paper's content; the category claim remains `probable` until the primary text is read. ScienceDirect 403s on every route despite a bronze-OA flag. |
