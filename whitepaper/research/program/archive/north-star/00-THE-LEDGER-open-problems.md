...
`docs/research/grafts/2026-06-19-soma-windags-source-audit.md` — not the framing
...
|----|------|------|-----------------|-------|
| **RCP-1** | Convergence detector — semantic task-shape similarity for "another agent is doing a similar thing," no keyword matching; **gates the parley** | research/build | 1a (windags cascade `packages/core/src/core/skill-matcher.ts`): **PORTABLE**, but plan-time only. 1b (run it on live agent *outputs*): **OPEN** — the real gap; neither source repo does runtime overlap | RQ-3; gates RCP-3 |
| **RCP-2** | Parley trigger — cost-aware (Signal-Detection) threshold for when to break formation and convene; resist MAS-overhead Goodhart | research | 2a (windags `P(fail)×waste>cost`, `evaluation-engine.ts`): **PORTABLE**. 2b (soma density threshold ρ\*): **OPEN** | **RQ-7** (same SDT spine) |
| **RCP-3** | Parley protocol — typed multi-party Contract-Net round over existing performatives → role allocation + commitments | build | 3a wave-reconvention (windags): **PORTABLE**. 3b discourse-typed bus (FIPA `act/relationship`, windags `topologies/swarm.ts`): **PORTABLE** | IMP-2; RCP-14 |
| **RCP-4** | Attestable-but-not-inspectable learned memory — the learned rung carries attestation/bonding guarantees | proof/research | 4a **CORRECTED**: windags **rejected** Thompson/Beta as a "category error" (`CLAUDE.md:86`); shipped signal is **attribution-kNN**. Narrow-interval trust is **OPEN** (needs non-bandit form). 4b (soma memory cells as attested hashes): **ABSENT** (soma wk-4 unbuilt) | **PRV-8**; IMP-3 |
...
| **RCP-10** | Pre-federation halt gate — a validity check that must pass *before* work decomposes or bonds are written | build | windags Polya halt gate (`meta-dag-predict.ts`): **PORTABLE** | RQ-1 (precondition for bonds) |
...
| **RCP-13** | Inter-agent output contracts — runtime schema validation between agents before downstream propagation | build | **CORRECTED**: windags `io-contract` is **DESIGN-ONLY** — free-text metadata, **no `ContractValidator`** | IMP-2 (envelope substrate) |
| **RCP-14** | Argumentative lineage — epistemic-ancestry spans (Toulmin claim/data/warrant); digest-with-zoom for *reasoning* provenance | build | windags `SwarmTracer` (`topologies/swarm-tracer.ts`): **PORTABLE** | RQ-3; RCP-8 / RCP-1b consume it |
