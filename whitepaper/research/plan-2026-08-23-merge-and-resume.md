...
> **Historical execution record.** The PR, branch, and queue instructions below describe the 2026-08-23 state and are not current commands. Start present-day work from `whitepaper/research/HANDOFF.md` and the corpus manifest.

## Context

The whitepaper chain has landed (#7344 renderer pin → #7278 collected volume → carrier branch), and PR **#9450** (`purser/pr-7278-tests`, head `24052b84d`) is now the omnibus publish vehicle: 72 files, +2606/−986, carrying the collected volume, the One Spine appendix, the treatise adjudications, and the entire `whitepaper/research/` corpus. The user's instruction: **merge #9450, then resume the actual research program** (proofs, simulation, verification, expansion), informed by five agent-critique PDFs they uploaded.
...
- `whitepaper/research/pdf/Sheaf-Cohomology-Lit-Review-Assessment-Prototyping-Plan.pdf` (upload `e0de2c53…`) — replaces the `literature/sheaf-assessment-notes.md` stub's "not yet rescued" status; contains the experiment matrix + three theorem candidates gating W8.
- `whitepaper/research/pdf/The-Harbor-After-the-Harbor.pdf` (upload `312d5943…`) — the rigor review (correctness audit, exercise solution key, build sequence) cited by doc1.
- Update `literature/sheaf-assessment-notes.md`, `HANDOFF.md` (§3.7 rescue status, delta v4), and `pr7698-reconciliation.md` F4 → discharged. The remaining chat-only artifact ("Theorem-Proving Stack SotA") stays flagged — HANDOFF §3.2 already extracts its decisive content.
- Resolved during corpus consolidation: `whitepaper/research/README.md` and `HANDOFF.md` now state that reference PDFs are committed and distinguish rebuildable renders from external-source evidence.
