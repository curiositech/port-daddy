...
### Added
- **Family nicknames work for every vendor, not just the one whose CLI happened to accept them.** `sonnet` resolved before this and `sol` did not — purely because the claude-code CLI takes its own family words on `--model` while OpenAI's API takes ids, so a transport quirk read as a capability only one family had. `vocabularies.modelAliases` is an INPUT vocabulary resolved on the explicit pin: `sol` / `terra` / `luna` reach `gpt-5.6-sol` / `-terra` / `-luna`, `opus` / `sonnet` / `haiku` / `fable` reach their current ids, case-insensitively, and an id the registry has never seen still passes through untouched because an explicit pin is the operator's call. The rule for adding a row is that the vendor itself uses the nickname in its own materials, so this stays a translation table for names people already read rather than a second place model names get invented; generation fails closed if a nickname names no catalog row, and a nickname may never collide with a capability rung (`opus` names a model, `high` names a job).
- **One canonical model reference for the whole app: `config/models.yaml`.** A concrete model id used to have to be independently correct in four or five places — the declarative registry, the price table, the context-window table, the backend catalog's advertised `models[]`, and the cloud plane's own hardcoded copies — and every model-drift incident in the 2026-08-22 audit was a consequence of that split. One catalog row per id now carries provider, plane, price, context window, capabilities, GA status, and how the id was verified, so "registry ⊆ priced ⊆ verified" is a structural tautology instead of a rule someone has to remember. `scripts/generate-model-registry.ts` supplants the old refresh script and emits two committed artifacts — `lib/model-registry-data.ts` for the daemon and `apps/shared/model-registry.generated.ts` for the Workers plane, which cannot import `lib/` and is exactly why `apps/` forked its own constants — with a drift test asserting both byte-match a fresh generation, and a `--probe` mode that asks each provider which models it actually serves. The probe found two live phantoms before this landed: `gemini-3.1-pro` (only the `-preview` id is served) and the Cloudflare `high`/`max-thinking` pin `@cf/moonshotai/kimi-k2-instruct`, which Cloudflare had retired — and an unknown Workers AI id does not 404, it hangs, which is how the fleet went dark on 2026-07-03. One row also carries the reasoning-effort values that model accepts, which is not decoration: the OpenAI adapter hardcoded `effort: 'minimal'`, a value only the oldest model in the lineup still takes, so four of five capability rungs returned HTTP 400 on every call while the registry looked correct — pinning the id is only half of what a request has to get right. The cloud plane additionally declares named ROLES (`shipDefault`, `shipMid`, `reviewBot`, `author`, `synthesisOfficer`, `mediator`, `embed`, …). A role is what a ship gets by DEFAULT and explicitly not a ceiling: an earlier revision of this work made "no ship may pin its way onto the most expensive model" mechanical, and that ratchet is deleted, because production data retired it — over a live 14-day window the busiest ship's entire Workers AI spend was under $0.90 while the guard was silently demoting two pins an operator had deliberately tiered up. Any admitted id is pinnable; the guard that remains is that the id must be REAL, since an unknown Workers AI id does not error, it returns a blank the parser reads as clean.
- **The daemon's richest live endpoint finally has a consumer: a multiplex lane view, and a capture harness that produces auditable proof.** `GET /agents/:id/stream` — merging status, tube, and transcript into one typed envelope feed — had NO client anywhere in the repo, so "watch several bodies work at once" was a claim nobody could check. `public/lanes.html` is that client: one column per lane, live tokens, a heartbeat chip that reports silence AS silence (a lane that stopped receiving events must not look like a lane that is thinking), the failover chain drawn from the predecessor edge so a succession reads as one piece of work, and a real/fixture source label on every lane. It is deliberately one static file served by the daemon that produces the data, so a capture cannot photograph a stale bundle. `scripts/demo-lanes.mjs` boots a real daemon on its own port and database, runs spawns inside a dedicated worktree (honouring the isolation guard rather than bypassing it), sets an explicit budget and bond rather than removing the spend rails, and writes a provenance manifest naming the daemon port, run ids, commit, and source label. Two modes with an honesty line between them: `--fixture` is hermetic (fake binaries, fake streaming server, real daemon/transcripts/SSE) and labels everything `fixture`; `--live` uses real providers and SKIPS any backend whose credentials are absent rather than substituting a fixture wearing a real label. `tests/unit/lanes-view.test.js` pins the two defects a screenshot cannot catch, both of which actually occurred while building this: the daemon writes NAMED SSE events, so a client relying on `onmessage` alone renders connected, green, and permanently empty; and every transcript envelope carries the whole message array, so appending "the last message" per update double-renders the tail on `end` and replays the entire run on reconnect.
- **Every API backend streams its answer live, so a lane shows work as it happens instead of a spinner.** Until now exactly two backends streamed — the cli-tube pair parsing claude-code `stream-json` and codex `--json` per line — while Cloudflare, Gemini, OpenAI (and its compatible family: Groq, DeepSeek, xAI, LM Studio), and the Anthropic SDK all sent `stream: false` and returned one blob at the end. A lane watching an API body showed nothing until it finished, which is the Infinite Spinner the operator-surface rules forbid: no way to tell a slow run from a hung one. `LLMCompletionRequest` gains an optional `onTextDelta`, and a request streams only when someone is listening — streaming is not free (Workers AI gives up its usage block, and OpenAI needs `stream_options` to report usage at all), so it is a caller's choice with an identical final result either way. **Both** OpenAI shapes are implemented, because the registry's entire `gpt-5` ladder routes through the Responses API and leaving that on batch would have meant OpenAI never streams in practice; its reasoning events are deliberately not streamed into the answer, the same rule that keeps Gemini's `thought` parts out of the operator's output. Streaming does not cost capability: Gemini's separately-recorded thinking turn survives, rebuilt from the stream, and the structured turns a text stream cannot carry (reasoning summaries, function calls) are appended after it. Deltas are coalesced into a small number of transcript turns before they land — flushing on size, on time (so a slow model still shows life), and on a newline (so seams fall on sentence boundaries) — and the coalescer's finish signal is what tells the spawn loop to skip the batched re-append, because recording both is how an operator reads the same answer twice. Two robustness rules are pinned by tests: a frame split across network chunks does not drop a token (the classic silent SSE bug), and a provider that ignores `stream: true` and answers with plain JSON degrades to batch rather than reporting an empty failure. `cli:agy` stays batch with the reason written down in the provider spec: Antigravity documents no streaming format, and a parser invented for an undocumented one drops turns while looking live.
- **A dispatch that dies on one backend continues on the next, resuming rather than restarting (ADR-0121).** When failover is enabled (`PD_DISPATCH_FAILOVER=true`), a failed dispatch mints a SUCCESSOR on the next backend in preference order, binding the same work and the REMAINING budget, and the predecessor settles to `salvage` rather than `failed` — because `failed` is reaped by the worker and the worktree is where the transcript the successor's brief is built from lives. `lib/dispatch/handoff-from-transcript.ts` is the first thing in the repo that ever CONSTRUCTS an ADR-0118 handoff capsule: the schema, the fail-closed sanitizer, the successor-prompt renderer and the `/continue` route were all built and wired, and every path took a capsule from a client, so cross-backend continuation was a contract with no producer. The capsule is deliberately a BRIEF and not a replay — objective, operator turns, a bounded tail, and artifact pointers — and the rendered prompt tells the successor to revalidate repository truth rather than trust it. A capsule that cannot be proven clean by the secret scanner is not sent at a lower standard; the successor runs cold with the original goal, and the continuation receipt records which it got. The policy itself (`lib/dispatch/failover.ts`) is a pure function: one same-backend retry for a genuinely transient cause, immediate failover for a missing binary (retrying an absent binary is pure latency), no second body at all for a goal the backend rejected outright, a hard cap of two successors, and a successor budget that is the original minus everything already spent. Three enabling changes ride along: the dispatch worker's `backend` option becomes a DEFAULT rather than an override — it used to shadow the per-dispatch column, which made cross-backend failover impossible by construction — the dispatch row gains a `spawned_agent_id` join key (transcripts are keyed by the Conductor's agent id, so without it a dispatch could not be joined to what its own body did), and the succession is modelled as an EDGE between two dispatches rather than an eleventh state on a deliberately closed state machine.
...
- **The raw session transcript is now a readable page, not just a download — pd-transcript.v1 Phase 2.** `GET /fleet/runs/:id/transcript/:ship` renders each ship's captured conversation as a no-script turn-card timeline in the run page's own visual system: kind chips (SYSTEM/USER/MODEL/ERROR), phase chips (`MAP 3/7`, `PLAN`, `GATE`…), per-turn model, token counts, cost, latency, UTC clock, and `#t{seq}` permalinks; system and user prompts fold in `<details>` (sysRef-deduplicated repeats render as a link to the first occurrence), model output renders escaped under the page's no-script CSP, truncation is explicitly badged, and malformed or unknown-major envelopes are skipped with an honest notice — never silently. The masthead links back to the run receipt, across attempts, and to the raw `.jsonl` download, all carrying the viewer's own capability token; authorization is the run page's exact scheme via a new shared `authorizedForRun`, so the receipt, the viewer, and the raw route can never drift on who may see a run's deliberations. The run page's per-ship transcript link now targets this viewer (the viewer owns the raw-download link).
- **Every fleet ship's raw model conversation is now captured, stored, and downloadable — pd-transcript.v1 Phase 1** (`docs/FLEET-SESSION-TRANSCRIPTS.md`). A `runCaptured` wrapper (`apps/fleet-executor/src/transcript-capture.ts`) taps every conversational `env.AI.run` call site — MAP fan-out, REDUCE, contract repair, and all five purser pipeline steps (steelman/plan/author×2/repair), phase-labeled per turn — recording versioned JSONL envelopes with monotonic `seq`, per-call model/usage/cost/latency, hash-deduplicated system prompts (`sysRef`), a credential-shape scrub on every body, and explicit truncation markers under hard per-turn (256 KB) and per-object (~10 MB) caps. Capture is fail-open by contract: a recording or flush error can never change a run, and a thrown ship still flushes its partial conversation — exactly when the forensics matter. Buffers flush once per (run, ship, attempt) to a new `fleet-transcripts` R2 bucket (provisioned; `TRANSCRIPTS` binding on both Workers) with one `fleet_run_transcripts` D1 index row (`migrations/2026-08-24-fleet-run-transcripts.sql`) — the rows double as the per-ship × per-model outcome ledger. The relay serves `GET /fleet/runs/:id/transcript/:ship.jsonl` (newest attempt; `?attempt=N`) under exactly the run page's capability scheme, and each ship's config panel on the run page now carries a "Raw session transcript (JSONL)" link — the link the run-page overhaul promised. A capture-skew invariant test walks every Workers AI call site and fails unless it is wrapped or explicitly exempted (embeddings, XO passes), so a future call site cannot silently vanish from the record.
- **RFC: fleet session transcripts** (`docs/FLEET-SESSION-TRANSCRIPTS.md`) — the design for capturing every ship's raw multi-turn model conversation at the `env.AI.run` chokepoint into a versioned `pd-transcript.v1` JSONL format (R2 for bytes, a D1 index for joins), read back through one token-gated relay API, and rendered by four surfaces: a web turn-card viewer on the run page, `pd fleet transcript` in the CLI, a pd-console transcript view, and a PWA-first path to iOS. Closes the run receipt's missing-provenance gap (the receipt shows what a ship concluded; nothing durable shows why) and turns the capture index into the per-ship × per-model outcome ledger adaptive routing needs.
- **Fleet's Workers AI call timeout is now a per-repo setting, defaulting to 5 minutes instead of a hardcoded 60 seconds.** The `/account/repos` screen gains an "AI call timeout" field (1–10 minutes) alongside the SITREP dial, stored in the same `repo_settings.settings_json` bag (`apps/shared/repo-ai-settings.ts` is the shared constant/validator/D1-resolver both `apps/relay` and `apps/fleet-executor` read, since both Workers bind the same D1 database). `FleetAiCircuit` now also tracks, per ship, aggregate Workers AI call counts/timeouts/errors/elapsed time across a whole run (`FleetAiCircuit.runForShip`), flushed once per ship into a new `fleet_ai_call_stats` D1 table rather than logged per call — `node scripts/fleet-ship-stats.mjs` renders it as a new "Workers AI call latency by ship" section, flagging any ship that is actually hitting its configured deadline. Every Workers AI failure now also carries the real measured `elapsedMs` (not just the configured deadline) in its summary text, since no fleet-facing surface previously showed how long an AI call had actually run.
- **The landing page's terminal demos are now real, gated recordings instead of scripted GIFs.** A new Porthole VT100/xterm-subset interpreter (`website-v2/src/lib/porthole/vt.ts`) replays genuine asciicast captures of the released `pd` CLI against a live daemon as real, selectable DOM text with unbounded scrollback, ANSI colors mapped onto the site's own semantic tokens, and `prefers-reduced-motion` support — closing the gap the porthole audit (`demos/porthole/AUDIT-2026-08-18.md`) flagged between the site's "these are real recordings" claim and what actually shipped. Five new captures (collision, visibility, ports, recovery, quickstart) replace the old typewriter-animated demo strings. A new CI gate (`check-porthole-casts.mjs`) decodes and replays every cast before scanning for leaked paths or error output — closing the audit's raw-text-regex blind spot, where a leak split across multiple PTY write events was invisible to a plain `grep`. Also fixes the `object-cover` cropping bug the audit flagged on four terminal-recording `<img>` tags (Agents, Harness, and Mac workflow demo pages).
- **Harbor Editor is recoverable, writable, and readable as a real code editor.** Failed file opens preserve the current pane, failed editor deep links can return to Files with Escape, physical keyboard input reaches the in-memory Loro buffer, and each editor view exposes optional grapheme-safe line wrapping, nonblocking git blame against unsaved text, and visible syntax language state with JSON highlighting.
- **The fleet run receipt page (`/fleet/runs/:id`) now shows what a ship IS, what the PR actually is, and every other pass over it — not just a raw event log.** Each ship's card carries the model(s) it ran on and its cost this run (`ship-spend`'s already-recorded fields, now surfaced instead of discarded), a config strip with its role, blocking/execution posture, and a link to its real prompt source on GitHub (`fleet-ship-config`, one new transcript step written once at run start), and the purser's full steel-manned obligations list, not just a count. The masthead fetches the PR's live title, size, and unified diff via the same GitHub App installation every other relay read uses (`getPrMeta`/`getPrDiff`), rendering the diff as one expandable panel per changed file with +/− coloring and a bound against pathological PRs, always alongside a link to the real diff on GitHub; a new strip lists every other review generation of the same PR (`listFleetRunGenerationsForPr`), since a PR's full history is one `fleet_run_intents` row per push and nothing previously queried across them. Every step now carries an absolute UTC clock time (the run-relative offset moved to a tooltip), `ship-error`/`ship-resumed`/`ship-checkpoint` get their own narratives instead of falling to a raw default line, and repeated `delivery-attempt`/`delivery-failed` rows collapse into one consolidated entry with a per-attempt breakdown — the same treatment MAP-chunk noise already had. The permanently-dead "Neurons" stat tile (a column the executor never wrote) is replaced with a real Spend total summed from the same per-ship cost data.
- **The relay can now mirror a repository's roadmap for the account that pushed it (roadmap command center, PR 1).** A daemon pushes its per-repo roadmap snapshot to `PUT /v1/roadmap/snapshot` (device token or session), and the relay stores an account-scoped replica — items with tombstones, hierarchy/dependency edges, and a capped activity tail — behind an honest two-clock watermark (daemon `generatedAt` in unix ms, relay `receivedAt` in unix seconds). Each push fully replaces the previous snapshot in one transaction; `GET /v1/roadmap/mirror` serves the board grouped by lane, item detail with edges in both directions, and the activity tail. The mirror rides the account lifecycle: it exports with `/account/export`, dies with account erasure, and the retention sweep keeps activity bounded at 200 rows per repository.
...
- **`pd seamanship visibility <skill-id> [private|listed|public]` — change a skill's tier from the tool instead of by hand.** Operator directive: a gate deciding what leaves your machine should not be operated by hand-editing YAML across a 300-skill catalog. The writer (`withVisibility`) lives beside the reader in the dependency-free visibility module, is a pure text transform so it stays Worker-importable, and edits only the one line inside the frontmatter span — a `visibility:` in a fenced example in the body is never touched, and every other byte is preserved so the diff is the decision rather than a wall of reformatted YAML. Setting `private` REMOVES the line rather than writing `visibility: private`, because private is the absence of a grant, which makes flipping a skill out and back a byte-identical round trip with no residue. Writes are atomic (tmp + rename) and read back through the same `parseVisibility` every publish path uses before success is reported, so the tool can never claim a tier the catalog disagrees with. `pd seamanship list --visibility <tier>` filters, and the listing footer states the exposure surface outright.
- **Private skills now render as private, everywhere.** Both the CLI listing and the account page previously left private UNMARKED — the reasoning being that the absence of a grant should read as ordinary rather than as a missing label. That is right about grants and wrong about gates: an unmarked tier is one you cannot audit by looking. Every tier carries a marker now, private included, on both surfaces. Private remains the default and remains the absence of a frontmatter line; it is no longer the absence of a rendering.
...
- **The Gantt anchors a bar to its real dates when it has them.** `lib/roadmap-items.ts`'s `startedAt`/`dueAt` (epoch ms, "the Gantt's left/right date anchor when present") existed in the schema and API since ADR-0086 but were never read by either Gantt renderer — every bar rendered on the kernel's purely-relative CPM schedule regardless. Both `core/pd-console`'s `PlannerPane` (`PlannerItem::from_value`) and `lib/planner-board.ts`'s `renderBoard` now parse the pair and, when BOTH are present and `dueAt` lands after `startedAt`, draw that task's bar at its real wall-clock day-offset from the axis's today-anchor instead of the CPM offset — the render lane widens to fit a real due date past the CPM makespan, and the bar carries a `DATED`/`dated` tag so the operator can tell which bars are real and which are still relative. `critical`/`slack` membership is untouched — it still comes from the kernel scheduler (ADR-0120 TCB boundary; `pd_anchor::schedule` and its TS parity twin `lib/planner-schedule.ts` are unmodified), dates only ever change WHERE a bar is drawn. An item missing either field (the common case until items are backfilled) keeps today's plain relative bar exactly as before.
...
- **pd-console gets a typed roadmap-home data layer (no view wiring yet).** `core/pd-console/src/roadmap_projection.rs` mirrors `lib/roadmap-projection.ts`'s `RoadmapProjection` output (not yet shipped — that file lands with PR #9223, branch `claude/roadmap-projection`) as serde structs, plus a pure `RoadmapLiveEvidence::display_state()` matching the iOS port's law-13 rule field-for-field: no `dispatch_id` is always `NoEvidence`, and the server's own `live` flag is checked before any age arithmetic, so a settled dispatch with fresh evidence renders `Stale`, not `Live`. Since PR #9223 ships no fixture of its own, `core/pd-console/tests/fixtures/roadmap-projection.fixture.json` was generated by running the real `buildRoadmapProjection`/`serializeRoadmapProjection` against a seeded in-memory DB covering six law-13 cases (no evidence, live, stale age, future-dated evidence, a settled-but-fresh dispatch, and an orphaned dispatch stamp with no timestamp) — reused, not reimplemented by hand. Hard edge: these types track `RoadmapProjection` as #9223 defines it today; this merges after #9223, not before.
- **Remote harbors can now be joined by single-use invite, and every membership change ticks the harbor's authority epoch (ADR-0122 §4).** A member mints an invite (`POST /v1/harbors/:ns/:name/invites`): a JTI bound to the harbor and the inviter with a mandatory expiry (default 72h, max 7d), whose bearer token is returned exactly once and stored only as a SHA-256 hash — no key material rides an invite in either direction; the relay stays a phone book, never a key holder. Redeeming it (`POST /v1/harbors/:ns/:name/join`) is single-use by compare-and-swap — one UPDATE gated on `consumed_at IS NULL`, so two concurrent redeems have exactly one winner — and records a plain `member` row (the role is CHECK-pinned in the migration: an invite can never grant ownership) while bumping the new `harbors.authority_epoch` column atomically with the membership INSERT; the operator add-member route ticks the same clock, and a refused duplicate rolls the tick back with the batch. Joins are idempotent for the winning member (replays answer 200 without re-ticking), invites are revocable by their inviter or any owner while unredeemed (`POST .../invites/:jti/revoke`), and to an invalid invite holder every refusal — nonexistent harbor, unknown token, expired, revoked, consumed by someone else — is one byte-identical 404, so join never becomes a harbor-existence oracle. The epoch is the membership-change counter of the phone book the relay already owns: the relay still signs nothing, grants no writer lease, and authors no authority record.
- **Shared-harbors decision groundwork (ADR-0122–0127).** ADR-0122–0126 accept one-writer harbor authority, separate signing/encryption roots with device-wrapped epoch keys, fail-closed transcript redaction, a native iOS HITL surface, and the resequenced private/team-harbor build gates; ADR-0127 records the still-Proposed chain-hash reconciliation question. These are prerequisites and launch contracts, not claims that the vault or iOS implementation already ships.
...
- **The daemon now runs a real relay connection with an honest status surface (WS-A cluster R, C1).** `lib/relay-connection.ts` replaces the honest-disconnected stub in `server.ts`: handshake (TOFU-pinned) → SSE subscribe → exponential backoff, wired into `pd relay status` / `GET /relay/status`. `connected: true` is asserted only while the relay has ACCEPTED the subscribe stream (a new `onOpen` signal fires on HTTP 200 of the SSE fetch); handshake success alone still reports disconnected, every failure lands its code in a new `last_error` field, and the backoff resets only on an accepted stream. A successful `POST /relay/exchange` now stores the returned card so the lifecycle can reconnect across restarts, and config/card writes restart the live connection without a daemon restart.
- **Daemon seal path — one classification chokepoint, sealed with the pd-vault construction (WS-A cluster R, S1 / A3 slice 1).** `lib/relay-seal.ts` is the single place a relay-bound event becomes an A1 envelope: `sealRelayEvent` (pd-vault AEAD — HKDF-SHA256 channel key, XChaCha20-Poly1305, harbor/channel/epoch/seq bound into the tag) or `relayReadableEvent` (REQUIRED stated reason). The product is a branded `ClassifiedRelayEvent` that only the chokepoint can mint, and the publish wire (`publishToRelay`'s transit slot, `DaemonRelayConnection.publish`) accepts only that type — an unclassified event on the wire is a compile error, and a cast past the brand dies at a runtime re-classification before any bytes leave the daemon. The AEAD is a pure-TS reference implementation (`lib/pd-vault-ts.ts`) behind a narrow `VaultSealProvider` interface so the pd-vault Rust FFI can replace it with zero call-site changes; it is held to the crate's known-answer vectors via the shared fixture `tests/fixtures/pd-vault-parity-vectors.json` (ADR-0120: no second implementation without a shared fixture), and the daemon's envelope signature binding reproduces the relay suite's pinned digests byte for byte. The sealed `alg` closed set gains `xchacha20-poly1305` as an explicit schema revision — the algorithm the kernel actually implements — in schema, classifier, and tests together.
...
### Changed
- **No surface outside the canonical config names a model id any more, and the guard that enforces it can now see the planes that drift.** `pd-fleet.yml` loses all 35 `model:` pins for `capability:` on a backend fallback and `cf_role:` / `plan_cf_role:` / `author_cf_role:` / `map_cf_role:` on a ship or step; the six `roles/*.yml` presets follow. Both fleet parsers, the fleet executor, the GitHub App receiver, and the relay resolve those tokens through the generated registry, and the relay's divergent duplicate — which honored ANY `@cf/`-prefixed pin with no guard at all, so it would certify a config naming a model the executor refuses — now shares the executor's. The executor's spend and context-window tables are derived rather than hand-kept in sync, and `lib/backend-catalog.ts`'s advertised `models[]` is derived too, so the picker can no longer offer a model the resolver would never pick (the Cloudflare row advertised the retired kimi id under a comment calling it the real slug). `tests/unit/no-hardcoded-model-ids.test.js` widens from `lib/ routes/ cli/ mcp/ core/ config/` to also cover `apps/`, `scripts/`, `schemas/`, `roles/`, `fleet/`, `website-v2/src/`, `pd-fleet.yml`, and `README.md`, and its allowlist shrinks — `lib/backend-catalog.ts` and `lib/shipwright/archetypes.ts` are no longer exceptions. Documentation surfaces keep their illustrative ids under a new companion check, `tests/unit/docs-model-ids-current.test.js`, which asserts every id the docs show is one the registry still maps; it caught the website advertising `gemini-2.5-flash` and a retired Workers AI id long after they stopped resolving. The conversion also made one thing visible worth recording: most of the `model: '@cf/openai/gpt-oss-120b'` pins in `pd-fleet.yml` were already INERT — the review model was never in the honored pin set — so those ships had been running on the cheap model while the config said otherwise.
...
- **The fleet's model choices are now made against a verified roster and a live scoreboard.** `skills/port-daddy-internal-dev/references/cloudflare-model-roster.md` records the Cloudflare model catalog + pricing snapshot, the three-part admission contract (verified existence, verified rate, known context window), and the standing decision record; `node scripts/fleet-ship-stats.mjs` renders per-ship × per-model spend, broken/repair health, and the purser authoring funnel straight from the relay D1, flagging any honored-but-unpriced model.
- **The Steward Worker now deploys from CI like the fleet-executor and relay.** `deploy-steward.yml` typechecks, tests, and `wrangler deploy`s `apps/steward` on every push to main that touches it, so the live seat can no longer drift behind main; runtime secrets survive deploys and an uncommissioned seat still answers 503 safely.
- **The model dossier is now a product surface on the Shipwright page.** `apps/relay/src/model-dossier.json` is the canonical reviewed-model board (23 honored Workers AI models with verified prices, served context windows, verdicts, fleet assignments, and the named exclusion rulings); the signed-in `/account/shipwright` page renders it as a collapsible Model Board in the masthead, and the Shipwright's own system prompt now recommends `model:` ids from that board — replacing the stale hardcoded qwen folklore — with an exact-quote rule so an emitted pd-fleet.yml can never carry an id the executor would silently remap or blank. A fleet-executor parity suite reads the identical JSON and pins every id, rate, and context window to `KNOWN_GOOD_CF_MODELS`/`WORKERS_AI_RATES`/`MODEL_CONTEXT_TOKENS`, so the operator-facing board can never drift from what the executor actually honors and meters.
- **The fleet's model repertoire expanded from three workhorses to the full eligible universe: 23 verified models, assigned by role.** Beyond the ten role-assigned admissions, every current, priced, Cloudflare-hosted text-generation model is now honored (the llama 3.x/4 family, mistral-small-3.1, qwen2.5-coder-32b, qwq-32b, qwen3.8-27b, deepseek-r1-distill, granite-4.0-h-micro, gemma-sea-lion-v4-27b), each with a pricing-page-verified rate and model-page-verified context window; the remaining ~200 catalog entries are image/video/speech/embedding/safety or gateway-only models that cannot serve as reviewers, and the handful of by-name exclusions (deprecated tier, unpriced llama-3.1-8b-fast, llama-guard classifier, the #654 kimi-k2.6 phantom, LoRA bases) are recorded as rulings in the roster reference. Honored ≠ endorsed: assignments stay evidence-driven and the scoreboard judges everything. The role assignments themselves: GLM-4.7-Flash (the agentic 30B "repo mechanic": 59.2% SWE-bench at cheap-tier price) takes qa, lookout, purser PLAN, and code-reviewer MAP; DeepSeek V4-Flash-0731 (strongest independent agentic-coding record in its class, 1M context) becomes the purser AUTHOR tier; GLM-5.2 (strongest independently-verified open coding model) replaces the vendor-benchmark-only Kimi pin on code-reviewer REDUCE; DeepSeek V4-Pro-0813 gives red-team a true reasoning flagship; Gemma 4 26B joins as cheap-class diversity so the ship-stats scoreboard can judge three model families empirically, with qwen3-30b ships kept as the control population. Every admission carries a pricing-page-verified rate and a model-page-verified context window.
- **The purser authors on the strongest verified tier, and declared premium ship pins are honored.** With `@cf/openai/gpt-oss-20b` authoring, 14 days of live D1 evidence showed 121 authored-test sets non-executable, 83 of 110 author-repairs failing, and 124 purser broken-ship runs washing 249 of 584 fleet verdicts to neutral (#8870) — the author default is now `@cf/openai/gpt-oss-120b` (~$1 per two weeks of extra spend) and the one-shot author-repair rewrite always escalates rather than retrying the tier that just failed. The known-good model set now guards verified existence instead of price: code-reviewer's declared `kimi-k2.7-code` pin and red-team's `gpt-oss-120b` pin run as written instead of being silently remapped down, stale premium pins on the cheap-tier ships were rewritten to the cheap id they actually run, a MAP pin pricier than its ship's REDUCE model is dropped as economically backward, and every honored model is priced (the purser's author calls no longer meter $0).
- **A release only cuts a new pd-console binary when the console actually changed.** `release.yml` diffs `core/pd-console` (and its packaging script) against the previous tag — ignoring the version-string churn `sync-version.ts` stamps on every release — and skips the Rust build, signing, notarization, and upload when nothing real changed. The newest console stays on the last release that built one, that release's `latest.json` omits the console entry (an already-supported feed shape), and `workflow_dispatch` with `force_console: true` rebuilds unconditionally when needed.
...
### Fixed
- **A slash inside a harbor name no longer silently addresses a different relay route.** `RelayRoute.encode` percent-encoded path components with `.urlPathAllowed`, which includes `/`; a harbor named `fleet/presence` could therefore address a different route. Components now exclude `/` from path-component encoding and use `percentEncodedPath`, with tests covering `/`, `?`, and `#`.
- **Streaming a Cloudflare body made it unbillable, so a perfect answer still failed the run.** The streaming reader returned no token counts under a comment asserting that "streamed Workers AI responses carry no usage block". They do — measured live on 2026-08-23 over 112 frames from `@cf/qwen/qwen3-30b-a3b-fp8`, the final frame carries `prompt_tokens 11, completion_tokens 111`. The claim was load-bearing in the wrong direction: telemetry is fail-closed, so a body with no counts is refused as unbillable, and the live capture showed the cloudflare lane reporting "Exact telemetry required, but cloudflare did not return token counts" with a complete, correct answer sitting directly above it. Usage is now read from the stream; zero-valued frames are ignored rather than assigned, because an early `0` landing last would assert a free call to the one meter that is fail-closed on exactly that.
- **A model that runs out of budget mid-thought no longer reports as a model with nothing to say.** Live-probing all 23 admitted Workers AI ids found that more than half are reasoning models, which stream a scratchpad first and the answer last out of ONE shared output budget. On the fleet's own default (`@cf/qwen/qwen3-30b-a3b-fp8`) the same prompt gives 113 frames / 105 reasoning / 3 content at `max_tokens 2000`, and at 400 gives HTTP 200 with **zero** content frames. That is not an error, it is a BLANK — the shape a parser reads as clean, the same silent-blank class that took the fleet down in #654 by a different route. The failure now names the cause and says to raise `maxTokens`; the reasoning text is deliberately NOT promoted into the answer, the same rule that keeps Gemini's `thought` parts out of operator output. Nine catalog rows now declare `reasoning` so a caller can size the budget before spending it, and `lib/shipwright/survey.ts` asks for 600.
- **Gemini's failures now distinguish three different events that all used to read the same.** "Returned no text response" covered a stream that closed without a single frame (transient load-shedding after the request was accepted), a stream that carried only `thought` parts (the model was still thinking when it ended — what the live capture's gemini lane actually hit), and a genuine empty answer. Each now says which, with `finishReason` where the provider sends one, and the reasoning text is never leaked into the error.
- **An admitted Workers AI model that this account cannot call at all.** `@cf/meta/llama-3.2-11b-vision-instruct` was catalogued from the vendor list as `[text-generation]`, which admitted it — and admission is a promise that a ship pinned there will run. Live-probed: **HTTP 403**, gated behind a per-account Model Agreement and an EU-domicile representation. Not transient, and not something a failover hop clears. It is now catalogued as `[vision]` so admission cannot pick it up, named in the Shipwright board's `excluded` rulings with the probe result, and named in the executor's deliberately-unadmitted list so a second such row costs a diff. Every remaining `@cf/` row moves from `verifiedBy: cf-catalog` to `live-probe`.
- **A provider that answers a quota failure with HTTP 200 and an SSE content type no longer reports it as "the model returned nothing".** Found live: Gemini answers a 429 with status **200**, `content-type: text/event-stream`, and a plain JSON error object as the body. Every honest check passes — the status is fine, the content type says SSE — and the stream parser then finds zero frames, so the run was reported as "Gemini returned no text response". That is technically true and completely useless: it hides a quota error behind a message that reads like a model problem and sends an operator hunting the wrong thing. All three streaming readers (Cloudflare, Gemini, and the OpenAI-compatible pair) now distinguish "the stream yielded no FRAMES" from "the stream yielded no TEXT": the first means the body was never a stream whatever it claimed, so the provider's own error is read out of it; the second is a real model outcome and keeps its own message, including Gemini's `finishReason`.
- **Fleet cron schedules honor supported absolute times and reject unsupported shapes.** Daily/hourly `M H * * *` and `M * * * *` schedules re-arm against the next host-local wall-clock occurrence; valid `*/N` steps retain their interval path. Host-local `Date` semantics apply at DST boundaries: spring-forward gaps advance by the gap and fall-back folds select the earlier occurrence, rather than calendar-walking both instants. Malformed or calendar-constrained schedules now fail closed instead of silently firing every ten minutes, and the operator forecast reports those refusals as zero launches instead of projecting the retired fallback. Fleet YAML also gains a strict `enabled: false` admission gate: disabled or malformed-enabled declarations remain inspectable in the source AST but never enter executable runtime config. The nightly dispatch runner uses that gate until its first ready-PR artifact receives operator review; fixed-clock support lands without arming it. The configured fleet-wide $8.50 value is honestly a settled-spend prelaunch threshold, not an atomic reservation; concurrent starts can oversubscribe it.
- **`pd account` default relay is the branded production origin.** `DEFAULT_RELAY` now targets `https://relay.portdaddy.dev` instead of the pre-cutover workers.dev URL.
- **A GitHub rate limit can no longer silently unpublish an operator's skill directory.** `listInstallationRepos` swallowed a non-ok `/user/installations/:id/repositories` response with a bare `continue` and returned an empty repo list rather than signalling a failed read. Because only a `null` marks a scan degraded, `installationsKnown` went true, `syncSkillListings` passed its own guard, and its unconditional `DELETE FROM skill_listings WHERE namespace = ?` withdrew every listed skill — because GitHub was briefly rate-limiting the account. A failed or unparseable read now degrades the whole scan, so publishing refuses with `CATALOG_UNAVAILABLE` and changes nothing, which is what the function's documentation already promised.
- **The Remote Harbors tutorial now shows invites and `/join` as live, not designed-not-built.** The page was rewritten (#9776) to be honest about the harbor surface just before invite/join (`apps/relay/src/invites.ts`) actually merged (#9772); it still described minting an invite and redeeming it as a future `pd harbor invite`/`pd harbor join` CLI that doesn't exist. Updated to cite the real, live routes (`POST/GET .../invites`, `POST .../invites/:jti/revoke`, `POST .../join`) with a working `curl` example against the raw relay API, and narrowed the remaining designed-not-built list to what's actually still unbuilt: device pairing and end-to-end channel-key sealing — the real gate on shared harbors.
- **Retired interactive post-tool hooks stay retired, even inside already-running agent sessions.** The stable `pd-hook-post-tool` wrapper is now an immediate zero-work tombstone before clocks, debug logging, daemon probes, or delegation, so a provider that cached an obsolete PostToolUse registration cannot resurrect per-edit fan-out. Squid debug-log compaction now normalizes BSD/macOS `wc -c` padding and discards the first partial record after byte-tail trimming, keeping the promised 2 MiB ceiling without corrupting the TSV timeline.
- **Purser now rejects malformed generated JavaScript and TypeScript before it can create GitHub side effects.** Every authored test file is parsed as a complete program using the source type implied by its extension, after discovery and trusted-runner evidence are established. A syntax failure gets one bounded repair; if that still fails, Purser reports broken machinery and stops before sandbox execution, stacked-PR creation, or parent-PR retargeting.
- **The release train now falls back from a configured-but-dead preferred credential.** Version cutting and tag publication live-probe repository push permission for `RELEASE_TRAIN_TOKEN`, then try `HOMEBREW_TAP_TOKEN` when the preferred PAT is expired or under-scoped. Only the selected source name crosses step outputs; secret values stay inside the consuming steps, and mutation fails closed before a commit or tag when neither credential can push.
- **Signed macOS installs load the local semantic runtime without a dynamic-linker escape hatch.** Release builds now embed an executable-relative ONNX rpath before Bun packages the N-API binding, keep `DYLD_*` out of the hardened launcher contract, and import the native runtime again after signing the exact release binary. Linux retains its pre-start `LD_LIBRARY_PATH` contract.
- **Source files can no longer go binary to git, repo-wide.** Six files carried literal NUL bytes as join/sentinel separators, which is legal JavaScript that compiles fine but makes git classify the blob as binary. Two consequences are silent and both had already cost work: ripgrep reports `binary file matches` with no lines, so a symbol search reads exactly like "this is used nowhere" (three real hits were hidden in `lib/suggestions.ts`), and git's three-way merge takes one side wholesale rather than merging hunks, which is how #9352's ADR-0086 planner columns silently vanished from `lib/roadmap-items.ts` in `47964247a` behind a green build. All separators are now written as the source escape, leaving every runtime string byte-identical — including the two that feed persisted hashes (`deterministicFindingId`, `hashPayload`), whose digests are unchanged. Measured and worth recording: no `.gitattributes` setting rescues a NUL'd file — `text diff`, `text diff merge`, `merge=union` and a custom driver all still refuse and still drop a side — so not writing the NUL is the only fix, and `tests/unit/source-is-text.test.js` is now the guard. It walks every tracked file via `git ls-files` rather than a directory list, because the previous guard's two hardcoded directories are precisely why all six offenders shipped past it; it carries a vacuity check so a bad pattern list fails loudly instead of passing on an empty set, and it also asserts `.gitattributes` keeps `text`/`diff`/`merge` set — `merge` closes a separate real hole where a clone-local `merge.default=binary` silently loses a side of any merge, NUL or not. The superseded per-workspace guard under `apps/fleet-executor/` is deleted rather than left running beside the repo-wide one.
- **`pd roadmap upsert` now rejects bad planner values instead of silently changing them.** `--start`/`--due` inherited `Date.parse`'s leniency, which does not reject calendar overflow — it rolls it forward (`2023-02-30` became 2023-03-02, `2026` became Jan 1) — so a typo'd date was stored as a real, different day. `--priority`/`--estimate` went through a bare `Number.parseInt`, and since `NaN` serializes to `null` and `null` is the API's "clear this column" sentinel, `--estimate abc` wiped an estimate another surface had recorded and `--priority xyz` reset a stored priority to the default. All four flags are now validated at the CLI boundary and rejected with a message naming the flag and the accepted shape. The daemon's own sanitizers are unchanged and still saturate for non-interactive callers.
...
- **An unexpected auth status from APNs no longer stops every push until someone deploys.** `sendApnsPush` mapped 403 (the documented provider-token family — Expired/Invalid/MissingProviderToken) to "drop the cached JWT and retry", and let everything else fall through to a terminal `failed`. 401 is not in Apple's documented status list for this API, which is exactly why it is now handled alongside 403: the cost of being wrong is asymmetric, since retrying an auth-shaped status self-heals on the next sweep while treating one as terminal is an outage. The device token is deliberately not marked dead on that path — an auth problem is the relay's, not the device's.
- **Two silently-swallowed D1 failures now say so in the Worker log.** `markTokenDead` and the device-token read in `sendInterruptionPushes` both catch and continue, which is right — a lost dead-mark costs one extra 410, and a registry read failure must not read as a delivery failure to the sweep. But a persistent failure was invisible in the delivery numbers, and a read failure was indistinguishable from "no devices registered" to an operator wondering why their phone is silent. Both log via `console.warn` rather than `recordHookEvent`, because the latter writes to the same D1 the call just failed against and would swallow the report for the same reason it swallowed the write. The nag sweep also logs once when asks are due but no transport is configured at all — inert is correct with no secrets set, so it stays quiet unless something would actually have been paged.
...
- **The website's Remote Harbors tutorial no longer claims harbors don't exist.** It said "Remote Harbors do not exist yet" while the relay's harbor surface v1 (`POST/GET /v1/harbors`, `GET /v1/harbors/:namespace/:name`, `POST /v1/harbors/:namespace/:name/members`) has been live since `apps/relay/src/harbors.ts` shipped. The page now separates what's live (create/list/get/add-member, the account page's `#harbors` section) from what's designed-not-built (signed invites + `/join`, device pairing, end-to-end channel-key sealing) and names full end-to-end encryption as the gate on launching shared harbors, not a ship date.
- **Envelope signatures now actually bind their routing tuple.** The binding message joined its components with `|` and hashed `JSON.stringify(payload)`. Both were unsound: an unprefixed join is ambiguous when a field can contain the separator (harbor `a|b` + channel `c` produced byte-identical bytes to harbor `a` + channel `b|c`, so one signature validated for two different routing tuples — a cross-channel replay in the mechanism meant to prevent it), and insertion-order serialization means a verifier that rebuilt the payload differently rejects a valid signature. Components are now length-prefixed by UTF-8 byte count (injective framing) and payloads canonicalized with recursively sorted keys.
- **An unpaired surrogate can no longer forge an envelope's routing tuple.** Length-prefixing made the binding injective over bytes, but the string-to-bytes step above it was not: `TextEncoder` maps every unpaired surrogate to U+FFFD, so `channel="ops\uD800"` and `channel="ops\uFFFD"` — distinct strings, reachable because `JSON.parse` preserves a lone surrogate verbatim — encoded to the same bytes, took the same length prefix, and produced a byte-identical binding message. One signature validated for two routing tuples, which is the cross-channel replay the prefixing was added to prevent. Routing metadata with no lossless UTF-8 encoding is now rejected at classification and again in the framing itself.
- **The envelope schema documented the ambiguous binding it was replacing.** Both `sig` descriptions in `schemas/relay/v1/envelope.schema.json` specified the signature as sha256 over a bare `|`-join of the eight components — the exact unprefixed construction that let harbor `a|b` + channel `c` produce the same bytes as harbor `a` + channel `b|c`. The schema is the cross-implementation contract, so it was instructing every future Swift, Rust, and Python verifier to build the format the code had just stopped using: they would have rejected valid signatures, or reimplemented the cross-channel replay. Both descriptions now specify the length-prefixed framing (UTF-8 byte counts), the canonical payload serialization, the well-formed-UTF-16 requirement on routing metadata, and a known-answer digest per variant to check an implementation against.
- **Canonicalization of objects nested inside arrays is now pinned.** `canonicalJson` recurses into array elements and always did, but every array in every test held only primitives, so the property had nothing behind it — replacing the recursive branch with a plain `JSON.stringify` left all 866 tests green. The shape it protects is the ordinary one, a payload carrying a list of records: without recursion `{items:[{b,a}]}` and `{items:[{a,b}]}` hash differently, so a verifier that rebuilt the list in another key order rejects a valid signature. Pinned at both layers now — the serialization directly, and the binding that inherits it — with element order still asserted as semantic.
- **A `relay_readable` reason of only whitespace is no longer a valid justification.** The check was `reason.length === 0`, so `'   '` passed — non-empty by length, empty by meaning, in the one field whose whole purpose is that a class which cannot say why it transits unencrypted does not ship. Now rejected via `trim()`, which also covers non-ASCII blanks (NBSP, line/paragraph separators); the schema gained a matching `\S` pattern. A reason with real content surrounded by whitespace is still accepted — the rule is "must say something", not "must be pre-trimmed".
- **An envelope can no longer claim a different channel than the frame it travelled in.** The envelope type documented `harbor`/`channel`/`sender`/`seq`/`iat` as "must equal the outer frame's", and nothing enforced it: the publish path decoded the body for the classification probe and discarded the result. A mismatch was never a forgery — the frame signature covers the encoded envelope — but the envelope carries its own signature over its own routing tuple, so an event delivered on channel C whose envelope said channel D was filed under C by the relay's chain and under D by every consumer that verifies the envelope. Two signed answers to where the event lives. `handlePublish` now rejects a classified body whose routing tuple disagrees with its frame (`ENVELOPE_FRAME_MISMATCH`, 400) and audits the attempt; unclassified bodies keep their detect-and-warn treatment unchanged.
...
### Security
- **Removed a committed API key from `.mcp.json`.** The `TWENTYFIRST_API_KEY` literal is replaced with `${TWENTYFIRST_API_KEY}` environment expansion. Provider-side rotation remains required because the old value exists in history.
...
### Added
- **Whitepaper Suite Collected Treatise Overhaul (Version 2.0).**
  - **Publication-grade Cover & Architecture Lattice**: Integrated high-craft TikZ vector diagram depicting the complete L0--L3 coordination hierarchy (Single-Writer Effect Hypervisor, Capability Security Mesh, Evidence-Bearing Work Unit $W$, Epistemic Substrate, 2-of-3 Multisig Clearinghouse, and 6-Rung Risk Ladder).
  - **Five Assurance Levels & Risk Ladder**: Consolidated implementation ledger in Appendix A defining discrete, falsifiable assurance modes (Observed, Coordinated, Brokered, Confined, Attested) and sequential risk-containment staging ($\text{PREVENT} \to \text{CONTAIN} \to \text{CHECKPOINT} \to \text{OBSERVE} \to \text{ADJUDICATE} \to \text{COMPENSATE}$).

### Fixed
- **Table of Contents (TOC) & Typographic Hierarchy**: Redefined `\l@part` and `\pdchapter` in `coordination-papers-mega-volume-preamble.tex` to eliminate duplicate Roman numeral headers (`VI VI`) and enforce consistent hierarchical indentation across parts, sections, subsections, figures, and tables.
- **Game-Theoretic Claim Signaling (Chapter VI)**: Corrected the stage-game payoff bimatrix in §VI.7.4 to a proper Prisoner's Dilemma ($(T,T)=(3,3), (T,F)=(0,4), (F,T)=(4,0), (F,F)=(1,1)$), re-derived the trigger discount factor threshold ($\delta^\star_{k=3} \approx 0.342$, grim trigger $\delta \ge 1/3$), and updated downstream mechanized proofs.
- **Capability Delegation Chain (Chapter V)**: Corrected Algorithm 3 (Harbor Card verification) to enforce monotonic attenuation at every hop ($\text{cap}_i \subseteq \text{cap}_{i-1}$), preventing transitive re-escalation.
- **Settlement Ledger & Escrow Accounting (Chapter IV)**: Separated the four fund buckets (Requester Bounty, Provider Performance Bond, Insurer Reserve, Fees), clarifying that refunded collateral is not income and verifying algebraic balance conservation per bucket.
- **Newcomer Whitewashing Theorem (Chapter III)**: Reframed Property III.6.1 and Theorem III.6.1 to state the valid conditional on accessible economic utility and aggregated exposure limits at the principal root.
- **Signal Detection Theory & Criteria Direction (Chapter I)**: Corrected criterion movement in Figure 1 and §I.5 (left = stringent, right = lax) with an explicit loss objective.
- **Conflict Domains & Fencing Epochs (Chapter II)**: Restated Theorem II.6.1 over canonical conflict domains (exact keys, intervals, path prefixes) with strictly monotonic fencing epochs enforced by downstream effect brokers.
