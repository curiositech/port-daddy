...
### Added
- **Voice-rule CI guard** — `scripts/check-voice-rules.ts` + `.voice-rules.yml` + `.github/workflows/voice-rules.yml`. Prevents byline drift and forbidden corporate-hedging phrases ("Port Daddy Engineering Team", "engineering@portdaddy.dev", "our engineers", "we believe") from leaking into the published surface (whitepapers, docs, marketing site). 427 files scanned per run; exact-string matching against structured YAML config (NOT keyword-NLP); inline override syntax `<!-- voice-rule:ok reason=... -->` (HTML, LaTeX `% voice-rule:ok`, and JS `// voice-rule:ok` forms all accepted). 7 unit tests in `tests/unit/voice-rules.test.js` cover clean/dirty/override/multi-violation/empty cases plus a regression test against the live repo. Wired into `npm run check:voice-rules`. CI runs on every push to `main` / `research/**` and every PR to `main`. Same PR fixed the byline drift in `website-v2/public/whitepaper/{agent-transactions,anchor-protocol}-whitepaper.tex` (replaced `engineering@portdaddy.dev` with `hello@curiositech.ai`) and in `docs/reports/PORT_DADDY_ANCHOR_WHITEPAPER.md` (replaced "The Port Daddy Engineering Team" with "Erich Owens / Curiositech LLC"). Lands feature-requests item #10. Wave 0 of the `research/empirical-closure` lab.
