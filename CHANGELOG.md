...
### Fixed
- **Fleet cron schedules honor absolute times.** `parseCronInterval` coerced absolute-hour crons (`0 1 * * *`) into a fixed ~10-minute interval, so a daily-at-1am agent fired every 10 minutes — the named blocker on the dispatch-runner. Absolute `M H * * *` / `M * * * *` schedules now compute the delay to the next real occurrence and re-arm via a timeout chain; `*/N` fast paths and the 1-minute floor are unchanged. The `dispatch-runner` ship in `pd-fleet.yml` is un-commented behind its (updated) preconditions comment — the operator-review-of-first-dispatch-PR precondition remains honestly open.
- **`pd account` default relay is the branded prod origin.** `DEFAULT_RELAY` pointed at the pre-cutover `port-daddy-relay.erich-owens.workers.dev`; it now targets `https://relay.portdaddy.dev`.
...
### Security
- **Removed a committed API key from `.mcp.json`.** The `TWENTYFIRST_API_KEY` literal is replaced with `${TWENTYFIRST_API_KEY}` env expansion; the exposed key needs operator rotation.
