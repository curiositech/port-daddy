...
- **Codex-piloted Claude Code is visually identifiable.** `pd squid codex` now injects `PD_SQUID_PILOT=codex` into the launched client env; `pd-statusline` flips the badge to a magenta `◆ PD⇄CODEX` so a session answered by Codex-through-ChatGPT-Pro can never be mistaken for a direct Anthropic seat. `pd setup` stages the statusline alongside the tentacles, and `pd init` wires it (plus `/squid`) into every pd project.
