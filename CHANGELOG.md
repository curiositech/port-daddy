...
### Added
- **Shared-harbors decision groundwork (ADR-0122–0126).** Accepted architecture now fixes one-writer harbor authority, separate signing/encryption roots with device-wrapped epoch keys, fail-closed transcript redaction, a native iOS HITL surface, and the resequenced private/team-harbor build gates. These are prerequisites and launch contracts, not claims that the vault or iOS implementation already ships.
