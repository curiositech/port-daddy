...
### Fixed
- **A malformed parley URL no longer answers differently from a missing one.** The `/account/parleys/` router decoded path segments without guarding `decodeURIComponent`, so a bad percent-escape threw past the routing into the global boundary and returned `500 INTERNAL_ERROR`, while an unroutable path returned a bare nine-byte `Not Found` and a genuinely missing parley returned the full page. Three different replies on a surface whose 404 exists to give exactly one, so that a non-member and a nonexistent parley are indistinguishable. All three now serve the same page, with the same `no-store`, `noindex`, script-free headers.
