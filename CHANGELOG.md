...
- **An unsupported control verb can no longer reach an operator wearing a generic apology.** `ControlVerbs.unsupportedReason` ended each backend's switch in `default:` returning "this backend's adapter does not implement <verb>" — non-empty, so the existing reason test passed, and generic, so it told the operator nothing and used the exact phrasing that function's own contract forbids ("what the backend cannot do — not 'not implemented'"). Both fallbacks were unreachable today, which is what made them dangerous: removing a verb from `supportedVerbs` would have shipped one silently. The switches are now exhaustive with no `default:`, so a new verb is a compile error rather than a shrug, and a verb removed from the matrix without a written reason lands on an explicit "the tables disagree — this is a bug in Port Daddy, not a limit of the backend" marker that a new test forbids on any genuinely unsupported pair.
- **A control command's failure reason of only whitespace no longer counts as recorded.** `isMissingRequiredReason` used `(failureReason ?? "").isEmpty`, so `"   "` read as a recorded reason — the silent half-control state ADR-0122 §5 forbids, wearing a filled-in field's clothes. Now trimmed before the check, matching the rule the relay applies to a `relay_readable` envelope's `reason`.
...
- **pd-console now opens on a mission-first Work surface.** Enter a plain-language goal to submit one governed WorkIntent, then follow its exact dispatch, agent, transcript, worktree, checks, and pull request without substitution by an unrelated global session.
...
- **The fleet's web surfaces are installable.** Phase 5 of the transcript RFC: the run receipts and transcript viewer now ship a web app manifest, home-screen icons, and theme-color/safe-area polish, so Add to Home Screen on iOS (and install on Android/desktop) pins the fleet as an app — the RFC's honest answer for phones, with a SwiftUI reader still a demand-driven later step.
...
- **The PR template no longer disables the changelog gate on every PR.** The template pasted a live `<!-- changelog-exempt: <reason> -->` comment into a checklist line as an example; the same body scanner that reads real markers matched it, so any PR opened from the template auto-exempted the changelog requirement. The template now describes the marker in prose — the way `visual-exempt` was already documented — and a regression test runs the real template file through the guard and asserts the changelog rule still fires.
...
### Fixed
- **A slash inside a harbor name no longer silently addresses a different relay route.** `RelayRoute.encode` percent-encoded path components with `.urlPathAllowed`, which is the RFC 3986 path grammar `path = *( pchar / "/" )` — so `/` is a MEMBER of that set and the one character that decides where a segment ends was the one character left untouched. A harbor named `fleet/presence` built the same path as the presence route of a harbor named `fleet`, and a namespace could carry `..` segments into another account's URL space. `URLComponents.path` compounded it: that setter takes a DECODED value, so it would have re-decoded any `%2F` back to `/` even after the encoder was fixed. Components now encode against `.urlPathAllowed` minus `/`, and the built route is assigned through `percentEncodedPath`; both halves are needed, and a test pins each. `?` and `#` were already encoded — they are excluded from `pchar` — and are now asserted so narrowing the set later cannot widen them.
