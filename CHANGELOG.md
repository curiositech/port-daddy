...
### Added
- **Fleet citations are now audited against the repository they claim to describe.** A measured audit of 268 posted fleet findings found 36 citing files that exist on no ref (`csp-validator.ts`, `cli/visibility.py`, `docs/roadmap.md` — each presented as "evidence"). Two leaks let them through: proposal ships' `evidence` arrays were never validated at all, and the reviewer-ship changed-files filter fails open on possibly-truncated file lists. Both now audit path-shaped citations against the repo tree at the PR head: a finding citing a fabricated file is withheld and named as such in the posted comment (never shipped with a one-click issue button), a proposal's fabricated evidence is stripped, and a proposal grounded ONLY in fabricated files is withheld outright. Concepts ("PR description") stay out of scope — judging those is a human task, the same boundary `check-doc-citations` draws for docs — and the audit fails open, loudly, when tree evidence is unavailable. `scripts/fleet-citation-scoreboard.mjs` is the measurement half: a per-ship accuracy table (path-missing, line-past-EOF) computed from the comments the fleet actually posted, so citation reliability is a re-runnable number rather than an impression. <!-- cite-exempt: the backticked paths in this entry are the fleet's fabricated citations, quoted as evidence — they do not exist, which is the point -->
...
### Fixed
- **A purser suite that never ran a test can no longer block a pull request.** Three defects in the purser's own instrument each read as evidence against reviewed PRs. First, its sandbox verdict was exit-code-only, so an authored suite that failed to LOAD — a 13-line sketch with zero test declarations, four suites dead on a module import, an ESM `require` crash — blocked the PR it was reviewing even though no assertion ever touched that PR's code; the runner's outcome now records whether any test actually executed — the structured Jest summary's zero-total count where the marker protocol applies, the runners' own literal zero-count records (jest/vitest, pytest, go test) everywhere else — a zero-test run classifies as a harness failure on ANY exit code, green included, and routes to the broken-ship path (the purser errors, the PR is neither gated nor retargeted) at all three verdict sites. Second, an unfenced model response was accepted as a test file whenever it contained code-looking fragments anywhere, which raw chain-of-thought does by quoting the very imports it is deliberating about — two multi-kilobyte deliberation transcripts were committed as `.test.ts` files this way; the bare-response fallback now also requires the response to BEGIN the way a source file begins. Third, the authoring prompt asked the model to derive its own `..`-depth and said nothing about the CI platform, and both live authored-suite failures were the same off-by-one with an unguarded `xcodebuild` on a Linux runner; the prompt now states the exact segment count for the planned path and requires a probe-and-skip guard around platform-specific binaries.
- **The citation audit's posted note now names every proposal that lost fabricated evidence, and two citation shapes are no longer mis-handled.** Proposals whose fabricated paths were stripped while the proposal survived on real or concept evidence were logged in the transcript but invisible in the rendered note — only fully-withheld proposals appeared; `renderCitationAuditNote` now renders the stripped set too, naming each surviving proposal and its fabricated citations. `bareCitedPath` also strips a dangling bare colon (`src/file.ts:` previously kept the trailing colon and could never match the tree), and a backslash-containing citation (`C:\path\file.ts:10`) is now classified not-a-path — kept, never flagged fabricated — because the evidence this audit checks is the git tree at the PR head, which is POSIX (forward-slash separators only), so a backslash path can never be tree-validated and a "missing" verdict on one would be a false fabrication charge. <!-- cite-exempt: `src/file.ts` and `C:\path\file.ts` are illustrative citation shapes, not repository files -->
