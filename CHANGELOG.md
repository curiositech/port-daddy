...
- **App watcher — the operator's app lanes refresh themselves.** The 3.24 app-watcher LaunchAgent (later retired) polled every three minutes, rebuilt + relaunched the **latest** pair (`pd-console-latest.app`, `FleetBar (dev-latest).app`) whenever `origin/main` moved, and rebuilt the **prod** pair (`pd-console-prod.app`, `FleetBar.app`) after a Homebrew cut. It used a dedicated clone under `~/.port-daddy/app-watch/repo`, never a working checkout; failed SHAs/versions were not retried until they moved again. Historical script paths are intentionally omitted because those entrypoints no longer exist.
