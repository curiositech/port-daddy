...
- Added `scripts/check-pr-requirements.mjs` and CI job `pr-requirements-guard` (`npm run check:pr-requirements`): the machine half of AGENTS.md § Pull Request Operating Procedure. Fails the merge queue when a PR has an empty/boilerplate Summary or Test Plan, or when a visual surface (`core/pd-console/`, `website-v2/`, `fleet-config-ui/`, `public/fleet-ui/`, `public/`, `dashboard/`, `apps/FleetBar/`) changes without a screenshot + a GIF/recording. Escape hatches: `<!-- visual-exempt: … -->` and `<!-- pr-requirements-exempt: … -->`.
