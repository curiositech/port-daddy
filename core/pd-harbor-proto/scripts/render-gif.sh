...
echo "[gif] paletteuse → docs/harbor.gif"
ffmpeg -y -framerate "$FPS" -i "$FRAMES/frame_%03d.png" -i docs/palette.png \
  ...
  -loop 0 docs/harbor.gif >/dev/null 2>&1

# A still for the PR's screenshot slot (mid-wave frame).
cp "$FRAMES/frame_075.png" docs/harbor.png 2>/dev/null || cp "$FRAMES/frame_000.png" docs/harbor.png

echo "[gif] done:"
ls -la docs/harbor.gif docs/harbor.png
