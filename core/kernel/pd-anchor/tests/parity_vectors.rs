...
fn vectors() -> Value {
    let p = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../../tests/fixtures/macaroon-parity-vectors.json");
    serde_json::from_str(&fs::read_to_string(&p).unwrap_or_else(|e| panic!("read {p:?}: {e}"))).unwrap()
}
...
fn first_party_matches_canonical_vector() {
    ...
    let mut m = Macaroon::mint(root, fp["identifier"].as_str().unwrap(), fp["location"].as_str().unwrap());
    for c in fp["caveats"].as_array().unwrap() {
        m = m.add_first_party_caveat(c.as_str().unwrap()).unwrap();
    }
    assert_eq!(m.signature_hex, fp["expected_signature_hex"].as_str().unwrap());
}
...
fn third_party_grant_and_discharge_match_canonical_vector() {
    ...
    assert_eq!(g.macaroon.caveats.last().unwrap().vid.clone().unwrap(), tp["expected_vid_hex"].as_str().unwrap());
    assert_eq!(g.macaroon.signature_hex, tp["expected_signature_hex"].as_str().unwrap());

    let db = &v["discharge_bound"];
    let d = discharge_rent_paid(ckey, &g.rent_caveat_id, RentVerdict::Paid,
        db["discharge_now_ms"].as_i64().unwrap(), db["discharge_ttl_ms"].as_i64().unwrap()).unwrap().unwrap();
    let bound = g.macaroon.prepare_for_request(&d).unwrap();
    assert_eq!(bound.signature_hex, db["expected_signature_hex"].as_str().unwrap());
}
