...
impl ClaimsPane {
    ...
    /// Radar cards sorted by escalation rank — the one ordering both the view
    /// and selection share, so a clicked index always lands on the card the
    /// operator saw.
    fn ranked_trouble(&self) -> Vec<&TroubleEntry> {
        let mut radar: Vec<&TroubleEntry> = self.trouble.iter().collect();
        radar.sort_by_key(|trouble| Self::trouble_rank(&trouble.state));
        radar
    }

    /// Select one bounded claim-tree trouble card. Inspect-only UI state: it
    /// retargets the focused evidence card and never issues coordination.
    /// Distinct from ledger-row selection (`SurfaceAction::SelectRow`), whose
    /// index namespace is the sorted claims ledger, not the radar.
    pub fn select_trouble(&mut self, index: usize) {
        let radar = self.ranked_trouble();
...
impl Pane for ClaimsPane {
    ...
    fn view(&self) -> Vec<Block> {
        ...
        if !self.trouble.is_empty() {
            ...
            blocks.push(Block::KeyVal(
                "legend".into(),
                "you  \u{2192}  claimed surface  \u{2192}  finite-state action".into(),
            ));
            let radar = self.ranked_trouble();
...
mod tests {
    ...
    fn selecting_a_radar_card_retargets_only_the_inspector() {
        ...
        // Selection is a purely local UI mutation; index 1 addresses the
        // rank-sorted radar (COORDINATE first, WATCH second).
        p.select_trouble(1);
