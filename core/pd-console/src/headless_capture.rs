    ...
    for b in blocks {
        match b {
            ...
            Block::Flag { letter, label, tone } => {
                ...
                draw_text(&mut c, x0 + sq / 2 - 4, y + 9, 2, letter.encode_utf8(&mut letter_buf), bg);
                draw_text(&mut c, x0 + sq + 12, y + 9, 2, label, ink2);
                y += ROW_H;
            }
            Block::NodeRow { selected, live, flag, name, badge, badge_tone, meta, age, tone, .. } => {
                ...
                let mut fb = [0u8; 4];
                draw_text(&mut c, x0 + 8 + sq / 2 - 4, y + 10, 2, flag.encode_utf8(&mut fb), bg);
                // liveness dot
                let dot = if *live { to_rgb(t.engaged) } else { to_rgb(t.resting) };
                ...
                c.fill_rect(x0 + inner - bw - 70, y + 8, bw, 16, tone_rgb(*badge_tone, t));
            ...
            Block::Spark(vals) => {
                ...
                for (i, v) in vals.iter().enumerate() {
                    ...
                    let bx = x0 + 8 + i * bw;
                    c.fill_rect(bx, base.saturating_sub(bh), bw.saturating_sub(2).max(1), bh.max(1), accent);
                }
                y += ROW_H + 20;
            }
            Block::ControlButton { verb: _, label, enabled, primary, .. } => {
...
pub fn sample_console_blocks() -> Vec<Block> {
    vec![
        ...
        Block::Row(vec!["MODEL".into(), "TIER".into(), "STATE".into(), "AGE".into()]),
        Block::NodeRow {
            index: 0,
            selected: true,
            live: true,
        ...
        Block::Gap,
        Block::Row(vec!["engaged".into(), "gated".into(), "landed".into(), "alarm".into()]),
        Block::Flag { letter: 'A', label: "underway".into(), tone: Tone::Engaged },
        Block::Chip { label: "claimed".into(), tone: Tone::Accent },
        Block::Chip { label: "conflict".into(), tone: Tone::Conflicted },
        Block::Chip { label: "CRITICAL".into(), tone: Tone::Alarm },
        ...
        Block::TranscriptLine { text: "wrote docs/artifacts/gpui/headless-capture-sample.png".into(), tone: Tone::Landed },
        Block::WrappedText {
...
mod gpui_headless {
    ...
    impl Render for Probe {
        fn render(&mut self, _win: &mut gpui::Window, _cx: &mut gpui::Context<Self>) -> impl IntoElement {
    ...
    fn element_pipeline_runs_with_no_display() {
        ...
        let ok = window.update(&mut cx, |_view, _win, _cx| true).unwrap_or(false);
