...
const TS_KEYWORDS: &[&str] = &[
    "any", "as", "async", "await", "break", "case", "catch", "class", "const", "continue",
    "default", "delete", "do", "else", "enum", "export", "extends", "false", "finally", "for",
    "from", "function", "if", "implements", "import", "in", "instanceof", "interface", "let",
    "new", "null", "of", "private", "public", "readonly", "return", "static", "switch", "this",
...
const TS_TYPES: &[&str] = &["bigint", "boolean", "never", "number", "object", "string", "symbol", "unknown"];
const PY_TYPES: &[&str] = &["bool", "bytes", "dict", "float", "int", "list", "set", "str", "tuple"];
...
pub fn highlight_line(text: &str, lang: Lang) -> Vec<(u32, SyntaxKind)> {
    ...
    while i < bytes.len() {
        ...
        if slash_comments(lang) && b == b'/' && bytes.get(i + 1) == Some(&b'*') {
            let close = text[i + 2..].find("*/").map(|p| i + 2 + p + 2).unwrap_or(bytes.len());
...
mod tests {
    ...
    fn keyword_tables_are_sorted_for_binary_search() {
        for table in [RUST_KEYWORDS, TS_KEYWORDS, PY_KEYWORDS, SHELL_KEYWORDS, RUST_TYPES, TS_TYPES, PY_TYPES] {
            assert!(table.windows(2).all(|w| w[0] < w[1]), "table not sorted: {table:?}");
        }
    }
    ...
    fn unterminated_string_swallows_rest_of_line_but_lifetime_does_not() {
        ...
        assert_eq!(got.last().unwrap(), &("\"open".to_string(), SyntaxKind::Str));
        // A lifetime tick stays plain (no close quote on the line).
        let got = kinds("fn f<'a>(x: &'a str)", Lang::Rust);
        assert!(got.iter().any(|(s, k)| s == "str" && *k == SyntaxKind::Type));
    ...
    fn adjacent_same_kind_runs_merge() {
        ...
        assert_eq!(runs.len(), 1, "punctuation coalesces into one Plain run: {runs:?}");
    }
