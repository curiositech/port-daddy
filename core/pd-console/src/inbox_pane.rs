...
//!
//! Calls `GET /attention?agentId=pd-console&peek=true&limit=30` on the daemon.
...
impl Pane for InboxPane {
    ...
    fn view(&self) -> Vec<Block> {
        ...
        if let Some(err) = &self.last_error {
            blocks.push(Block::KeyVal("note".into(), "GET /attention failed".into()));
        ...
        if self.messages.is_empty() {
            ...
            for msg in &self.messages {
                ...
                let sender = if msg.sender.len() > 16 {
                    &msg.sender[..16]
                } else {
                    &msg.sender
                };
                ...
                let tone = if msg.read {
                    Tone::Resting
                } else {
                    Tone::Engaged
                };
                blocks.push(Block::Row(vec![ts, sender.to_string(), subject_trunc]));
                let _ = tone;
            }
        }
    ...
    fn refresh<'a>(
        ...
        Box::pin(async move {
            let url = format!(
                "{}/attention?agentId=pd-console&peek=true&limit=30",
                daemon.base()
            );
