...
- **Fleet checkpoint continuations need their own queue and honest coordinates**: never bind `FLEET_CONTINUATIONS` back to the serialized `fleet-runs` consumer, because an unrelated review can then starve a logical run between checkpoints. Keep `fleet-continuations` on a separate one-message consumer with bounded retries and the same fail-closed DLQ contract. Preserve the monotonic `sequence * 100 + platformAttempt` cursor only for durable compare-and-swap/deduplication; receipts, APIs, and transcript detail must expose `continuationSequence` and the per-message `platformAttempt` separately and decode legacy cursor-only rows. Test interleaved admission/continuation invocations, duplicate delivery, checkpoint-commit/send interruption, missing bindings before model spend, config parity, and a legacy cursor such as 101 rendering as continuation 1 plus platform attempt 1.
