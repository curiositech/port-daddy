...
#   session-resume.sh --identity myapp:api --purpose "..." --roadmap maintenance --no-claim-files
#
...
[[ -z "$roadmap"  ]] && { echo "--roadmap required"  >&2; exit 1; }
...
# 3. Sitrep through the selected daemon-aware CLI.
sitrep=$(pd sitrep --json 2>/dev/null || echo '{}')

# 4. Start a session if none
if [[ -z "$session_id" ]]; then
  start=$(pd begin "$purpose" --identity "$identity" --lifecycle durable --roadmap "$roadmap" --json 2>/dev/null)
...
if (( claim_files )) && (( ${#files[@]} )); then
  claims=$(pd session files add "${files[@]}" --json 2>/dev/null || echo '[]')
fi
