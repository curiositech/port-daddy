...
- **Before every meaningful task** — run `jury-rig search` (or `pd jury-rig search` when Port Daddy is intentionally available). It returns at most 10 metadata matches and caps every description at 240 characters. Graft a selected skill only when its full guidance is needed, then load only the references the task needs.
