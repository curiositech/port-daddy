...
description: >-
  Distribution pipeline for signed Rust desktop applications -- code signing, notarization, auto-updates, and package
  manager distribution. Activate on: code signing, macOS notarization, Apple Developer, Authenticode, Windows signing,
  auto-updater, Homebrew formula, winget manifest, Scoop bucket, Tauri updater, app distribution, release pipeline. NOT
  for: library crate publishing (use crates.io docs), web deployment (use devops-automator).
...
metadata:
  ...
  io-contract:
    kind: deliverable
    produces:
      - kind: distribution-pipeline
        description: >-
          Complete CI/CD workflow configuration for code signing, notarization, and package manager publishing across
          macOS and Windows
        format: yaml
      - kind: certificate-setup-guide
        description: >-
          Step-by-step instructions for obtaining and configuring code signing certificates (Apple Developer ID, Windows
          EV, Azure Trusted Signing)
        format: markdown
      - kind: auto-updater-implementation
        description: >-
          Tauri updater configuration with signing key generation, manifest JSON structure, and endpoint setup for
          GitHub releases or custom servers
        language: json
        format: json
      - kind: package-manager-manifest
        description: >-
          Homebrew formula, winget manifest, or Scoop bucket configuration for distributing the application across
          package managers
        format: yaml
      - kind: troubleshooting-runbook
        description: >-
          Diagnostic procedures and fixes for notarization failures, SmartScreen warnings, signature mismatches,
          certificate expiry, and cross-platform binary issues
        format: markdown
---
...
- `diagrams/01_flowchart_decision-points.md` — Mermaid flowchart routing requests through scope validation, constraint assessment, and pattern selection. **Read when** unsure whether a request fits this skill or which distribution path to follow.
...
**root**
- [`CHANGELOG.md`](CHANGELOG.md) — Rust App Distribution — Changelog — - Normalized frontmatter into the canonical metadata-based repo shape - Added or refreshed repo-local provenance metadata - Recorded this au
