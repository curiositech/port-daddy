...
description: >-
  Generate images with Google's Nano Banana family (Gemini 2.5 Flash Image, Nano Banana 2, Nano Banana Pro) using
  character and style references. Use when the user mentions "nano banana", "gemini image", any "gemini-*-image*" model
  id, character consistency for image generation, style transfer with Gemini, or multi-reference image composition. NOT
  for Ideogram (use existing Ideogram scripts), Qwen on Apple Silicon (see ~/.claude/QWEN_IMAGE_GUIDE.md), generic
  ...
  io-contract:
    ...
    produces:
      - kind: image-generation-plan
        description: >-
          Model choice, reference-image roles, prompt structure, aspect ratio, and iteration plan for Nano Banana image
          generation
        format: markdown
      - kind: image-asset
        description: Generated or edited image file plus saved prompt and provenance when generation is executed
        format: png|webp|json
---
...
The **relationship instruction** is the structural piece — explicitly label what role each reference plays. Without role labels, the model partly absorbs and partly ignores the reference.
