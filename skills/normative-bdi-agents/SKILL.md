...
metadata:
  ...
  io-contract:
    ...
    produces:
      ...
      - kind: refactor-plan
        description: >-
          Guidance for integrating norms as desires within existing BDI frameworks, eliminating separate norm-following
          pathways, and extending consequence evaluation depth
        format: markdown
...
- `references/desire-internalization-as-norm-adoption-mechanism.md` — Explains why norms should be added as desires rather than beliefs or intentions. **Read when** deciding how to represent adopted norms in agent mental state.
...
- `references/normative-conflict-resolution-through-consequence-ranking.md` — Details consequence-based ranking for choosing between conflicting norm sets. **Read when** evaluating which maximal subset to execute.
- `references/separation-of-norm-recognition-and-norm-internalization.md` — Distinguishes detecting norms (ANB) from adopting them (NIB). **Read when** designing norm detection vs. commitment mechanisms.
- `references/three-types-of-consistency-for-norm-adoption.md` — Defines strong/weak/no consistency checks for norm compatibility. **Read when** evaluating whether a new norm can be safely adopted.
