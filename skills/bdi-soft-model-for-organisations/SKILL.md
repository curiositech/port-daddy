---
name: bdi-soft-model-for-organisations
description: >-
  Model organizations as BDI systems with filtered perception, discourse-shaped desires, accommodations, and
  abstraction-level hierarchy. Use for socio-technical coordination and organizational modeling. NOT for mechanistic
  workflows, individual psychology, or non-social domains.
license: Apache-2.0
...
metadata:
  ...
  io-contract:
    ...
    produces:
      - kind: design-doc
        description: >-
          BDI organizational model mapping beliefs (filtered perception by role), desires (discourse-shaped goals),
          intentions (commitments that bound deliberation), and abstraction-level hierarchy for a specific
          socio-technical coordination challenge
        format: markdown
      - kind: diagram
        description: >-
          Mermaid flowchart or entity diagram showing agent roles, perception filters, desire-formation negotiation
          points, and accommodation patterns that permit action despite persistent disagreement
        format: svg
      - kind: critique
        description: >-
          Analysis of failure modes in the current organizational model (consensus assumption, objective world-state
          assumption, desires as fixed inputs, single-level blindness, system-as-organization thinking) with specific
          fixes
        format: markdown
      - kind: refactor-plan
        description: >-
          Concrete steps to reframe a socio-technical design or process as cognitive support for organizational
          reasoning rather than replacement of human deliberation
        format: markdown
---
...
- `references/perception-through-filters-no-objective-observation.md` — Explains why agents perceive through cognitive filters tied to role, not objective truth. **Read when** modeling how different organizational roles see the same situation differently.

- `references/discourse-to-action-emergence-of-organizational-desires.md` — Shows how organizational goals emerge through conversation rather than being fixed inputs. **Read when** analyzing how desires form and shift during negotiation.

- `references/intention-as-commitment-bounds-deliberation.md` — Describes how commitments limit reconsideration and enable action despite uncertainty. **Read when** modeling why organizations stick to decisions or struggle to pivot.

- `references/accommodations-over-consensus-coordination-without-agreement.md` — Argues organizations coordinate through accommodation, not consensus. **Read when** designing for persistent disagreement or misaligned incentives.

- `references/hierarchies-abstraction-levels-expertise-coordination.md` — Maps how specialized knowledge at different abstraction levels must coordinate. **Read when** choosing modeling level or explaining expertise silos.

- `references/soft-systems-cognitive-gap-formal-modeling.md` — Identifies why SSM/POM models describe but don't explain internal processes. **Read when** deciding whether BDI adds explanatory power over soft systems alone.
