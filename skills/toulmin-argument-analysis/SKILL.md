...
description: >-
  Structure and analyze arguments using the Toulmin model (Claim-Data-Warrant-Backing-Qualifier-Rebuttal). Use when the
  user wants to build logical arguments, evaluate reasoning, prepare formal debate, clarify messy reasoning, write
  persuasive pieces, or mentions 'argument structure', 'warrant', 'backing', 'logical framework', 'structure this
  argument'. Useful as input for speech-writing, fallacy detection, and steelmanning.
metadata:
  io-contract:
    kind: deliverable
    produces:
      - kind: critique
        description: >-
          Toulmin structural analysis of an argument, identifying and evaluating each of the six components (Claim,
          Data, Warrant, Backing, Qualifier, Rebuttal) with explicit reasoning about logical strength and gaps
        format: markdown
      - kind: refactor-plan
        description: >-
          Recommendations to strengthen the argument by surfacing hidden warrants, adding backing, qualifying claims
          appropriately, or anticipating rebuttals
        format: markdown
      - kind: design-doc
        description: >-
          Structured argument blueprint for building a new logical case from scratch, mapping all six Toulmin elements
          before drafting persuasive prose
        format: markdown
license: Apache-2.0
...
- `diagrams/01_flowchart_decision-points.md` — Mermaid flowchart routing incoming requests through scope-check, path selection (plan/design/debug), and quality gates. **Read when** deciding which Toulmin analysis path to take or validating your approach.

- `references/theory/toulmin-model.md` — Deep historical background on Stephen Toulmin, critiques of formal logic, and comparative analysis with other argument frameworks. **Read when** you need to explain why Toulmin differs from syllogistic logic or want scholarly grounding.

- `references/examples/relationship-boundary-examples.md` — Fully worked Toulmin analyses of personal and interpersonal disputes, showing how to surface unstated warrants and relationship norms. **Read when** analyzing arguments about boundaries, communication, or relationship conflicts.
