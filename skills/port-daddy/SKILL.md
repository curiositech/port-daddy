...
metadata:
  ...
  provenance:
    kind: first-party
    owners: [workgroup-ai, port-daddy]
  authorship:
...
io-contract:
  ...
  inputs:
    - name: command
      type: string
      required: true
      description: "CLI command (status, briefing, salvage, begin, advise, note, session, claim, guard, pub, sub, watch, lock, unlock, with-lock, fleet, sortie, spawn, agent, tuple, pheromone, ideas, arbiter)"
    - name: args
      type: array
      required: false
      description: "Command-specific arguments (project, task, path, identity, etc.)"
    - name: flags
      type: object
      required: false
      description: "Optional flags like --project, --identity, --purpose, --scope, --ttl"
  outputs:
    - name: result
      type: object
      description: "Command output: session_id, port, claims, notes, status, fleet_status, tuple_data, etc."
    - name: error
      type: string
      description: "Error message if command fails"
---
...
For skill edits, treat this bundle as a release surface. Update this
workgroup skill, the repo `skills/port-daddy-cli/` mirror, references, tests,
and changelogs together, then mirror the validated result to the user-level
installed copy when filesystem permissions allow.
...
## Background Agent Commits (Non-TTY / Headless Context)

**Problem:** The compiled `pd` Bun binary hangs on ALL network calls (unix socket and TCP) when
run from a non-TTY background process. This makes `pd guard check --staged --hook` fail or hang
in every background Claude Code session. Additionally, `pd` 3.19.x Homebrew installs can ship
only the 33KB launcher without the actual Bun binary next to it — fix with:
```bash
ln -sf ~/coding/port-daddy/dist/port-daddy \
  /opt/homebrew/Cellar/port-daddy/$(pd --version)/bin/port-daddy
```

**The correct pattern for background agents in repos with Coordination Guard (enforce):**

**Step 1 — Register session + claim files via HTTP API** (curl works fine in non-TTY):
```bash
DAEMON=http://localhost:9876
SESSION_ID="session-$(date +%s)-bg"

# Register session
curl -s -X POST $DAEMON/sessions \
  -H "Content-Type: application/json" \
  -d "{\"sessionId\":\"$SESSION_ID\",\"agentId\":\"claude-code-bg\",\"purpose\":\"task description\"}"

# Claim staged files in batches (50 per request max)
...
for i in range(0, len(files), 50):
    ...
    req = urllib.request.Request(
        f'http://localhost:9876/sessions/{os.environ[\"SESSION_ID\"]}/files',
        data=json.dumps({'files': batch}).encode(),
        headers={'Content-Type': 'application/json'},
        method='POST')
    urllib.request.urlopen(req)
    print(f'Claimed {len(batch)} files')
" SESSION_ID="$SESSION_ID"

# Write context file so guard can find the session
mkdir -p .portdaddy
echo "{\"agentId\":\"claude-code-bg\",\"sessionId\":\"$SESSION_ID\",\"purpose\":\"task description\"}" \
  > .portdaddy/current.json
```

**Step 2 — Commit**: Two authorized options:

*Option A (preferred): Leave staged, have user commit from their terminal.*
Tell the user: `git commit -m "..."` — the guard works normally in an interactive shell.

*Option B (when user explicitly authorizes HUSKY bypass):*
```bash
HUSKY=0 git commit -m "your message"
```
Only use Option B when the session + claims are properly registered via the HTTP API above.
`HUSKY=0` bypasses all hooks — the coordination evidence must already exist in Port Daddy.

**Step 3 — End session:**
```bash
curl -s -X PUT $DAEMON/sessions/$SESSION_ID \
  -H "Content-Type: application/json" \
  -d '{"phase":"done","outcome":"committed"}'
```
