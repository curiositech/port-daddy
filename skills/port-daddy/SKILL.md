...
**Pre-loaded tools (available immediately):**
...
| `release_port` | Release a claimed port (supports wildcards like `myapp:*`) |
| `list_services` | List all claimed services with ports, status, and metadata |
...
| `pd_discover` | Find additional tools by category (see below) |

**Discoverable tools (call `pd_discover` first, then use by name):**

Tuple space, pheromone trails, messaging, inbox, DNS, tunnels, webhooks, and more are available but must be discovered first. Call `pd_discover` with a category to load them:

| Category | What You Get |
|----------|-------------|
| `tuples` | `tuple_out`, `tuple_read`, `tuple_take`, `tuple_scan`, `tuple_count` — shared swarm memory |
| `pheromone` | `file_heat`, `pheromone_spray`, `pheromone_sniff` — ambient signals and file contention maps |
| `messaging` | Pub/sub channels, direct messaging between agents |
| `inbox` | `talk_to_agent` — send direct messages to specific fleet agents |
| `dns` | Service identity resolution |
| `tunnels` | Expose local services via ngrok/cloudflared/localtunnel |
| `webhooks` | Register webhooks for external system integration |
| `integration` | `integration ready` / `integration needs` — service dependency coordination |
| `briefing` | Generate project briefing snapshots |
| `changelog` | Track changes per semantic identity |

All categories: `session-lifecycle`, `ports`, `sessions`, `notes`, `locks`, `messaging`, `agents`, `inbox`, `webhooks`, `integration`, `dns`, `briefing`, `tunnels`, `projects`, `changelog`, `activity`, `system`, `tuples`, `pheromone`
...
| Problem | Solution |
|---------|----------|
| Dev server port conflict | `claim_port` MCP tool or `pd claim myapp:api -q` |
| List what's running | `list_services` MCP tool or `pd find '*'` |
| Release ports when done | `release_port` MCP tool or `pd release myapp:api` |
| Need to coordinate with other agents | `begin_session` + `pd session files claim` |
| Agent-to-agent signaling | `pd pub` + `pd watch` (discover via `pd_discover messaging`) |
| Direct message to a specific agent | Discover `talk_to_agent` via `pd_discover inbox` |
| Background automation | `fleet_init` MCP tool + `pd fleet up` |
| Share knowledge across agents | Discover tuple tools via `pd_discover tuples` |
| Track "hotness" of resources | Discover pheromone tools via `pd_discover pheromone` |
| See file contention at a glance | Discover `file_heat` via `pd_discover pheromone` |
| Crashed agent left work behind | `pd salvage` or check `salvageHint` from `begin_session` |
| Exclusive operations (deploys, migrations) | `acquire_lock` MCP tool or `pd with-lock` |
