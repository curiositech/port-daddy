...
**`beautiful-gui-design` — the VISUAL SYSTEM (Layer 1).** Owns hierarchy, color, type, spacing, the three-tier token model, light/dark, WCAG contrast, accessibility, component states. **Core, not polish:** the maritime OKLCH semantic tokens in `palette.rs`/`theme.rs` and the `Tone` enum (pane.rs:16) *are* this skill's three-tier model already realized. The conflict-band UX, the claim chips, the authorship-gutter colors all obey its rules: color is semantic tokens never raw hex, body ≥14px (the hard font-floor canon), every interactive control has default/hover/active/focus/disabled. The skill's "Invisible in Light Mode" and "Rainbow Vomit" anti-patterns gate every status color you add for M agents.
...
## 7. Structural vs polish (the honest cut)
...
**FOUNDATIONAL — the product is a lie without these:**
