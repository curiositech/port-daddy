...
io-contract:
  ...
  produces:
    - kind: design-doc
      description: >-
        Onboarding flow architecture with decision trees, stage sequencing, permission timing strategy, and crisis
        detection integration specific to recovery/wellness context
      format: markdown
    - kind: critique
      description: >-
        Assessment of existing onboarding flows against recovery-specific failure modes (rubber stamp, crisis overwhelm,
        permission walls, feature FOMO, privacy anxiety) with remediation guidance
      format: markdown
    - kind: refactor-plan
      description: >-
        Step-by-step transformation plan to convert generic onboarding into compassionate recovery-app flow, including
        user state branching, progressive disclosure, and accessibility gates
      format: markdown
    - kind: code
      description: >-
        Decision tree logic, permission request sequencing, and crisis path validation implementations for onboarding
        state management
...
- [assets/example_asset.txt](assets/example_asset.txt) — Example Asset File.
- [references/api_reference.md](references/api_reference.md) — Reference Documentation for Recovery App Onboarding.
- [scripts/example.py](scripts/example.py) — example.py.
