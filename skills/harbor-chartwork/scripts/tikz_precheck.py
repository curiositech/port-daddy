...
# P15-P17 were RESERVED for claude/figures-that-were-missing (PR #10190)
# while this branch's typographic-law rules took P18-P25 instead (see the
# CHANGELOG entry for the full story). That reservation is discharged as of
# this merge: P15 (caption-promise), P16 (identifier-consistency) and P17
# (caption-vocabulary) are implemented below, alongside P18-P25. RULE_IDS
# above is the single source of truth for what is implemented. Kept as an
# empty dict, not deleted, so TestRuleIdRegistry keeps checking the shape of
# a future reservation rather than needing its own rewrite the next time one
# happens.
RESERVED_RULE_IDS = {}
