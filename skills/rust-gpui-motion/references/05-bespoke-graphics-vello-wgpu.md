...
- **Embedded surface (path 2):** the viz lives **in the pane tree** next to text panes. Choose this when the viz must be *ambient and persistent* — the ghost filetree, merge-as-light, the biofield. Cost: sharing the `wgpu` device with gpui's renderer (or compositing an offscreen Vello texture into a gpui quad). This is the essential v12 bet; do not pretend it's free.
