...
description: >-
  Foundational impossibility result proving consensus cannot be guaranteed in asynchronous systems with even one faulty
  process
metadata:
  ...
  io-contract:
    ...
    covers:
      ...
      - distributed systems coordination trade-offs
      - failure detection and timing assumptions
      - consensus protocol design patterns
...
- `references/asynchrony-and-impossibility-boundaries.md` — Foundational explanation of FLP theorem: no consensus protocol guarantees termination in asynchronous systems with one faulty process. **Read when** understanding why pure asynchrony + consensus is impossible.

- `references/bivalence-and-decision-commitment.md` — Defines bivalent configurations (states where either decision value is reachable) and their role in the impossibility proof. **Read when** analyzing why a system cannot escape uncommitted states.

- `references/failure-indistinguishability-problem.md` — Core insight: asynchronous systems cannot distinguish crashed processes from slow ones without timing assumptions. **Read when** diagnosing why failure detection requires timeouts.

- `references/initially-dead-processes-solution.md` — Constructive positive result: consensus IS possible if all failures occur before execution (static failure set). **Read when** exploring when consensus becomes tractable.

- `references/message-delivery-and-commutativity.md` — Formal model of message passing and nondeterministic delivery; explains why message ordering matters. **Read when** understanding the proof's assumptions about communication.

- `references/synchrony-assumption-trade-offs.md` — Explains the fundamental trade-off: guaranteed termination requires relaxing pure asynchrony (adding timeouts/heartbeats). **Read when** choosing between consistency, availability, and timing assumptions.

- `references/testing-impossibility-boundaries.md` — Testing strategy for systems near impossibility boundaries: validate assumptions hold and failures degrade gracefully. **Read when** designing tests for consensus protocols or failure detectors.
