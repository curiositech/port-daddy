...
flowchart TD
  ...
  G -->|"User sees own rows"| H["CREATE POLICY ... FOR SELECT TO authenticated<br/>USING ((select auth.uid()) = user_id)"]
  F --> I{INSERT validation?}
  I -->|"User can only create own row"| J["CREATE POLICY ... FOR INSERT TO authenticated<br/>WITH CHECK ((select auth.uid()) = user_id)"]
