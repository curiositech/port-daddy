...
| `accent` / `bg` | `palette.rs` roles (the `accent`/`bg` roles as defined in `core/pd-console/src/theme.rs` (theme-specific; do not copy hexes from docs), `bg: 0xf5f5f0`) | convert `Hsla`/`rgb` → **linear** `[f32;4]`; re-push on theme flip (`Ctrl-A g`) |
