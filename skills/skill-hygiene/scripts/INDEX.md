...
| `install_hooks.sh` | You want the pre-commit guard installed locally so every commit that touches a skill bundle is auto-audited. Idempotent — safe to re-run. Works in linked git worktrees (installs into the shared common hooks dir). |
