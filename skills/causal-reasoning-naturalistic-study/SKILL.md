---
name: causal-reasoning-naturalistic-study
description: >-
  Naturalistic causal reasoning for failures and disputed attribution. Use when postmortems or root-cause debates need
  reframing. NOT for causal inference, fault trees, or fully observed failures.
license: Apache-2.0
allowed-tools:
  - Read
  - Grep
...
metadata:
  ...
  io-contract:
    kind: deliverable
    produces:
      - kind: causal-analysis
        description: >-
          A structured causal narrative identifying the explanatory frame (event, condition, story, list, or
          abstraction), the partial-cause set with propensity/reversibility/covariation judgments, and explicit
          uncertainty boundaries.
        format: markdown
      - kind: frame-critique
        description: >-
          An evaluation of competing causal frames used by stakeholders, showing how each frame determines which causes
          are visible and which are missed.
        format: markdown
      - kind: action-cause
        description: >-
          A compressed, actionable single cause or minimal cause-set suitable for immediate decision-making, with
          explicit documentation of what complexity remains unresolved.
        format: markdown
      - kind: diagnostic-probe-plan
        description: >-
          A prioritized list of evidence gaps and targeted questions to resolve indeterminacy, organized by which
          frame-choice or attribution criterion each probe would clarify.
        format: markdown
---
...
- `diagrams/01_flowchart_decision-points.md` — Decision tree for selecting explanatory frame (event, condition, story, list, abstraction). **Read when** choosing which causal form fits the failure.
- `references/five-forms-of-causal-explanation.md` — Taxonomy of event, condition, story, list, and abstraction explanations with strengths/failure modes. **Read when** building or evaluating a causal explanation.
- `references/reciprocal-framing-and-causal-search.md` — How explanatory frames and identified causes co-construct each other in a loop. **Read when** understanding why different stakeholders see different causes.
- `references/domain-expertise-and-causal-frame-selection.md` — Why domain knowledge is critical for identifying causes but not for coding them into frames. **Read when** assessing whether an agent has enough context.
- `references/criteria-for-causal-attribution.md` — Propensity, reversibility, and covariation as three evaluation lenses for causal claims. **Read when** judging whether a proposed cause is credible.
- `references/three-causal-criteria-for-agent-diagnosis.md` — Detailed explanation of propensity, reversibility, covariation with examples. **Read when** applying criteria to evaluate competing causes.
- `references/indeterminate-vs-determinate-problems.md` — Distinction between problems with single discoverable answers vs. inherently multiple valid explanations. **Read when** deciding if closure is possible or if multiple frames must coexist.
- `references/indeterminate-causation-and-partial-explanations.md` — How to reason about causes in indeterminate domains where no single answer exists. **Read when** facing contested attribution with no ground truth.
- `references/causal-stories-as-system-models.md` — Why story explanations model mechanism and system interaction, not just correlation. **Read when** building mechanistic narratives for complex failures.
- `references/causal-story-construction-and-mechanism.md` — How to construct and evaluate story explanations as mechanism maps. **Read when** designing a narrative explanation of interacting causes.
- `references/closure-and-action-in-causal-reasoning.md` — The stopping problem: when explanation is complete enough to act. **Read when** determining if analysis should continue or decision should be made.
- `references/closure-decisions-and-the-action-understanding-tradeoff.md` — Tradeoff between depth of understanding and time to action in causal closure. **Read when** balancing investigation completeness against urgency.
- `references/reductive-tendency-and-when-to-resist-it.md` — Systematic simplification patterns that distort causal reasoning (chopping, sequencing, linearizing). **Read when** detecting oversimplification in a proposed explanation.
- `references/the-reductive-tendency-and-agent-simplification.md` — How compression necessary for action introduces predictable epistemic distortions. **Read when** auditing an agent's causal reasoning for hidden assumptions.
- `references/domain-calibrated-causal-reasoning.md` — Why sports, economics, politics, military analysis systematically prefer different explanation forms. **Read when** adapting reasoning to a new domain.
- `references/coincidence-correlation-and-causal-initiation.md` — Rehabilitating correlation as investigative evidence in indeterminate domains. **Read when** evaluating whether co-occurrence counts as causal evidence.
