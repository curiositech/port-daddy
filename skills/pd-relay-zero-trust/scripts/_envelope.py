"""Shared script-io envelope helpers.

All scripts in this skill read a Request envelope from stdin and write a
Response envelope to stdout. See schemas/script-io.schema.json.
"""
...
def write_ok(result: dict[str, Any], trace_id: str | None = None) -> None:
    out = {"kind": "response", "version": VERSION, "ok": True, "result": result}
    if trace_id:
...
def write_error(code: str, message: str, hint: str | None = None,
    ...
    if hint:
        err["hint"] = hint
    out: dict[str, Any] = {
        "kind": "response", "version": VERSION, "ok": False, "error": err,
    }
    if trace_id:
        out["trace_id"] = trace_id
    json.dump(out, sys.stdout, separators=(",", ":"), sort_keys=True)
...
def run(handler: Callable[[dict[str, Any]], dict[str, Any]],
    ...
    if "--selftest" in sys.argv[1:]:
        # Subclasses provide selftest; default is no-op success.
...
def canonical_json(obj: Any) -> str:
    """RFC 8785-ish canonical JSON. Stdlib-only; sort keys, no spaces."""
