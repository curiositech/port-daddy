...
export function audit(plan){
  ...
  const prior=new Set([...plan.positions.map(x=>x.authorId),...plan.reviews.map(x=>x.reviewerId),...plan.corrections.map(x=>x.authorId),...managers.map(x=>x.participantId)]);const beats=new Set(),beatAuthors=new Set();const beatRoles={ENGINEERING:"ENGINEERING_BEAT",PRODUCT:"PRODUCT_BEAT",DESIGN:"DESIGN_BEAT"};
  for(const[i,bx]of(plan.realityBeats??[]).entries()){exact(bx,BEAT,`$.realityBeats[${i}]`,out);add(out,beats.has(bx.beat),"reality-beat-duplicate",`$.realityBeats[${i}].beat`);beats.add(bx.beat);add(out,prior.has(bx.authorId),"reality-beat-not-fresh",`$.realityBeats[${i}].authorId`);add(out,participants.get(bx.authorId)?.role!==beatRoles[bx.beat],"reality-beat-role-invalid",`$.realityBeats[${i}].authorId`);add(out,beatAuthors.has(bx.authorId),"reality-beat-author-duplicate",`$.realityBeats[${i}].authorId`);beatAuthors.add(bx.authorId);add(out,bx.packetSha256!==plan.packet.packetSha256,"packet-digest-mismatch",`$.realityBeats[${i}].packetSha256`);}
