...
description: >-
  Plan and run manager-led, role-based teams where assignments change by round, roles may be added midstream, and the
  manager decides when the work is ready to ship. Use when delegation should adapt to emerging evidence rather than a
  fixed review loop. NOT for static approval chains, uncoordinated message swarms, or blackboard-style shared-state
  diagnosis.
license: Apache-2.0
  ...
  io-contract:
    ...
    produces:
      - kind: orchestration-plan
        description: >-
          Manager-authored role catalog, ship condition, and round-by-round assignment decisions with evidence summaries
          and role activation/retirement rationale
        format: markdown
      - kind: team-execution-log
        description: >-
          Timestamped record of each round: active roles, worker outputs, manager decisions, and evidence that triggered
          role changes or ship readiness assessment
        format: markdown
      - kind: final-artifact-synthesis
        description: Consolidated deliverable(s) produced by the team across all rounds, with manager sign-off on ship readiness
        format: varies by objective
---
