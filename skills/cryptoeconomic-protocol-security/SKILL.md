...
description: >-
  Analyze bonded agent systems, escrow workflows, slashing rules, oracle trust, Sybil cost, and front-running risk in
  protocols where agents post bonds or stake reputation. Use for collateral sizing, griefing analysis, dispute design,
  and bonded-agent security reviews. NOT for smart contract auditing, DeFi tokenomics, blockchain consensus design, or
  trading strategy.
license: Apache-2.0
metadata:
  tags:
  ...
  io-contract:
    ...
    produces:
      - kind: critique
        description: >-
          Attack-class vulnerability assessment identifying which of the five bonded-agent attack classes
          (undercollateralization, griefing, oracle manipulation, Sybil cost, front-running) apply to the protocol and
          severity of each
        format: markdown
      - kind: design-doc
        description: >-
          Collateral sizing and bond-level recommendations with decision-tree rationale for structural, economic, and
          social defenses
        format: markdown
      - kind: refactor-plan
        description: >-
          Ordered list of protocol changes to close identified attack vectors, prioritized by exploitability and
          implementation cost
        format: markdown
      - kind: diagram
        description: >-
          Attack surface map showing agent roles, escrow flows, slashing triggers, and oracle decision points with
          threat annotations
        format: svg
...
- `diagrams/01_flowchart_decision-points.md` — Mermaid flowchart routing incoming requests to scope boundaries and attack-class assessment paths. **Read when** you need to triage a protocol review or decide which attack class to focus on first.
- `references/float-plan-escrow-analysis.md` — Escrow workflow template for manifest-based work (bond posted before execution, oracle acceptance decision, salvage triggers). **Read when** analyzing a bonded-agent system with pre-published task specs and requester approval gates.
...
**Skill Evaluations:** See [`evals/evals.json`](evals/evals.json) for benchmark prompts and expected outputs used to validate this skill on bonded-agent security reviews.
