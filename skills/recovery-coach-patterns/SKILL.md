...
io-contract:
  ...
  produces:
    - kind: code
      description: >-
        TypeScript/Next.js components, API routes, and database queries following Recovery Coach security patterns
        (auth, RLS, rate limiting, PHI audit)
      language: typescript
      format: typescript
    - kind: architecture-decision
      description: >-
        Component type selection (Server vs Client), API security level classification, and database query pattern
        recommendations based on data sensitivity
      format: markdown
    - kind: anti-pattern-detection
      description: >-
        Identification and remediation of common Recovery Coach codebase violations (RLS bypass, missing rate limits,
        PHI leaks, validation skips, unhandled auth)
      format: markdown
    - kind: quality-checklist
      description: >-
        Pre-submit verification covering authentication, rate limiting, validation, RLS, PHI logging, error handling,
        type safety, testing, HIPAA compliance, and accessibility
      format: markdown
---
