...
8. **Persist intent:** durable dispatch intent precedes transmission. Unknown
   transmission becomes `AMBIGUOUS`; it cannot retry, release capacity, or reuse
   the key.
...
11. **Cancel as protocol:** request, acknowledgement deadline, fence,
    ...
    are separate facts.
12. **Settle then replay:** every attempt ends `SETTLED`, `RELEASED`, or
    `HELD_AMBIGUOUS` before reuse. Replay reconstructs projections with effects
    denied.
...
- `STATIC_VALID` proves contract consistency only, never runtime readiness.
