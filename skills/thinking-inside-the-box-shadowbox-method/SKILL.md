---
name: thinking-inside-the-box-shadowbox-method
...
metadata:
  ...
  io-contract:
    kind: deliverable
    produces:
      - kind: training-system-design
        description: >-
          Framework for calibration-based cognitive skill development using expert panel comparison, constraint-driven
          reasoning, and multi-dimensional assessment without ground truth
        format: markdown
      - kind: evaluation-rubric
        description: >-
          Multi-dimensional assessment criteria across Attention, Action Priority, Information, Anticipation,
          Assessment, and Monitoring dimensions to diagnose expertise gaps
        format: markdown
      - kind: agent-calibration-protocol
        description: >-
          Methodology for agents to log reasoning divergences against expert corpora, use constraint-forcing functions,
          and learn from ensemble consensus patterns
        format: markdown
      - kind: scenario-design-guide
        description: >-
          Approach for structuring progressive-revelation scenarios with irrevocable commitment checkpoints and expert
          panel consensus feedback loops
        format: markdown
---
...
- `references/calibration-driven-learning-for-agent-systems.md` — How agents learn by comparing their reasoning to expert patterns and discovering systematic gaps. **Read when** designing self-supervised learning systems or building agent evaluation frameworks.

- `references/constraint-as-cognitive-forcing-function.md` — Why tight constraints (one-inch box) force clearer thinking and prevent surface-level responses. **Read when** designing training scenarios or implementing reasoning capture mechanisms.

- `references/expert-consensus-without-ground-truth.md` — How to extract learning signal from expert disagreement when deterministic correctness doesn't exist. **Read when** building judgment training in ambiguous domains (architecture, security, strategy).

- `references/facilitation-free-scalable-expertise-transfer.md` — How ShadowBox enables expertise transfer at scale without expert presence. **Read when** planning scalable training systems or reducing expert bottlenecks.

- `references/multi-dimensional-cognitive-assessment.md` — Box types (Attention, Action Priority, Information, Anticipation, Assessment, Monitoring) that diagnose specific expertise gaps. **Read when** creating evaluation rubrics or designing multi-dimensional assessment.

- `references/progressive-revelation-and-commitment.md` — Why forward-only information flow and irrevocable commitment mirror real decision-making. **Read when** structuring scenario sequences or implementing checkpoint-based learning.

- `references/scenario-based-expertise-transfer.md` — Why context-rich cases outperform abstract principles for developing genuine expertise. **Read when** designing training scenarios or case libraries.
