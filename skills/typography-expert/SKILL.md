...
Typography is a project decision, not a fashionable slot to fill. Before naming
a typeface, extract 3-5 specific visual adjectives from the product, inspect its
existing materials and neighboring products, then render the real words and
data in at least three materially different candidates. Compare silhouette,
terminals, rhythm, x-height, numerals, axes, language coverage, license, and
delivery cost.

Check recent project choices. A display face used elsewhere in the last 90 days
is in cooldown unless the user explicitly wants cross-product consistency.

**Overuse blacklist (opt-in only):** Fraunces, Bricolage Grotesque, Playfair
Display, DM Serif Display, Poppins, Montserrat, and Space Grotesk. They are not
inherently bad; they are forbidden as unprompted defaults because repeated use
has made them AI-design tells. Use one only when the user names it or a rendered
comparison proves it uniquely fits the brief.

Never select from a trend list or specimen pangram alone. Record the rejected
candidates and why they lost so the next project does not restart from the same
house default.
...
**Voice-to-structure decision tree (shortlist, never automatic selection):**
```
IF technical/operational → sharp grotesque or engineered variable sans; test numerals first
IF editorial/narrative → compare quiet, sharp, and expressive serifs on real copy
IF signal/poster graphics → variable family with meaningful width/casual/slant axes
IF long-form reading → screen-tuned text face with optical sizing and strong italics
IF dense data UI → high x-height sans + mono with tabular figures and slashed zero
```

Contemporary candidate pool (rotate; do not treat as defaults): Geist, Recursive,
Martian Grotesk, Instrument Serif, Newsreader, Source Serif 4, IBM Plex, Switzer,
PP Neue Montreal, Söhne, and a project-specific licensed foundry candidate.
...
| Casual | CASL | 0-1 | Recursive: rational to expressive without changing family |
| Mono | MONO | 0-1 | Recursive: proportional to monospaced |
