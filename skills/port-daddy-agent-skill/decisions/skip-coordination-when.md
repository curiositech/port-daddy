...
The default rule in this repo is: `pd briefing` + `pd begin` + claim + note + done. The cost of running them is ~5 seconds. Skipping is rarely worth it. But here are the genuine exceptions.
...
1. Your shell lost its session anchor (likely due to cwd mismatch). Run `pd begin` here.
