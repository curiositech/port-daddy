...
Every write body includes `projectDir`, `actorId`, and non-empty `citations`.
Optional provenance (`sessionId`, `runId`, and model/harness/worktree metadata)
should be supplied when available. An otherwise valid record without a citation
is rejected.
...
a matched factual control and matched treatment run. Prompt-only or unmatched
...
outcome attribution. Contested and deprecated doctrine remain history, but are
not returned as active orders.
