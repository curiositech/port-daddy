...
|---|---|---|---|---|
| `I/fig:stack-map` | Where does legibility sit in the system, and what depends on it? | stratified stack with one vertical dependency direction | four named layers; interface boundaries; upward/downward dependency | decorative tower; radial stack; prose boxes |
...
| `I/fig:abdication` | How does operator awareness decay with autonomy, and what do canaries restore? | line or step plot with aligned role stations and intervention resets | authority on x-axis; awareness on y-axis; unaided baseline; role transitions; canary impulses and recovery | role boxes beneath an unrelated curve; unlabeled decorative slope |
| `I/fig:readpoverty` | When does accumulated state outrun human reading capacity? | aligned cumulative curves with shaded capacity gap | common time/work axis; state growth; readable throughput; backlog/gap; intervention point if claimed | funnel metaphor; unscaled piles; prose-only warning |
...
| Atlas ID | Reader question / claim | First-choice grammar | Must encode | Reject |
|---|---|---|---|---|
| `II/fig:swk-stack-map` | Where does the kernel sit among the seven-paper layers? | stratified stack with highlighted current layer | all layers; current layer; adjacent interfaces; one reading direction | ornamental tower; perspective architecture |
...
| Atlas ID | Reader question / claim | First-choice grammar | Must encode | Reject |
|---|---|---|---|---|
| `III/fig:stack` | Where does personhood/continuity sit in the seven-volume stack? | stratified stack with current layer highlighted | layers; current layer; dependency direction | decorative body/tower; chapter art |
...
| `III/fig:rate-raters` | How is evaluator recursion bounded and audited? | two-tier audit loop with explicit stopping rule | evaluator; evaluator-of-evaluator; sampled re-audit; termination/budget; public outcome | infinite spiral; self-looping bubbles |
...
| Atlas ID | Reader question / claim | First-choice grammar | Must encode | Reject |
|---|---|---|---|---|
| `IV/fig:he-stack-map` | Where does the economy sit in the seven-volume stack? | stratified stack with current layer highlighted | layers; current layer; dependency direction | decorative market arena; chapter art |
