...
| `I/fig:ls-cooperation-modes` | As write authority moves from person to agent, what must the person keep watching to stay the operator rather than the audience? | five-panel small multiples ordered by authority handoff, filled vs. hollow circles for who holds the pen, an ordinal cost bar under each | Solo with staff, Pair with agent, Ensemble, Nightshift, and Harbor co-edit panels sharing one baseline grid; filled circle for write authority vs. hollow circle for observation only; each panel's "sees:" text; the claim-gate state in Harbor co-edit; the ordinal (not measured) rank 1--5 | a single authority dial; a org chart of the five modes; a measured cost axis where only an ordinal rank exists |
...
| `II/fig:swk-taint-automaton` | Does the supervisor ever refuse the uncontrollable read, or only the controllable egress once tainted? | two-state automaton, states relabelled from Schneider's own Figure 1 | `ok` and `tainted` states; the `in_context_read` transition marked uncontrollable and never refused; `net_egress` self-loops on both states, allowed at `ok` and disabled at `tainted`; the gate-the-channel-not-the-token framing | a bouncer illustration checking every token; a flowchart that gates the read instead of the egress; a state diagram without the controllability labels |
...
| `III/fig:stp-resurrection-gates` | Which three gates must a migrating reputation clear, and what does dropping each one buy an attacker? | actor-to-actor chain with three named gates, each with its own dropped-gate attack panel below it | predecessor and successor actors; the lineage-witness, engine-attestation, and escrow-closure gates in order; the shortest attack under each dropped gate (whitewash-by-resurrection, engine-history shedding, liability stranding, each at 2 steps); the (principal, engine) vs. principal-alone keying split that reopens a fourth, one-step attack if collapsed | a single pass/fail migration checkmark; a generic three-step flowchart without named attacks; a table of attack names with no gate-to-attack binding |
...
| `V/fig:anchor-assurance-chain` | Where in the three-layer verification stack does a human, not a tool, assert that one layer's claim carries over to the next? | three-layer chain with descending obligations and ascending evidence | protocol-model, Rust-source, and running-binary layers with their tools (ProVerif; Kani and tests; conformance suite) and what each shows; the specification gap and deployment gap as the only human-asserted bridges; evidence-narrows-assumption arrows running back up both gaps | a single unbroken proof chain with no human-asserted gap; a checklist of tools with no descend/ascend structure; a stack diagram that hides which bridge is unverified |
...
| Atlas ID | Reader question / claim | First-choice grammar | Must encode | Reject |
|---|---|---|---|---|
| `VI/fig:bonded-three-layers` | Why is no single layer of the commons authority's trust stack sufficient on its own? | three-layer defense-in-depth stack, each labeled with what it stops and what it cannot stop | Layer 1 Structural Prevention (stops accidental scope violations), Layer 2 Immutable Attribution (stops anonymous harm), Layer 3 Economic Alignment (stops profitable misbehavior); the caution arrows naming what legitimate access, mere attribution, and traceability alone each fail to prevent; the closing composition claim | a single security shield icon; three unconnected feature boxes; a pyramid implying rank rather than sequential gap-closing |
