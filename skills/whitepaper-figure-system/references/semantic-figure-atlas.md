...
| `VI/fig:bc-graduated-trigger` | How fast does the patience premium for a forgiving, $k$-round punishment window fall as $k$ grows, against the never-forgiving grim-trigger floor? | single xy plot of the threshold $\delta^\star(k)$ against the grim floor as a reference line | the grim-trigger floor $\delta>1/3$; the threshold curve for $k=1\ldots8$; the chapter's adopted $k=3$ point marked with its premium over grim named; the closed form's stage-game parameters | a bar chart per $k$; the floor and the curve on separate axes |
| `VI/fig:bc-oracle-audit-rate` | Does the protocol's default 10% human-audit sample clear the detection rate the cartel-resistance model actually needs, and by how much? | single xy threshold plot with the region below it shaded as "cheating still pays" | the fragility-threshold curve over the oracles' patience $\delta$; the protocol's default sampling rate as a flat reference line; a candidate cheaper rate for comparison; the chapter's worked anchor point named | the threshold and the sampling rate on separate charts; a rate compared only in prose |
