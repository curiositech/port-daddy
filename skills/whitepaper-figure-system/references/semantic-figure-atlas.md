...
| `V/fig:anchor-handshake-ladder` | Who signs each message of the issue--delegate--verify ceremony, and which participant stops being spoken to? | three-plus-one-party sequence ladder on a shared clock | the four participants; the signing key above each arrow and the capabilities below it; the narrowing capability set; the issuer's lifeline going silent after the first message | a static chain with no clock; arrows around a metaphor; prose boxes standing in for messages |
