...
| `VI/fig:bc-graduated-trigger` | How much patience does a k-round graduated trigger cost relative to grim trigger, and how fast does that premium shrink? | threshold curve against round count, with a horizontal asymptote | round-count axis; delta*(k) curve; grim-trigger bound as a reference line; the marked delta*(3) point and its gap to grim | bare number in prose ("adds barely X"); a single static comparison with no curvature shown |
| `VI/fig:bc-oracle-audit-rate` | How much headroom does the default human-audit sample buy over the folk-theorem detection threshold, and where does a cheaper sample stop working? | threshold curve against discount factor | discount-factor axis; required-detection-probability curve p_d*(delta); the default sample rate as a reference line; the crossing point where the default stops clearing the threshold | a bare percentage in prose with no threshold curve beside it |
