...
description: >-
  Meta-skill that orchestrates the discourse skill suite: routes between steel-man-argument, socratic-questioning,
  toulmin-argument-analysis, logical-fallacy-detector, bad-faith-rhetoric-detector, discourse-elision-analyzer,
  forensic-speech-structure, and productive-discourse-facilitator. Use when analyzing conversations, facilitating
  discourse, or operating as a dialogue copilot. Maintains mode (silent analysis, coaching overlay, active facilitator)
  and decides which skills to invoke based on conversation dynamics.
metadata:
  version: "1.0"
  io-contract:
    kind: structured
...
- `diagrams/01_flowchart_decision-points.md` — Mermaid flowchart routing incoming requests through scope validation, constraint assessment, and pattern selection. **Read when** deciding whether a request belongs to this skill or needs redirection.

- `references/routing-rules.md` — Authoritative orchestration guide with full decision tree, routing patterns, edge cases, and multi-skill sequences for all discourse coordinator scenarios. **Read when** making complex routing decisions or designing a multi-skill intervention.
