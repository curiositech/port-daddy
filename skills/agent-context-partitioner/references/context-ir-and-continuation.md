...
`PrepareContinuation` may package assignments, authorized transfers, coverage proof, omissions, translation requirements, and a separately supplied one-use nonce. `AdmitSuccessor` belongs to a different lifecycle writer and independently checks predecessor fence, effect disposition, capacity, current guidance, context coverage, and nonce consumption.
