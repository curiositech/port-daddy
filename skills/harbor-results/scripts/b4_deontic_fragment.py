...
# Implementation boundary: the membership construction described above permits
# specialized O(T log T + C log C) detection. This reference implementation
# enumerates all active pairs (even non-clashes), so its interval loops are
# worst-case O(T^2 + C^2). Finite oracle agreement does not prove the faster bound.
...
def _sweep_pairs(items, clash):
    ...
    This reference enumerator is quadratic even if no active pair clashes."""
    events = []
...
def of_conflicts(drules, fired):
    ...
    iff they share a key - then enumerate each key group (quadratic worst case)."""
    by_key = {}
    for i in fired:
...
def check(pol, propagate=True):
    """Reference L_c checker: O(H + T^2 + C^2 + V*E), not the optimized detector."""
...
def run_experiments():
    """Run the original seeded research sweep only on explicit invocation."""
    print("IMPLEMENTATION NOTE: theoretical detector bound != this pair enumerator's quadratic interval cost.")
    # ------------------------------------------------------------------
