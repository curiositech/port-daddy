...
const cases=[
 ...
 ["duplicate claim",()=>{const x=clone();x.claims.push(structuredClone(x.claims[0]));return x},"E_DUPLICATE_CLAIM"],
 ["duplicate certificate",()=>{const x=clone();x.steelmanCertificates.push(structuredClone(x.steelmanCertificates[0]));return x},"E_DUPLICATE_CERTIFICATE"],
 ["duplicate critique",()=>{const x=clone();x.critiques.push(structuredClone(x.critiques[0]));return x},"E_DUPLICATE_CRITIQUE"],
 ["duplicate exchange round",()=>{const x=clone();x.exchangeRounds.push(structuredClone(x.exchangeRounds[0]));return x},"E_DUPLICATE_EXCHANGE_ROUND"],
 ...
 ["duplicate dissent",()=>{const x=clone();const d={dissentId:"d1",participantId:"alice",scope:"terminal",statement:"Retained objection",status:"PRESERVED"};x.dissent=[d,structuredClone(d)];return x},"E_DUPLICATE_DISSENT"],
 ["duplicate assent",()=>{const x=clone();x.terminal.assent.push(structuredClone(x.terminal.assent[0]));return x},"E_DUPLICATE_ASSENT"],
 ...
 ["duplicate integrity event",()=>{const x=clone();const e={eventId:"e",participantId:"bob",code:"OTHER_PROTOCOL_VIOLATION",evidenceRefs:["receipt:1"],status:"DOCUMENTED",disposition:"OPEN",motiveClaimed:false};x.integrityEvents=[e,structuredClone(e)];return x},"E_DUPLICATE_INTEGRITY_EVENT"],
