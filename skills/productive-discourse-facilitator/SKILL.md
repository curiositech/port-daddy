...
description: >-
  Facilitate collaborative, emotionally aware conversations and guide toward productive dialogue. Use when the user
  wants de-escalation, mutual understanding, structured discussion, conflict resolution, or mentions 'productive
  discussion', 'collaborative debate', 'dialog vs debate', 'find common ground', 'help us talk about', 'de-escalate'.
  Includes a couples-in-recovery mode for fragile nervous systems. Can orchestrate other discourse skills in a
  structured sequence.
metadata:
  io-contract:
    kind: deliverable
    produces:
      - kind: dialogue-structure
        description: >-
          Facilitated conversation framework with mode selection (dialogue/debate/deliberation/recovery), turn-taking
          rules, emotional regulation checkpoints, and escalation/retreat signals
        format: markdown
      - kind: steelman-summary
        description: Strongest-version restatement of each party's position with underlying needs and values made explicit
        format: markdown
      - kind: synthesis-document
        description: >-
          Joint identification of genuine agreements, remaining disagreements, temporal tensions, and shared values
          discovered through structured discourse
        format: markdown
      - kind: intervention-plan
        description: >-
          Specific de-escalation moves, mode switches, or recovery protocols triggered by detected failure modes
          (activation blindness, false neutrality, premature synthesis)
        format: markdown
license: Apache-2.0
...
- `diagrams/01_flowchart_decision-points.md` — Mermaid flowchart routing incoming requests through scope assessment, constraint evaluation, and pattern selection. **Read when** deciding whether a request fits this skill or needs redirection.

- `references/theory/dialog-vs-debate.md` — Defines dialogue, debate, and deliberation modes with appropriate triggers, facilitator roles, and mode-switching signals. **Read when** selecting which discourse mode fits the current conversation state.

- `references/playbooks/sac.md` — Structured Academic Controversy protocol: cooperative learning method for intellectual conflict that generates synthesis rather than winners. **Read when** both parties are intellectually curious, power is balanced, and you need to escalate to debate mode.

- `references/playbooks/couples-early-recovery.md` — Communication scaffolding for partners in early recovery (first 1–2 years sobriety) with neurological and relational reorganization needs. **Read when** either party mentions recovery, addiction history, or shows nervous system fragility.
