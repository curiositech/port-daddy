...
Six functions. Each maps to exactly one earcon. Names are essential — use them in code (`Sound::Confirm`, `Sound::Arrival`) so the *intent* is legible at the call site, never the waveform.
