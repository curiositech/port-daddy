# Reference Index

Load only the file that matches the current blocking question. Do not read the entire reference tree by default.

| File | Topic | Lines | When to load | Grep hint |
|---|---|---:|---|---|
| `references/manager-driven-team-dynamics.md` | Manager-Driven Team Dynamics | 75 | Load when you need details about manager driven team dynamics. | `rg -n "manager-driven-team-dynamics" /Users/erichowens/coding/some_claude_skills/.claude/skills/coordination-topology-architect/references/manager-driven-team-dynamics.md` |
| `references/runtime-honesty-and-legacy-labels.md` | Runtime Honesty and Legacy Labels | 48 | Load when you need details about runtime honesty and legacy labels. | `rg -n "runtime-honesty-and-legacy-labels" /Users/erichowens/coding/some_claude_skills/.claude/skills/coordination-topology-architect/references/runtime-honesty-and-legacy-labels.md` |
| `references/swarm-blackboard-contract-net.md` | Swarm, Blackboard, and Contract Net | 67 | Load when you need details about swarm blackboard contract net. | `rg -n "swarm-blackboard-contract-net" /Users/erichowens/coding/some_claude_skills/.claude/skills/coordination-topology-architect/references/swarm-blackboard-contract-net.md` |
| `references/topology-decomposition-playbook.md` | Topology Decomposition Playbook | 92 | Load when you need details about topology decomposition playbook. | `rg -n "topology-decomposition-playbook" /Users/erichowens/coding/some_claude_skills/.claude/skills/coordination-topology-architect/references/topology-decomposition-playbook.md` |
| `references/workflow-patterns-review-gates.md` | Workflow Patterns, Review Gates, and Rework | 61 | Load when you need details about workflow patterns review gates. | `rg -n "workflow-patterns-review-gates" /Users/erichowens/coding/some_claude_skills/.claude/skills/coordination-topology-architect/references/workflow-patterns-review-gates.md` |
| `references/four-plane-topology.md` | Four-plane topology | 12 | Load when planning, communication, authority, and execution differ. | `rg -n "Four planes" skills/coordination-topology-architect/references/four-plane-topology.md` |

## Bundle files

- [`topology-evidence-addendum.md`](topology-evidence-addendum.md)
