---
name: hierarchical-skills-representation
description: >-
  Use for affordances, hierarchical control, nullspace composition, and information-seeking action selection in embodied
  agents. NOT for pure symbolic planning, generic RL tuning, or domains without sensorimotor grounding.
license: Apache-2.0
metadata:
  ...
  io-contract:
    ...
    produces:
      - kind: design-doc
        description: >-
          Hierarchical skill decomposition and affordance-grounded control architecture for embodied agents, including
          priority structure, nullspace composition rules, and convergence criteria
        format: markdown
      - kind: code
        description: Skill primitive definitions, controller composition patterns, and information-seeking action selection logic
        language: python
      - kind: critique
        description: >-
          Assessment of whether a proposed embodied control system is grounded in executable affordances or relies on
          feature-only representations; identification of monolithic policy, weighted-controller, or
          premature-commitment failure modes
        format: markdown
      - kind: refactor-plan
        description: >-
          Concrete steps to restructure a control system around reusable skill primitives, explicit priority
          hierarchies, and information-driven exploration instead of end-to-end or weighted-objective approaches
        format: markdown
...
- `references/bayesian-affordance-models-for-action-selection.md` — Frames object recognition as predictive control: what controller stabilizes interaction with this object? **Read when** designing perception systems that must inform action selection, not just classification.

- `references/control-affordances-as-knowledge-representation.md` — Bridges the representational gap between continuous sensorimotor control and discrete symbolic reasoning through executable affordance programs. **Read when** architecting an agent that must unify perception, planning, and control layers.

- `references/failure-modes-in-complex-control-systems.md` — Catalogs how hierarchical skill composition creates novel failure modes: level propagation, priority conflicts, convergence breakdown. **Read when** debugging unexpected behavior in multi-level skill stacks or assessing robustness.

- `references/hierarchical-abstraction-for-problem-decomposition.md` — Shows how task hierarchies reduce combinatorial explosion by recursively decomposing complex goals into reusable subtasks. **Read when** structuring a new skill curriculum or justifying why flat policies fail.

- `references/hierarchical-composition-and-nullspace-projection.md` — Explains nullspace projection and priority-based composition for running multiple controllers without conflict. **Read when** implementing concurrent objectives or resolving controller interference.

- `references/information-theoretic-action-selection-under-uncertainty.md` — Formalizes when to explore (gather affordance information) vs. exploit (commit to task action) using information gain. **Read when** designing exploration strategies or deciding whether to probe an object before manipulation.

- `references/intrinsic-motivation-for-skill-acquisition.md` — Proposes convergence-based intrinsic rewards for autonomous skill discovery without task labels. **Read when** bootstrapping a skill learner or justifying curriculum learning over end-to-end RL.
