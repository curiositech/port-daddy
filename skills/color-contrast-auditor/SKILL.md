...
metadata:
  ...
  pairs-with:
    - skill: beautiful-gui-design
      reason: Implement the accessible color fixes this audit recommends inside an actual interface design.
    - skill: web-design-expert
      reason: Choose brand/palette colors up front; this skill verifies the resulting pairs actually meet WCAG contrast floors.
    - skill: ux-friction-analyzer
      reason: Places a contrast failure in the broader context of a user's cognitive and visual friction, not just the ratio.
  io-contract:
    kind: deliverable
    consumes:
      - kind: color-pair-spec
        format: json
      - kind: design-brief
        format: markdown
    produces:
      - kind: contrast-audit-report
        format: json
      - kind: contrast-audit-report
        format: markdown
---
...
- A written report (see `templates/output-template.md`) that a designer or reviewer can act on without re-deriving the math.
...
**`examples/`**
- [`examples/expected-output.md`](examples/expected-output.md) — Example Output: Color Contrast Auditor — Scenario: a design brief eyeballs `#777777` gray body text over a white card because it "reads fine on the design file" — the classic gray-t
- [`examples/sample-input.json`](examples/sample-input.json) — sample input (data/schema)

**`references/`**
- [`references/safe-color-pairs.md`](references/safe-color-pairs.md) — Pre-Calculated Safe Color Pairs — Ready-to-use color combinations that pass WCAG AA (4.5:1 for normal text, 3:1 for large text).

**`schemas/`**
- [`schemas/contrast-spec.schema.json`](schemas/contrast-spec.schema.json) — contrast spec.schema (data/schema)

**`scripts/`**
- [`scripts/contrast_audit.mjs`](scripts/contrast_audit.mjs)

**`templates/`**
- [`templates/output-template.md`](templates/output-template.md) — Contrast Audit Report Template — Fill in every section before handing this back to a designer or reviewer.
