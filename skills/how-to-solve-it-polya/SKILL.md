---
name: how-to-solve-it-polya
description: >-
  Use when stuck on a hard problem, decomposition is unclear, a related problem is needed, or explicit looking-back
  review will improve transfer. NOT for routine execution with a known procedure, rote checklist work, or domain
  questions that need specialized subject matter expertise instead of heuristic search.
license: Apache-2.0
metadata:
  ...
  io-contract:
    kind: deliverable
    produces:
      - kind: problem-restatement
        description: Clarified unknown, givens, and conditions separated from the original problem statement
        format: markdown
      - kind: solution-strategy
        description: Explicit plan identifying direct attack or related/auxiliary problems that unlock progress
        format: markdown
      - kind: reasoning-trace
        description: Step-by-step execution with heuristic moves labeled as provisional and validated moves marked as demonstrative
        format: markdown
      - kind: transferable-method
        description: >-
          Extracted lesson, generalization, or alternate derivation from the solved problem that applies beyond this
          case
        format: markdown
---
...
- `references/polya-look-at-the-unknown.md` — Master heuristic: match the unknown to a familiar problem. **Read when** restating the problem or searching for a productive neighboring problem.
- `references/polya-auxiliary-problems-and-stepping-stones.md` — How to systematically change a blocked problem into solvable stepping stones. **Read when** direct attack fails and you need a productive problem variation.
- `references/polya-decomposing-and-recombining.md` — Two-phase operation: break the whole into parts, then reassemble into a better-adapted structure. **Read when** the problem feels tangled and you need to isolate independent subproblems.
- `references/polya-heuristic-vs-demonstrative-reasoning.md` — Distinction between provisional heuristic moves and validated arguments; why both are necessary. **Read when** labeling reasoning steps or deciding whether a direction is safe to pursue.
- `references/polya-four-phase-problem-solving.md` — Polya's framework: Understanding, Plan, Execution, Looking Back. **Read when** you need the structural phases that organize the entire solving process.
- `references/polya-expert-intuition-and-signs-of-progress.md` — How to navigate under uncertainty by reading imperfect signs of progress. **Read when** you are stuck and need to diagnose whether you are moving toward the solution.
- `references/polya-mobilization-organization-progress.md` — How mobilization (retrieving relevant knowledge) and organization (structuring it) drive progress. **Read when** you have data but cannot see how to move forward.
- `references/polya-looking-back-and-knowledge-compounding.md` — Why the review phase is where a solved problem becomes reusable method and expertise compounds. **Read when** you have a solution and need to extract the transferable lesson.
