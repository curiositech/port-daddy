...
- `diagrams/01_flowchart_multiagent_system_design_decis.md` — Decision tree flowchart for multiagent system design. **Read when** choosing between distributed constraint satisfaction, mechanism design, or bounded rationality paths.

- `diagrams/02_stateDiagram-v2_information_structure_&_equili.md` — State diagram mapping information structure (perfect/imperfect, recall) to equilibrium solution methods. **Read when** selecting representation (backward induction, sequence form, or correlated equilibrium).

- `diagrams/03_quadrantChart_mechanism_design_tradeoff_spac.md` — Quadrant chart positioning mechanisms by computational tractability vs. desirable properties achieved. **Read when** evaluating VCG, correlated equilibrium, or heuristic trade-offs.

- `references/bounded-rationality-cooperation.md` — Explains how finite-state automata enable cooperation in repeated games despite backward-induction defection. **Read when** agents have memory/computation limits or cooperation emerges unexpectedly.

- `references/computational-equilibrium-complexity.md` — Contrasts Nash existence (guaranteed) with findability (PPAD-hard for general games). **Read when** equilibrium computation becomes intractable and alternatives needed.

- `references/distributed-constraint-solving.md` — Covers asynchronous backtracking and local decision-making under bandwidth/power constraints. **Read when** designing distributed CSP or sensor network coordination without central authority.

- `references/mechanism-design-constrained-reality.md` — Addresses mechanism design when strategy spaces are fixed and designer cannot reshape them. **Read when** VCG assumptions fail due to pre-existing agent constraints.

- `references/mechanism-design-impossibilities.md` — Details Arrow's impossibility (voting), Myerson-Satterthwaite (bilateral trade), and other fundamental limits. **Read when** requirements appear contradictory or efficiency + truthfulness both needed.

- `references/representation-and-tractability.md` — Shows how normal form vs. extensive form vs. sequence form encoding changes computational complexity. **Read when** equilibrium computation is intractable and representation switch might help.
