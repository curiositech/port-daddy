...
**Empirical artifact:** `proofs/bonded/pareto/simulation.mjs` + run log.
...
`proofs/bonded/pareto/simulation.mjs` runs a Monte Carlo over:
...
> `proofs/bonded/pareto/dominance.md` with explicit assumptions
> (public reputation, no collusion, solvency, insurer efficiency).
> A Monte Carlo simulation in `proofs/bonded/pareto/simulation.mjs`
