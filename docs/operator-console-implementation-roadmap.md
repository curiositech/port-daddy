...
**Ships:**
- Token extraction script: parses `tokens.semantic.css`, generates `apps/pd-console/src/tokens.rs` with all `--name` → OKLCH constants
- ANSI color mapping: OKLCH → truecolor (`\x1b[38;2;R;G;Bm`) → 256-color fallback → 16-color fallback (single-hue ramp)
- CI check: `scripts/check-token-mirror.sh` — runs extractor, diffs against committed `tokens.rs`, fails if diff
...
- [ ] `scripts/check-token-mirror.sh` runs in CI; zero diff against current CSS
...
All colors in pd-console are OKLCH values resolved from `website-v2/src/styles/tokens.semantic.css`. The Rust token mirror (`apps/pd-console/src/tokens.rs`) is generated by `scripts/extract-tokens.py` and must not be hand-edited.
...
- Automated contrast audit runs in CI via `tests/contrast_audit.rs`
