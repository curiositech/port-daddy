...
A memory/persistence-subsystem audit run the same day this RFC was opened found that **ADR-0040's actor-souls reputation ledger is currently read without real writers**: `lib/budget-guard.ts:176` genuinely reads soul classification on every spend decision, but `POST /actors/register` (`routes/actors.ts:153`) and `recordCleanExit()` (`lib/actor-souls.ts:458`) have zero callers anywhere in the live session lifecycle (`begin_session`, `pd begin`, `pd done`). Every agent handle resolves to `soulClass: 'unknown'` and stays permanently floored to the shared newcomer pool — the ADR's own "Proposed" status is accurate, not just cautious. Design §4 and Phase 3 below originally assumed this ledger as a working substrate; it is not one yet. Phase 3 is revised below to include registering and graduating actor-souls as an explicit prerequisite, not an assumption.
...
| Actor identity / "persons not spawns" — identity keystone, scoped to one daemon's SQLite | `docs/adr/0040-non-forgeable-actor-identity.md` (actor-souls); registration path `routes/actors.ts:153`, spend-side consumer `lib/budget-guard.ts:176` | The intended substrate for a persona that outlives one spawn — but the registration/graduation write path (`POST /actors/register`, `recordCleanExit`) has no live caller today, so every agent is currently `soulClass: 'unknown'`. This proposal's Phase 3 needs to wire that registration into `begin_session`/dispatch spawn before personas can mean anything (see correction note above and Phase 3, below). |
...
3. **Per-persona sub-identity + reputation rollup** — the highest-risk, most novel piece, and now known to have a real prerequisite: actor-soul registration (`POST /actors/register`) and graduation (`recordCleanExit`) have to actually be wired into the live session lifecycle before a persona's outcomes mean anything, since today nothing calls either. This phase is really two steps — (3a) make the base actor-souls ledger live for every agent, not just dispatch/hackathon personas, since that's a prerequisite improvement the whole fleet benefits from independent of this RFC; (3b) build the per-persona sub-identity scoping on top of a ledger that's actually populated. Also needs an explicit operator decision on the Sybil/whitewashing boundary (does a bad persona genuinely cost the base agent, every time, no exceptions?) before either step is built.
...
- **Building persona accountability on a ledger that isn't populated.** Confirmed by the audit cited above: the reputation ledger this RFC's Phase 3 depends on has a read path but no live write path today. Building the persona layer without first fixing that would produce the exact reputation-laundering failure this RFC otherwise guards against — every persona would resolve to the same inert `unknown` state, making the "real accountability boundary" claim false in practice even though it's true on paper.
