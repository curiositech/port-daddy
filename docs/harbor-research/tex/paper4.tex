...
%   Standalone paper : docs/harbor-research/tex/paper4.tex (this file, S1-S9)
%   Chapter          : website-v2/public/whitepaper/sealed-harbor.tex
%                       (Chapter 3, The Sealed Harbor; the four-pillar
%                       assurance argument -- Pillars I, III, IV; Pillar II
%                       restates swk's R5)
...
rationale, not a theorem, and it is the one critical claim in this paper that carries no proof and no script. It rests on an
...
\textbf{Verification.} The checker builds the two worlds side by side, one per secret, and drives both through every interleaving
of the same action sequence --- \emph{self-composition} \cite{bdr04}, the standard reduction of a 2-safety hyperproperty like
noninterference to an ordinary safety check, applied here concretely rather than through a general program logic. Exhaustive over
all interleavings to depth 7 $\times$ all secret pairs: zero distinguishing observations in
...
[verified framework, \cite{ramadge-wonham}]. Both hypotheses on $K$ are critical and easy to drop by accident: $K=\emptyset$
...
width of \S\ref{sec:budget}, the paper's headline quantity. The closest applied-security precedent for this move --- sequential
hypothesis testing at a guaranteed operating point, deciding at a fixed error rate that a rare adversarial signal has occurred ---
is Jung, Paxson, Berger and Balakrishnan's Threshold Random Walk portscan detector, which derives the identical SPRT-based
trade-off for a live network-security stream \cite{jpbb04}.
...
\begin{boundary}
\textbf{Honest boundary.} Independence and \emph{secrecy} of planting are critical: an adversary who obtains the canary list
...
metered one --- which is why a budget contract and a detection theory are critical here and absent there --- and we justify
