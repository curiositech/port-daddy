...
[verified]. \emph{Now you try:} at $c{=}2$, $\rho{=}0.3$, does $r=1.5$ justify a sole owner? ($g(0.3,2)=1+0.6-0.09=1.51$ --- no,
...
arithmetic; refutation internal]. \emph{Now you try:} at the reference instance, is a sole owner with a same-day succession plan
($\eta=2$, so $K$ recomputed with the same pool: $\eta K \approx 2\times1.0362 > 1$) ever capped? (No: $\eta K>1$ makes
$D^\star=\infty$ --- a fast enough succession plan removes the viability ceiling entirely, leaving only the Theorem-2 premium
test. That is what a succession plan \emph{buys}.)
