...
This paper draws on two fields that share no readers. Part~I is deontic logic and complexity theory; Part~II is queueing
theory. A reader fluent in one is very unlikely to be fluent in the other, so both dictionaries are below, in the order the
terms are needed. Nothing here is deep; it is all notation someone else fixed decades ago.

\paragraph{Part I: logic and complexity.} \emph{Deontic} logic is the logic of obligation and permission, as opposed to the
...
process is guaranteed to terminate at one canonical answer. That answer is the \emph{least model}, and its defining property
is the one Theorem~1a leans on: it contains precisely the facts forced in \emph{every} consistent world, so a status it
...
A \emph{difference constraint} is an inequality of the form $x_j - x_i \le c$ over deadline variables --- ``the runbook
...
``tractable at scale.'' It is \textbf{NP-complete} when a proposed solution can be \emph{checked} in polynomial time but no
polynomial algorithm for \emph{finding} one is known, and finding one would settle the P versus NP question. The working
...
\paragraph{Part II: queueing.} An \emph{$M/M/1$ queue} is one server, arrivals landing at random at average rate $\lambda$,
each job taking a random time averaging $1/\mu$. \emph{$M/M/c$} is the same with $c$ interchangeable servers sharing one
line --- the airport-check-in arrangement, not the supermarket one. \emph{Utilisation} $\rho$ is the fraction of capacity in
use; as it approaches $1$ waiting time diverges, which is the entire reason these comparisons are not obvious.
...
\emph{Pooling} is the practice of putting all work in one queue served by everyone, as opposed to \emph{dedicating} a
specialist to it. The classical fact --- and the tension the paper prices --- is that pooling wins on waiting time through
statistical smoothing, while a specialist may win on service rate through skill. The \emph{skill premium} $r = \mu_s/\mu_g$
is how much faster the specialist is, and Theorem~2 asks exactly how large it must be to pay.
...
NP-complete, and the language designer is offered nothing in between. We claim the location, not the pattern --- the same
step is familiar one formalism over, where the simple temporal problem is polynomial and admitting disjunctions of
difference constraints makes consistency NP-complete~\cite{sk00}. That is why $\mathcal{L}_c$'s boundary should be read as a
