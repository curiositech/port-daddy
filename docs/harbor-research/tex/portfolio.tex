...
\gives Chapter I's \S I.6 grows from one worst-case bound into a small theory: average-case floors, the formal value of adaptivity, and a knob (the tangent point) that turns operator time-accounting into digest sizing. It also gives A7's experiment its second axis. Of everything here, it is the strongest single candidate for a standalone paper --- the mathematical identity of ``summarize the state, link to the work.''
