...
% Row 1
\node[align=center,font=\scriptsize] at (2.0,8.20) {$N$ artifacts total,\\$k$ of them load-bearing:\\$\binom{N}{k}$ ways to place\\the $k$ criticals among $N$};
\node[align=center,font=\scriptsize] at (10.0,8.20) {Shared codebook:\\every flagged $m$-subset\\covers only $\binom{m}{k}$ of\\those $\binom{N}{k}$ placements};
...
\node[align=center,font=\scriptsize] at (10.0,1.80) {$m{=}8$ opened $\Rightarrow$\\$B^\star = 5.98$ bits;\\$0/16$ floor violations\\{[verified, \texttt{a7\_experiment.py}]}};
