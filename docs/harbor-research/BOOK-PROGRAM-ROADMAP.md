...
|---|---|---|
| Book-only CI builds, R2 mirror, blocking on failure | **Done** | ADR-0144, `scripts/publish-book-to-r2.mjs`, wired into `.github/workflows/whitepaper-build.yml`, PR #10265 |
...
| tufte-evidence-design skill actually run against the Book | **Started 2026-09-16**: `scripts/margin_lint.py` run repo-wide → 0 enforced findings, 10 advisory (`\footnote` in body, not yet converted to sidenotes, across 8 chapter files). `scripts/ink_audit.py` run against a stack-map render → no chartjunk flags, but that render predates today and should be re-taken once the visual redraw (task #13) lands. |
