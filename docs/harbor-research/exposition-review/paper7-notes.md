...
And add to §8 (before line 369), giving the boundary — the section the paper itself calls load-bearing — the same express-lane treatment as the theorems:
...
**Issue:** `submission_lint.py` returned no overclaim warnings, but its dictionary does not cover these forms. Four are load-bearing:
