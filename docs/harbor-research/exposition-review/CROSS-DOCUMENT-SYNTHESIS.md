...
| B3 | Missing Move 5 (no hand-checkable numbers on a critical result) | paper1, paper4, paper5, paper6, paper7, legible-swarm, single-writer-kernel, spawn-to-person, harbor-economy, bonded-commons, federated-harbor | **11 of 14** |
...
|---|---|
| `a` | paper1: the calibrated posterior (critical, in a boxed theorem). legible-swarm `def:sdt`: the per-decision attention cost. paper1 uses $c_{\mathrm{att}}$ for the latter. "Same letter, adjacent results, opposite roles." |
...
| 30 | 9 docs | **One colour rule, corpus-wide.** Status by weight/small-caps/border-style, never hue; add a redundant non-colour channel wherever hue is critical. Exemplars already exist: `fig-swk-comm-organ` (border style), `tab:anchor-status` / `tab:verification-status` (weight), bonded-commons matplotlib (colour+shape). Settle the red/green question in `CONVENTION.md`, not per-paper. | §B4 |
