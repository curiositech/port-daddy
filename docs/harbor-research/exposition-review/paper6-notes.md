...
### 19. The honest boundary is missing two items it has earned
...
**Will they get it?** — Cleveland–McGill does not apply (no quantitative encoding); the applicable test is the house rule "Label arrows with the *relation*, never with nouns." Rows 2 and 3 pass ("where the rulebook ends / chartered RESOLUTION begins"; "sole OWNERSHIP is priced, not presumed"). Row 1's label, "mechanical DETECTION / no authority needed", is a noun plus a verdict rather than a relation — it says *what* the row is, not what maps to what. **Greyscale**: the two domain boxes are `fill opacity=0.12` in harborblue vs. seagreen, which render as near-identical light greys; the columns are then distinguished only by their header text. It survives, but only just, and the colour is carrying no information a greyscale reader loses. **Legibility [needs render]**: row 1's target cell is seven `\tiny` lines in a 2.7 cm column, and the file's own header comments (L21–28) document a previous text-on-text collision at exactly this spot that was fixed by hand-breaking lines against measured CI widths — this is the fragile cell to re-inspect on the next build.

**Verdict** — Good; two small fixes.

**Concrete fix** —
1. Row 1 arrow label (fig L32), make it a relation: replace `{mechanical\\DETECTION\\no authority needed}` with `{computed identically\\by anyone $\Rightarrow$\\no authority consumed}`.
...
**Will they get it?** — The curves are position-on-a-common-scale, Cleveland–McGill rank 1: the right encoding, well chosen. The exact/falsified distinction is solid-vs-dashed, so it survives greyscale. **But the two shaded lenses are distinguished only by hue** (`fill=shipred,opacity=0.18` vs. `fill=harborblue,opacity=0.15`) and become the same grey in print — and the caption's key sentence ("the shaded lenses are the two error regimes --- false-certify below the crossing, false-reject above it") then depends entirely on the reader reconstructing which lens is which from the marker labels. That is recoverable but unnecessarily hard. Second problem: `ymax=4.0` clips the dashed curve from $\rho \approx 0.77$ onward, though its coordinate list runs to 12.5 at $\rho=0.92$. The reader sees the dashed line leave the top of the frame and cannot see the paper's sharpest contrast — **$g$ saturates while $\tilde g$ diverges** (L248, and the $\rho{=}0.9,c{=}8$: $7.73$ vs $64$ instance at L253), which is the whole reason the replacement matters at high utilisation. I verified the plotted data independently: $g(0.1,2)=1.19$, $g(0.5,2)=1.75$, crossing at $0.381966$, $g(0.9,8)=7.733$, $\tilde g(0.9,8)=64$ — all correct, so this is a framing problem, not a data problem.

**Verdict** — Strong panel, two fixes.

**Concrete fix** —
1. Replace the two `fill opacity` shadings with `pattern` fills (`pattern=north east lines` for false-certify, `pattern=north west lines` for false-reject, keeping `pattern color=shipred`/`harborblue`), and put a small in-plot text label inside each lens — `false-certify` and `false-reject` — rather than only on the two marker points. Greyscale then carries the panel unaided.
2. Either annotate the clipped dashed curve (`\node[shipred,font=\tiny,anchor=south east] at (axis cs:0.92,4.0) {$\tilde g \to 12.5$};`) or, better, add the third panel proposed in C4.
...
1. **Theorem 3's boxed `iff` does not survive its own degenerate cases** (A1/F1) — wrong strictness at $\xi/\eta = D^\star$ (the infimum is not attained, so pooling still wins there), and no $\eta K < 1$ guard, without which the closed form goes negative and the stated criterion inverts. Compounded by three different inequality conventions for one fact across the abstract, the express lane, the box and the summary table. The mechanical linter flagged exactly this line and asked exactly this question; the answer is no. Fix the box first, then propagate.
2. **Three more precision defects in Part I's foundations**, each cheap and each the kind that a referee finds before a reader does: "definite Horn rules … where the head may be $\bot$" is not a coherent object and undermines the least-model justification (A2/F2); the NP-complete definition omits hardness, i.e. the half the 3-SAT reduction supplies (A3); Bellman–Ford's `iff` needs its super-source, and the complexity term changes with it (A7/F3). Plus one arithmetic tension to resolve at source — 149 propagation-only conflicts vs. 100 missed by the mutation, and a 1013-vs-885 census (A18/F4).
3. **Two structural fixes that would help the reader most per line changed**: move the queueing dictionary from L124–140 down to §6 where it is first used, 110 lines later (A5), and float/caption/label the inventory table so the paper's central summary artifact can be referenced and read alone (A6/B4). Both are mechanical; neither requires a new idea.
4. **Part I has no figure of its own** — half the paper, the theorem the express lane leads with, and the only Rail-B regime diagram in the document is the queueing one. C1 (the expressiveness→complexity strip), C2 (conflict kind × detector × cost term × witness, currently one un-zippable run-on sentence in the box) and C3 (the witness drawn as an interval timeline) are the three highest-value additions, in that order. On existing figures: both regime panels are well-chosen encodings with correct data (I recomputed $g(0.1,2)$, $g(0.5,2)$, the crossing, $g(0.9,8)$, $C(4,0.208)$, $K$, $D^\star$ and $W_\infty(1.5D^\star)$ — all match the text), and the only real defects are Panel A's hue-only shading failing greyscale and its clipped $\tilde g$ hiding the saturate-vs-diverge contrast.
5. **The bibliography stops in 2000.** All 11 references are 1917–2000, in a program whose other reviewed papers reach into the 2010s and 2020s. The conspicuous gap is Dechter–Meiri–Pearl 1991 for the temporal-constraint dichotomy the paper explicitly invokes; the risky gap is the modern normative-MAS conflict-detection literature, which is where a competing statement of Theorem 1's frontier would live if one exists; and Halfin–Whitt may be mis-cited for a pooling-economies claim it does not make (F5). Flagged as questions, not chased (A11).
