...
**Kind** — table (as a captioned float, per the figure convention — `booktabs`, no rules beyond `\toprule/\midrule/\bottomrule`, per the corpus's existing table style). Caption states the finding: "Each clause is critical: dropping any one admits a named attack whose shortest trace the model checker exhibits in two steps or fewer. The scope lemma's row needs no migration at all [internal, \texttt{b5\_engine\_substitution.py}]."
