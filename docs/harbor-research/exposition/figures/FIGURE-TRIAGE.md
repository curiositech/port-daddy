...
| 4.15 | legible-swarm-rate-regime | §4 the two-constraint digest rate R(delta,f), pinned-joint theorem | threshold/phase-boundary plot on (delta,f), tinted priced wedge, zero-rate line, three worked points | carries | **add** | *phase-boundary plot* (pgfplots, `tpl-regime-plot.tex` family): axes delta,f; wedge tinted between f=p-delta and f=1-delta/p; corner H(p)=0.2864, and (0,0.10)->0.1864, (0.04,0.06)->0.0087 marked, from `b1_frontier.py` |
| 4.16 | legible-swarm-zoom-advantage | §4 the zoom-advantage theorem, adv(d) crossing 1 at d=1/12 | threshold/phase-boundary plot, guaranteed advantage step function vs density (log x), breakeven line, two worked configurations | carries | **add** | *regime plot* (pgfplots, step/const-plot): adv(d)=1/(d(2*ceil(log2(1/d))+4)) on log-x density; crossing at d=1/12; F=2500,k=10 (12.5x) and F=4096,k=32 (7.1x, tight) marked, from `zoom_bound_check.py` |
| 4.17 | legible-swarm-escalation-band | §4 the escalation threshold tuning band, feasible debit interval vs attention budget | threshold/regime plot: two walls (alarm-load, miss-loss), feasible band shaded where they cross, A=2.0 (empty) and A=3.3 ([0.035,0.072]) worked points | carries | **add** | *regime plot* (pgfplots, fillbetween soft-clip past the crossing): delta_min(A) alarm-load wall falling to 0 at A=3.5, delta_max=0.071975 constant miss-loss wall, crossing at A*~3.11, from `b7_escalation_band.py` |
| 4.18 | legible-swarm-split-penalty | §4 the super-additive split penalty, Floor(N,2k,m)/(2 Floor(N,k,m)) vs k | measured-quantity plot: two review-budget curves (m=8, m=20), naive-doubling guide at ratio=1, k=4 and k=10 endpoints labeled | carries | **add** | *measured-quantity plot* (pgfplots, marker points joined): m=8 curve 1.029 to 1.224 over k=1..4, m=20 curve to 1.392 at k=10, closed form from Thm. lowerbound |
| 4.19 | legible-swarm-completion-verifier | §4 the completionist obligation: completion as a checkable verifier, not a vibe | small state machine: claimed -> run verifier -> three-way gate (done / fail-loud / refused), guards keyed to a ruled legend | carries | **add** | *state machine* (tikz-diagram-craft tpl-state-machine, reusing the swk commitment-oracle claim-to-gate shape): predicate met -> done, predicate unverifiable -> fail-loud (never a silent auto-pass), predicate unmet -> refused, from Def. completionist |
| 4.20 | legible-swarm-consent-lifecycle | §4 the consent-grant lifecycle protocol: issue, act, witness, expire/revoke | state machine: issued -> active -> guarded loop {admitted / escalated} -> expired/revoked, three-clause admit guard keyed to a ruled legend | carries | **add** | *state machine* (tikz-diagram-craft tpl-state-machine, bend-left/right loop-back arcs): scope+stakes+reversibility guard on admit, ttl-or-revoke guard on the terminal transition, from Def. consent and the Consent-grant lifecycle protocol |
| 4.21 | legible-swarm-inception-canary | §4 the Inception Canary protocol: a blinded, ground-truth-known defect replay | protocol/decision flow: replay -> surface in queue -> operator zooms? -> {rubber-stamp failure / vigilant detection}, opposite lease effects named | carries | **add** | *decision-flow diagram* (tikz-diagram-craft, two-way gate off a three-step spine): approves-without-expanding halts pre-commit and tightens the autonomy lease, zooms-and-rejects widens the autonomy grant, from Protocol canary |
| add | legible-swarm-readpoverty | §4 R1 information floor: 5.98 bits at N=60, k=2, m=8 | — | — | **add** (done — folded into 4.9's redraw as an open-circle measured-point overlay on the existing curves, per register row ch4-62; r1-floor.csv committed beside the fragment) | *xy plot* from `a7_experiment.py`: bits vs N for two digest designs, measured points |
...
| add | fig-fh-settle-protocol | §8.6.3 the three-step settlement protocol (bond post, damage claim, verify), previously conflated with fig-fh-settlement (see 8.6) under one label | — | — | **add** | *message-ladder sequence*: bond post / damage claim / verify-and-clear with escrow as its own lifeline |
