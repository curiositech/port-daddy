...
|---|---|---|---|---|---|---|
| 1.1 | fig-swk-stack-map | the kernel sits under legibility and economy, above a machine floor | four labelled bands in `pd row label`/`pd axis label` `\footnotesize`, this chapter's two bands in one focus tint, the machine floor as one dark rule naming its four dependencies | supports | **keep** (redrawn per this row's spec: `\footnotesize` house labels, one dark machine-floor rule, ad hoc per-row tints collapsed to the shared focus/neutral fills — no more word soup) | — |
