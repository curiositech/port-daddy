...
- **Open.** A leading defect in the Book only: lines containing monospaced text inside a worked example get an extra line of space above them from chapter 2 onward (not in a standalone chapter, not in a test document that loads the Book's whole preamble and chapter 2 alone); bisected to the front matter or chapter 1's body at the time of writing. Redraws for chapters 2, 5 and 6–8 in flight from three workers. Remaining adds: the R12 lineage DAG (chapter 5), the fleet `pd status` session (chapter 4, needs the fleet fixture), sessions for chapters 3–6, the plots from `sweep-delta.sh`, `b7_escalation_band.py` and `sheaf_consistency_radius.py`. The corpus audit reference needs regenerating once the fragment set settles. Worked-example floor for the remaining sections; `check_style_sections.py`'s two new rules; sidenotes for provenance; the product-appeal page of the audit.
