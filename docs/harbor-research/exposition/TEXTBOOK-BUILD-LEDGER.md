...
- **The Book's engine is xelatex, in CI as well.** The fontspec block above is a Unicode-engine package and the CI build (`latexmk -pdf`) aborted on it. `scripts/build-whitepapers.sh` now selects `-xelatex` for the Book root only, so the published Book comes off the same engine as the interim builds; the standalone chapters stay on pdfTeX. The block is guarded with `iftex` and falls back to the `tgcursor` Type 1 package, so a pdfTeX compile of the preamble degrades instead of failing.
- **Openers recut for the 7 x 10 measure; margin column on the right of every page.** The part and chapter openers had kept their A4 sizes and box fractions: the part title ran off the page, its one-liner justified and hyphenated in a 7 cm strip, the chapter question had been moved above a shrunken plate, and the fontspec switch had taken the Times italic and small caps away. The part page now sets its type across the full 6 in (column and margin) at the first printing's proportions in TeX Gyre Termes; the chapter opener is back to title, epigraph, plate, question, with the plate cut to the room the page has (so it cannot spill) and the title at regular weight. The layout is one-sided, margin column always on the right (Tufte's asymmetric page), so a single-page viewer sees every page the same way.
- **Nothing leaves the page.** pgf's picture-typesetting hook measures every finished picture: one wider than the column is set across column and margin, one wider still is scaled to that width and logged as a `pd-book` warning; `scripts/harbor-research/page_overflow.py` reports what still needs a redraw. Monospace paths break at their separators inside `\texttt` (XeTeX inter-character tokens), sessions run at the full 6 in width, six display equations and the ten-column stack-map table were recut to the column. The margin Recall block is bottom-aligned to the line that issues it, sharing the section's height instead of hanging below it.
- **Figures are judged only under the Book preamble.** `compile_fragment.sh --preamble book` compiles a fragment with the Textbook Edition's preamble (Palatino, 4.5 in column). The former Latin Modern/A4 chapter mode passed figures that overflowed the Book and is now rejected by the tool.
- **Three editions from one source.** `\pdedition` (maritime by default; `swiss` and `technical` through two driver roots) selects the opener family, the display faces and a figure-language override file that restyles the same style names, so every fragment renders in each edition's character; the build script produces all three PDFs. First builds: 528 / 530 / 530 pages, no errors, no undefined references, no ink past the paper edge in any of them.
- **Sessions and listings as a printed manual sets them.** Source Code Pro sized to Palatino's lowercase; sessions at 8.6/10.6 with typed input bold and output regular, no box, never starting within seven lines of the foot; listings without frame, background or stripes, small grey numbers. Overfull lines fell from 281 to 130 with the denser face; the pseudocode listings still want the CLRS register (a follow-up).
...
### Round 6 (2026-09-06, night): the three editions get their own faces; math in Palatino; the openers hold their pages

- **Math typography.** Under xelatex the newpxmath operators font and the `\mathrm`, `\mathsf` and `\mathtt` alphabets had resolved to Latin Modern, so every digit, `\log`, `\max`, text subscript and pgfplots tick numeral in the Book was in the wrong face while the words around it were Palatino (220 pages carried Latin Modern; the maritime build now carries none). The faces are declared under fixed NFSS family names in the preamble's XeTeX branch and the math alphabets bound to them; `\textsf` and `\mathsf` are TeX Gyre Heros, `\mathtt` Source Code Pro. Verified by a fragment compiled under the Book preamble and by the font inventory of the built PDFs.
- **Chapter uniformity, measured.** Openers, running heads, pedagogy and listings are uniform across chapters. Not uniform and recorded for the redraw wave: `\resizebox` only in chapters 5–8 (figure text sizes differ between the halves of the book); two chapter-7 figures and the anchor-phases figure set labels in sans; chapter 7 embeds three matplotlib PDFs (Type 1 Palatino and STIX); chapter 4 figures name raw colours; chapter 5 keeps two Roman-numeral references; fragments that name `hh*` colours directly bypass the edition overrides.
- **Openers.** The technical edition's drawing sheet was placed by its bounding box and sat half an inch low and left, cutting the running head and epigraphs; it declares the page as its bounding box now. The Swiss and technical part pages set their chapter list compact so three-chapter parts no longer spill; the technical part and chapter pages end with `\vspace*` so nothing lands on the title block.
- **Imprint.** "Mundus Press" was an invented name from the art-system commit; the imprint is Curiositech on the title page and colophon of every edition.
- **Covers and plates per edition.** Maritime keeps the watercolour jacket. Technical: the author pointed at the engraving system of the art-system commit (exploded and cutaway drawings with empty callout boxes, white on the part's ink for parts, stipple on cream for chapters); those eleven plates are recovered from `175f3e753` into `plates/technical/`, a Sealed Harbor plate and a cover are generated in the same register with an existing plate as the style reference, the part pages set the engraving across the lower half of the solid-colour page with the type in paper above it, the chapter openers cut the plate to the room left above the question (the maritime idiom) so no opener spills, and the cover carries the engraving under the sheet's frame and title block. A first attempt in a drafting-paper grid register was generated and set aside in the scratchpad. Swiss: a research brief (`SWISS-BRIEF.md`: the canon, the grammar, a print palette with press references, a cover composition with three alternatives, twelve plates specified as geometry with stated number series, and ten changes for the figure language) and, because the plates are exact geometry, they are drawn in TikZ (`coordination-papers-mega-volume-swiss-plates.tex`), not generated: the cover's arcs in Müller-Brockmann's 16:8:4:2:1 with a 6 mm red square at the origin, four part plates inverted onto the part's ink, eight chapter plates on paper in the part's tint ladder. Three print inks (Reflex Blue, Konkret Violet, a reserved signal red) are registered in the palette twins, the token file, the brand document and the palette guard; the Swiss edition repoints cobalt and violet to them and nothing else changes.
- **Handoff.** `HANDOFF-TEXTBOOK.md` (context, build, contracts, measured state, standing instructions, open decisions, next work, which skills help and which mislead) and `NAMES-AND-SUBTITLES.md` (every title, subtitle, part name, question and edition label the book has carried, from git history). A Sonnet companion session was opened on the branch for the author to think with; it logs the author's decisions on its own branch.
- **Marginalia.** Wikimedia Commons answers from the sandbox now. A worker fetched the candidates, read each licence from the API (eleven cleared; Coase's bespoke permission is not a CC badge and is excluded; Ramadge has no free portrait), cropped, converted to duotone at the margin width and wrote a sidecar per image, a credits fragment and a placement proposal. Ten are placed with `\pdmarginfigure` at the sentence where the person's idea is critical (Lampson, Wonham and Lamport in the kernel; Hobbes's *Leviathan*, Scott and Shannon in the legible swarm; Wald in the sealed room; Parfit in the identity chapter; Ostrom and Aumann in the commons); the note shifts itself up when it would run off the foot; the Image credits page closes the appendices; `check_marginalia_sidecars.py` runs in library-checks.
- **Open.** The author's decisions on the hypervisor as the prescribed substrate, on page-one honesty about what is built, and on the threads to cut; the figure standard and the redraw wave; the thread inventory; pseudocode to the CLRS register; the Tufte margin apparatus.

**CI page drift (2026-09-07, 03:15Z).** The CI TeX Live build sets all three editions at 531 pp; the local tectonic builds set 530. The extra page is not an opener spill: the CI xelatex runs chapter 1 one line ahead from about §1.8 (Palatino/TeX Gyre metrics differ between the pinned TeX Live image and the local tectonic bundle), the break moves at §1.8.5 (p. 47 local, p. 48 CI), the drift is absorbed by the end of Part III and re-emerges in chapter 5, so Part IV opens on p. 270 in CI against p. 269 locally. The published PDFs and the registry are the CI numbers. Two half-empty pages in chapter 1 §1.8 (the figure-only page carrying Figure 1.8 and, in CI only, the page carrying Listing 1.2 with nothing below it) are a flow defect for the kernel redraw pass in Wave 11.4, not a build defect. Rule for future rounds: quote page numbers from the CI PDFs, never from a local build.

**The drift, settled (2026-09-14).** The note above guessed at the cause because nothing could build locally at the time; a TeX Live now exists on the build machine and the guess was wrong. Three renders of the same source, same `SOURCE_DATE_EPOCH`, at today's `origin/main`: tectonic 0.15.0 with its own bundle, **549 pp**; apt TeX Live 2023 `xelatex`, **549 pp**; the committed CI artifact, **551 pp**. tectonic and local TeX Live agree on all 355 structural hyperref destinations — page-for-page identical, same 27–28 overfull boxes, same zero undefined references. So there is no "tectonic metrics drift": tectonic is not the odd build, CI is, and the difference is not an engine-family difference at all (all three are XeTeX). What differs is one LaTeX package. The Book loads `\usepackage[fontsize=10.5pt]{fontsize}`, which generates its size ladder at run time; **v0.8.5** — the version in TeX Live 2023 *and* in tectonic's bundle — sets every size except `\normalsize` at a 1.19 leading ratio, while the newer version in CI's image uses 1.20. Measured off the PDFs: `\small` is 9.58131 pt on 11.40178 pt locally against 11.4976 pt in CI, `\Large` 15.12003 pt on 17.9929 pt against 18.0762 pt; `\normalsize` (10.5/12.59996) is identical in both, which is why only `\small`-set material — abstracts, callouts, exercises, captions, the margin apparatus — drifts. Forcing the 1.2 ratio locally (a four-line patch to `\@setfontsize`) takes the local Book from 549 to 550 pp, makes the front-matter page's leading match CI to 0.001 pt, and recovers the page CI gains in chapter 6 §6.3. The residual page is the one chapter 1 gains: the renders first disagree at §1.7.2 and the page is banked by the chapter 1 appendix — the region the old note named, for the wrong reason. That residue is a page-break decision at a `\section*` preceded by `\Needspace*{8\baselineskip}`, about 6.7 pt of vertical space, not a font metric. The rule stands unchanged — quote page numbers from the CI PDFs — but for a stated reason now: CI's TeX Live is newer than the one any local engine has, and it is the published one.

## Round 7 (2026-09-07, morning): the author's four decisions

- **Substrate.** The author's answer to "prescribe the hypervisor?": the guard and the shim say nothing interesting about collaboration; if the enforcement point below the agent is the real thing, say so; if we do not know, run targeted realistic simulations in a harness separate from the product. Taken as two claims. Confinement: the kernel chapter already argues that only a separate-user or separate-VM boundary with forced egress is a wall; it now says that boundary is the prescribed substrate and the guard and shim are interim instruments of an observed mode. Collaboration: whether the single-writer rail beats a worktree per agent with a merge queue is open, so `studies/substrate-study/PROTOCOL.md` pre-registers the study (S2: six substrates including advisory-with-bypass and the merge-queue baseline, on replayed real commit histories, five fixed metrics, four hypotheses with a kill criterion that would demote the rail; S1: substrate cost and controllability, needs KVM, runs on the author's machine). The harness is being built by a worker on `wave-14/substrate-harness`; the pilot's numbers land in `studies/substrate-study/PILOT.md` (doesn't exist yet), never in the book until `[verified]`.
- **Page one.** A new front-matter section after the abstract, "What is built, what is modelled, what is proposed": what runs (daemon, WAL path, projections, records, bounded Rust verifier, relay), what is machine-checked but not running, what is proposed and neither, and the two reading consequences (interceptors observe but do not regiment; the rail is a design invariant, not a theorem about collaboration).
- **Threads.** Cut nothing; legibility comes from the front matter and the express lanes.
- **Swiss part pages.** The plate gets the page: the chapter list moved to the facing page in the part's ink, the plate takes the room at the foot and is capped only by the line width. Local Swiss build 535 pp (four list pages added), no spills, chapter openers still on the page after the list. Sheet: `openers/swiss-parts-v2.png` in the scratchpad.
- **Housekeeping.** PR activity subscription and the hourly check-in were stopped at the author's request; one deferred check-in remains to fast-forward the bot regen and sync metadata after this round's push.

## Round 8 (2026-09-07, afternoon): the parts of the product the book left out

- **Coverage.** An audit found none of Grand Harbor/Chartroom, pd-console, the cooperative coding modes, or the hook harnesses in the eight chapters. Plan: `COVERAGE-PLAN-PRODUCT-PARTS.md`. Landed: front matter, "How a claim about the harness earns its grade" (Genesis, Voyage, Wake, Logbook, Receipt, the provable transition as the work unit's receipt chain); chapter 1, the hook harness as the third instrument beside guard and shim (ADR-0108, dials, the ADR's own "blast-radius reduction for the cooperative case, not a vault") and a new subsection "The second writer: institutional truth above the local harbor" (Chartroom per ADR-0137, projection-or-writer-never-both, the six-step authority transfer by writer epoch, dual-write forbidden; grade Designed; answers Porthole decision 6); chapter 4, "The operator's instrument" (the console's panes as projections under the information floor, its commands as Coordinated-mode work units, the honest build grade) and "Modes of cooperation" (the binder's five modes as supervision regimes, three governance rules, costs per mode an empirical hypothesis); chapter 3 and chapter 5 cross-references (the room replaces the hook harness; body-to-body resumption is resurrection soundness with the provider fixed). Status row for the console corrected from "text-only, not on disk" to the native shell, built weakly. Local maritime build: 0 errors, 537 pp, only a font-shape warning.
