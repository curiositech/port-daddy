...
Not "when it looks Swiss". Not "when the team agrees". When `website-v2/scripts/swiss-normalization-ratchet.json` (not yet shipped to `main` — it lands with #10153) contains zero entries, and CI has been failing on every regression the whole way there.
