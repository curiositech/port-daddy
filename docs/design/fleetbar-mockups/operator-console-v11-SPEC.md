...
- All of this is **DB-backed** — no `.portdaddy/current.json` in the target
  architecture. Today the CLI still writes that file (`cli/utils/current-context.ts`);
  the console reads worktree attachment from the daemon directly, making the JSON
  file unnecessary for GPU-native operation. Attachment is a query by worktree.
...
**Voyage manifest** (planned as `docs/design/fleetbar-mockups/voyage-manifest.html`
— sketched in the session, not yet committed) is the Watch/Sight home screen: every
voyage, status chip, destination, **outcome** column, grouped *in-session / fridge /
landed*, with HITL gate on top.
...
- House rules enforced: **≥14px body** (no tiny fonts), Tufte sidenotes in the right
  gutter, rendered Mermaid, syntax-highlighted code (`syntect`), the v10 palette.
...
**Crate:** `core/pd-tui` (ADR-0046 codename) is the new GPUI crate, to be added to
the existing workspace (alongside `core/pd-bosun`, `core/harbor-card-rs`). Note: a
ratatui prototype named `core/pd-tui` exists on the `design/tui-fleetbar-mockups`
branch but has NOT been merged to main — the GPUI build starts fresh. It is a GPUI
...
0. **Scaffold `core/pd-tui`** — new GPUI crate in the existing Rust workspace (the
   ratatui prototype on `design/tui-fleetbar-mockups` is NOT the starting point;
   start clean as GPUI); a window that opens, the v10 palette/Geist loaded, one
   frame painted. Proves the substrate + toolchain.
