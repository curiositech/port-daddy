...
- **Source:** Ostrom Principle 4 (monitors accountable to the monitored / are the appropriators; immutable evidence trails; reject the Panopticon where the daemon logs everything and agents see nothing) + game-theory Step 5 'why observable history is decisive' (remove immutable notes -> deviation undetectable -> game collapses to one-shot defection)
- **Novelty:** common-baseline
