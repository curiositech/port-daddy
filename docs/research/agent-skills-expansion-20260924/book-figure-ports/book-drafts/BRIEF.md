# Draft: unknown effect reconciliation

Planning artifact only; no Book source changed. Input: Terra Mermaid D02, copied in unknown-effect-source.mmd and hash-bound by source-receipt.json. Book placement: Chapter 1 recovery/work-unit discussion; existing coverage is recorded in ../BOOK-CANDIDATES.md. This is a teaching extension, not a novelty claim.

Reader question: Which next action is justified after a write times out? Claim: timeout alone leaves the operation outcome unknown; recovery depends on operation-specific authoritative evidence and retry protection.

Grammar: a shared-time sequence over three roles, followed by an aligned evidence/action table. The table preserves all three Mermaid alternative branches without forcing paragraphs into a flowchart. Rejected grammar: a timeout-to-failure arrow or unconditional retry loop.
