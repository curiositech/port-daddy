...
When a new subtask arrives, **retrieve similar skills** using semantic similarity (SentenceBERT embeddings, task-tuned similarity threshold threshold, task-tuned retrieval depth) and provide them as reference examples.
...
- Parameters: k=2, task-tuned similarity threshold
...
Table 2 reports 49.08 for TDAG and 46.69 for the “w/o Agent Generation” ablation (a 2.39 average-score difference) under the paper’s ItineraryBench setup. The ablation does not isolate the skill library’s contribution from agent generation. But this aggregate masks important dynamics:
...
```
Task 1:  49.08 average score (TDAG full system in the paper; not a sparse-library baseline)
