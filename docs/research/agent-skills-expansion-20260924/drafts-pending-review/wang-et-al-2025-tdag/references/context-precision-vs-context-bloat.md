...
The TDAG paper notes that irrelevant context can hinder agents and reports P&E at 42.85 vs. ReAct at 43.02 on its ItineraryBench average (Table 2). That comparison does not isolate context as the cause; it only motivates task-specific context design and measurement.
...
Section 4.2.1 describes tool-document generation by restructuring, clarifying, and enriching source documentation for a subtask. The paper reports design intent; it does not show that every supplied item is optimal or validate this reference’s context-scoring procedure.
...
Define a task-specific context-relevance measure before evaluating it. No universal “effective context” percentage is established by the TDAG paper.
