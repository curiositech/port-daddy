...
Section 5.4 and Figure 3 of TDAG compare binary task success (defined as full Level 1 executability) with the paper's fine-grained ItineraryBench score. The authors report that binary scoring has difficulty distinguishing methods on their challenging test set. Do not infer statistical significance or a fixed “20 percentage point” gap from this figure. Table 2 averages and Figure 3's binary/fine-grained comparison are distinct measurements.

A binary completion metric can hide validated partial progress, while partial credit can also overstate a run that misses one critical requirement. Report both with predeclared semantics and denominators.
...
The paper notes that binary evaluation may lead to misconceptions about emergent ability. Its benchmark comparison does not establish that model-scale transitions are generally measurement artifacts.
...
Under the paper’s benchmark rule, this level gives partial credit for valid checks. A local test might: if an itinerary correctly books 4/5 trains and visits 6/8 attractions, it scores ~(10/13) × 60 ≈ 46 points, not 0.
...
## Why This Matters: Scoring Rules Change What Counts
...
The TDAG introduction warns that binary reporting can lead to misconceptions. Treat this as a motivation for layered measures, not proof that any particular emergence claim is false.

Consider this constructed teaching scenario:
...
Figure 3 presents the benchmark-specific comparison; it does not establish statistical significance. Use the exact task split and metric when reproducing its interpretation.

## Example local method: Hierarchical Scoring

A local rubric can borrow the paper’s separation of action feasibility, task constraints, and efficiency, but must define task-specific criticality, denominators, and weights:
...
A local team can display validated progress over time, but should not interpret an illustrative trajectory as an empirical model-development result.
