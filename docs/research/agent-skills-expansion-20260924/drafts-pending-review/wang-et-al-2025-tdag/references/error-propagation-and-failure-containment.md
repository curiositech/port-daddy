...
The TDAG paper's Table 3 reports CTF as 34.78% of P&E's categorized errors and 4.35% of TDAG's categorized errors on the paper's ItineraryBench setup. The table caption defines each entry as that method's percentage of total errors in the named category; do not read the values as task failure rates or an 8× deployment guarantee.
...
The contrast concerns whether later task descriptions are updated from observed results. A static system can also validate preconditions and stop safely; dynamic updates do not guarantee success.
...
**Option B: Abort** — stop a dependent task slice or the whole task when its contract requires it. The benchmark scores (Table 2: P&E 42.85; ReAct 43.02) are specific to ItineraryBench and do not prove an implementation always aborts.
...
On this benchmark the reported CTF error shares differ (TDAG 4.35%, P&E 34.78%). These are error-category shares, not general task-level rates. This reveals:
...
**For an implementation**: Different error types need different recovery strategies. Don't use one-size-fits-all retry logic.
...
**For an implementation**: Add task metadata indicating whether containment is appropriate:
...
The paper reports a higher average fine-grained score for TDAG than the listed ItineraryBench baselines (49.08 vs. 43.02–44.74); it does not establish latency or deployment success tradeoffs in this reference.
...
**For the target orchestrator design**: The question isn't "should we support failure containment?" but "what's our default stance?"
...
The paper motivates dynamic updates for uncertain, dependency-rich tasks in its studied environments. Choose locally by testing task quality, cost, latency, failure containment, and effect reconciliation against an equal-budget baseline.