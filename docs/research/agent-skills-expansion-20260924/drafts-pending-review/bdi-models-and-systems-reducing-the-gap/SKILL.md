...
description: >-
  Design executable BDI reasoning using scoped explicit negation, Event Calculus,
  abductive feasibility checks, and preference-governed revision. The cited Móra et al.
  paper is a cognitive-model proposal, assumes initially consistent beliefs, and leaves
  belief update out of scope. Use for runtime semantics and conflicting candidate desires.
  NOT for purely axiomatic modal
...
Use this skill when the main problem is closing the gap between a BDI theory and a reasoning engine that can run. Móra et al. (ATAL 1998; LNAI volume 1999) propose ELP with explicit negation/WFSX, Event Calculus, abductive feasibility, and preference relations over revisions. The full text was read through an author-uploaded copy on 2026-09-24; this is paper-method evidence, not a complete implementation, empirical validation, or belief-observation update algorithm.
...
### Contradiction Should Trigger Scoped Deliberation

Explicit positive and negative evidence can remain distinguishable during deliberation. Do not let contradictory evidence become contradictory authorization: retain source, time, scope, and authority, then fail closed or escalate where policy requires it. The paper does not specify how new sensor observations update beliefs.
...
- Choose explicit negation when the required semantics need a represented negative literal; absence-only representations can be valid under a separately declared closed-world semantics.
- Treat contradiction as input to a revision mechanism, not as a reason to abort reasoning.
- Specify local trigger inputs, delivery/fallback behavior, and measured cost; preserve or reconsider commitment under a declared policy.
...
An operational routing example must avoid a clinical determination: two incoming cases have contradictory candidate routing obligations. Represent the negative intention explicitly, preserve evidence provenance, and let a declared policy choose or escalate the surviving intention set.

### Field Robotics with Deadlines

A robot intends to inspect a site before its locally declared reserve condition. New terrain evidence makes the route infeasible. Use trigger-based reconsideration with abductive reasoning about missing preconditions and alternatives; record that the observation-update mechanism is a local extension, not a Móra result.
...
See [`references/paper-scope-and-conformance.md`](references/paper-scope-and-conformance.md) for paper scope, a scoped-evidence trace, and conformance-vector boundaries.
