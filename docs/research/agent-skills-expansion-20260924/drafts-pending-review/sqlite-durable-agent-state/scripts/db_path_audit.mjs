...
export function auditDbPlan(plan) {
  ...
  if (journalMode === 'wal') {
    const busyPolicy = String(plan.busyHandlingPolicy ?? 'unspecified').toLowerCase();
    if (busyTimeoutMs <= 0 && !['caller_retry', 'serialized_writer'].includes(busyPolicy)) {
      pushFinding(
        'warning',
        'WAL_BUSY_POLICY_UNDECLARED',
        'WAL may return SQLITE_BUSY under writer contention. Declare a caller-deadline-bounded busy timeout or an explicit safe retry/serialization policy.',
        { busyTimeoutMs, busyHandlingPolicy: busyPolicy },
      );
    }
  ...
  if (journalMode === 'wal' && busyTimeoutMs <= 0 && !['caller_retry', 'serialized_writer'].includes(String(plan.busyHandlingPolicy ?? '').toLowerCase())) {
    recommendations.push('Declare a caller-deadline-bounded busy timeout or an explicit safe retry/serialization policy for SQLITE_BUSY.');
  }
  ...
  if (recommendations.length === 0) {
    recommendations.push('Plan shape looks consistent: single canonical env-pinned path, declared busy handling, verified migrations, and a serialized writer topology.');
  }
