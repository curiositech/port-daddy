...
- Temporal documents child workflows as separate executions with their own histories, and warns that parent history contains child-status events; its current guidance says a single parent should not spawn more than 1,000 child workflows. Continue-As-New has parent/child lifecycle implications, so check current version-specific docs rather than treating the mapping as direct equivalence: https://docs.temporal.io/child-workflows . A WinDAG proposal node is not a Temporal child workflow until an adapter maps and tests its semantics.
