...
The solution requires a *trigger mechanism*: Specific conditions that signal "deliberation is warranted." A local trigger design should declare and measure:
1. **Monitoring cost and coverage**: index size, evaluation cost, and missed/delayed/lost-event behavior.
2. **Semantics**: which typed, trusted inputs can invalidate a commitment.
3. **Fallback**: a bounded reconciliation sweep or explicit abstention when delivery is incomplete.
...
When a separately supplied local observation/update adapter invokes belief revision, the system detects: "We identify such contradiction by testing if rev_int is in the selected revision for the beliefs set, i.e., if it has to have its truth value modified in order to restore consistency. The intention revision process is triggered when one of these constraints is violated" (p. 23).

This is architecturally beautiful: The paper gives trigger constraints within its revision formalism. A local implementation may use event indexing, polling, or a hybrid; none is universally cheaper. Measure trigger evaluation, missed/delayed events, and the fallback reconciliation cost.
...
This implements *context-sensitive reprioritization*. The agent doesn't constantly check "are there more important things to do?" (expensive). Instead, the arrival of conditions making high-priority desires relevant can request reconsideration through a separately specified local update path.
...
Later, condition C becomes true (a typed local event or consequence record). Now Des1 is achievable. The trigger fires: "A previously impossible but important desire is now feasible."

The formal model records trigger constraints for rejected desires. A local runtime must state whether it indexes those conditions, polls them, or batches them; it must also handle missing or delayed events before asserting that reconsideration occurred.
...
commitment = Commitment(
    ...
    triggers=[
        ...
        ("trusted_alert.severity >= policy.reconsideration_severity", reconsider_deployment),
    ]
)
```

**2. Local event integration option**: If typed trusted events arrive (time ticks, sensor updates, messages), check if any active trigger conditions now hold:
```python
...
**4. Coverage is measured, not assumed**: record which conditions are covered, delayed, lost, or unknown; use a declared reconciliation sweep or escalation for uncovered conditions.

**5. Trigger cost is measured**: event indexing can still scan many commitments or triggers. Compare the chosen path with any polling/batch fallback on a representative trace.
...
**Exception-Based Over Polling**: Implement "reconsider when necessary" as exception handling (constraint violation) rather than polling (check every cycle). This is a local design option whose cost and loss behavior must be measured.
