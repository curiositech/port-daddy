...
The paper's answer: Use abductive reasoning to hypothesize a course of actions that would satisfy the goal. A witness makes a goal provisionally feasible only within the declared logical action model. Mark it `infeasible-in-model` only after an exhaustive complete search under stated assumptions; timeouts, incomplete search, missing actions, and insufficient modeling are `unknown`, not impossibility.
...
If abduction succeeds, produce a skill chain: [fetch_code → parse → build_call_graph → taint_analysis → vulnerability_report]. If no witness is found, distinguish an exhaustive failure in the stated model from unavailable source, incomplete catalog, or timeout; the latter cases stay unknown or request more evidence.
...
The key: Abduction is hypothesis checking, not full planning; its cost depends on the model and search procedure. It answers "is this goal feasible in principle?" not "what is the optimal plan?"
