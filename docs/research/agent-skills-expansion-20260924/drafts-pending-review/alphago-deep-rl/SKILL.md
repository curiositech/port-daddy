...
AlphaGo evidence is system- and evaluation-specific. A transfer to another domain is a hypothesis requiring a stated action space, simulator or outcome oracle, baseline, ablation, and held-out evaluation. The repaired quadrant chart is illustrative, not a measured speed or accuracy result.
...
flowchart LR
    ...
    State --> ValueEstimate
    PolicyPrior --> Search
    ValueEstimate --> Search
    Search --> Action
    Action --> NextState
    NextState --> SelfPlayData
    SelfPlayData --> Training
```

```mermaid
flowchart TD
    Hypothesis --> Oracle{reliable evaluation?}
    Oracle -->|no| Stop[do not claim transfer]
    Oracle -->|yes| Baseline
    Baseline --> Ablation
    Ablation --> HeldOutTest
    HeldOutTest --> ScopedFinding
```
