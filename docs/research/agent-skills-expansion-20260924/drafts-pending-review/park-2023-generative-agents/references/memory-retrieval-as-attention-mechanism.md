...
## Exact source algorithm and local-policy boundary

Park §4.1 uses creation timestamp/most-recent access time, sandbox-hour recency decay factor 0.995, LLM importance 1–10, cosine similarity of generated embeddings, min-max normalization, equal alphas, and top-ranked records that fit context. A local production adaptation must keep this source description distinct from its own hybrid lexical+dense retrieval, immutable compatible space identity, and authority filtering before ranking.

## The Three Retrieval Factors

### Recency: Temporal Decay as Cognitive Realism

Recency assigns higher scores to recently accessed memories through exponential decay over game-time hours since last retrieval. The decay factor of 0.995 means memories fade gradually, not abruptly. This is Park et al.'s Smallville implementation setting; it is not a validated human-memory claim.

**Scope of recency-only comparison:** An agent using only recency would be purely reactive, responding to immediate stimuli without drawing on formative experiences or long-term patterns. The breakfast you ate this morning would matter more than your decade-long vegetarianism.
...
**Source setting**: importance is an LLM-generated integer 1–10 at memory creation; the paper does not validate it as human intuition. "Cleaning up the room" scores 2; "asking your crush out on a date" scores 8. These ratings are generated at memory creation time, making them computationally cheap.

**Scope of importance-only comparison:** An agent using only importance would obsess over dramatic events while forgetting practical details. Knowing your apartment flooded is important, but so is remembering where you put your keys *today*.
...
**Scope of relevance-only comparison:** Pure semantic similarity ignores that yesterday's chemistry discussion matters more than last year's, and that the dramatic announcement that you're switching majors out of chemistry matters more than routine homework.
...
In the implementation, all α values equal 1 after min-max normalization; these are verified source settings, not universal production defaults. The top-ranked memories that fit the language model's context window are retrieved.

**Key insight**: This is a simulation retrieval design, not evidence of a cognitive model of human memory. Psychological research shows human recall combines these factors (we remember recent events, important events, and contextually relevant events). The architecture operationalizes this in a form that can condition language model generation.
...
As language models support longer contexts (a target model context budget), the temptation is to stuff everything into context. This paper argues against that approach: retrieval is a feature, not a limitation. Selective attention—surfacing the *right* subset of experience—produces more coherent behavior than overwhelming the model with everything.
