...
These are plausible engineering failure classes, not measured universal rates or an exhaustive taxonomy from Voyager. Keep the concrete failure and mitigation examples, but make validation, cost control, and safety policies application-specific; model output or a timeout alone does not establish the underlying cause.
