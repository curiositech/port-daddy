...
Code is a useful executable action representation in Voyager, with reuse and debugging benefits that depend on the runtime and sandbox. Preserve the program examples and tradeoffs; do not claim programs are inherently safer, interpretable, compositional, or more capable than policies. Capability/API access must be explicit and constrained.
