...
Voyager's self-verification is a model-mediated success check for its setting and can miss success. Preserve the critic and feedback examples as a self-check design, but distinguish them from an independent outcome oracle. A generated completion statement, stored skill, or passing self-critique is not an external effect receipt.
