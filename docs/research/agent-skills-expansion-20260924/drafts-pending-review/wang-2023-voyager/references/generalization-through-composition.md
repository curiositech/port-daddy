...
## Source scope and corrections (2026-09-24)

Composition in the Voyager setting is reuse of executable skills under a particular Minecraft API and curriculum. Preserve dependency and interface examples, while treating generalization, portability, and hierarchy as hypotheses to test on declared states and versions rather than demonstrated universal properties.
...
## Compositional depth: a dependency graph

This constructed dependency graph retains the source example while making the
prerequisite relation explicit. Each edge still needs a versioned precondition
and an outcome test before reuse.

```mermaid
flowchart TB
  D[craftDiamondPickaxe] --> MD[mineDiamond]
  D --> CS[craftSticks]
  MD --> IP[craftIronPickaxe]
  MD --> EU[exploreUntil]
  IP --> IO[mineIronOre]
  IP --> SI[smeltIronOre]
  IO --> SP[craftStonePickaxe]
  SP --> MS[mineStone]
  SP --> WP[craftWoodenPickaxe]
  SP --> CS
```
