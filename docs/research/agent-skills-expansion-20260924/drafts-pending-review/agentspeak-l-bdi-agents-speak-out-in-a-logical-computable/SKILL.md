...
This path preserves AgentSpeak(L) language and operational-semantics focus. Name the grammar, interpreter, selection functions, and environment adapter. Messages and distributed agreement remain external protocol concerns; do not infer them from a local BDI transition.
...
stateDiagram-v2
    ...
    ApplicablePlans --> SelectedIntention: selection policy
