...
description: >-
  A bounded conceptual guide to Belief-Desire-Intention design: separate evidence,
  candidate goals, commitments, and externally authorized effects. Use it to reason
  about commitment and reconsideration trade-offs; it does not itself provide learning,
  multi-agent coordination, safety, or authorization guarantees.
license: Apache-2.0
metadata:
  provenance:
  ...
  io-contract:
    ...
    covers:
      ...
      - Intelligent systems design principles
      - Multi-agent coordination and commitment
...
### 1. The Three Components Are a Useful Design Separation
...
### 5. Cross-Architecture Similarities Are Hypotheses, Not Universal Principles
...
- [`references/commitment-boundary-and-trace.md`](references/commitment-boundary-and-trace.md) — source/access boundary, commitment lifecycle, and an intention-to-effect trace.
...
- `references/why-beliefs-desires-intentions-exist.md` — Explains BDI components as computational necessities, not philosophy. **Read when** justifying architectural separation or understanding why conflating components fails.

- `references/rational-commitment-and-reconsideration.md` — Kinny-Georgeff empirical evidence on replan frequency trade-offs in dynamic environments. **Read when** tuning commitment thresholds or debugging thrashing/stalling behavior.

- `references/task-vs-goal-orientation.md` — Contrasts conventional task-oriented software with goal-oriented BDI systems; explains automatic recovery and opportunistic replanning. **Read when** migrating legacy systems or designing failure-adaptive behavior.

- `references/learning-the-missing-piece.md` — Identifies learning and adaptation as gaps in basic BDI; Pollack's acknowledgment of limitations. **Read when** extending BDI with learning or evaluating model fit for adaptive domains.

- `references/multi-agent-commitment-and-coordination.md` — Tambe's multi-agent extensions; joint commitments and coordination protocols. **Read when** architecting multi-agent systems or handling inter-agent dependencies.

- `references/soar-bdi-convergent-evolution.md` — Convergent evolution between BDI (logic-rooted) and Soar (psychology-rooted); architectural universality. **Read when** validating BDI necessity or integrating with cognitive architectures.
