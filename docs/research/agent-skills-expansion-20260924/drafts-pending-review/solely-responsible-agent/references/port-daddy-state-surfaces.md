...
This is a historical local inventory, not current runtime proof. Preserve the state-surface distinctions as an archaeology aid, but verify any route, persistence property, visibility, or enforcement mechanism against current source or deployed evidence before relying on it. A durable owner uses documented state and a handover receipt, not an assumed private namespace.
