...
## Evidence, replay, and promotion boundary

See [field evidence and uncertain effects](references/field-evidence-and-uncertain-effects.md) for the route/witness/effect matrix and promotion boundaries.


A sandbox label is a hypothesis until each claimed boundary has independent evidence.
For every attack case, record the route attempted, boundary expected to stop it,
controller/broker decision, independent host or provider witness, effect truth, and
replay recipe. A missing receipt after process death or network loss is
`UNKNOWN/HOLD`; query the independent target before any retry. Reset the guest,
ledger, credentials, provider fixture, virtual clock, and schedule deterministically
between repetitions, and record any state intentionally retained.

Keep evidence classes separate: fake/simulated, replayed fixture, host-observed,
broker-observed, model-checked, and provider-reconciled. A result promotes only the
exact tier and route it tested. Schema or lint success does not establish isolation;
a guest report does not establish host truth; a local reservation does not prove a
provider-enforced spend ceiling. While the operator halt applies, dynamic tiers stay
`NOT_PROVISIONED` or `BLOCKED`; no test execution is authorized by this skill.

- [Adversarial run and deterministic replay](diagrams/research-s02-adversarial-run-and-replay-lifecycle.md)
- [Evidence tier and allowed claim](diagrams/research-s03-evidence-tier-and-allowed-claim.md)
- [Lost acknowledgement and effect reconciliation](diagrams/research-s04-lost-acknowledgment-is-unknown-until-reconciled.md)
