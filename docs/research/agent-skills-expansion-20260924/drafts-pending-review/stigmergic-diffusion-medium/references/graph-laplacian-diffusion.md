...
For `p_next=(I-αdtL)p`, each Laplacian eigenmode is multiplied by
`1-αdtλ`. Linear stability requires `0 <= αdtλ_max(L) <= 2`.
Because `λ_max(L) <= 2d_max`, `dt <= 1/(αd_max)` is sufficient for the
unweighted, undirected, synchronous operator. Thus `0.9/(αd_max)` is a 10%
safety factor for this operator; it is not a transferable guarantee for weighted,
directed, normalized, nonlinear, asynchronous, or in-place variants.
...
- The `0.9/(α·d_max)` guard is meaningful only when `d_max>0` and the stated
  simple Laplacian/operator ordering applies. An edgeless graph has no diffusion.
