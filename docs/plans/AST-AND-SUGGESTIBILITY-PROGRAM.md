...
| --- | --- | --- | --- | --- | --- | --- |
| D0 | `ast-suggest-demo-harness` | LANDING | existing GPUI proof harness | `demos/ast-suggestibility/` supplies the schema, fixture daemon, synthetic clock, action driver, receipt writer, provenance validator, and exact-window adapter once | `proof-harness.e2e.mjs` proves two CI-safe captures have identical semantic contracts; missing/stale daemon, receipt, source label, private path, and broad capture fail closed | Reference fixture bundle is explicitly marked `FIXTURE DATA` in every Harbor frame; native PNG/MP4/GIF evidence is captured only from the proof-owned GPUI window |
