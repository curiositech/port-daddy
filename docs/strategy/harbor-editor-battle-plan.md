...
Zed proved humans and agents can be co-present in one CRDT buffer, then stopped at co-presence: a CRDT guarantees bytes merge, never that *intent* agrees. Port Daddy already owns file/symbol/region claims (`routes/symbols.ts`, `POST /conflicts/predict`), the commit guard, Ed25519 capability cards (`core/harbor-card-rs/src/lib.rs`), and an immutable-note audit trail. It also registers authenticated editor-recovery routes, but those routes are 503-only scaffolding until their canonical Rust and P3 authorities are implemented. **We win by making the CRDT *governable*: take the one genuinely-missing primitive (a fast Rust collaborative buffer) off the shelf — Loro v1.13.x — bind its awareness layer to PD claims so a claim *is* a presence range the guard can refuse to merge across, and treat every editing actor (human OR agent) as a first-class Loro replica keyed to its PD identity.** The buffer merges bytes; the daemon governs intent; the harbor card decides who may write the region at all. **The target wedge is the salvageable agent in a shared buffer** — kill an agent mid-edit, validate its complete typed ledger through canonical Rust, then atomically transfer one released claim and provenance. That remains unimplemented P3.5 product work; P2 snapshots, notes, generic salvage, and `apply_remote_ops` do not provide it.
...
3. **Salvage** — daemon-owned typed operation receipts seal a dead replica's complete sequence-zero op log; authenticated editor recovery canonically validates replay before one P3-owned released-claim transfer and provenance write. Zed loses the work when the ACP process drops.
...
- Durability: P3.5 salvage uses append-only typed operation receipts plus a sealed abandonment high-water receipt. Notes carry provenance only; they are never operation evidence, and editor sync/checkpoint transport remains a separate P1/P2 contract.
...
- **Shared (default, P3):** host's daemon is authoritative for the claim/governance ledger; joiners connect over daemon HTTP+SSE. Lowest friction, pure reuse.
...
7. **Salvage (the wedge).** A's process dies mid-edit. Daemon-owned typed operation receipts and a sealed abandonment high-water mark preserve the complete op log from sequence zero. An authenticated successor uses `POST /editor/recovery/request`, `/prepare`, `/replay`, then `/finalize`: canonical Rust validates the real Loro bytes and terminal state before finalize leases an opaque content-hash/parser-generation symbol witness, retains root/file descriptors, holds and consumes a daemon file-mutation generation, and atomically invokes P3's released-claim transfer with that exact tuple. Provenance, outbox, token consumption, and preparation finalization share the transaction. No token or phase row is written until every public authority is available; the sidecar returns 503 until the canonical Rust and P3 seams land. Zed loses this entirely.

> **Implementation truth (2026-08-24).** These registered routes are security scaffolding, not a usable recovery pipeline. The P1 Rust operation-receipt producer, P1B, canonical Rust Loro recovery adapter, and P3 same-database released-claim transfer adapter are all unimplemented external gates. Daemon scope minting also cannot yet provide the required verified worktree root device/inode witness; content-bound symbol lease and daemon file-mutation generation authorities are likewise unwired. Keep the routes 503-gated until every authority lands. The mutation lease governs daemon-authorized writers, not arbitrary OS processes; P3 must persist the transferred root/file device+inode, content hash, and generations and reject later edits after any bypassing replacement or in-place write. Editor-owned provenance seals that same exact file hash as `scope.canonical_content_hash`, and provenance plus its append-only outbox may commit canonically. Every projection read, attempt-log write, or local publication-receipt failure after that commit is fail-soft: finalize remains a canonical 200 with the stable outbox ID and a truthful pending reason, and restart retries the same sink-deduplicated idempotency key until one local receipt exists. Without an atomically idempotent derived-note sink there is no drain timer or retry churn and the row stays pending; with one, bounded startup/periodic repair retries the exact key. `sessions.addNote` is not such a sink. The schema is reconstructed only in its current unshipped form; no intermediate PR migration is supported.

Agents reach claim and conflict coordination through agent-neutral MCP tools (`claim_region`, `release_region`, `coordination_preflight`) — first-class, **never Claude-specific**. Recovery itself has no CLI/MCP bypass; it uses only the authenticated `/editor/recovery/{request,prepare,replay,finalize}` sidecar.
...
| **P2 — LAN multiplayer over the bus (3–4 wk)** | Two humans co-edit one file, self-hosted | Loro Protocol multiplexed over the daemon's tube SSE (`Subscription::Editor`, `on_stream` folds remote ops); cursors/selections via `EphemeralStore`; checkpoint/reconnect behavior belongs to the canonical editor-sync contract, while P3.5 captures separate typed salvage receipts; follow-mode. **Reaches Zed's LAN baseline — but self-hosted, no vendor cloud** | tube pub/sub, canonical editor-sync contract, `AgentTranscript` SSE plumbing |
...
| **P3.5 — Salvage + provenance (2 wk, fold into P3 demo)** | Kill an agent mid-edit, recover the edit | Append-only typed op receipts plus sealed abandonment; authenticated `POST /editor/recovery/{request,prepare,replay,finalize}`; canonical replay receipt before one atomic P3 claim transfer, provenance write, and token consume. **The headline demo; all authorities beyond the 503 route scaffold remain unimplemented.** | `routes/editor-recovery.ts`, `lib/editor-recovery.ts`, **unimplemented** canonical Rust Loro adapter, **unimplemented** P3 released-claim finalizer |
...
> LAN substrate — editor sync/edit+coord channels/presence (#727), P3
