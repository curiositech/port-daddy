...
| 7 | adr-0050-phase-7-compulsion | **shipped (note-per-commit enforced; rent→slash advisory-default; reclaim sweep next)** | adr-0050-phase-0-pd-cutter-wrapper | **The compulsion — coordination is the price of the sandbox.** Mechanism design, not politeness: a voyage keeps its Coast-Guard sandbox only while it pays coordination rent. **Done when:** an un-noted commit blocks the next commit; a dark/abandoned sandbox is reclaim-eligible — and reclaim can NEVER touch the live main checkout. **Landed:** `lib/coast-guard/compulsion.ts` (pure rent evaluator + reclaim safety gate, 19 tests), `lib/coast-guard/compulsion-facts.ts` (git+daemon fact gatherer, fails open), wired into the Coordination Guard as `requireNotePerCommit` (default on) so "no note, no commit" is enforced at commit-time (7 guard tests). **Rent → slash (the economic-enforcement loop):** `lib/coast-guard/rent-slash.ts` (pure graduated-punishment policy — first miss is grace, repeats escalate a small fraction capped at 50%, never the whole bond) + `lib/coast-guard/rent-breach-ledger.ts` (per-PRINCIPAL escalation memory; cure decays toward grace — graduated, not grim) + `lib/coast-guard/rent-slash-enforcer.ts` (resolves the BREACHING principal's own bond, **ADVISORY by default** — logs the slash that WOULD happen and debits nothing; only `PD_RENT_SLASH_MODE=enforce` calls `bonds.slash`, preserving the conservation invariant), surfaced at `POST /coast-guard/rent-breach` / `rent-cure` / `GET /coast-guard/rent-status` (the breaching principal is derived server-side from the caller's session — never the request body, the Sybil/griefing defense). 75 tests incl. a conservation-property test on the rent→slash path. **OPERATOR DECISION:** enabling `enforce` mode and tuning the slash fraction/escalation/grace is an operator opt-in; the default debits nothing. The reclaim sweep (`shouldReclaim` is built + gated; the reaper CLI surface is the next slice). |
