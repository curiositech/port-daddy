...
| G2 | **Enforced lock gate** (the decisive claim) | agent B's Edit of a file agent A holds locked is blocked by `PreToolUse` `exit 2`; the file content/mtime is unchanged in **10/10** trials; the block is logged | a reproducible test + the hook stderr the agent received | Tentacles |
