...
```ts
type MemoryTier = 'Core' | 'Recall' | 'Archival' | 'Recall→Archival';

interface TierRow {
  construct: string;        // stable construct name; see TIER_TABLE in cli/commands/memory.ts
  tier: MemoryTier;
  eviction: string;         // operator-facing description; not parsed
  access: string;           // operator-facing description; not parsed
  count?: number;           // present when the live daemon count succeeded
  countError?: string;      // present when the live daemon fetch failed; mutually exclusive with `count`
}

// pd memory tiers --json
interface MemoryTiersOutput { rows: TierRow[]; }

// pd memory tier <construct> --json
interface MemoryTierOutput extends Pick<TierRow, 'construct' | 'tier' | 'eviction' | 'access'> {}

// pd memory summary --json
interface MemorySummaryOutput {
  tiers: Array<{
    tier: MemoryTier;
    count: number | undefined;     // sum of constructs in this tier whose count fetcher succeeded
    constructs: string[];           // construct names in this tier (alphabetical)
    errors: string[];               // "construct: <message>" for any failures
  }>;
}
```

**Partition invariant** (locked by the test
`recall_count + archival_count = total_count over the same fixture` in
`memory-tiers-cli.test.js`): for any node count `n`,
`active-notes.count + archived-notes.count === total_notes` under the
configured `PD_MEMORY_RECALL_TIERS_TTL` window (default `30d`). The
partition holds by construction in `buildCountFetchers()` — `archived =
max(0, total - recall)`.

**Wire-shape compatibility note.** The CLI's `salvageable-sessions`
fetcher reads the daemon's authoritative `/salvage/pending` response, which
emits `{ success, agents: StaleAgent[], count: number, filtered? }`.
Prefer the explicit `count` field; fall back to `agents.length`. The
older `/resurrection/pending` alias still works but is deprecated.

When this schema changes, update both this section and the snapshot test
in the same commit; the snapshot will fail until both move.
