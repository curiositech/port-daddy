...
- `lib/squid/matrix.ts` — the **Ink Cloud**, a POSIX `KEY="value"` file at `~/.port-daddy/matrix.env`, mkdir-locked so K≥8 ephemeral agents append without torn lines. It carries `PD_LOCK_*`, `PD_PHEROMONE_*`, and `PD_ALERT_*`. **It is a hot cache, not a source of truth**. *(Addendum, 2026-08-04 — W1: the "unbuilt TODO" reconcile loop is now IMPLEMENTED in `lib/squid/reconcile.ts`: 15s tick + event fast-paths, single owner of `PD_HALT` / `PD_INBOX_*` / `PD_PARLEY_*` / `PD_CLAIM_*` / `PD_CI_*` / `PD_ALERT_FLEET_APPROVALS` / projected `PD_PHEROMONE_*`, raw-pheromone drain into the durable `ink_pheromones` store with the pheromone.ts decay law, stray-GC via `applyProjection`, and the `PD_RECON_HEARTBEAT_TS` staleness contract under which every enforcement rung fails OPEN. The former RECONCILE TODO block in matrix.ts is resolved and points there.)*
