# 0057. Unified Distribution — Port Daddy as one beautiful whole
...
| 7 | dist-update-channel | 4 | **CLI half LANDED.** `scripts/build-latest-json.mjs` generates a `latest.json` feed (schema + semver math in `lib/latest-manifest.ts`) from the built artifacts; `release.yml`'s `build-latest-json` job publishes it to each Release so `releases/latest/download/latest.json` is stable. `pd upgrade` (`cli/commands/upgrade.ts`) fetches the feed, compares the embedded version, surfaces the per-artifact SHA-256 + signed flag, and `--apply` re-points brew (`brew upgrade port-daddy` + service restart) — privileged in-place self-replace is deferred to brew by design. *Remaining:* GUI in-app update check + a real cryptographic signature over the manifest (the SHA-256 verifies asset integrity; the feed is not yet detached-signed against an embedded pubkey). |
