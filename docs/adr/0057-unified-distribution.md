...
| 3 | dist-ci-signing-secrets | 1,2 | wire the Developer ID + notary profile into CI secrets; invoke `scripts/sign-and-notarize.mjs` for the **daemon** artifact in `release.yml` (sign + notarize + staple) |
...
| 7 | dist-update-channel | 4 | `latest.json` per release; `pd upgrade` self-update; GUI in-app update check + signature verify against embedded pubkey |
