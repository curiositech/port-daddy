...
| **Fleet daemon integration — FleetBar launcher, UI prefs, setup CLI, fleet-ui serving** *(historical WIP snapshot)* | Later landed across the current control-center surfaces | The global spawn ceiling regression now lives in `tests/unit/spawner.test.js`; tunnel and tutorial coverage live with their current surfaces. This row records the 2026-04-05 work-in-progress state and is not an install or recovery instruction. |
