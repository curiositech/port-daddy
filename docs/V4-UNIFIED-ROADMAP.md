...
| Bosun watchdog (Rust) | Deployed | daemon writes `~/.port-daddy/heartbeat`; `core/pd-bosun/` supervises filesystem heartbeat, replaces legacy Barnacle, and the supervisor release is promoted |
...
> **Current repo truth — 2026-04-28:** `graph_edges` is implemented in-tree via `lib/graph-edges.ts`, `routes/graph.ts`, `tests/unit/graph-edges.test.js`, and MCP `graph_edges` / `graph_stats` surfaces. The symbol index, orchestrator registry, and merge queue are already wired into `server.ts`. `a8b310e` added symbol-path region claim support and `e3ea67e` added the semantic alias resolve diagnostic, so claim provenance is now symbol-aware in the working tree. The remaining Phase 1 risk is authority drift: stable promotion, docs, and operator/runtime truth still need to agree on what already exists.
...
>
> **Cartographer — 2026-04-28:** `3d3bedc`, `3172d0f`, and `a33fbd6` surfaced resource governance in the FleetBar shell and the overview UI while tightening bond admission. The operator can now see the budget story where decisions happen, but the economics still stop short of a finished pricing function.
...
>
> **Cartographer — 2026-04-28:** `f689337` and `4cba7ec` put Shipwright into the real Fleet Control Center and split it into subviews, while `28cbfe2` and `5a257e2` exposed project readiness and actor lifecycle in FleetBar. `7ba8d84`, `ca8ffad`, and `b29b005` also made roadmap/operator surfaces visible there. This is the live evolution of 3D from a dashboard panel into a broader fleet management product.
...
| **Roadmap progress + operator surfaces** | `7ba8d84`, `ca8ffad`, `b29b005`, `a8b310e` | The map got its own live operator surface and symbol-path claim support, but that shell was not named in the original V4 phase plan. |
| **Feedback primitive + salvage task board** | `8fcf93e`, `bc38d96`, `a350cbb`, `618678b`, `51a4fae`, `2bdf7f2` | Dogfood needed a first-class feedback ingress and the salvage queue needed task-board triage, so the fixes landed as operator pain relief rather than a named V4 phase. |
| **FleetBar resource governance + salvage visibility** | `3d3bedc`, `3172d0f`, `a33fbd6` | Resource governance became visible in FleetBar and the shell, and bond admission got tighter, because operators needed budget truth where they make decisions. |
| **Website visual / SEO rehab cluster** | `3331a8c`, `8670850`, `21ad6a4`, `389871f`, `c355c5b`, `b9928c2`, `9d30e34`, `c41268f` | The public site kept drifting, so generated visuals, SEO metadata, layout cleanup, and headline/button cleanup shipped as product truth work rather than a V4 phase deliverable. |
| **Operator/runtime truth hardening** | `a8ca8e5`, `b0c0c09`, `10179d2`, `000ff77` | Cartographer body matching, daemon env hygiene, and FleetBar daemon URL drift were repaired because the live control plane had to stay honest even when the work was not a named V4 phase. |
