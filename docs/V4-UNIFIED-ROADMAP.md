...
**Status:** Active — Phase 0 complete, Phase 4 partially complete (Fastify ✅, Trie ✅, Binary IPC ✅, Backpressure ⚡), Phase 3 now fully done (fleet engine ✅, Fleet Daemon ✅, pheromone ✅, Fleet Live Dashboard ✅), security hardening ongoing (3 audit passes)
...
>
> **Cartographer — 2026-03-31 (fleet daemon):** Fleet Daemon shipped (`37ee080`). `lib/fleet-daemon.ts` (422 lines) — always-on fleet subsystem auto-started on daemon boot, SIGHUP reload, per-project fleet runner start/stop/status, `FleetLimits` (max_concurrent_spawns, max_spawns_per_hour). `routes/fleet.ts` (155 lines) — 7 HTTP endpoints wired into `routes/index.ts` and `server.ts` with graceful shutdown. Fleet SSE event stream via `fleet:events` pub/sub channel. 482 unit tests in `tests/unit/fleet-daemon.test.js`. Fleet runners now survive terminal close via launchd. **3A is complete as originally specified** — "like docker-compose for agent swarms" now runs inside the daemon. The `.portdaddy/fleet.yaml` convention remains informal; top-level `pd-fleet.yml` is the in-practice convention.
...
| **Fleet Daemon** (`lib/fleet-daemon.ts`, `routes/fleet.ts`) — always-on fleet subsystem | `37ee080` | Fleet engine ran via CLI only — processes died when the terminal closed. Fleet runners now auto-start on daemon boot (launchd survives login), SIGHUP reloads all fleets, per-project HTTP API for remote control. Completes the "daemon-managed fleet" promise of 3A. Closest to planned work of anything in this table — this is 3A finishing what it started. |
| **Blog content engine** — 9 articles, 11 Ideogram V3 hero images, VHS demo tapes, BlogPage redesign | `37ee080` | Marketing and distribution: 8 feature tutorials reformatted as blog posts, "Six Superpowers" entry-point grid, demo GIFs for 7 features (7 rendered, 11 tapes authored). Ideogram V3 batch generation via `scripts/ideogram-batch.sh`. Not in any roadmap phase — pure marketing/content work that emerged from needing SEO distribution. |
| **`shared/paths.ts`** — centralized runtime path constants (`~/.port-daddy/`) | `d466103` | Security: `/tmp/port-daddy.*` files were world-readable and vulnerable to symlink attacks (CWE-59). New `PD_HOME = ~/.port-daddy/` (0700 directory) centralizes all runtime files. 9 source files updated to import from this module. Not in any roadmap phase — security hardening driven by audit. |
| **Webhook secrets encrypted at rest** (AES-256-GCM, same envelope as note encryption) | `f91195e` | Security: webhook secrets stored plaintext in SQLite could leak HMAC keys. Reused Phase 0 note-encryption infrastructure (master.key, AES-256-GCM envelope). Backward-compatible: plaintext pre-migration secrets still work. Extends Phase 0 note encryption to a second subsystem. |
| **Master key permission enforcement** — auto-fix 0o600/0o700 on startup | `f91195e` | Security: `lib/note-encryption.ts` now `statSync`-checks master key and `~/.port-daddy/` directory permissions on load. Auto-fixes with `chmodSync` + warning log; refuses to start if permissions unfixable. Hardens a gap in the Phase 0 encryption design. |
| **`docs/TROJAN-PORT-DADDY.md` + `docs/SITE-ARCHITECTURE-PLAN.md`** | `37ee080` | Go-to-market strategy. Multiple Trojan Horse entry points for the same daemon (port manager, multi-agent coordinator, fleet runner, dev server orchestrator). Site URL hierarchy and navigation plan. Not a technical feature — strategic positioning work. Energy signal: time is being spent on distribution, not just features. |
| **Comprehensive `skills/port-daddy/` skill** — full MCP skill rewrite (5 reference docs) | `f91195e` | The `skills/port-daddy-cli/` skill had drifted from v3.8 reality. New `skills/port-daddy/` with SKILL.md + 5 reference docs: `api-reference.md` (757 lines), `sdk-reference.md` (457 lines), `multi-agent-patterns.md` (169 lines), `portdaddyrc-spec.md` (172 lines). Separate from the CLI-focused skill. 2582 lines of new skill documentation. |
| **Blog content data + content planning docs** | `9d23ba7` | Continuation of `37ee080` blog work: 459 structured blog entries in `website-v2/src/data/blogData.ts`, `docs/BLOG-CONTENT-PLAN.md` (319 lines), `docs/IDEOGRAM-PROMPTS.md` (351 lines, batch image generation prompts for all 11 hero images), `docs/DOCUMENTATION-OUTLINES.md` (666 lines). Also: filled 7 parity gaps — SDK docs expanded for tuples, features manifest updated, `skills/port-daddy-cli/` SKILL.md + API reference patched, MCP tool mappings corrected, parity tests fixed. Energy signal: blog and content marketing is an active ongoing track, not a one-off sprint. |
| **Spider connections S18–S25** | `.spider/connections/2026-03-31-connections.md` (untracked) | Spider fleet agent ran 2026-03-31 on new corpus (tuple space, merge queue, symbol index, full IPC stack). 8 new syllogisms, all high/medium confidence: **S18** symbol+merge→semantic conflict prediction (session); **S19** pheromone+orchestrator plugins→heat-first plugin (trivial); **S20** tuple space+symbol index→symbol-level blocking rendezvous (trivial, convention + ~20 lines); **S21** tuple space+fleet engine→Contract Net Protocol task delegation (session); **S22** tuple space+note encryption→one-time rescue key delivery (small — makes prior S8 trivial); **S23** IPC router+Arbiter→peer-negotiation visibility (small — closes Arbiter blind spot over IPC); **S24** merge queue+activity log SSE→real-time pipeline broadcast (trivial, ~25 lines); **S25** pheromone+tuple space→stigmergic auto-dispatch (medium). S19, S20, S24 are self-contained trivial items waiting only on Phase 1 wiring. S23 is a security gap that may not wait for Phase 1. |
