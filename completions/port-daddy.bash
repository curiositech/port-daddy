...
_port_daddy() {
  ...
  local commands=(
    ...
    # Daemon lifecycle
    start stop restart install install-freshness uninstall dev use daemon ci-gate self-update upgrade mcp
  ...
  case "$cmd" in
    ...
    # -----------------------------------------------------------------------
    session)
      local session_subcommands='start end done abandon continue rm files phase'
      ...
      case "$subcmd" in
        ...
        abandon|rm)
          ...
          ;;
        continue)
          _pd_opts '--purpose -P --direction --backend --model --budget --deadline-ms --workdir --identity --project --idempotency-key'
          ;;
        files)
