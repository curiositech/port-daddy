...
The [provider-neutral retrieval design](docs/proposals/provider-neutral-retrieval-fabric.md) replaces the universal-model rule with corpus-approved profiles, compatible index generations, RRF of rankings, and privacy filters before both retrievers. The [research record](docs/research/embedding-retrieval-model-landscape-2026.md) identifies candidates, not production winners. Registry-backed role/tier/provider selection and fail-closed local artifact/output verification are source-present. Transcript search is the bounded persistent-index pilot: it requires an explicit harbor and exact repository, sanitizes every source derivative and query before tokenization or embedding, stores admission and embedding-production receipts, rebuilds unscoped legacy rows, and fuses BM25 plus the exact compatible dense space with RRF at `k=60`. The same admission and migration still need to reach every other corpus; signed benchmark promotion, remote budget integration, and operator controls remain separate implementation work. This documentation does not activate a remote model or prove an installed runtime upgrade.
