...
1. **Confine** — an OS sandbox (macOS Seatbelt; Linux Landlock/bubblewrap) **denies** reads to the crown jewels (`~/.ssh`, `~/.aws`, `~/.gnupg`, cloud creds, every secret-bearing `.env`/`.env.local` in `$HOME` and the workdir) while allowing normal work. A project-root `.env.example` remains readable because it is a tracked public template that Git must inspect; the exact exception does not open sibling dotenv files or symlink targets. `cat ~/.ssh/id_ed25519` → *Operation not permitted*.
