...
Search across the current stable runtime is **hybrid**: BM25 plus the local `Xenova/all-MiniLM-L6-v2` fallback prefetched at install per ADR-0061. That is transitional runtime truth, not the long-term architecture. The accepted direction is a provider-neutral, versioned profile registry with policy-selected local or remote quality tiers; separate text, code, UI/multimodal, and reranking roles; hard repo/harbor/privacy filters; golden-corpus promotion; cost/latency/quality receipts; and dual-index migration that never compares incompatible vector spaces. See the [Provider-Neutral Retrieval Fabric proposal](docs/proposals/provider-neutral-retrieval-fabric.md) and its [primary-source model landscape](docs/research/embedding-retrieval-model-landscape-2026.md). Skill Graft's Tool2Vec centroids are reconciled content-hash-by-content-hash across the full user catalog: setup and daemon ticks use only loopback Ollama, never an inherited fleet or cloud backend, while a manual `pd skill-graft warm` may use an explicitly pinned `PD_SKILL_GRAFT_BACKEND`. The SQLite lease and row checkpoints make daemon, setup, and manual callers safe to resume after interruption. `pd doctor` reports current, cold, reconciling, embedder-down, or generator-down coverage. `pd memory tiers` prints the three-tier vocabulary overlay (Core/Recall/Archival) over the same SQLite substrate.
