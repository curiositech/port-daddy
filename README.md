...
When an agent reaches a decision only the operator can make, `pd alert "<decision needed>" --body "<why the agent is blocked and what the answer unlocks>" --urgency high --wait` writes one idempotent `OperatorInterruption` to Relay and returns only after a durable receipt. MCP-native agents get the same first-class loop through the essential `alert_operator`, `list_operator_interruptions`, and `get_operator_interruption` tools, so a blocked agent does not have to discover an optional capability first. FleetBar asks before enabling privacy-safe macOS notifications; after the operator opts in, high and critical asks make a sound by default, and both alerts and sound have visible off switches. Both native surfaces show the complete decision card: question, blocked reason, urgency, exact source agent/session, and age. Their **Respond securely** action opens the authenticated form whose buttons say **Send answer to agent** or **I handled this elsewhere**; a stopped WorkIntent is labeled failed/rejected instead of impersonating a human question. CLI and MCP reads expose only interruptions emitted by the exact requesting agent/session without exposing the operator bearer token. Remote Fleet agents also await and transcript the exact Relay row for every escalation. A missing identity or transport configuration, dead daemon, rejected Relay write, or unreadable receipt fails loudly instead of pretending the operator was notified. The full contract is [`docs/hitl-interruptions.md`](docs/hitl-interruptions.md).
