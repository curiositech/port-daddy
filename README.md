...
jury-rig search "write tests for a flaky fleet trigger"  # standalone: no Port Daddy CLI or daemon startup
jury-rig graft "write tests for a flaky fleet trigger"   # standalone explicit bounded body load
...
Search across Port Daddy is **hybrid** — BM25 plus a compatible persisted Tool2Vec profile. Jury-rig computes BM25 TF/DF over the current deduplicated skill scan: one document per skill containing `name`, `description`, `category`, and `tags`, using the declared lowercase/alphanumeric/Porter tokenizer and `k1=1.2`, `b=0.75`. Every search result carries a compact corpus ID and document-frequency digest for that exact calculation. The standalone `jury-rig` executable is the halt-safe path: it imports only the skill command and retrieval libraries, never Port Daddy's monolithic CLI, daemon discovery, freshness checks, or update loop. `jury-rig search` and the `pd jury-rig search` shorthand return at most 10 metadata matches with every description capped at 240 characters; they do not put SKILL.md bodies into agent context. Suggestibility hooks and background scripts can call `jury-rig search --json --quiet`, inspect that bounded result out of band, and stay silent when no match warrants attention. They must not auto-graft every match. `jury-rig graft` is the explicit bounded body-loading step, and `reference` reads one contained supporting file on demand. Search and graft return before generation, then launch a detached local-only warm worker. That worker discovers only loopback Ollama, prefers the installed `qwen3-embedding:8b` semantic model, binds embedding and generator cache identities to exact Ollama digests, and fills every missing current-hash centroid under the SQLite lease. No compatible local pair means an honest identified-corpus BM25 result, never a remote fallback. Existing compatible centroids remain usable without their generator. `PD_JURY_RIG_OLLAMA_EMBED_MODEL` and `PD_JURY_RIG_OLLAMA_GENERATOR_MODEL` can select other installed local models explicitly. `pd doctor` remains the operator-facing coverage report when Port Daddy is available.
