...
Staleness is evidence to inspect, not permission to inherit an identity or its claims. AgentNode-bound work uses the `durable_ownership` SDK/API and an exact, short-lived signed grant: `bootstrapDurableOwnership` establishes the current canonical owner's epoch, `prepareDurableTakeover` records the proposed claim transfers/releases and sanitized briefing, and the named successor accepts with `takeoverSession(sourceSessionId, { grantId, nonce })`. Historical owners and notes remain intact; acceptance appends the new epoch and receipt. `getDurableTakeoverGrant` returns an authenticated, redacted lifecycle view, not the one-shot nonce. See the [SDK ownership contract](docs/sdk.md#durable-ownership-and-signed-takeover) and [API reference](docs/openapi.yaml).

Sessions created before AgentNode binding have a separate, narrower continuation: from the exact predecessor worktree and context slot, run `pd session takeover <old-session-id> --same-owner "reason"`. The CLI sends only that slot's existing actor credential over HTTP; it never mints a replacement or asserts the stored display label. The daemon requires the credential actor to equal the predecessor's daemon stamp, re-probes the physical Git worktree and HEAD, and moves the complete unreleased dual-store claim set from an active or abandoned predecessor in one transaction. It preserves the historical agent label for attribution but transfers no AgentNode, roadmap ownership, or durable-owner epoch. Any partial, mismatched, replayed, or AgentNode-bound case fails closed. Promote the successor through the ordinary sanitized-handoff `pd roster promote` ceremony before bootstrapping durable ownership; actor-only continuation never fabricates that node. Full threat model and recovery steps: [Actor-only session continuation](docs/operations/actor-only-session-continuation.md).

`pd session rm` is credential-authenticated archival: it releases active claims and writes a tombstone, but never deletes the session, notes, or claim history. Exact `session end`/`session rm` mutations derive authority only from the daemon credential and reject `agentId`, `X-Agent-Id`, and unauthenticated IPC.
