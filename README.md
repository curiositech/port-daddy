...
Roster agents are daemon-minted `AgentNode` identities that outlive any body or session. Their meaningful slug is a scoped human alias, profile edits append revisions, and promotion requires a fail-closed sanitized handoff episode bound to the native harness session being promoted. Port Daddy coordination can enrich that handoff but is not required for historical sessions. Expertise lookup requires an explicit repository or `system` scope, sanitizes the derivative and query before ranking, and fuses BM25 with the corpus-selected dense space using RRF; any lexical fallback is labeled degraded. Runtime choice does not change the person: `pd roster continue` uses the existing native-or-sanitized-handoff receipt ledger. Stored permissions and triggers are explicitly declarations until a witnessed runtime enforces them. See ADR-0119.
...
pd memory search "how did we deploy" --harbor myapp --repo github:org/repo
...
The [provider-neutral retrieval design](docs/proposals/provider-neutral-retrieval-fabric.md) replaces the universal-model rule with corpus-approved profiles, compatible index generations, RRF of rankings, and privacy filters before both retrievers. The [research record](docs/research/embedding-retrieval-model-landscape-2026.md) identifies candidates, not production winners. Registry-backed role/tier/provider selection and fail-closed local artifact/output verification are source-present. Transcript, roadmap, and durable-roster search now require explicit scope, sanitize source derivatives and queries before tokenization or embedding, store exact corpus-policy and vector-space generations, rebuild unverifiable model-only projections, and fuse independent BM25 and dense ranks with RRF at `k=60`. Separate-generation cutover and rollback are not yet source-present. The same admission and migration still need to reach Tool2Vec, whois, episodic memory, and semantic response caching; signed benchmark promotion, remote budget integration, and operator controls remain separate implementation work. This documentation does not activate a remote model or prove an installed runtime upgrade.
