...
**Key flags:** `--backend`, `--model`, `--tier`, `--identity`, `--purpose`, `--budget`, `--allowedTools` (both Claude CLI aliases), `--maxTokens`, `--workdir`, `--timeout`
...
**Backends in source:** `claude` (SDK), `claude-cli`, `codex`, `gemini`, `cloudflare` (Workers AI), `openai`, `groq`, `deepseek`, `xai`, `ollama`, `lmstudio`, `aider`, `custom`, and CLI-tube backends `cli:claude-code`, `cli:codex`, `cli:agy`, `cli:gemini`, `cli:groq`, `cli:grok`. Operator-facing launches are **fail-closed on telemetry**: metered API backends need exact token counts, an exact nonzero rate, and a persisted exact cost record. Spawn results and transcripts expose requested/effective backend+model provenance plus the override source when preflight or a forced CLI selection changes what actually ran. CLI-tube backends ride the operator's authenticated local CLI and record a flat session estimate. Gemini and agy currently capture the prompt plus final stdout/stderr; their documented stream formats still need fixture-backed parsers. Gemini CLI is blocked until a policy-preserving native skill-suppression overlay is implemented; Groq's unverified CLI and the incompatible Grok proxy are also refused before launch. The Gemini API backend is unaffected. `pd backend` switches the active provider/model configuration; `pd backend adapters` prints the generated N:N portability contract and `--probe` discovers installed binaries, advertised flags, and declared transcript roots without claiming spawn/resume conformance; `pd benchmark run` compares backends with real (paid) calls.

Managed CLI launches use one `executionIntent` in the spawn API/SDK: `manual` (default), `edit-only`, or `autonomous`. Coast Guard configuration never grants autonomy; actual `confined: true` wrapper proof is required before native bypass executes. Manual uses native permission defaults, never automatic approval review. Edit-only maps to verified Claude and agy controls; Codex refuses that unsupported mode. `permissionMode` is removed and rejected. Both Claude aliases share allowed tools, hooks, workspace/resume identity, and explicit caller environment while stripping inherited Anthropic credentials before brokering; direct Claude captures JSON, Tube Claude captures stream JSON.

Port Daddy does not preload the installed skill catalog. Codex receives the mandatory last-wins `skills.include_instructions=false` setting on fresh and resumed runs. Claude Code (both aliases) and agy receive mandatory `--disable-slash-commands`, disabling their native skills without disabling hooks. Caller prompt text is preserved without inventing a skill-selection receipt; dynamic selection belongs to Port Daddy Jury-rig or the caller. Gemini CLI, Groq CLI, and Grok are not launchable or advertised as available until their required contracts are verified; no existing system settings are overwritten. Spawn results and current-daemon history expose resolved `executionIntent` beside the Coast Guard receipt; intent is not proof of confinement.

`injectSquidHooks` is accepted only for the effective Claude backend, after forced-backend resolution. Requested hooks run after wrapper admission; installation failure blocks the child. Malformed existing settings are preserved byte-for-byte, not replaced. This does not claim managed Codex or agy HITL-hook support. Dispatch previews contain only non-executable semantic intent, never command/argv or a fabricated wrapper receipt. Reactive rules preserve explicit intent and default to autonomous only when it is absent.
...
Dispatch accepts `cli:claude-code`, `cli:codex`, and `cli:agy`, explicitly requests autonomous intent, and executes only through an injected Conductor adapter. The legacy raw launcher and ambient GitHub publisher are removed. Worktree creation keeps Guard enabled and uses the durable source project. Publication currently raises `GITHUB_APP_PUBLISHER_REQUIRED`: an authorized scoped App publisher must be wired before push/PR creation; Conductor records the attention error and Dispatch preserves completed work for salvage. Blocked Gemini/Groq/Grok CLI selections cannot become daemon defaults. Operator sorties and memory continuations remain manual. Fleet-engine triggers and Tube recursion retain separate admission contracts; this slice does not claim Conductor lineage for them.
