...
1. **FleetBar** (`apps/FleetBar/`) — the SwiftUI macOS menu-bar app. Daemon health at a glance, berth chip, cost dashboard, native Account Settings, account-backed relay health, operator interruptions with fail-closed critical launch gates, secrets pane, visual task intake, one-click "Open Operator Console". Auto-launched by the daemon.
