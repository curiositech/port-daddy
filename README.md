...
<p align="center">
  ...
  <a href="skills/port-daddy-agent-skill"><img src="https://img.shields.io/badge/MCP%20tools-178-blueviolet?color=3AADAD" alt="MCP tools"></a>
</p>
...
The MCP server exposes **178 tools** across the whole surface — session lifecycle (`begin_session` with the same required `lifecycle` enum, `end_session_full`), ports, notes, locks, messaging, salvage, actors, inboxes, webhooks, DNS, tunnels, sorties, tuples, pheromones, roadmap, commitments, parleys, symbols/conflict prediction, fleet control (bonds/wallets/panic), semantic graph/memory, and discovery (`pd_discover`) — plus **6 resources** (`port-daddy://skill`, `://services`, `://sessions`, `://agents`, `://locks`, `://tunnels`).
