...
\end{tikzpicture}
\caption{\textbf{One added rule kind turns a polynomial check into an
NP-complete one.} Both columns share the identical rule vocabulary of
$L_c$: Horn rules, deontic obligations/prohibitions on scope and interval,
exclusive claims, and difference constraints, decided in
$O(H+T\log T+C\log C+VE)$ with a polynomial witness per conflict
(the in-fragment theorem, left). Letting exactly one obligation become disjunctive
(right) is enough: the extended fragment's conflict-freedom is
NP-complete by reduction from 3-SAT (the frontier theorem), so the boundary is the
presence of that one feature, not a graded notion of expressiveness.
[verified, \texttt{b4\_deontic\_fragment.py}]}
