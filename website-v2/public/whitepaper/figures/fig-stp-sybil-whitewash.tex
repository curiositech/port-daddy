...
\begin{tikzpicture}[pd figure,x=1cm,y=1cm]
  \def\pw{3.70}  \def\ph{3.60}  \def\bx{5.20}
  ...
  \node[pd label,anchor=north east] at (\pw,-.75) {respawns};
  ...
  \end{scope}
  \node[pd note,anchor=north west,text width=9.5cm] at (0,-2.70)
    {scope: one trusted daemon and one bound principal; federation still owes cross-operator attestation};
\end{tikzpicture}
\caption{Binding identity to a principal preserves sanctions across restarts
and prevents aliases from multiplying quorum weight. This protection is
local to one trusted domain, not cross-operator attestation. [internal]}
\label{fig:sybil}
\end{figure*}
