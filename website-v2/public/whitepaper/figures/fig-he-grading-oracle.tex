...
\begin{tikzpicture}[
  ...
  oracle/.style={draw=hhcobalt,line width=1.4pt,fill=hhsand!30,
                 rounded corners=3pt,inner sep=5pt,text width=40mm,
                 align=center,minimum height=18mm},
  fix/.style={draw=hhink,thick,fill=hhsand!30,rounded corners=3pt,
              inner sep=4pt,text width=40mm,align=center,font=\scriptsize},
...
\draw[flow] (grade) -- node[above,font=\scriptsize]{public signal} (folk);

% the capture attack
\node[draw=hhcobalt,thick,fill=hhpaper,rounded corners=2pt,inner sep=3pt,
      font=\scriptsize\itshape,text=hhink,text width=44mm,
      align=center,above=8mm of grade] (capture)
  {\textbf{but the grade is produced by a capturable evaluator}\\
   (an adversarial reviewer or a ``neutral judge'')};
\draw[->,>=Latex,thick,hhcobalt] (capture) -- (grade);


% the two fixes
\node[fix,below=12mm of agent] (sp)
  {\textbf{Fix A:} a machine-checkable oracle the agent cannot author ---
   strategy-proof by construction};
\node[fix,below=12mm of grade] (bond)
  {\textbf{Fix B:} bond-and-slash the judges; a grade overturned on re-audit
   $\Rightarrow$ slash};
\node[fix,below=12mm of folk] (rec)
  {\textbf{Then prove:} the rate-the-raters recursion \emph{terminates} at a
   machine-checkable base};
