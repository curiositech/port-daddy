...
\begin{figure*}[t]
\centering
\begin{tikzpicture}[pd figure,x=.87cm,y=1cm]
  \def\lx{1.65}   \def\x0{2.05}   \def\pw{8.35}
  \node[pd title,anchor=north west,text width=9.5cm,align=left] at (0,5.15)
  ...
  \foreach \x/\lab in {2.05/Open,6.15/Conditional,10.25/Mechanized}
    ...
    \node[pd label,anchor=north,text width=1.7cm,align=center] at (\x,-6.60) {\lab};}
  \node[pd note,anchor=north west,text width=9.5cm] at (0,-7.05)
    {position, not colour, carries the claim: each row advances only when the
    missing artifact or theorem exists};
\end{tikzpicture}
\caption{Evidence differs by threat. Identity and replay have concrete
artifacts; revocation and settlement depend on stated connectivity and
custody assumptions. Admission, cartel monitoring, and centralization remain
open. [internal]}
