...
\begin{tikzpicture}[pd figure,x=1cm,y=1cm]
  ...
  \node[pd note,anchor=north,text width=3.6cm,align=center] at (\ix,-.30)
  ...
  \draw[pd rule,-{Stealth[length=1.8mm,width=1.1mm]}] (\ox,\oy) -- (\xmax,\oy);
  \draw[pd rule,-{Stealth[length=1.8mm,width=1.1mm]}] (\ox,\oy) -- (\ox,\ymax);
  ...
  \fill[pd neutral fill] (\kx,\oy) rectangle (\rx,\ky);
  ...
  \node[pd note,anchor=north,text width=3.1cm,align=center] at ({(\ox+\rx)/2},{\oy-.30})
    {extra deposit returns intact past $B_T$};
\end{tikzpicture}
\caption{Deposit-only Sybil deterrence saturates. One principal can fund $K$
interchangeable identities, but the loss attached to a defaulting identity
rises only until its deposit reaches the transaction coverage $B_T$. Past
that knee, $s=\min(B_{\mathrm{dep}},B_T)=B_T$: additional posted capital
returns intact on no-loss transactions and cannot increase the attacker
maximum loss. Figure~\ref{fig:sybil-mc-inline} shows the corresponding
simulated profit and commons-deficit sweeps. [internal, closed form]}
