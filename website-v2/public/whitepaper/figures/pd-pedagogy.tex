...
% \pdmargincaption[short list entry]{one-sentence caption} -- the numbered
% caption path for figures, diagrams, plots, pictures, and tables. In the Book
% it uses the outer margin and the same collision-aware allocator as glosses
% and evidence notes. Outside the Book it is exactly a normal caption.
% A margin caption that grows beyond a quarter of the text block is no longer
% concise; it falls back to the ordinary caption position instead of silently
% overrunning the column. Call \label immediately after this macro, exactly as
% with \caption.
\newsavebox{\pd@captionbox}
% The caption package is loaded before this shared apparatus. Save its real
% command once: the Book-level router below must be able to fall back without
% calling itself, and research documents must retain ordinary captions.
\let\pd@inlinecaption\caption
\newcommand{\pdmargincaption}[2][]{%
  \ifpdmargincolumn
    \ifx\@captype\@undefined
      \PackageError{pd-pedagogy}{\string\pdmargincaption outside a float}%
        {Use it inside figure, figure*, table, or table*.}%
    \else
      ...
      \sbox{\pd@captionbox}{\begin{minipage}[b]{\marginparwidth}\marginfont
        {\bfseries\@nameuse{fnum@\@captype}.}\enspace #2\end{minipage}}%
      \ifdim\dimexpr\ht\pd@captionbox+\dp\pd@captionbox\relax>.25\textheight
        \typeout{PD-CAPTION-FALLBACK: \@captype\space \@nameuse{the\@captype}}%
        \par\addvspace{\abovecaptionskip}%
        \@makecaption{\@nameuse{fnum@\@captype}}{#2}%
        \par\addvspace{\belowcaptionskip}%
      \else
        \pd@placemargin{\pd@captionbox}%
      \fi
    \fi
  \else
    \def\pd@captionshort{#1}%
...
% One caption path for the assembled Book. Figure and table fragments keep
% writing ordinary \caption calls; in Book mode those calls become numbered
% outer-margin captions, while long captions take the explicit inline fallback
% above. Longtable and other specialist environments retain their own local
% caption commands because this dispatcher only accepts the four real float
% environments. This keeps numbering, labels and list entries centralized
% without 200 per-fragment rewrites.
\newif\ifpd@routecaption
\newcommand{\pd@captiondispatch}[2]{%
  \pd@routecaptionfalse
  \ifpdmargincolumn
    \edef\pd@captionenv{\@currenvir}%
  ...
  \ifpd@routecaption
    \def\pd@captionshort{#1}%
    \ifx\pd@captionshort\@empty\pdmargincaption{#2}\else\pdmargincaption[#1]{#2}\fi
  \else
    \def\pd@captionshort{#1}%
    \ifx\pd@captionshort\@empty\pd@inlinecaption{#2}\else\pd@inlinecaption[#1]{#2}\fi
  \fi}
\newcommand{\pd@captionrouter}{\@ifnextchar[\pd@captionrouter@opt\pd@captionrouter@plain}
\long\def\pd@captionrouter@opt[#1]#2{\pd@captiondispatch{#1}{#2}}
\long\def\pd@captionrouter@plain#1{\pd@captiondispatch{}{#1}}
\AtBeginDocument{\let\caption\pd@captionrouter}
...
% 1. Maturity. The four grades single-writer-kernel.tex names in prose as
% "the series' honest labels": implemented / partial / specified / proposed.
...
% 4. Evidence for a stated result. \Unproved was called \Proposed until the
% maturity scale above became shared: both rendered the single word
% "proposed", in the same gray italic, eight lines apart in chapter 4's own
% legend -- one meaning "no design document yet", the other "no proof here".
% Two axes printing one word is the collapse this apparatus exists to
% prevent, so the evidence scale says what it means instead.
...
\newcommand{\pd@measurewidefield}{%
  ...
  \ifpdmargincolumn
    ...
    % Strict page checking resolves the physical leaf from the aux file.  The
    % outer margin is right on rectos and left on versos, so a wide apparatus
    % must move left by exactly the margin field on an even page.
    \checkoddpage
    \ifoddpage\else
      \setlength{\pd@wideoffset}{-\dimexpr\marginparsep+\marginparwidth\relax}%
    \fi
  \fi}
\lstnewenvironment{pdsession}[1][]{%
  ...
  \makebox[\pd@sessionwidth][l]{\color{pdinkmuted}\rule{\pd@sessionwidth}{0.4pt}}\endgraf\nobreak\vspace{3pt}%
  \noindent\pd@marginhead{At the terminal}%
  \ifx\relax#1\relax\else{\itshape #1.}\fi\endgraf\nobreak
  \lstset{style=pdsession,linewidth=\pd@sessionwidth,xleftmargin=\pd@wideoffset}%
}{%
  \endgraf\nobreak\noindent\hspace*{\pd@wideoffset}%
  \makebox[\pd@sessionwidth][l]{\color{pdinkmuted}\rule{\pd@sessionwidth}{0.4pt}}\endgraf
...
\newtcblisting{pdsampledata}[1][]{%
  ...
  width=\ifpdmargincolumn\dimexpr\linewidth+\marginparsep+\marginparwidth\relax\else\linewidth\fi,
