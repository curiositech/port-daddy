...
\begin{tikzpicture}[pd figure]
  \begin{axis}[pd axis,
    ...
    \addplot[pd rule,domain=1:8,samples=8,line width=.9pt,mark=*,mark size=1.1pt,mark options={fill=hhink}] {1-0.2^x};
    \addplot[pd focus rule,only marks,mark=*,mark size=1.7pt] coordinates {(3,0.992)};
    \draw[pd guide] (axis cs:3,0) -- (axis cs:3,0.992);
    \node[pd direct label,text=hhteal,anchor=south west,align=left] at (axis cs:3.25,0.86) {$k=3$: caught with probability $0.992$};
  \end{axis}
\end{tikzpicture}
...
\begin{tikzpicture}[pd figure]
  \begin{axis}[pd axis,
    ...
    \node[pd direct label,anchor=west,text=hhink] at (axis cs:436,1) {$431.9$};
    \node[pd direct label,anchor=west,text=hhink] at (axis cs:301,2) {$296.9$};
  \end{axis}
\end{tikzpicture}
