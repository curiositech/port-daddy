...
\end{tikzpicture}
\caption{The three parties a sealed room must separate, and the single
boundary that separates them. Derek must not receive Erin's model $M$;
Erin must not receive Derek's data $D$; the cloud operator hosting the
computation must receive neither. All three constraints are edges of one
triangle, and one enclosing mechanism, the sealed room of
Sec.~\ref{sec:sealed-design}, seals every edge at once rather than fencing
each pair separately. [internal]}
