...
\bottomrule
\end{tabularx}}
\caption{The conservation ledger of Theorem~\ref{thm:conservation}: three
buckets per project, joined by five named operations. Only \texttt{topUp}
(gold) grows the supply $S_P$; the other four, including the two-way split
that \texttt{slash} performs (red, dashed, two rails for one operation),
only move money between buckets. The instance under each bucket is the
chapter's own worked partial breach: before $(W_P,E_P,C_P)=(4,6,0)$, a
slash of $p=2$ retires the full escrow, and after $(8,0,2)$ --- $S_P=10$
both times. [internal, \texttt{Conservation.tla}]}
