% Fig 1 — the L0->L3 stack map, placing Floor 3 (the bridge). Harbor Volume idiom.
% Design: one bold title + small-caps whom per rung, one italic content line with
% an honest-state tag; Floor 3 is a cobalt bracket spanning the L2->L3 gap.
...
\node[rung,fill=hhsand!30,above=5mm of l2] (l3)
  {{\bfseries L3 --- Economy \& federation}\hfill{\footnotesize\textsc{for the market}}\\[2pt]
   \textit{anchor, escrow, reputation, federation, the labor/skill market}\hfill{\footnotesize$\square$~\textsc{designed}}};

% Floor 3 = the bridge from L2 to L3: a cobalt bracket spanning the gap
...
\node[paper,draw=hhcobalt,very thick,anchor=east,align=center]
  ...
  {\textbf{Floor 3}\\\emph{From Spawn to Person}\\\textit{the bridge}};

% companion papers, left column
\node[paper,anchor=east] at ([xshift=-9mm]l0.west) {Beam~A\\\emph{Anchor Protocol}};
\node[paper,anchor=east] at ([xshift=-9mm]l2.west) {Floor~2\\\emph{Legible Swarm}};
\node[paper,anchor=east] at ([xshift=-9mm]l3.west) {Floor~4\\\emph{Harbor Economy}};
\end{tikzpicture}}
...
the market. Beam~A (Anchor Protocol) supplies the cryptographic identity below; Floor~4 (Harbor Economy) consumes the
