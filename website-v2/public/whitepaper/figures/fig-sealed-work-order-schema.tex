...
\end{tikzpicture}
\caption{The work order $C$'s thirteen fields, grouped by the five roles
the chapter itself names. The four hashes (shaded, top) pin an attested
\emph{measurement} of each named object -- the data manifest, the agent
bundle, the runtime, and the policy -- not the object's live state at
query time; function and tools fix what may run; recipients and output
schemas fix who may receive a result and in what shape; budgets and
retention bound the engagement; validators, payment, and liability are
the remedy if something goes wrong. Any change to any field produces a
new hash for $C$, so both principals must sign again. [internal]}
