...
At this point, Acme's risk officer revokes Controller-7's \texttt{db:migrate} card and publishes $R_A^{(e+2)}$. Under Theorem~\ref{thm:fh-conv}'s connected reliable-round model, dissemination completes in expected $\Theta(\Delta\log m)$ time. During a partition, receiving harbors must rely on expiry or a fail-closed freshness policy; the example does not pretend gossip supplies a deadline.
