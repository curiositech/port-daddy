...
\fancyfoot[C]{\footnotesize\color{hhgray} The Harbor Volume \textperiodcentered\ Beam~C: Federated Harbor \textperiodcentered\ v0.9 (pre-print)}
...
\title{
  \vspace{-1cm}
  {\Large\textsc{The Harbor Volume --- Beam C: Federated Harbor}}\\[0.5cm]
...
\node[draw=hhebony,line width=0.8pt,rounded corners=5pt,fill=hhsand!22,
  ...
  {\color{hhcobalt}\textbf{This paper: Beam~C --- Federation.}}~Two harbors, each trustworthy, jointly useless --- and the witness log that fixes it. \textsc{$\square$ designed.}\\[6pt]
  {\color{hhebony!35}\rule{\linewidth}{0.4pt}}\\[5pt]
  {\footnotesize It builds on Beam~A (Anchor Protocol, \S\ref{sec:fh-xfer}), Beam~B (Bonded Commons, \S\ref{sec:fh-settlement}), and the Floor~1 kernel daemon. Either companion can be read first or in parallel.}
};
\end{tikzpicture}
\end{center}
...
\item \textbf{Independent publishability.} The act of publishing $R_X^{(e)}$ to $W$ does not require $D_X$ to trust any other party; the cross-signatures collected from other daemons in the federation are an auditor convenience, not an authorization gate.
\end{enumerate}
\end{property}
...
The federation is \emph{not} a permissionless market in identities. New harbors enter through the admission ceremony of \S\ref{sec:fh-admission}, which requires either a pre-existing bond or an existing harbor sponsor. This is by design; the cost is that the federation is invitation-bounded, the benefit is that Sybil attacks against the federation~\cite{douceur2002sybil} cost the attacker as much per identity as honest entry does. \emph{Note: the out-of-scope attacks discussion (\S\ref{sec:fh-attacks-out}, the Sybil paragraph) names the cost-ratio bound as open work; this claim should be read as a design goal, not an established result.}
...
Each of these is a deliberate design choice. The Federated Harbor's architecture is not more powerful than alternatives; it is more honest about what is happening at each step.
...
\paragraph{Mechanized (\Closed).} Only one claim in this paper is fully mechanized: single-harbor capability authenticity, inherited unchanged from Anchor (ProVerif, \Closed). Everything the Federated Harbor adds on top is at best \Partial. This row is the only \Closed entry in Appendix~\ref{app:fh-mech}, Table~\ref{tab:fh-status}, and the body must not claim more.

\paragraph{Proof sketch, pending full mechanization (\Partial).} The three central federation claims are \Partial, matching Appendix~\ref{app:fh-mech}, Table~\ref{tab:fh-status} exactly: cross-realm authenticity (draft ProVerif model, sound only under an unmechanized witness-log freshness assumption); attenuation monotonicity across the boundary (Lemma~\ref{lem:fh-att}, \Partial under the same caveat); and the cross-harbor Conservation invariant (TLA+/Apalache, model-checked only at $|F| \le 4$ and leaning on a structural argument beyond that scale). None of these is a closed theorem; each is a proof sketch awaiting full mechanization.
...
HTLC-style atomic swaps~\cite{herlihy2018atomic} achieve trustless settlement for fungible assets with a shared hash preimage. The Federated Harbor's bonds are non-fungible (reputation-priced under competitive insurance) and may need a trusted-but-bounded escrow rather than a trustless one. We adopt the bounded-escrow design (\S\ref{sec:fh-settlement}); the escrow mechanism is therefore \Partial, not closed.

\begin{conjecture}[Trustless settlement impossible for non-fungible bonds]\label{conj:fh-trustless}
Fully trustless settlement --- in the HTLC sense, with no trusted-but-bounded third party --- is impossible for non-fungible, reputation-priced bonds of the class used in the Federated Harbor.
\end{conjecture}

\noindent We state this without proof. A clean negative result would be a worthwhile separate paper.
...
A federation paper with five named open questions is doing its job. The point of the paper is to draw the new boundary cleanly so the open problems are visible; if every claim in this paper were closed, there would be no third paper to write. The substrate is in production; the federation layer is a research agenda built on it. Anchor and Bonded have a tighter theorem-to-conjecture ratio for the same reason the sky has a floor: they are the floor.
...
\paragraph{Maturity-vocabulary mapping.} The Beams (Anchor, Bonded, Federated Harbor) use the \Closed/\Partial/\Open\ vocabulary; the Floors (Single-Writer Kernel, Legible Swarm, From Spawn to Person, Harbor Economy) use Built/Designed/Vision. They map across the volume as: \Closed~$\approx$~Built, \Partial~$\approx$~Designed (or BuiltWeak), and \Open~$\approx$~Vision.
...
\vspace{0.8cm}
\noindent\emph{Exercises, traces, and open problems for this paper --- with worked solutions --- are in the Harbor Volume Workbook, Part~VII.}
