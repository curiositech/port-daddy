...
The Pareto-dominance and Sybil-deposit-floor Monte Carlo charts are placed inline next to their respective discussions (Figures~\ref{fig:pareto-mc-inline} and~\ref{fig:sybil-mc-inline} in \S\ref{sec:youle:welfare}--\S\ref{sec:youle:a5}). This appendix carries only the A6 repeated-game cartel sweep, whose finer-grain $(p_d, \delta)$ grid is supplementary to the body discussion. Raw run logs and the rendering script are bundled at \url{https://portdaddy.dev/whitepaper/artifacts/}.
...
\paragraph{E5 (Cartel folk-theorem).} Using the A6 grid (\S\ref{app:montecarlo}, Figure~\ref{fig:cartel-folk}), compute the minimum per-round detection probability $p_d^\star$ at $\delta = 0.99$. Then argue, qualitatively, what would change if cartel members can re-enter under fresh identities. Does the bond mechanism prevent re-entry, deter it, or merely price it?
