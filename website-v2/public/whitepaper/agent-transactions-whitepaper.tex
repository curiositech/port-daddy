...
\paragraph{Cost.} Each settlement adds an $O(1)$ amortized append to the epoch accumulator (de Marneffe 2011). Epoch rollover is $O(N)$ hashes plus one signature: at 100 sessions/hour, $\approx 10$ms. Negligible.
...
\emph{Under full information and competitive entry, the market-discovered premium Pareto-dominates any authority-chosen static parameter.} A formal proof is deferred to a companion appendix prepared by Youle.
