...
That third party has to be settled \emph{before} any interaction, not improvised transaction by transaction---otherwise establishing it is just the $O(n^2)$ trust negotiation of \S1.1 under a new name. We call it the \pdgloss{commons authority}{the one always-on, pre-trusted service that gives agents identity, capability bounds, shared knowledge, an immutable history, and settlement --- without deciding who is right.}: one always-on service that every agent already trusts, precisely because it was there before any of them showed up. Five capabilities make it that, and the definition below names them.
...
\input{figures/fig-bonded-three-layers}
...
For high-assurance environments and for fleets spanning multiple daemons, the evidence trail is strengthened to a three-level \pdgloss{Merkle Forest}{one Merkle tree per settlement epoch, each root signed by its own daemon, so an auditor can verify inclusion across daemons with a short proof instead of database access.}. The single-daemon Merkle chain---each note hashing the previous, with a per-session root---is necessary but not sufficient: the moment two daemons exist, an auditor without database access still needs to verify that a particular note was included in a particular session using only a short proof.
