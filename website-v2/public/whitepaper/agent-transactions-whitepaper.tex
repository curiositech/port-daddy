...
The original specification states the escrow invariant in TLA$^{+}$ but does not commit to an operational accounting. The April 2026 reference implementation in the daemon's bond-accounting subsystem collapses the abstract escrow into three monetary buckets per project---\textbf{wallet}, \textbf{escrow}, and \textbf{commons pool}---and lets us formalize what \texttt{EscrowInvariant} only asserted in prose.
...
\midrule
Merkle-Forest binding (\S\ref{sec:merkle-forest}) & EasyCrypt + fast-check & \Partial\ skeleton discharged; PROM-formal future work & \texttt{binding.ec} \\
...
\paragraph{Limitations.} Four mechanizations are carried indefinitely behind the empirical evidence above: a PROM-formal Merkle binding in EasyCrypt; a Coq or Lean mechanization of the Pareto strategic game; a formal analysis of the composite Sybil-deterrence mechanism ($C_{\mathrm{kyc}}$ $+$ reputation gating $+$ per-class $B_{\mathrm{dep}}$); and a multi-class generalization of the repeated-game cartel result to heterogeneous coordination classes (\S\ref{sec:taxonomy}). These are documented as known open problems rather than as defects in the present claims, which stand on their stated evidence.
