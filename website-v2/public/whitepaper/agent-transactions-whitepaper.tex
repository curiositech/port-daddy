...
\newcommand{\SpecOnly}{{\textcolor{hhcobalt}{\textbf{\textsf{spec-only}}}}}
...
\fancyfoot[C]{\footnotesize\color{hhgray} The Harbor Volume \textperiodcentered\ Beam~B: Bonded Commons \textperiodcentered\ pre-release}
...
\begin{abstract}\noindent
Two autonomous agents working on the same project corrupt each other's state, double-claim the same files, race for the same ports, and leave the operator to do recovery archaeology. The problem is not the agents---it is that they have no coordination substrate. This paper specifies one. We treat coordination as a \emph{correlated equilibrium} in the sense of Aumann~\cite{aumann1974subjectivity}: a daemon as correlating device sends private recommendations to each agent---which files to claim, which to defer, when to settle---and following the recommendation is each agent's best response given that others follow theirs. The substrate is built from three independently load-bearing layers---capability-attenuated tokens that physically bound scope, Merkle-chained evidence trails that make anonymous harm impossible, and collateralized work contracts that pre-fund damage regardless of intent---and the correlating-device reading is what makes the layers cohere. We formalize the construction in the context of Port Daddy, a coordination daemon for local agent swarms, and show that the resulting system does not require agents to be trustworthy, to share values, or to fear punishment. It requires only that their principals post collateral proportional to the risk their agents impose on the commons, and that the daemon's recommendation distribution be common knowledge. We ground the non-enforced design of the resource claim system in Sen's Impossibility of a Paretian Liberal, showing that enforced allocation with private agent knowledge is provably suboptimal: the daemon's job is to broadcast the conflict graph, not to adjudicate it. We further show how the same authority can be equipped with a \emph{mutable-signal ledger} (\S\ref{sec:pheromones}) that supports revocation, renaming, and provenance-aware propagation of coordination hints without sacrificing attribution integrity---revocations are themselves events in the chain. Finally, we describe a competitive-insurance pricing mechanism (\S\ref{sec:youle}, contributed by Youle) in which insurer agents bid to underwrite agent transactions, replacing static parameter selection with market-discovered bond prices; under the correlating-device reading, the same equilibrium structure governs agent obedience and insurer non-collusion, so the price of anarchy stays bounded at both layers. Hobbes had the right idea about sovereigns; he just lacked a way to deploy one on \texttt{localhost:9876}.
\end{abstract}
...
\node[draw=hhebony,line width=0.8pt,rounded corners=5pt,fill=hhsand!22,
  ...
  {\color{hhcobalt}\textbf{This paper: Beam~B --- Conservation \& bonding.}}~Walls, not laws: a correlated-equilibrium sovereign for mutually-suspicious agents. \textsc{$\square$ designed.}\\[6pt]
  {\color{hhebony!35}\rule{\linewidth}{0.4pt}}\\[5pt]
  {\footnotesize It can be read first or on its own; Beam~A (Anchor Protocol) supplies the cryptographic substrate this paper consumes.}
};
\end{tikzpicture}
\end{center}
...
The paper builds in four tracks; each carries a load-bearing theorem or empirical result. The four tracks use distinct primitives and can be read independently; the table below maps them. If you have less than 50 minutes, start at the track that matches your question.

\vspace{0.4cm}
\noindent
\begin{tabular}{p{3.5cm} p{8.5cm} p{3.5cm}}
\toprule
\textbf{Track} & \textbf{Load-bearing artifact} & \textbf{Lives at} \\
\midrule
...
We present:
\begin{enumerate}
  \item A reading of the coordination daemon as a \textbf{correlating device} in the sense of Aumann~\cite{aumann1974subjectivity}: claim recommendations are private signals drawn from a common-knowledge distribution, and agent obedience is an equilibrium rather than an enforced policy (Section~\ref{sec:commons}, esp.\ \S\ref{sec:correlating-device}). This reframes what earlier drafts called ``advisory'' coordination: the daemon's recommendations are incentive-compatible because the equilibrium structure makes them so, not because a sword is in reserve.
...
The daemon's role admits a precise game-theoretic name. Aumann~\cite{aumann1974subjectivity} introduced the \emph{correlated equilibrium}: a randomized strategy profile in which a trusted third party draws a recommendation tuple $(r_1, \ldots, r_n)$ from a joint distribution $\mu$ over action profiles, privately reveals $r_i$ to player $i$, and each player's best response---given posterior beliefs about the other players' recommendations conditional on its own---is to play $r_i$. The construction is strictly more permissive than Nash: every Nash equilibrium is a correlated equilibrium where $\mu$ is a product of marginals, and the correlated set strictly contains it when $\mu$ has non-trivial correlation. The mechanism does not need a sword. The recommendations are obeyed because, given common knowledge of $\mu$ and each player's own draw, deviating is dominated. \emph{To reconcile this with the coercive signals the daemon does ship (SIGTERM, bond-slash, session kill; \S\ref{sec:structural}): those signals are not in tension with the no-sword claim. By Theorem~8.1 of \emph{The Single-Writer Kernel} (Floor~1)~\cite{owens2026singlewriter}, a post-commit monitor cannot enforce a safety invariant---it can only detect and compensate. The coercive signals are therefore \emph{compensating responses} that fire \emph{after} a defection, not an ex-ante enforcement sword. The correlated-equilibrium claim holds for agents who follow the protocol; the compensating signals handle defectors after the fact. The two are complementary, and A5 (\S\ref{sec:youle:a5}) is why neither is dispensable: bonds alone are provably insufficient against an attacker whose fraud gain exceeds the deposit.}

This is exactly the daemon's role. The commons authority observes the global claim graph---who wants which resource, who holds which capability, whose history shows what. On a contested file claim, the daemon does not adjudicate; it broadcasts a recommendation pair: \emph{Agent~$\alpha$, you may proceed on \texttt{auth.ts}; Agent~$\beta$, the surface is held, defer or coordinate.} The recommendations are drawn from a deterministic policy over the conflict graph (the pheromone-lineage primitive of \S\ref{sec:pheromones} furnishes the draw; the policy is public). Each agent receives only its own recommendation, but the distribution over recommendations and the dependence on the public conflict graph are common knowledge.

\begin{property}[Daemon-as-Correlating-Device]\label{prop:correlating-device}
Port Daddy's daemon implements a correlating device for the agent-coordination game. Specifically:
\begin{enumerate}[nosep]
  ...
  \item \textbf{Obedience as best response.} Given that the other agents obey, an agent that defies its own recommendation faces (a) the bond it posted against the harm class it just realized (\S\ref{sec:economic}), (b) the reputation discount that compounds across future settlements (\S\ref{sec:rep-discount}), and (c) the conflict-graph completeness guarantee of \S\ref{thm:advisory}, which makes the deviation visible to every counterparty. Conditional on the others' obedience, deviation is strictly dominated.
\end{enumerate}
\end{property}

The reframe is what turns the paper's headline result from a description into a guarantee. The correlated-equilibrium reading answers the obvious question immediately: nothing stops them, and nothing needs to. The recommendation distribution and the bond structure jointly leave each agent's incentive constraint slack at obedience. Enforcement is the wrong frame; mechanism design is the right one. The detailed deviation analysis lives in Section~\ref{sec:economic}; the safety and liveness properties of the lifecycle are checked formally in Section~\ref{sec:tlaplus}.

A consequence worth surfacing: the \emph{price of anarchy} of this game---worst-case ratio of social cost at any correlated equilibrium to the social optimum---is one to one in the limit where bonds cover full cleanup cost and reputation noise is bounded. The simulation evidence of \S\ref{sec:youle:welfare} supports this empirically for $\sigma_r \le 0.1$. The analytical bound under realistic reputation noise is open; we do not close it here.
...
Structural prevention is enforced at two levels: \emph{statically} by the Anchor Protocol's ProVerif-verified token exchange (all three protocol phases verified in ProVerif 2.05~\cite{owens2026anchor}), and \emph{dynamically} by the \textbf{Arbiter}---an ambient security agent that subscribes to the daemon's event stream and checks every state transition against six formally derived invariants. These invariants map directly to the safety properties of the TLA$^{+}$ specification in Section~\ref{sec:tlaplus}: \texttt{NoteMonotonicity}, \texttt{EscrowInvariant}, and \texttt{LockOwnerValid} are compiled from the formal model into synchronous runtime checks. Violations are logged immutably and, in strict mode, trigger the salvage protocol---the sovereign's sword in code form. \emph{A reconciling note on what this `sword' can and cannot do: by Theorem~8.1 of \emph{The Single-Writer Kernel} (Floor~1)~\cite{owens2026singlewriter}, a post-commit monitor cannot enforce a safety property---it can only detect and compensate. The Arbiter's salvage signals (SIGTERM, bond-slash, session kill) are therefore \emph{compensating responses} that act \emph{after} a violation is observed; they do not enforce a safety invariant ex ante. Prevention proper is structural---boot gate, primary-key uniqueness (mutual exclusion), single-writer routing---and is what the correlated-equilibrium claim of \S\ref{sec:correlating-device} relies on. The two are complementary, not contradictory: the equilibrium governs agents who follow the protocol, and the compensating signals handle defectors after the fact.}
...
\begin{enumerate}
  \item \textbf{One-shot defectors} must post collateral \emph{before} receiving capabilities. The Sybil attack is priced: creating 100 disposable identities means posting 100 bonds. The cost of defection scales linearly with the number of attack identities. \emph{Note: A5 (\S\ref{sec:youle:a5}) proves that pure deposit deterrence alone is insufficient when fraud gains exceed the bond; the composite admission mechanism---per-identity onboarding cost and reputation gating (\S\ref{sec:youle:a5})---and bid-pattern detection (Anchor~Protocol Beam~A, \S6.4~\cite{owens2026anchor}) are the load-bearing defenses, not the bond alone.}
...
The bond machinery above prices \emph{outcomes}: an agent that destroys files pays for the destruction. The paper's headline claim is upstream of outcomes---that truthful file-claim signaling (\S\ref{sec:attribution-survives}, \S\ref{sec:advisory}) is incentive-compatible in the first place. Truthful claim signaling is incentive-compatible; we prove it here. The cartel analysis of \S\ref{sec:youle:a6} works the parallel argument for insurer pricing.
...
\emph{Note: This result is argued as economic rationale; it is not formally proved or mechanized. Companion papers that invoke this as a `proven bound' should be read as citing an argued economic principle pending formal mechanization.}
...
Competitive insurance (\S\ref{sec:youle}) is a thick-market mechanism. It needs insurers with capital, principals with reputation history, and enough transaction volume that bidders can statistically distinguish good risks from bad. A cold-start deployment has none of that. The Pareto-dominance result of \S\ref{sec:youle:welfare} is the end-state; Phases~1--2 below are the path to it, not optional preamble.
...
The Pareto-dominance result of \S\ref{sec:youle:welfare} is conditional on Phase~3 being reached and the listed metrics being met. Phases~1--2 are not bureaucratic preamble: they generate the data the competitive-insurance market needs to price at all. A deployment that skips them is running \S\ref{sec:youle} on uninformative priors, and the dominance claim does not hold.
...
\paragraph{Threat-class bond bands and their defensibility.}\label{sec:pricing:threats}
...
This result extends to the agent layer: under the correlating-device interpretation of \S\ref{sec:correlating-device}, deviation incentives for agents and insurers are computed against the same per-round equilibrium structure, so the cartel folk-theorem threshold $p_d^\star$ bounds collusion at both layers.
...
\bibitem{owens2026singlewriter}
Erich Owens.
\newblock The Single-Writer Kernel: Mutual Exclusion and the Limits of Post-Commit Monitoring for Local Agent Swarms.
\newblock Technical White Paper, Curiositech LLC, 2026.
...
\vspace{0.8cm}
\noindent\emph{Exercises, traces, and open problems for this paper --- with worked solutions --- are in the Harbor Volume Workbook, Part~VI.}
