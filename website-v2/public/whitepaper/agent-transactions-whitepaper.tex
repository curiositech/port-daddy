...
This matches two independent committed artifacts. \texttt{proofs/economics/delta-threshold.z3}'s second query (\texttt{delta-threshold.expected.txt}) asserts the root lies in $[0.34, 0.35]$ and returns \texttt{sat}, agreeing with the sign change computed here. Independently, \texttt{proofs/economics/sweep-delta.sh}'s committed sample sweep (documented in \texttt{proofs/economics/README.md}) reports the model-checked IC invariant \texttt{VIOLATED} at $\delta=0.34$ and \texttt{HOLDS} at $\delta=0.35$---the same sign change, checked by a state machine instead of a closed-form inequality.
\end{pdsolution}
...
Pinned verdict sequence (\texttt{proofs/economics/delta-threshold.expected.txt}):
\begin{lstlisting}
sat
(
...
The committed log (\texttt{proofs/bonded/conservation/Conservation.run.log}) reports:
\begin{lstlisting}
...
Claim-signaling IC (\S\ref{sec:economic}) & TLA+ (TLC) + Z3 SMT & \Closed\ unique root $\delta^\star{\approx}0.3425$; IC invariant fails at $\delta = 0.30$, holds at $\delta = 0.35$ & \texttt{claim\_signaling.tla}, \texttt{delta-threshold.z3} \\
