...
\paragraph{Setup.} The principal $P$ has a coordination budget and wants $\alpha$ to ship the fix. The commons authority is the local Port Daddy daemon. The acceptance criteria are: (i) the failing test \texttt{login.regression.test.ts} passes, (ii) no other test regresses, (iii) the diff touches only \texttt{auth.ts} and the test file. The estimated cleanup cost if $\alpha$ defects or sabotages is \$5 (one operator-hour at \$5/hr equivalent, since this is a local agent). Figure~\ref{fig:worked-example} traces the seven steps below across the three architectural layers.

\input{figures/fig-worked-example}

\paragraph{Step 1: Capability issuance (Layer 1).} $P$ requests a Harbor Card for $\alpha$ via the daemon, specifying the capability set $\{\texttt{fs:read:repo}, \texttt{fs:write:auth.ts}, \texttt{fs:write:tests/**}, \texttt{cmd:test}\}$ and a TTL of $30$~minutes. The Anchor Protocol (\S\ref{sec:structural}, companion paper~\cite{owens2026anchor}) signs the card; $\alpha$ can prove its identity and capabilities to any verifier, but it physically cannot write to, e.g., \texttt{database.ts}---attempts return a capability error before the file system is touched.

\paragraph{Step 2: Bond posting (Layer 3).} The Bonded Advisor (\S\ref{sec:pricing}) quotes a bond proportional to the cleanup cost: $B_\alpha = \$5 \cdot (1 + s) = \$6$ at slope $s = 0.2$. $P$ posts \$6 into escrow. The settlement preconditions are now (1) acceptance criteria met $\Rightarrow$ \$6 returned, (2) partial completion $\Rightarrow$ pro-rata, (3) sabotage $\Rightarrow$ \$6 liquidated to the commons treasury.

\paragraph{Step 3: Advisory claim (\S\ref{sec:sen}).} $\alpha$ claims \texttt{auth.ts} via \texttt{pd session files add auth.ts}. The daemon's conflict graph is currently empty for this path, so the claim succeeds with no conflict notification. The claim has a TTL bound by the Harbor Card's: $30$ minutes.

\paragraph{Step 4: Evidence chain begins (Layer 2).} $\alpha$ writes a scope note: \emph{``Scope: \texttt{auth.ts}. Hypothesis: cookie-domain handling on subdomain login fails. Validation: \texttt{npm test login.regression}.''} The note's content hash $h_1$ chains into the session's Merkle root $R_{\text{session}}^{(1)}$. $\alpha$ then reads \texttt{auth.ts}, writes a partial fix, runs the failing test. Each action becomes an evidence-chain entry: a read receipt, a diff blob, a test-runner stdout. Five notes deep, $R_{\text{session}}^{(5)}$ is the canonical commitment to $\alpha$'s state.

\paragraph{Step 5: Mid-task crash (\S\ref{sec:crash-recovery}).} At minute~12, $\alpha$'s process crashes (let's say the LLM provider timed out). The daemon's heartbeat monitor observes $\alpha$ as unresponsive after $90$~seconds and marks the session abandoned. $\alpha$'s claim on \texttt{auth.ts} releases on TTL; the evidence chain is sealed at $R_{\text{session}}^{(5)}$ and committed into the harbor root $R_{\text{harbor},\ell}$ at the next epoch boundary. \emph{Critically, no information is lost}: the partial diff, the test output, and $\alpha$'s scope note are all in the chain.

\paragraph{Step 6: Successor agent.} $P$ spawns $\beta$ with a fresh Harbor Card and the same capability set. $\beta$'s first action is \texttt{pd salvage}: the daemon returns the abandoned session, the scope note, the partial diff, and the last test output. $\beta$ reads the scope note (``cookie-domain handling on subdomain login''), inspects the partial fix, and finishes the work. The new evidence chain $R_{\text{session}}^{(\beta)}$ references $R_{\text{session}}^{(5)}$ as its salvage parent---attribution is preserved (\S\ref{sec:attribution-survives}).

\paragraph{Step 7: Settlement.} At minute~24, $\beta$'s final action is \texttt{pd done}: the acceptance-criteria checker runs the regression suite, confirms \texttt{login.regression.test.ts} passes, confirms no other test regresses, confirms the diff is bounded to the two declared files. All three criteria met. The daemon releases \$6 from escrow to $P$. The settlement record signs the evidence chain root and is appended to the harbor Merkle forest (\S\ref{sec:merkle-forest}).
