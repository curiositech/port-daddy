...
\paragraph{Mechanized claim-signaling.} Repeated claim-signaling between two agents under the daemon's correlating recommendation supports truthful coordination via a \emph{graduated trigger} strategy: cooperate by default; on observed deviation, both agents play mutual-defer for three rounds, then return. Standard one-shot-deviation analysis yields the discount-factor threshold $\delta^\star$ as the unique real root in $(0,1)$ of $3\delta^3 + 3\delta^2 + 3\delta - 1 = 0$, numerically $\delta^\star \approx 0.2531$. The cubic, the existence-and-uniqueness of its root in $[0.25,\,0.26]$, and a TLA$^{+}$ model checking that no one-shot deviation produces positive discounted payoff at $\delta = 0.26$ are mechanized as \texttt{proofs/economics/delta-threshold.z3} and \texttt{proofs/economics/claim-signaling.tla}. Both run unattended in CI (\S\ref{app:mech}, Table~\ref{tab:verification-status}).
