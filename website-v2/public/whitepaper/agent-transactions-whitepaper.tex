...
% ═══════════════════════════════════════════════════════════════════════════════
\section{Monte Carlo Visualizations}\label{app:montecarlo}

Three figures summarize the empirical evidence behind the §8.4.4 Pareto-dominance theorem and the two new regimes (A5, A6) closed in this round. All raw run logs are tracked at \texttt{proofs/bonded/pareto/*.run.log}; the rendering script is at \texttt{website-v2/public/whitepaper/figures/render\_montecarlo.py}.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/fig-pareto-dominance.pdf}
  \caption{Pareto dominance of competitive auction over static escrow.
  Left: dominance rate as a function of reputation noise $\sigma_r$ at $n=5$ insurers, split by cartel size. Dominance holds robustly with no cartel even at $\sigma_r = 0.5$; with 3 colluders it collapses past $\sigma_r \approx 0.1$. Right: dominance rate vs.\ $n$ at $\sigma_r = 0$ and no cartel; $n=3$ is the empirical optimum due to the winner's-curse order-statistic effect noted in \S\ref{sec:youle:welfare}.}
