...
% One key, used in the header and everywhere a claim is graded. The labels are
% deliberately neutral engineering-maturity terms, not project-internal jargon.
...
\node[draw=hhebony,thick,fill=hhpaper,rounded corners=2pt,
      inner sep=8pt,text width=\dimexpr\textwidth-22pt\relax,align=left]{%
  \small\textbf{\sffamily Maturity key (honest self-grading).}\enspace
...
\emph{credit} (\textsf{cr}); read it as a stablecoin cent if you like.}
...
\emph{Side~1 (labor).} Bob posts a float plan with a $400$\,\textsf{cr} bounty;
Alice takes it as a firm. She dispatches three of her own agents, posts a
$60$\,\textsf{cr} sub-bond per agent ($180$\,\textsf{cr} total), and keeps
$400 - (\text{slashed sub-bonds}) - \text{fee}$. If one of her three agents botches
its slice and is slashed $60$\,\textsf{cr}, Alice eats that loss --- she internalized
...
afternoon at $90$\,\textsf{cr}. Now there are three parties: Bob (poster), Alice
...
lint-and-type skill into Bob's float plan at $0.5$\,\textsf{cr} per file, metered,
...
$400$\,\textsf{cr}, worker bond $100$\,\textsf{cr}, acceptance criteria = ``the
...
$40\% \times 100 = 40$\,\textsf{cr} of its bond pro-rata; the remaining $60$ of the
...
\item Trace the flow of funds for a \textbf{Partial} settlement where the worker
  ...
  bond of $250$\,\textsf{cr} and $70\%$ completion).
...
$50$\,\textsf{cr} judge-bond and rules \emph{for Bob}. But Judy and Bob are
...
finds it indefensible, and \emph{contradicts} it. Judy's $50$\,\textsf{cr} bond is
