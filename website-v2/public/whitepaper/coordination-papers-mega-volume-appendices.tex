...
\subsection*{Open problems, all three lists in one place}
...
the cost of that decision is that no reader can see the whole set at once. This
concordance indexes them; it does not renumber them, and the owning chapter's
table stays the one to trust for status.

\begin{table}[H]
\centering
\caption{Open-problem concordance. Statuses are the owning chapter's own, as of
this edition; where a later chapter bears on a problem without closing it, the
last column says how.}
...
ch.~8 & The federation limitations section: deployment-specific propagation tail bounds, and reconciling sheaf diffusion with epidemic gossip. & The second now carries a falsifier (rounds-to-first-alarm against $1/\lambda_2$) and no scheduled run. \\
Cross-chapter & Claim signalling under \emph{private} monitoring: ch.~7's equilibrium assumes perfect public monitoring, which one harbor's ledger supplies and gossip does not. & Open in both ch.~7 and ch.~8; neither re-derives it, and ch.~8's settlement uses bonds rather than triggers to avoid needing it. \\
\bottomrule
\end{tabularx}
\end{table}
