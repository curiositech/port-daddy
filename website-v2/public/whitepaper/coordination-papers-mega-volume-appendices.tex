...
which channels the institution can see and control. Two public gradings sit
beside this one on different axes: Sheridan's levels of automation grade how
much of a task a system performs unassisted, and the Common Criteria's
evaluation assurance levels grade a security claim by how much independent
verification backs it. This ladder grades neither task share nor verification
effort; it grades a runtime claim by the evidence the mechanism itself
produces.
...
% \flushbottom (the book's default) stretches every page to \vsize exactly;
% this generated table set is a chain of xltabular blocks with no
% stretchable glue of its own between rows, and one page break in it lands
% 2.96pt over \vsize -- an Overfull \vbox from the output routine, not a
% line of type. A dense reference appendix does not need flush-bottom
% uniformity as strongly as body prose does, so it is scoped ragged here:
% pages are free to fall a hair short instead of being forced to a height
% no combination of its own rows actually fits.
{\raggedbottom
...
\clearpage
\IfFileExists{figures/pd-marginalia-credits.tex}{\input{figures/pd-marginalia-credits}}{}
