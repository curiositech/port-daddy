...
\fancyfoot[C]{\footnotesize\color{hhgray} The Harbor Volume \textperiodcentered\ Floor~3: Personhood}
...
\title{
  \vspace{-1cm}
  {\Large\textsc{The Harbor Volume --- Floor 3: Personhood}}\\[0.5cm]
...
single-operator tool (Floor~2 and Beam~A) to the cross-operator market (Floor~4). We
...
assuming it. We prove the necessity of non-forgeable identity (Theorem~\ref{thm:necessity})
for any sanction-respecting reputation, and \emph{sketch} a cost bound for
whitewashing attackers under an unproven schedule-existence assumption
(Theorem~\ref{thm:whitewash-cost}, proof sketch); the grading-oracle
termination on which the central incentive-compatibility argument rests
remains a conjecture (Conjecture~\ref{conj:contract}).
\end{abstract}
...
Swarm} (Floor~2) and \emph{The Anchor Protocol} (Beam~A) supply the
legibility surfaces and the cryptographic identity ceremony this paper builds on;
\emph{The Harbor Economy} (Floor~4) consumes this paper's reputation
...
\node[draw=hhebony,line width=0.8pt,rounded corners=5pt,fill=hhsand!22,
  ...
  {\color{hhcobalt}\textbf{This paper: Floor~3 --- Personhood.}}~Identity is the gate, not the score --- role + continuity = an accountable principal. \textsc{$\square$ designed.}\\[6pt]
  {\color{hhebony!35}\rule{\linewidth}{0.4pt}}\\[5pt]
  {\footnotesize This is the bridge of the volume: it carries the through-line from the legible single-operator tool (Floors~1--2) to the cross-operator market (Floor~4). Most of the substrate is honestly not built yet; the paper says so, in the same words, everywhere the volume leans on it. Any Floor can be opened first.}
};
\end{tikzpicture}
\end{center}
...
\emph{not} a bandit), \S\ref{sec:multidim-reputation} (the reputation protocol),
...
\bottomrule
\end{tabular}
\caption{Reader's Map. Within the volume the suggested order is Floor~2 (the
wedge) $\to$ Beam~A (the formal core) $\to$ \textbf{Floor~3 (this one, the
bridge)} $\to$ Floor~4 (the market). This paper can be read directly after
Floor~2 if you only want the ``why a person can be priced'' argument.}
...
across machines, work settled against bonds (The Harbor Economy, Floor~4) --- is impossible until a
...
% --- FIGURE 1: the L0->L3 stack map, placing Floor 3 (From Spawn to Person) ---
...
\text{reputation}
\;\to\;
\underbrace{\text{a tradeable asset}}_{\text{Floor~4}}.
\]
...
How to score quality? & Elo/BT/TrueSkill (scalar) & multi-dimensional vector, judge-per-axis (\S\ref{sec:multidim-reputation}) \\
...
execution snapshot --- is owned by the formal-core paper of this volume (The Anchor Protocol, Beam~A),
...
$\star$ \textit{Open.} (3) When an LLM agent dies mid-thought, what is recoverable
...
\begin{itemize}[leftmargin=1.4em]
  ...
  cliques.'' It is the right tool for the \emph{federated} reputation The Harbor
  Economy (Floor~4) needs (trust that travels across harbors), and it is \emph{not} a bandit.
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  argument warned against (\S\ref{sec:multidim-reputation}).
...
% ═════════════════════════════════════════════════════════════════════════════
\section{The multi-dimensional reputation protocol}\label{sec:multidim-reputation}
% ═════════════════════════════════════════════════════════════════════════════
...
skill's author? \\
$\star$ \textit{Open.} (4) If you publish the 3-vector and buyers weight it, do
...
Make judging a bonded role (\S\ref{sec:multidim-reputation}): $J$ posts a bond before grading;
...
$\star$ \textit{Open.} (3) Prove or refute Conjecture~\ref{conj:contract}. If the
...
$\star$ \textit{Open.} (4) Arbitration capacity: human oracles are scarce. Design
...
$\star$ \textit{Open.} (3) Reputation-claim revocation with an append-only,
...
the cross-harbor analogue of capability revocation (The Harbor Economy, Floor~4).
\end{exercises}

% ═════════════════════════════════════════════════════════════════════════════
\section{What this paper hands the market (The Harbor Economy, Floor~4)}\label{sec:handoff}
% ═════════════════════════════════════════════════════════════════════════════

This paper is the bridge; its output is a precise hand-off to \emph{The Harbor
Economy} (Floor~4). We summarize what is delivered, what is borrowed, and what is
explicitly owed.
...
Multi-dimensional reputation (Def.~\ref{def:qvec}, Protos.~\ref{proto:judge},~\ref{proto:skill}) & \DESIGNED$\to$\VISION &
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item \textbf{Vector-reputation disclosure (\S\ref{sec:multidim-reputation}).} Publish a
  ...
  under load?
  \item \textbf{Skill-version reputation (\S\ref{sec:multidim-reputation}).} A portfolio proof
...
Multi-dimensional reputation & \DESIGNED$\to$\VISION &
No reputation component yet; the substrate it scores over is partly built. \\
Neutral-judge ceremony (Proto.~\ref{proto:judge}) & \DESIGNED$\to$\VISION &
...
\vspace{0.8cm}
\noindent\emph{Exercises, traces, and open problems for this paper --- with worked solutions --- are in the Harbor Volume Workbook, Part~III.}
