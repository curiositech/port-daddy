...
\node[draw=hhebony,thick,fill=hhsand!30,rounded corners=2pt,
      ...
      align=left] (cbox) {%
  \small\textbf{\sffamily accompanying chapter.}\enspace This paper is the cryptographic substrate. Its companion, \emph{The Bonded Commons: Pre-Transactional Trust Infrastructure for Multi-Agent Systems}, builds the economic and game-theoretic layer on top: bonded participation, a conditional auction/static comparison, A5/A6 mechanism analysis, and a Sen-inspired warning against turning advisory claims into coercive entitlements. The Anchor Protocol is a self-contained subset; \emph{The Bonded Commons} consumes it as the trust kernel for the wider mechanism design. Read this paper for ``how does the daemon authenticate an agent?''; read the companion for ``why should there be a daemon at all?''
};
\end{tikzpicture}
\vspace{4pt}
