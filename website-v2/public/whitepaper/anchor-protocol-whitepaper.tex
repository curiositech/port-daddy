...
% Maturity markers for the Appendix A status table. The glyph carries the grade;
% plain text after it carries the degree.
\newcommand{\Closed}{{\textcolor{hhteal}{\textbf{\textsf{$\blacksquare$ built}}}}}
\newcommand{\Partial}{{\textcolor{hhamber}{\textbf{\textsf{$\square$ designed}}}}}
\newcommand{\Open}{{\textcolor{hhink}{\textbf{\textsf{$\cdot$ open}}}}}
...
\fancyfoot[C]{\footnotesize\color{hhebony!50} The Harbor Volume \textperiodcentered\ Beam~A: Anchor Protocol \textperiodcentered\ v1.0 (pre-print)}
...
This paper introduces the \textbf{Anchor Protocol}, a cryptographic and semantic identity framework built into the Port Daddy daemon. We detail the protocol's evolution from symmetric MACs to asymmetric Ed25519 signatures and multi-hop delegation chains (inspired by Macaroons~\cite{birgisson2014macaroons}). Furthermore, we present the formal verification of the protocol's design using the \textbf{ProVerif} symbolic analyzer~\cite{blanchet2016modeling} and the mathematical proof of its implementation, demonstrating immunity to algorithm confusion~\cite{mclean2015critical}, impersonation, and timing side-channels. We are precise about scope: the multi-hop delegation soundness is machine-checked at the chain depth realized in deployment (depth~3), and we state explicitly (\S\ref{sec:limitations}) that extending the symbolic proof to unbounded chain depth---as distinct from the unbounded \emph{sessions} ProVerif already quantifies over---remains future work.
\end{abstract}
...
\vspace{0.4cm}
\noindent\textit{Reading time: approximately 30 minutes end-to-end. The companion paper, \emph{The Bonded Commons: Pre-Transactional Trust Infrastructure for Multi-Agent Systems} (Owens 2026), carries the economic and governance argument; either paper can be read first.}
...
\node[draw=hhebony,line width=0.8pt,rounded corners=5pt,fill=hhsand!22,
  ...
  {\color{hhcobalt}\textbf{This paper: Beam A --- Identity \& capability.}}~Math-based identity for a process: a verified control plane on localhost. \textsc{$\blacksquare$ built \,$\cdot$\, ProVerif, Kani.}\\[6pt]
  {\color{hhebony!35}\rule{\linewidth}{0.4pt}}\\[5pt]
  {\footnotesize It can be read first or on its own.}
};
\end{tikzpicture}
\end{center}
...
\node[draw=hhebony,thick,fill=hhsand!30,rounded corners=2pt,
      ...
      align=left] (cbox) {%
  \small\textbf{\sffamily Companion paper.}\enspace This paper is the cryptographic substrate. Its companion, \emph{The Bonded Commons: Pre-Transactional Trust Infrastructure for Multi-Agent Systems}, builds the economic and game-theoretic layer on top: bonded participation, conditional-Pareto-dominant auctions, A5/A6 mechanism analysis, and the Sen-grounded justification for why coercive coordination is required at all. The Anchor Protocol is a self-contained subset; \emph{The Bonded Commons} consumes it as the trust kernel for the wider mechanism design. Read this paper for ``how does the daemon authenticate an agent?''; read the companion for ``why should there be a daemon at all?''
};
\end{tikzpicture}
\vspace{4pt}
...
The four phases are not four proofs. Phases~1--3 --- pinning, asymmetric identity, and multi-hop delegation --- carry the protocol's authentication and attenuation guarantees, and those three are what ProVerif mechanizes for an unbounded number of sessions (\S\ref{sec:algorithms}). Phase~4, revocation, narrows acceptance rather than granting it, so it is enforced at runtime against monotonicity and reanimation invariants rather than re-proved symbolically (\S\ref{sec:revocation}). When this paper says ``three verified phases,'' it means those first three; ``four phases'' counts the full Harbor Card lifecycle.
...
Agent A takes its existing Harbor Card, appends a new set of restricted capabilities (e.g., \texttt{["db:read"]} reduced from \texttt{["db:read", "db:write"]}), and signs the \emph{entire chain} with its own ephemeral private key.
...
The two figures for this bound state it two ways that agree. The ``$O(\log m)$ rounds'' phrasing counts gossip rounds asymptotically; the $\Delta(1+\ln m)$ bound multiplies the expected round count by the period $\Delta$ to give wall-clock time. The companion federation paper proves the same bound in its cross-harbor setting; both papers use $\Delta(1+\ln m)$. Plugging in $m=10$ and $\Delta=30$\,s gives $\approx 100$\,s expected, $\approx 2$\,min worst case --- the figure's headline number.

\paragraph{Soundness preservation.} Revocation strictly narrows acceptance, so existing ProVerif soundness proofs (Theorem~\ref{thm:injective}) are unaffected. The new proof obligation is small: revocation is \emph{enforced}, which follows by inspection of the verification path---the revocation check is unconditional and precedes admission. The card lifecycle gains a state ($\texttt{valid} \to \texttt{revoked} \to \texttt{expired}$) modeled as an additional transition.
...
\end{align}
This \emph{injective agreement} property~\cite{lowe1997hierarchy} guarantees that capabilities cannot be escalated and trust is perfectly transitive.
\end{theorem}
...
Formal proofs guarantee protocol \emph{design} correctness. At \emph{runtime}, Port Daddy deploys the \textbf{Arbiter}---an ambient security agent that subscribes to the daemon's activity log and checks every state transition against six formally derived invariants:
...
\paragraph{Detect-and-compensate, not enforcement.} The Arbiter is a post-commit monitor: it observes the activity log \emph{after} a transition has been committed. By the impossibility result of \emph{The Single-Writer Kernel} (Floor~1, Theorem~8.1, after Schneider~2000), a post-commit monitor cannot \emph{enforce} a safety property; it can only detect a violation and compensate. The Arbiter's coercive signals --- SIGTERM, bond-slash, session kill, the ``man overboard'' salvage --- are therefore \emph{compensating responses}, not enforcement of a safety property. The daemon \emph{prevents} only at three points: the boot gate, the SQLite primary-key uniqueness constraint (mutual exclusion on the single-writer slot), and the single-writer routing convention. Everything downstream of the commit, including every Arbiter invariant above, is detect-and-compensate.
...
To prevent timing attacks where an adversary guesses signatures byte-by-byte~\cite{brumley2011remote}, we implemented a custom constant-time byte comparator. Kani formally verified that this comparator is branch-free regarding the contents of the secret arrays.\footnote{Branch-freedom in source (confirmed by Kani) is necessary but not sufficient for constant-time execution on real hardware; the compiler and microarchitecture can reintroduce timing variation. The side-channel resistance claim is therefore \emph{partial} (see Appendix, Table~\ref{tab:anchor-status}): we document library trust rather than a side-channel proof.}
\end{property}
...
\textbf{\color{maydayred}Warning:} Algorithm~\ref{alg:delegation} shows the \emph{unsound} final-vs-root verifier for exposition --- note that the in-loop subset check (\texttt{cap\_i not\_subseteq cap\_root}) compares each hop against \texttt{cap\_root}, not against the immediately preceding hop's grant. A secure implementation MUST check each hop's capabilities against the immediately preceding hop's grant (per-hop checking), not against the root capability. See \S\ref{sec:attn-soundness} (Appendix A.5.3) for the attack (\texttt{v6}) and the correct fix (\texttt{v7}).%
}}\par\vspace{4pt}

\textbf{Security Property:} The subset check on line 7 and line 22 ensures \emph{capability attenuation}---delegated capabilities can only be restricted, never expanded. This is the mechanized content of Theorem~\ref{thm:attn-sound} (\S\ref{sec:attn-soundness}): under an enriched model with a real capability order and an active insider escalation adversary, ProVerif proves no escalating delegation is ever accepted, and a negative control confirms the guard is load-bearing. Combined with Theorem~\ref{thm:injective}, this guarantees transitive trust.
...
\begingroup\sloppy
\begin{enumerate}
  \item \textbf{Delegation-chain depth.} The symbolic proof of delegation soundness (Table~\ref{tab:anchor-status}, \texttt{chain-replay.pv}) is machine-checked at the chain depth realized in deployment, depth~3. It is not a proof for unbounded chain length. ProVerif's unbounded-session guarantee quantifies over the number of protocol \emph{sessions}, not over the number of hops in a single delegation chain; the hop count is a fixed structural bound in the modeled process. Extending the proof to unbounded depth, via recursive replication of the delegation process, is future work.

  Two facts about the current depth handling matter. First, the depth bound is a spawn-time policy check, not a verification-time invariant: \texttt{assessDelegation} in \texttt{lib/\allowbreak tube-spawner-router.ts} caps depth at \texttt{HARD\_MAX\_DELEGATION\_DEPTH}\,=\,8 (default~4) when a spawn request arrives. Second, the cryptographic chain verifier (\texttt{lib/\allowbreak delegation-chain.ts}) accepts arbitrary depth, and the attenuation monitor (\texttt{lib/\allowbreak cap-attenuation-monitor.ts}) enforces only per-hop capability monotonicity, not total depth. Machine-enforced depth at verification time is a planned hardening item.
...
\paragraph{Maturity key.} This paper uses \Closed/\Partial/\Open\ for Beam~A status. The companion Floor papers (\emph{The Single-Writer Kernel}, \emph{The Legible Swarm}, \emph{From Spawn to Person}, \emph{The Harbor Economy}) use \textsc{Built}/\textsc{Designed}/\textsc{Vision} with equivalent meaning: Closed~$\approx$~Built, Partial~$\approx$~Designed (or BuiltWeak), Open~$\approx$~Vision.

\paragraph{Artifact availability.} Every artifact named in Table~\ref{tab:anchor-status} is bundled at \url{https://portdaddy.dev/whitepaper/artifacts/}. The verified Rust core is the open-source \texttt{harbor-card-rs} crate; runtime components are deployed inside the Port Daddy daemon binary. The companion paper, \emph{The Bonded Commons}, carries the cross-paper status table; this appendix lists only the items that pertain specifically to the Anchor Protocol.

\renewcommand{\arraystretch}{1.15}
\begin{table}[H]
\centering
\caption{Anchor Protocol verification status. \Closed: model verified and runtime conformance test passes. \Partial: design verified, runtime empirically tested rather than proved. \Open: known unmechanized. The glyph carries the grade; the text after it carries the degree.}
\label{tab:anchor-status}
\small
\begin{tabular}{@{}p{56mm}p{32mm}p{28mm}p{30mm}@{}}
\toprule
\textbf{Claim} & \textbf{Method} & \textbf{Result} & \textbf{Artifact} \\
\midrule
Authentication + injective agreement, all three phases (\S\ref{sec:algorithms}) & ProVerif 2.05 & \Closed\ all three phases & \texttt{harbor\_card\_v\{1,2,3\}*.pv} \\
...
GitHub-App webhook origin authentication & ProVerif (Dolev-Yao relay) & \Closed\ origin auth TRUE; daemon re-verifies GitHub HMAC (runtime conformance) & \texttt{github\_ingress\_origin\_auth.pv} \\
Multi-tenant relay namespace isolation & ProVerif + runtime & \Closed\ member of $A$ cannot publish into $B$; \texttt{HARBOR\_MISMATCH} conformance & \texttt{github\_ingress\_tenant\_isolation.pv} \\
\midrule
Cuckoo-filter revocation soundness & inspection + empirical & \Partial\ gate strictly narrows acceptance; gossip bound reproduced from~\cite{demers1987epidemic} & runtime test \\
Constant-time signature comparison & audited library (\texttt{subtle::ConstantTimeEq}) & \Partial\ no side-channel proof; library trust documented & --- \\
Relay payload secrecy via HPKE (daemon key pinned) & ProVerif (design) & \Partial\ secrecy TRUE under pinned key; seal not yet implemented & \texttt{github\_ingress\_hpke\_secrecy.pv} \\
Webhook replay-freedom + in-order delivery & TLA+ / TLC (exhaustive at bound) & \Partial\ design model-checked; runtime via ordered-chain ingest & \texttt{WebhookDelivery.tla} \\
Card revocation finality under backup/restore (ADR-0018 Attack~2) & TLA+ / TLC & \Partial\ rollback attack + revocation-epoch mitigation both model-checked & \texttt{CardRevocation.tla} \\
\midrule
Formal gossip-convergence bound & --- & \Open\ leaning on standard anti-entropy analysis~\cite{demers1987epidemic} & --- \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Limitations.} The cuckoo-filter pollution closure handles the local-filter side; cross-peer gossip propagation still leans on the standard anti-entropy analysis rather than a mechanized proof. The side-channel resistance of signature comparison is inherited from an audited library rather than re-proved. Both are open problems, named here and tracked in Table~\ref{tab:anchor-status}.
...
\subsection{Phase 2: Asymmetric Ed25519}\label{app:phase2}
...
\subsection{Phase 3: Multi-hop Delegation}\label{app:phase3}
...
\vspace{0.8cm}
\noindent\emph{Exercises, traces, and open problems for this paper --- with worked solutions --- are in the Harbor Volume Workbook, Part V.}
