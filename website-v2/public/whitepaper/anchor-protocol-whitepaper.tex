...
\fancyfoot[C]{\small Port Daddy v3.7.0 — Formally Verified}
...
\author{
  \textbf{The Port Daddy Engineering Team}\\[0.2cm]
  \texttt{engineering@portdaddy.dev}
}
\date{March 16, 2026\\Version 1.1 (Port Daddy v3.7.0)}
...
% ═════════════════════════════════════════════════════════════════════════════
\section{Implementation: Rust Core}
% ═════════════════════════════════════════════════════════════════════════════

The following Rust implementation demonstrates the verified cryptographic core:

\begin{lstlisting}[style=rust, caption={Rust Implementation — Constant-Time Verification}]
//! Harbor Token Verification Core
//! 
//! This module implements constant-time cryptographic verification
//! to prevent timing side-channel attacks.

use subtle::ConstantTimeEq;
use ed25519_dalek::{VerifyingKey, Signature, Verifier};

/// A Harbor Card containing identity and capabilities
#[derive(Debug, Clone)]
pub struct HarborCard {
    pub agent_id: String,
...
/// 
/// # Security
/// This function is verified to be branch-free with respect to
...
/// Verifies an Ed25519-signed Harbor Card
pub fn verify_harbor_card(
    token: &[u8],
    signature: &Signature,
    harbor_pubkey: &VerifyingKey,
) -> Result<HarborCard, VerifyError> {
    // Verify signature first
    harbor_pubkey
        .verify(token, signature)
        .map_err(|_| VerifyError::InvalidSignature)?;
    
    let card = parse_token(token)?;
...
pub fn verify_delegation_chain(
    ...
    let root = &chain.hops[0];
    root_pubkey
        .verify(&root.token, &root.signature)
        .map_err(|_| VerifyError::InvalidRootSignature)?;
    
    let root_cap = parse_capabilities(&root.token)?;
    
    // Verify each delegation hop
    for window in chain.hops.windows(2) {
        let parent = &window[0];
        let child = &window[1];
        
        let parent_pk = extract_pubkey(parent)?;
        
        parent_pk
            .verify(&child.token, &child.signature)
            .map_err(|_| VerifyError::InvalidDelegationSignature)?;
        
        let child_cap = parse_capabilities(&child.token)?;
        if !is_subset(&child_cap, &root_cap) {
            return Err(VerifyError::CapabilityEscalation);
        }
    }
    
    parse_token(&chain.hops.last().unwrap().token)
}

fn is_subset(a: &[String], b: &[String]) -> bool {
    a.iter().all(|cap| b.contains(cap))
}
\end{lstlisting}
...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Property} & \textbf{Verified} & \textbf{Tool} & \textbf{Reference} \\
\midrule
...
Memory Safety & Yes & Kani & \cite{aws2022kani} \\
Constant-Time Comparison & Yes & Kani & \cite{brumley2011remote} \\
\bottomrule
\end{tabular}
\caption{Verified Security Properties}
