...
\S\ref{sec:correct}, Arbiter invariant \texttt{PID\_SQUATTING} &
...
The bound in each case is recorded, not just asserted here: \path{whitepaper/corpus.json} carries an \texttt{evidencePolicy} field for \texttt{harbor-card-kani-verify-logic-only}, \texttt{harbor-card-kani-constant-time}, and \texttt{harbor-card-kani-capability-attenuation} stating exactly these limits, so a claim that drifted past this chapter's wording would be visible against the manifest rather than only against memory.
\end{pdsolution}
...
The only change is in the verifier: v6's single check \texttt{is\_subset(final\_cap, root\_cap)} becomes two checks in sequence, \texttt{is\_subset(mid\_cap, root\_cap)} for hop~1 ($A \to B$ against the root) \emph{and} \texttt{is\_subset(final\_cap, mid\_cap)} for hop~2 ($B \to C$ against what $B$ actually held). Same malicious $B$, same forged \texttt{cap\_write} delegation, same query --- only the verifier's comparison target moves from ``root'' to ``immediate parent.''
