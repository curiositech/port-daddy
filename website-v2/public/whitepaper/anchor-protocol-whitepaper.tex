...
\begin{itemize}
\item \textbf{Cuckoo-filter pollution attack} (raised in v2.4, carried through v2.5). An adversary who can inject many false-positive entries could degrade revocation precision. We have a defensive design (insertion-rate bound + per-issuer namespace) but no implementation in \texttt{lib/cuckoo-filter.ts} yet, and no formal model. This is the primary outstanding security question for the next round.
\end{itemize}

\subsection*{How to read these claims}

``Closed'' means the property is mechanized: a verifier or proof assistant has checked the design against an explicit attacker model. ``Partially closed'' means the design is verified but the implementation is exercised empirically rather than proved. ``Open'' means the answer is plausible but has not been written down formally.
