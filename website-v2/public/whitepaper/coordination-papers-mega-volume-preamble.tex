...
%
% Two-sided margins (2026-09-16): the margin column now sits on the OUTER
% edge of every page -- left on a verso (even) page, right on a recto (odd)
% page -- via geometry's own inner=/outer= keys plus the \documentclass's
% twoside option, rather than fixed left=/right=. inner=0.8in is the narrow
% gutter-side margin on every page; outer=1.7in is the wide margin that
% holds marginparwidth on every page, whichever physical side "outer" is.
% This is safe for the same reason \marginnote (loaded in pd-pedagogy.tex,
% see its own note there) is safe and a bare \ifoddpage read at hook time is
% not: \marginnote resolves the actual side from LaTeX's own \@mparswitch
% machinery at output-routine time, when the page's parity is finally
% settled, not when the macro issuing the note happens to fire. Turning on
% twoside here is what makes that resolution meaningful; it does not
% introduce a new hook-time parity read anywhere.
...
\def\pgfsys@typesetpicturebox#1{%
  ...
  \ifdim\@tempdima>\linewidth
    ...
    \else
      % Two-sided margins (2026-09-16): this branch used to let a picture
      % wider than \linewidth hang into the margin column, flush toward it.
      % That choice needed to know, AT THIS POINT IN THE SOURCE, which
      % physical side this picture's page will finally have its margin on --
      % and that reading was already tried once (a \checkoddpage/\ifoddpage
      % branch) and reverted, because \thepage (and anything built on it)
      % names the page TeX currently THINKS it is composing, not the page a
      % picture pushed across a break by the page builder actually ships on:
      % confirmed on this Book, where fig-sealed-pillar-pipeline's hook fired
      % at \thepage=118 (even) for a figure that ships on page 119 (odd).
      % Now that the margin genuinely alternates sides (see the twoside/
      % inner=/outer= geometry above), that same stale reading would put the
      % figure's excess into the WRONG side's gutter on any page where the
      % hook and the shipout disagree on parity -- worse than useless, since
      % the reader-facing margin apparatus (\marginnote-based, resolved
      % correctly at output-routine time, not here) sits on the other side.
      % Rather than reintroduce a hook-time parity read to chase which side
      % is outer, a full-width picture is simply fit to \linewidth like any
      % other -- it no longer intentionally spans into the margin. This is a
      % deliberate scope reduction, not an oversight: a figure that needs
      % the extra room can still be drawn at \pdfullwidth's aspect ratio and
      % resized down, same as any other, and \pdfullwidth remains defined
      % above for that purpose. Revisit only alongside a real shipout-time-
      % safe horizontal-offset mechanism (paralleling how \marginnote itself
      % resolves side, not a \thepage read), not with a new hook-time guess.
      \PackageWarning{pd-book}{picture \the\wd\pd@picbox\space wide scaled to a \the\linewidth\space box (two-sided margins: no longer spans into the margin column)}%
      \resizebox{\linewidth}{!}{\usebox{\pd@picbox}}%
    \fi
  \else
    \usebox{\pd@picbox}%
