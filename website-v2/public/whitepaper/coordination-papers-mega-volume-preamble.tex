...
% inner/outer, not left/right: the Book is twoside (see the documentclass
% option), so a fixed left=/right= pinned the wide margin column to the same
% physical side of every page -- recto AND verso -- instead of the outer
% edge each page actually has. inner is the narrow, spine-side margin (what
% was "left"); outer is the wide margin carrying the column (what was
% "right"). geometry mirrors inner/outer per page automatically once twoside
% is active, and marginnote's own \if@twoside/\ifodd\c@page check (see
% figures/pd-pedagogy.tex) then places each \marginnote on whichever side
% \marginparwidth actually landed on for that page.
...
\newsavebox{\pdqbox}\newsavebox{\pdpartbox}\newlength{\pdplateh}
\makeatletter
% Force the next page to a recto (odd-numbered, right-hand) start, the
% standard book convention for a chapter or part opener -- inserting a
% genuinely blank verso page when needed rather than leaving one half-empty.
% This is the kernel's own \cleardoublepage (\if@twoside\ifodd\c@page...),
% not a from-scratch reimplementation, with one addition: the kernel's
% version does not touch the inserted blank page's style, so on a page that
% has a running head/foot (any \pagestyle other than empty) the blank verso
% page would still print a header -- \thispagestyle{empty} on it here is
% what makes the forced page actually blank, the way this Book's own
% \pdchapter/\pdpart already do for the opener page itself.
