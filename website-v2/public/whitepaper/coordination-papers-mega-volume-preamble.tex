...
\geometry{paperwidth=7in,paperheight=10in,inner=0.8in,outer=1.7in,%
  top=0.85in,bottom=0.95in,marginparsep=0.2in,marginparwidth=1.3in}
\normalmarginpar
...
% baseline cadence. amsthm's class defaults carry highly stretchable vertical
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[RE,LO]{\small\itshape\color{hhgray} \pdeditiontitle}
...
% Swiss-modern contents spread, generated semantically from textbook.json.
% Each page carries two part-colour fields using the same grotesk, hard rules,
% numerals, and uncoated-paper ground as the part and chapter plates.
\newsavebox{\pdbandbox}
\newlength{\pdbandwidth}
\newcommand{\pdbandcolor}{pdcobalt}
\newcommand{\pdcontentsspreadbegin}{%
  \thispagestyle{empty}\pagecolor{hhpaper}\color{hhink}%
  \begin{adjustwidth*}{0pt}{-\dimexpr\marginparsep+\marginparwidth\relax}%
  \vspace*{0.15cm}%
  \noindent{\pdgrotesk\fontsize{9.5}{12}\selectfont\bfseries Contents\hfill 01 / 02}\par
  \vspace{0.3cm}\noindent{\color{hhink}\rule{\linewidth}{1.4pt}}\par
  \vspace{0.55cm}%
  \noindent{\pdgrotesk\fontsize{38}{41}\selectfont\bfseries The argument\\chapter by chapter}\par
  \vspace{0.3cm}%
  \noindent\begin{minipage}{0.76\linewidth}\pdgrotesk\small
    Each chapter stands on the ones before it. A proof chapter closes a promise
    an earlier chapter had to leave open.
  \end{minipage}\par\vspace{0.55cm}}
\newcommand{\pdcontentsspreadbreak}{%
  \end{adjustwidth*}\clearpage
  \thispagestyle{empty}\pagecolor{hhpaper}\color{hhink}%
  \begin{adjustwidth*}{0pt}{-\dimexpr\marginparsep+\marginparwidth\relax}%
  \vspace*{0.15cm}%
  \noindent{\pdgrotesk\fontsize{9.5}{12}\selectfont\bfseries Contents\hfill 02 / 02}\par
  \vspace{0.3cm}\noindent{\color{hhink}\rule{\linewidth}{1.4pt}}\par
  \vspace{0.55cm}}
\newcommand{\pdcontentspart}[3]{%
  \hypersetup{linkcolor=hhpaper}%
  \renewcommand{\pdbandcolor}{#3}%
  \setlength{\pdbandwidth}{\dimexpr\linewidth-28pt\relax}%
  \begin{lrbox}{\pdbandbox}\begin{minipage}[t]{\pdbandwidth}%
    \color{hhpaper}\pdgrotesk\vspace{12pt}%
    {\noindent\fontsize{28}{28}\selectfont\bfseries #1\hspace{0.5em}%
      \fontsize{15}{18}\selectfont\bfseries Part #1\enspace\textperiodcentered\enspace #2}\par\vspace{8pt}}
\newcommand{\pdcontentschapter}[5]{%
  \par\noindent\begin{minipage}[t]{1.6em}\raggedleft\bfseries\pdchaplink{#2}{#1}\end{minipage}\hspace{0.65em}%
  \begin{minipage}[t]{\dimexpr\pdbandwidth-2.25em\relax}\raggedright
    \pdchaplink{#2}{\bfseries #3}\ifx\relax#5\relax\else\enspace{\scriptsize\itshape #5}\fi\par
    {\footnotesize #4\par}\end{minipage}\par\vspace{5pt}}
\newcommand{\pdcontentspartend}{%
  \vspace{7pt}\end{minipage}\end{lrbox}%
  \par\vspace{8pt}\noindent{\setlength{\fboxsep}{0pt}%
    \colorbox{\pdbandcolor}{\hspace{14pt}\usebox{\pdbandbox}\hspace{14pt}}}\par}
\newcommand{\pdcontentsbackmatter}{%
  \vfill\noindent{\pdgrotesk\fontsize{9.5}{12}\selectfont\bfseries Continue}\par
  \vspace{0.3cm}\noindent{\color{hhink}\rule{\linewidth}{0.7pt}}\par\vspace{0.35cm}%
  \noindent\begin{tabularx}{\linewidth}{@{}>{\bfseries}l X@{}}
    Introduction & The unit of account is the work.\\
    Result Atlas & Claims, evidence, limits, and status in one ledger.\\
    Solutions & Checks, traces, and open obligations from all eight chapters.\\
    References & One bibliography with live back-references.\\
  \end{tabularx}}
\newcommand{\pdcontentsend}{%
  \pdcontentsbackmatter\end{adjustwidth*}\clearpage\nopagecolor\color{hhink}%
