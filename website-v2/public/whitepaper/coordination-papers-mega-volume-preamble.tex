...
% Leading. \changefontsize's optional argument defaults to 1.2\@tempdima, so
% loading fontsize with only a size left the Book on a 1.20 ratio -- 10.5 on
% 12.6, measured 10.46 on 12.55 in the built PDF. That is LaTeX's default, not
% a decision, and it is tight for a long measure: the further the eye has to
% travel back along a line, the more vertical separation it needs to find the
% start of the next one, which is why the band for a long-form book runs about
% 1.25-1.32 rather than the 1.2 that suits a narrow column.
%
% 13.4 pt on 10.5 pt is 1.276 nominal (1.28 on the 10.46 pt Pagella actually
% sets). Three reasons for that number and not another in the band:
%
%  - Pagella is a Palatino, and Palatino's x-height is among the largest of the
%    book faces (about 0.47 of the em). A large x-height fills the interline
%    space, so a Palatino looks tighter than its ratio and wants the upper half
%    of the band, not its floor.
%  - The measure after the re-cut below is about 64 characters. At the bottom of
%    Bringhurst's 45-75 band 1.25 would do; at 64 it does not.
%  - 13.4 pt is the LARGEST value in the band that still fits 44 lines in this
%    text block: \textheight is 10 - 0.85 - 0.95 = 8.2 in = 590.4 pt, and
%    44 x 13.4 = 589.6 pt, with 0.8 pt to spare. 13.5 pt drops the page to 43
%    lines and costs another 15 pages for a difference no reader would name.
%
% \changefontsize scales \small, \footnotesize and the rest by the same 127.6%,
% and leaves \baselinestretch at 1, so every explicit \fontsize{a}{b} in this
% preamble (listings, the display sizes, the plate type) is untouched.
\changefontsize[13.4pt]{10.5pt}
...
% 5.5 (§5.5, The three organs of continuity) and 7.7.5 (Pricing the Bond) landed
...
% ---------------------------------------------------------------------------
% Page geometry: a two-sided 7 x 10 in book block, cut for perfect binding.
%
% The horizontal budget, inner edge to outer edge, sums to the 7 in trim:
%
%     inner (gutter)   0.95 in
%     textwidth        4.25 in
%     marginparsep     0.18 in
%     marginparwidth   1.12 in
%     outer trim white 0.50 in
%     -----------------------
%     total            7.00 in
%
% geometry's own equation takes three of those as one outer margin:
% inner 0.95 + textwidth 4.25 + outer 1.80 = 7.00, where the 1.80 in outer
% margin is the margin column (0.18 + 1.12 = 1.30) plus 0.50 in of trim white
% beyond it. \marginnote reads \marginparwidth and, under twoside, flips the
% column to the outer edge of each leaf; that flip is the whole point.
%
% Why these numbers rather than the previous left=0.8, right=1.7, text=4.5,
% sep=0.2, mpw=1.3:
%
%  - The old sum left 7 - (0.8 + 4.5 + 0.2 + 1.3) = 0.2 in of trim margin. A
%    routine +/-0.125 in trim tolerance puts the guillotine 1/16 in from live
%    margin text. 0.50 in is the floor trade binderies ask for; this takes it.
%  - The gutter gets more than the outer white, not less, because a 551-page
%    perfect-bound block loses roughly a quarter to three-eighths of an inch at
%    the spine to binding creep and to the page simply not opening flat. 0.95
%    in of specified gutter reads as about 0.6 in to the eye once bound, which
%    is about the 0.5 in the outer edge keeps without losing anything.
%  - The 0.75 in this buys (0.15 in of gutter, 0.30 in of trim white, and the
%    0.30 in the two together still needed) is paid for by the two things that
%    can pay without touching the body type: 0.25 in off textwidth and 0.20 in
%    off the margin column (0.18 in off marginparwidth, 0.02 in off
%    marginparsep). The body type size does NOT move -- see \changefontsize.
% ---------------------------------------------------------------------------
...
\pagestyle{fancy}
\fancyhf{}
% Twoside running heads. fancyhdr's bare [L]/[R] mean "left and right of every
% page", which under twoside puts the folio against the spine on every verso.
% The folio belongs on the outer edge of each leaf, where a thumb finds it:
% right on odd (recto), left on even (verso). The running title takes the
% inner position opposite it.
\fancyhead[LO,RE]{\small\itshape\color{hhgray} \pdeditiontitle}
\fancyhead[RO,LE]{\small\color{hhgray}\thepage}
\fancyfoot[C]{\footnotesize\color{hhgray} Textbook Edition \textperiodcentered\ \pdeditiondate}
\renewcommand{\headrulewidth}{0.4pt}
\renewcommand{\footrulewidth}{0pt}

% \pdcleartorecto -- \cleardoublepage, but the blank leaf it inserts is blank.
%
% Everything two-sided keys off \c@page: geometry's inner/outer, \marginnote's
% choice of margin, the running heads above. So the page COUNTER's parity has
% to agree with the physical leaf's parity, odd meaning recto, or every one of
% them is mirrored. A \pagenumbering that restarts the counter on an even leaf
% breaks that agreement for the whole book after it, which is exactly what the
% front matter did: title, imprint and frontispiece are leaves 1-3, and
% \pagenumbering{roman} then set the counter to i on leaf 4. Calling this
% before each \pagenumbering carries the book to a recto first, so the restart
% lands on an odd leaf and odd keeps meaning recto.
%
% LaTeX's own \cleardoublepage leaves the blank page with the current page
% style, which here is \pagestyle{fancy} -- a rule, a running title and a folio
% on a page with nothing else on it. This one is empty.
\makeatletter
\newcommand{\pdcleartorecto}{%
  \clearpage
...
\newcommand{\pdchapter}[4]{%
  ...
  \fancyhead[LO,RE]{\small\itshape\color{hhgray} Chapter #1. #2}%
...
\def\pd@part@swiss#1#2#3#4#5{%
    ...
    \fi
    % The part opener is TWO pages, and an adjustwidth* decides which way to
    % widen once, at \begin, from the page it opens on. Spanning the \clearpage
    % would hand the second page the first page's decision -- which is how the
    % Part I chapter list came to widen into the spine and 0.35 in off the paper
    % on a verso. Each page gets its own environment and its own parity test.
    \end{adjustwidth*}
    \clearpage
    \thispagestyle{empty}%
    \begin{adjustwidth*}{0pt}{-\dimexpr\marginparsep+\marginparwidth\relax}
...
% \strictpagecheck makes changepage decide parity from a \label written at the
% point of use and read back from the .aux, rather than from \value{page} at
% the moment the box is built. That distinction matters here: a full-width
% figure is built inside a float, so \value{page} is the page LaTeX happens to
% be filling, not the page the float lands on, and guessing it wrong puts a
% 5.55 in picture into the gutter. It also makes the adjustwidth* openers above
% mirror on the leaf they actually land on. The cost is one more latexmk pass.
\strictpagecheck
...
\def\pgfsys@typesetpicturebox#1{%
  ...
  \ifdim\@tempdima>\linewidth
    ...
    \else
      ...
      \fi
      % The overhang goes OUTWARD, so it has to know which side is outward.
      % Flush left in a \textwidth box overhangs to the right, which is the
      % outer margin on a recto and the spine on a verso; flush right is the
      % mirror of it. \checkoddpage sets \ifoddpage from the label written
      % here, which is why \strictpagecheck is on above.
      \checkoddpage
      \ifoddpage
