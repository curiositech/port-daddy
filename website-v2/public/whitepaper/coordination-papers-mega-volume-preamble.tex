...
\def\pd@chapter@swiss#1#2#3#4{%
  ...
  {\noindent\small\itshape\pdchapteroneline\par}
  \vspace{0.9cm}
  \noindent\pdswissplate{#3}{0.7\linewidth}{#4}\par
  \vfill
  \begin{flushright}
...
\def\pd@part@swiss#1#2#3#4#5{%
    ...
    \par
    % the Konkrete plate takes the room left at the foot, 24 x 16 units, capped
    \ifdim\pagegoal=\maxdimen\setlength{\pdplateh}{\dimexpr\textheight-\pagetotal\relax}\else\setlength{\pdplateh}{\dimexpr\pagegoal-\pagetotal\relax}\fi
    \addtolength{\pdplateh}{-1.0cm}%
    \ifdim\pdplateh>2.8in \setlength{\pdplateh}{2.8in}\fi
    \vfill
    \ifdim\pdplateh>2cm
...
\def\pd@part@technical#1#2#3#4#5{%
    ...
    \IfFileExists{plates/technical/part-#1.jpg}{%
      ...
      \AddToShipoutPictureBG*{\AtPageLowerLeft{\makebox[\paperwidth][c]{\includegraphics[width=0.74\paperwidth]{plates/technical/part-#1.jpg}}}}%
      \begin{adjustwidth}{0pt}{-\dimexpr\marginparsep+\marginparwidth\relax}
      \vspace*{0.3cm}
      ...
      \vspace{0.7cm}
      \noindent{\pdgrotesk\fontsize{72}{72}\selectfont\color{hhpaper}#1\par}
      \vspace{0.4cm}
