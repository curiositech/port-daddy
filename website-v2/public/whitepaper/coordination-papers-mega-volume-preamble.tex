...
\newcommand{\pdchapter}[4]{%
  ...
  \end{flushright}
  \vspace{0.5cm}
  % The plate is capped at a third of the page so that the question block
  % below it always fits on the opener; the block itself is one unbreakable
  % minipage, so the label can never be stranded at the foot of the page with
  % the question on the next one.
  \IfFileExists{plates/chapter-#3.jpg}{%
    \begin{center}\includegraphics[width=\textwidth,height=0.33\textheight,keepaspectratio]{plates/chapter-#3.jpg}\end{center}}{}%
  \vfill
