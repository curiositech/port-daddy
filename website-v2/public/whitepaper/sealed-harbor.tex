...
workload that neither a one-shot confinement nor a transparent container serves,
and for which the answer turns out to be a priced exit rather than a stronger box.

\begin{table}[H]\centering\footnotesize
\pdmargincaption{How the sealed room differs from Ryoan. Ryoan confines a stateless
...
$g(s)=s\bmod 2$, which takes two values and therefore carries exactly $1$ bit
--- and, since $s$ written in binary has its parity in the low position, that
release is literally one of the secret's two bits. Now count. An honest gate
hands Erin $1$ of those $2$ bits: the bit Derek sold her, and no more. The
leaky gate of the mutation suite, which forwards $s$ itself, hands over $2$ ---
the whole secret, which is why the checker catches it four steps in. And the
taint tracker of the pitch above, run \emph{soundly} at token granularity, has
...
her. That $0$-bit column is real, it is sound, and nobody will buy it; the
other horn, the tracker that lets the parity through, cannot then certify that
what the worker submitted was $s$ rather than some $f(s)$ it chose. Every
honest design in this chapter therefore lives in the middle column, and the
...
one \pdcite{rs10}. No sound token-granularity flow analysis for a generative model
...
produced sits at $\{D,E\}$. There are four edges up that lattice and four
...
needs nobody's permission. All four downward edges are locked, and --- this is
the part worth doing by hand --- they're locked \emph{by owner}: the two that
drop $D$, namely $\{D\}\to\{\}$ and $\{D,E\}\to\{E\}$, open only with Derek's
delegated declassification authority, and the two that drop $E$, namely
$\{E\}\to\{\}$ and $\{D,E\}\to\{D\}$, only with Erin's. Two keys, four locks,
...
\begin{table}[H]\centering\footnotesize
\pdmargincaption{The four links of the sealed room and the evidence behind each. The
...
Derek's secret, under every possible sequence of moves --- self-composition,
in the name the verification literature gave the move~\pdcite{bdr04} --- and prove that Erin's
...
mechanically \pdcite{rushby}. \textbf{Mutations.} A gate that leaks the raw
...
into one and state an ordinary invariant about the pair. We do it to a finite
model with a Python checker, where they do it to a program with a logic; that
is the small version of the technique, but it is the technique --- and this book said
``run the world twice'' for two editions before noticing the move had a name.
At each step the checker projects both states onto what Erin can observe and
compares. A difference at a
...
must keep Erin's worlds identical forever, and did\pdprov{c1\_noninterference.py}{20260816}{verified}. The first
...
\texttt{git\_push}, \texttt{spawn\_child}. Three it can only watch go past:
\texttt{model\_emit\_token}, \texttt{in\_context\_read},
\texttt{internal\_plan}. Eight events, and therefore eight policies of the
...
first pile?'' [internal, \texttt{b3\_controllability.py}]. Now the ninth
...
checker agrees. \emph{Now you try:} classify \emph{no
...
\begin{table}[H]\centering\footnotesize
\pdmargincaption{Five policies over the room's alphabet, classified by the
...
\noindent Rogers, Roth, Ullman and Vadhan coined two names for a ledger like
this one, tracking cumulative differential-privacy expenditure across
adaptively chosen queries \pdcite{rruv16}: a \textbf{privacy odometer} reads the
running total with no pre-specified ceiling, and a \textbf{privacy filter}
halts before a pre-specified budget is exceeded. Because $\varepsilon_{\max}$
below is fixed in advance and every over-budget release is refused, this
ledger is their filter; the odometer is the same conservation discipline
...
Rogers, Roth, Ullman and Vadhan \pdcite{rruv16}: a stopping rule that halts
...
artifact \emph{means}. A worker that computes $t=f(s)$ and submits $t$ so the
...
\pdcite{drv} only for engagements in which $k$ and the per-release
...
Whitehouse et al.\ \pdcite{wrrw23}, which recovers the advanced-composition rate
...
gives $6.40$ but advanced gives $3.30$, and it pays\pdprov{a3\_epsilon\_ledger.py}{20260816}{verified}. The crossover
...
Wald--Wolfowitz sense \pdcite{wald,wald-wolfowitz}. The miss target is written
...
This is textbook sequential analysis pointed at a live security detector, and
the move has a precedent worth naming: Jung, Paxson, Berger and Balakrishnan's
threshold random walk did the same thing to portscan detection in 2004,
modelling the benign-or-malicious question as a random walk between two
hypotheses and reading a guaranteed false-positive-against-latency trade-off
straight off Wald's test \pdcite{jpbb04}. The canary is that detector with the
network swapped for a gate.

\paragraph{Proof idea.}\pdmarginfigure{wald}{Wald's sequential probability ratio test, made for wartime quality control, is still the fastest way to decide that a canary has tripped at a guaranteed error rate.}%
...
approximation, against $0.554$ exact\pdprov{a4\_canary\_sprt.py}{20260816}{internal}. \emph{Now you try:} at the
...
Multiply, and that's the account: $200\times 64=12{,}800$ bits of adversarial
capacity, before timing. Two things about that arithmetic are worth a second
...
sells a bounded account. It does not sell an innocent one. \emph{Now you try:}
...
outside the possibilistic model. That timing term is not an estimate of ours:
it is the bound Askarov, Zhang and Myers \pdcite{azm10} prove for
\emph{predictive mitigation}, where a scheduler quantises releases into a
bounded set of permitted delays and the leakage through the resulting channel
is at most the logarithm of the number of distinguishable slots. The
mitigation this section describes --- scheduled release into coarse buckets ---
is theirs, and so is the reason it works: an adversary who can modulate
duration continuously has an unbounded channel, and the only thing that bounds
it is destroying the resolution, never observing the content. In Smith's vocabulary \pdcite{smith-qif}, the
...
question is undecidable in practice for prose; the second is arithmetic, and it
is the arithmetic the example above did in one multiplication. A granted
capacity is a quantity a contract can price, bond, and canary-police exactly,
which is the whole of what the four links, between them, make possible.
...
\item \textbf{Canaries have a power curve and a clock}: with per-canary miss probability $\beta$ and $k$ independent canaries, the chance a smuggled item passes is $\beta^{k}$ ($0.008$ at $\beta=0.2$, $k=3$), and Wald's sequential test bounds how long detection takes.
...
the exact hypergeometric $0.554$\pdprov{a4\_canary\_sprt.py}{20260816}{internal}.
\end{pdsolution}
...
budget needs a different theorem when the queries and the stopping point are
chosen adaptively, and the odometer/filter pair that supplies it, is Rogers,
Roth, Ullman and Vadhan's \pdcite{rruv16}; the fully adaptive filter that
recovers the advanced rate at worse constants is Whitehouse and colleagues'
...
sequential probability ratio test \pdcite{wald}, optimal in the sense he and
...
did exactly that to portscan detection in 2004 \pdcite{jpbb04}, and the canary
gate is their threshold random walk with the network swapped out.
