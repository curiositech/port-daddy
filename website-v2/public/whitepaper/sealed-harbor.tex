...
taxonomy \cite{ss09}, \emph{what} is released (the $\varepsilon$-priced
...
interleaving of the same action sequence to depth 7 --- \textbf{self-composition}
\cite{bdr04}, the standard technique for reducing a 2-safety hyperproperty
like noninterference (a claim that quantifies over pairs of runs) to an
ordinary safety check by composing two copies of the system and asserting an
invariant relating their states, applied here concretely rather than through a
general program logic. At each step it projects
...
process can decline to invoke gates nothing. This ladder grades a runtime
claim by the evidence a mechanism itself produces, a different variable from
Sheridan and Parasuraman--Sheridan--Wickens's levels of automation, which
grade how much of a task a system performs unassisted \cite{parasuraman2000}, and
from the Common Criteria's evaluation assurance levels, which grade a
security claim by how much independent verification backs it
\cite{cciso15408}.
...
adaptively chosen queries \cite{rruv16}: a \textbf{privacy odometer} reads the
...
This is textbook sequential analysis applied to a live security detector, and
the closest prior instance of exactly that move is Jung, Paxson, Berger and
Balakrishnan's Threshold Random Walk, which models portscan detection as the
same benign-versus-malicious random walk and derives a guaranteed
false-positive/latency trade-off from Wald's SPRT \cite{jpbb04}.
...
\item \textbf{Noninterference is checked by running the world twice}: identical except for the secret, the two runs must agree on everything that leaves through the slot, modulo the declassification the gate performs. The finite clean-room model passes this check to depth seven.
...
\bibitem{cciso15408} ISO/IEC 15408. \emph{Common Criteria for Information Technology Security
Evaluation}. International Organization for Standardization, current edition.
