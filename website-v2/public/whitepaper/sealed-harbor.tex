...
\caption{The thirteen elements of Work Order Contract $C$. Mutual non-disclosure between data owner Derek and model owner Erin is enforced by pinning cryptographic measurements, execution permissions, output schemas, resource ceilings, and economic remedies before any decryption key is released.}\label{tab:sealed-work-order-elements}\\
\toprule
\textbf{Element} & \textbf{Role} & \textbf{Security Specification \& Guarantee} & \normalfont\textbf{Example Value} \\
\midrule
\endfirsthead
\multicolumn{4}{@{}l}{\small\itshape Table~\ref{tab:sealed-work-order-elements}: The thirteen elements of Work Order Contract $C$ (continued)}\\
\addlinespace[3pt]
\toprule
\textbf{Element} & \textbf{Role} & \textbf{Security Specification \& Guarantee} & \normalfont\textbf{Example Value} \\
\midrule
\endhead
\bottomrule
\multicolumn{4}{r}{\itshape\scriptsize Continues on next page $\,\blacktriangleright$} \\
\endfoot
\bottomrule
\endlastfoot
$H(\text{manifest})$ & Pins & Cryptographic digest over dataset chunks, schema layout, and provenance. Guarantees guest reads only approved data. & sha256:7f83b165... \newline s3://derek/q3.parquet \\
\addlinespace[2pt]
$H(\text{agent})$ & Pins & Digest of model weights, tokenizer, prompt AST, and tools. Guarantees Erin's proprietary agent is unmodified. & sha256:c0ffee4b... \newline weights.safetensors \\
\addlinespace[2pt]
$H(\text{runtime})$ & Pins & Measured guest enclave launch digest (SEV-SNP / TDX MRTD) verifying kernel and monitor before execution. & mrtd:e4a8109d... \newline (Linux 6.8 + monitor) \\
\addlinespace[2pt]
$H(\text{policy})$ & Pins & Digest of declassification lattice rules, label propagation logic, and differential privacy noise parameters. & sha256:5b91df... \newline eps=0.5, delta=1e-6 \\
\midrule
$\text{function}$ & Permissions & Whitelisted top-level entrypoint symbol. Restricts agent from executing arbitrary subroutines or unvetted scripts. & portfolio\_stress\_test \newline (portfolio\_id: str) \\
\addlinespace[2pt]
$\text{tools}$ & Permissions & Closed whitelist of in-guest tool capability handles. Direct sockets disabled; all calls route through monitor. & ["yield\_oracle", "spot"] \newline (no raw sockets) \\
\midrule
$\text{recipients}$ & Recipients & Authorized recipient identities and public keys for each egress channel. Derek gets results; Erin gets telemetry. & derek: did:key:z6M.. \newline erin:  did:key:z6M.. \\
\addlinespace[2pt]
$\text{schemas}$ & Recipients & Strongly-typed schema definition (Cap'n Proto / JSON Schema) for declassified outputs. Forbids free-form payloads. & RiskReportV1 \{ \newline \enspace var95: f64, flag: bool \} \\
\midrule
$\text{budgets}$ & Bounds & Hard resource and leakage ceilings: maximum declassification bits ($q \cdot b$), CPU time, memory, and timeout. & b=64 bits, q=1 exit, \newline max\_cpu=300s, 32GB \\
\addlinespace[2pt]
$\text{retention}$ & Bounds & Ephemeral session lifetime and zeroization policy. Volatile memory cryptographically wiped upon exit; no NVRAM. & zeroize\_on\_exit; \newline no\_swap; max\_ttl=900s \\
\midrule
$\text{validators}$ & Remedies & Independent appraisal rules for attestation evidence (IETF RATS) and third-party arbitration endpoints. & rats\_profile: v1.2; \newline arbitrator: did:key:.. \\
\addlinespace[2pt]
$\text{payment}$ & Remedies & Consideration terms, escrow identifiers, and conditional release mechanisms upon verified receipt delivery. & 250.00 USDC in \newline Escrow\#0x8f2a... \\
\addlinespace[2pt]
$\text{liability}$ & Remedies & Liquidated damages, penalty covenants, and bonded stake forfeiture on attested breach or canary leak receipt. & 10,000 USDC bond; \newline forfeit\_on\_leak: canary \\
\end{xltabular}}
