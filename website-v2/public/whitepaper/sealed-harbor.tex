...
\pdmargincaption{Ryoan confines a stateless module that sees its input once; the sealed room confines a looping agent, so it needs a priced exit and a detector where Ryoan needed neither.}
...
or nothing about the design is provably enforceable. \pdgloss{Noninterference}{The room's silence: nothing about the data inside changes anything an observer outside can see, except through the one declared release.} proves
...
\pdgloss{self-composition}{Barthe, D'Argenio and Rezk's trick: noninterference is a claim about pairs of runs, so you compose two copies of the system and state it as one invariant.}, exactly as Barthe, D'Argenio and Rezk set it
...
adaptively chosen queries \pdcite{rruv16}: a \pdgloss{privacy odometer}{a running total of differential-privacy expenditure read with no pre-specified ceiling.} reads the
...
straight off Wald's test \pdcite{jpbb04}. The \pdgloss{canary}{A planted, known-bad item whose appearance outside the room proves a leak, and whose continued absence bounds the leak rate.} is that detector with the
