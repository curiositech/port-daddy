...
chapter needs a budget and a detector where Ryoan needed neither.
...
\end{aligned}
\]}
The order names the measured runtime and agent bundle, the encrypted data
inputs, the allowed tools and external providers, the result and telemetry
schemas, the output-bit, query, and timing budgets, the declassification
authorities, retention and key destruction, success evidence, dispute
procedure, and guarantee. Any change makes a new hash and requires both
signatures. It is a work unit in the sense of \pdchapref{swk}{The Single-Writer
...
two fences, two gates, and which of the four claims each part produces.
...
At $n=10^4$, $c=100$ (density one percent), $\beta=0.2$: a 100-span leak is
