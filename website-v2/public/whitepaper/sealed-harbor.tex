...
\input{figures/fig-sealed-three-principals}

Every week some enterprise negotiates this standoff with
...
\input{figures/fig-sealed-work-order-schema}
...
\input{figures/fig-sealed-dual-attestation}
...
draws $R$'s bound fields as a tree and flags the one leaf that proves
only what was released, not that it was correct.

\input{figures/fig-sealed-receipt-provenance}
...
\begin{table}[H]\centering\footnotesize
\caption{The room's six side properties and the later result each underwrites.
None is a theorem; each is backed by the implementation and its tests.
Complete mediation is flagged because it is the one property later sections
name explicitly, not merely assume.}
...
Figure~\ref{fig:sealed-laundering-fork} draws the fork: the honest path and
the laundering path are identical through \texttt{submit}, because that is
the one node the checker's model can see, and only the honest gate's
\emph{argument} differs.
...
\input{figures/fig-sealed-controllability-quadrant}
...
\input{figures/fig-sealed-two-adversaries}
...
\input{figures/fig-sealed-residuals-converge}
...
\input{figures/fig-sealed-leakage-ledger}
...
\begin{table}[H]\centering\footnotesize
\caption{How many unordered secret pairs a sound room must keep
Erin-indistinguishable forever (equal-parity), against how many may
legally differ at a release step (different-parity), for $g(s)=s\bmod 2$.}
\label{tab:sealed-parity-pairs}
\begin{tabular}{@{}c c c c@{}}
\toprule
Secrets & Unordered pairs & Equal-parity (hidden forever) & Different-parity (may differ at release) \\
\midrule
4 & $\binom{4}{2}=6$ & 2 & 4 \\
