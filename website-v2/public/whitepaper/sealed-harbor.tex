...
\midrule
$\text{function}$ & Permissions & Whitelisted top-level entrypoint symbol and signature. Restricts the agent from executing arbitrary subroutines or unvetted scripts inside the guest. & portfolio_stress_test \newline (portfolio_id: str, \newline \enspace shock_bps: int) \\
\addlinespace[3pt]
$\text{tools}$ & Permissions & Closed whitelist of in-guest tool capability handles. Direct external network access is disabled; all calls route through the attested monitor. & ["yield_curve_oracle", \newline \enspace "fx_spot_lookup"] \newline (no raw sockets) \\
\midrule
...
\addlinespace[3pt]
$\text{retention}$ & Bounds & Ephemeral session lifetime and zeroization policy. Enclave volatile memory is cryptographically wiped upon job completion or fault; zero NVRAM persistence. & zeroize_on_exit; \newline no_swap; no_nvram; \newline max_ttl=900s \\
\midrule
$\text{validators}$ & Remedies & Independent appraisal rules for attestation evidence (IETF RATS format) and third-party arbitration endpoints for cryptographic dispute resolution. & rats_profile: v1.2; \newline arbitrator: \newline did:key:z6MkuArb... \\
\addlinespace[3pt]
$\text{payment}$ & Remedies & Consideration terms, escrow identifiers, and conditional release mechanisms. Payment unlocks only upon verifiable delivery of a dual-signed release receipt. & 250.00 USDC in \newline Escrow#0x8f2a...; \newline release_on: receipt \\
\addlinespace[3pt]
$\text{liability}$ & Remedies & Liquidated damages, penalty covenants, and bonded stake forfeiture in the event of an attested invariant breach or canary leakage receipt. & 10,000 USDC bond; \newline forfeit_on_leak: \newline canary_receipt_valid \\
\bottomrule
\end{tabularx}
\end{table}
