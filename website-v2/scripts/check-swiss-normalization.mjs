...
const BRAND_MD_PATH = join(ROOT, 'docs', 'design', 'BRAND.md')
const brandMdText = existsSync(BRAND_MD_PATH) ? readFileSync(BRAND_MD_PATH, 'utf8') : null
...
// A directory or entry this scan cannot read is not zero violations, it is
// an unknown -- and a ratchet whose whole premise is a trustworthy count
// cannot silently treat unknown as zero. An unreadable directory used to
// return an empty list for itself and everything under it; an unreadable
// entry used to be skipped. Both are collected here instead and turned into
// a hard failure in main(), named, rather than a quietly undercounted scan.
// A directory that is a genuine, known exception (e.g. a symlink into a
// gitignored build output) belongs in SKIP_DIR_NAMES by name, declared and
// never walked in the first place -- not tolerated here as a silent read
// failure.
const unreadable = []
...
function classTokenRegex(base, requireSuffix) {
  ...
  // ternary cases.
  //
  // Uses a lookbehind assertion, which requires Node >= 9 (V8 >= 6.2); CI
  // pins `node-version: 22` for this job (see swiss-normalization-guard in
  // .github/workflows/ci.yml), well past that floor. If this script is ever
  // run outside CI on an older runtime, that is the requirement to check.
...
// A reason may cite a BRAND.md section as its justification (the escape
// hatch's own doc gives `BRAND.md §Hard borders` as the example shape). A
// citation like that is checkable in ten seconds by a human -- but only if
// it still resolves. Rather than match the reason against a keyword set
// (which would let anyone silence the guard by writing one of the words),
// this resolves the citation against BRAND.md's actual section anchors and
// fails the hatch when it does not: a stale citation means the exemption's
// stated rationale has been edited or deleted out from under it, and that
// is drift this guard exists to catch, not a false positive to tolerate.
//
// "Section" here covers both a markdown heading (`#`..`######`) and a bold
// lead-in on a bullet (`- **Like this.** ...`), because BRAND.md uses both
// as anchors and the escape hatch's own example cites the latter kind.
const BRAND_CITATION_RE = /BRAND\.md\s*§\s*([^,.;\n]+)/i
...
function brandSectionTitles(brandMdText) {
  const titles = []
  for (const m of brandMdText.matchAll(/^#{1,6}\s+(.+)$/gm)) titles.push(m[1])
  for (const m of brandMdText.matchAll(/\*\*([^*]+)\*\*/g)) titles.push(m[1])
  return titles.map(normalizeSectionTitle)
}

function resolveBrandCitation(reason, brandMdText) {
  const cited = reason.match(BRAND_CITATION_RE)
  if (!cited) return { citesBrand: false, resolved: true }
  if (brandMdText === null) return { citesBrand: true, resolved: false, reason: 'BRAND.md not found' }
  const wanted = normalizeSectionTitle(cited[1])
  const titles = brandSectionTitles(brandMdText)
  const resolved = titles.some((title) => title === wanted || title.startsWith(wanted) || wanted.startsWith(title))
  return { citesBrand: true, resolved, cited: cited[1].trim() }
}

function scanEscapeHatch(rawLines, rel, brandMdText) {
  ...
  rawLines.forEach((line, idx) => {
    ...
    const citation = resolveBrandCitation(reason, brandMdText)
    if (citation.citesBrand && !citation.resolved) {
      errors.push(
        `${rel}:${idx + 1}  swiss-allow cites BRAND.md §${citation.cited ?? '?'}, which does not resolve to any heading or bolded lead in BRAND.md -- fix the citation or update BRAND.md: ${clause.slice(0, 90)}`
      )
      return
    }
...
function scanFile({ abs, rel }) {
  ...
  const { allowMap, errors: hatchErrors } = scanEscapeHatch(rawLines, rel, brandMdText)
...
function main() {
  if (unreadable.length) {
    console.error(`swiss-normalization guard: ${unreadable.length} path(s) under the scanned roots could not be read:\n`)
    for (const entry of unreadable) console.error(`  ${entry}`)
    console.error(
      '\nAn unreadable path is an unknown violation count, not a zero one, and this guard never treats unknown as clean. ' +
        'If one of these is a genuine, known exception (e.g. a symlink into a gitignored build directory), add its name to ' +
        'SKIP_DIR_NAMES so it is a declared exclusion rather than a silent read failure.'
    )
    process.exit(2)
  }
