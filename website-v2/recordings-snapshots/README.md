...
real `pd` commands** (`scripts/record-site-terminal-gifs.sh`,
`scripts/record-agent-terminal-gifs.sh`). That makes them *living integration
...
1. **Extract** (`scripts/lib/cast-transcript.mjs`): a `.cast` → its concatenated
...
`scripts/seed-recording-world.*` and the `website-terminal-recordings` job.

## Adding a new ephemeral

If a real CI run flags "drift" that is actually an un-scrubbed id/timestamp, add a
rule to `EPHEMERAL_RULES` in `scripts/lib/cast-transcript.mjs`. Do **not**
