...
describe('check-swiss-normalization.mjs — the escape hatch', () => {
  test('a well-formed swiss-allow with a real reason suppresses the violation', () => {
    const root = makeFixtureRoot()
    writeFixture(root, 'docs/design/BRAND.md', ['# Brand', '', '## Hard borders', '', 'Square corners by default.', ''].join('\n'))
    writeFixture(
      root,
  ...
  test('a swiss-allow suppresses only its named rule, not a different rule on the same line', () => {
    const root = makeFixtureRoot()
    writeFixture(
      root,
      'src/components/Avatar.tsx',
      `export const Avatar = () => <img className="rounded-full" style={{ color: '#ff0000' }} /> // swiss-allow: radius — avatar is a system primitive\n`
    )
    const ratchet = writeRatchet(root, EMPTY_RATCHET)
    const { code, out } = run(root, ratchet)
    // radius is suppressed; the literal on the same line is not, and is not itself malformed
    expect(out).not.toMatch(/\[radius\]/)
    expect(out).not.toMatch(/malformed swiss-allow/)
    expect(out).toMatch(/\[literal\]\s+#ff0000/)
    expect(code).toBe(1)
  })

  test('a rule-mismatched swiss-allow does not suppress the violation actually present', () => {
    const root = makeFixtureRoot()
    writeFixture(
      root,
      'src/components/Avatar.tsx',
      `export const Avatar = () => <img className="rounded-full" /> // swiss-allow: literal — this reason is definitely long enough\n`
    )
    const ratchet = writeRatchet(root, EMPTY_RATCHET)
    const { code, out } = run(root, ratchet)
    // the hatch names "literal", but the only violation on the line is "radius" -- it
    // fires anyway. A well-formed hatch for a rule that isn't violated is not itself
    // an error; it just suppresses nothing.
    expect(out).not.toMatch(/malformed swiss-allow/)
    expect(out).toMatch(/\[radius\]\s+rounded-full/)
    expect(code).toBe(1)
  })

  test('a swiss-allow citing a BRAND.md heading that exists resolves and suppresses', () => {
    const root = makeFixtureRoot()
    writeFixture(root, 'docs/design/BRAND.md', ['# Brand', '', '## Corner radius policy', '', 'Square by default.', ''].join('\n'))
    writeFixture(
      root,
      'src/components/Avatar.tsx',
      `export const Avatar = () => <img className="rounded-full" /> // swiss-allow: radius — BRAND.md §Corner radius policy\n`
    )
    const ratchet = writeRatchet(root, EMPTY_RATCHET)
    const { code, out } = run(root, ratchet)
    expect(out).not.toMatch(/malformed swiss-allow/)
  ...
  test('a swiss-allow citing a bolded bullet lead-in (not a heading) also resolves', () => {
    const root = makeFixtureRoot()
    writeFixture(
      root,
      'docs/design/BRAND.md',
      ['# Brand', '', '## Visual language', '', '- **Corner radius is restricted.** Square by default.', ''].join('\n'),
    )
    writeFixture(
      root,
      'src/components/Avatar.tsx',
      `export const Avatar = () => <img className="rounded-full" /> // swiss-allow: radius — BRAND.md §Corner radius is restricted\n`
    )
    const ratchet = writeRatchet(root, EMPTY_RATCHET)
    const { code, out } = run(root, ratchet)
    expect(out).not.toMatch(/malformed swiss-allow/)
  ...
  test('a swiss-allow citing a BRAND.md section that does not exist is malformed and does not suppress', () => {
    const root = makeFixtureRoot()
    writeFixture(root, 'docs/design/BRAND.md', ['# Brand', '', '## Palette', '', 'Colours live here.', ''].join('\n'))
    writeFixture(
      root,
      'src/components/Avatar.tsx',
      `export const Avatar = () => <img className="rounded-full" /> // swiss-allow: radius — BRAND.md §A section that was deleted\n`
    )
    const ratchet = writeRatchet(root, EMPTY_RATCHET)
    const { code, out } = run(root, ratchet)
    expect(out).toMatch(/does not resolve to any heading or bolded lead in BRAND\.md/)
    expect(out).toMatch(/\[radius\]\s+rounded-full/)
    expect(code).toBe(1)
  })

  test('a swiss-allow with no BRAND.md citation is unaffected by section resolution', () => {
    const root = makeFixtureRoot()
    writeFixture(
      root,
      'src/components/Avatar.tsx',
      `export const Avatar = () => <img className="rounded-full" /> // swiss-allow: radius — a system primitive, no doc citation needed\n`
    )
    const ratchet = writeRatchet(root, EMPTY_RATCHET)
    const { code, out } = run(root, ratchet)
    expect(out).not.toMatch(/malformed swiss-allow/)
...
describe('check-swiss-normalization.mjs — an unreadable path fails loudly', () => {
  test('a broken symlink under the scanned tree fails the build by name, not as zero violations', () => {
    const root = makeFixtureRoot()
    // A symlink whose target does not exist: statSync throws ENOENT on it
    // regardless of who is running the process (unlike a chmod-based
    // fixture, which a root-run CI job would simply read straight through).
    symlinkSync(join(root, 'src', 'components', 'does-not-exist.tsx'), join(root, 'src', 'components', 'Broken.tsx'))
    const ratchet = writeRatchet(root, EMPTY_RATCHET)
    const { code, out } = run(root, ratchet)
    expect(code).toBe(2)
    expect(out).toContain('could not be read')
    expect(out).toContain('Broken.tsx')
    // Never silently continues to a violation count computed over whatever
    // it did manage to read.
    expect(out).not.toMatch(/in-scope files scanned/)
  })
})
