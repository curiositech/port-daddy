      ...
      <motion.div className="space-y-12">
        ...
        <section className="space-y-4">
          <motion.div className="flex items-center gap-3">
            ...
            <motion.h2 className="m-0">The Second Agent Problem</motion.h2>
          </motion.div>
          <motion.p>
            One agent is easy. Two agents is a distributed system. Who owns which files? How do they signal state changes? What happens when one crashes mid-task? Port Daddy gives you three primitives: <strong>file claims</strong>, <strong>pub/sub channels</strong>, and <strong>session notes</strong>.
          </motion.p>
        </section>
        ...
        <section className="space-y-4">
          ...
          <motion.p>
            Before modifying files, claim them. Claims are advisory — they warn other agents about conflicts without hard-locking anything.
          </motion.p>
...
$ pd session files claim src/middleware/auth.ts
  ...
  Holder session: session-a1b2c3d4`}</CodeBlock>
          <motion.p className="text-sm" style={{ color: 'var(--text-muted)' }}>
            Claims are released when the session ends (<code>pd done</code>) or when the agent crashes and enters the salvage queue.
          </motion.p>
        </section>
        ...
        <section className="space-y-4">
          <motion.div className="flex items-center gap-3">
            <Surface depth="inset" radius="xl" padding="none" className="w-10 h-10 flex items-center justify-center">
              <MessageSquare className="text-[var(--brand-secondary)]" size={20} />
            </Surface>
            <motion.h2 className="m-0">2. Pub/Sub: Signaling Between Agents</motion.h2>
          </motion.div>
          <motion.p>
            File claims handle ownership. Pub/sub handles communication. When Agent A finishes, it publishes a message. Agent B reacts instantly.
          </motion.p>
...
$ pd watch myapp:events --exec "npm test"
  ...
  Tests passed (14/14)`}</CodeBlock>
          <motion.p>
            <code>pd watch</code> subscribes via SSE and runs your command on every message. No polling. No cron. Agent A publishes, Agent B reacts in sub-second time.
          </motion.p>
        </section>
        ...
        <section className="space-y-4">
          <motion.div className="flex items-center gap-3">
            ...
            <motion.h2 className="m-0">3. Session Notes: The Audit Trail</motion.h2>
          </motion.div>
          <motion.p>
            Every agent writes notes as it works. Notes are immutable — once written, they can&apos;t be edited or deleted. If an agent crashes, its notes survive for the next agent to read.
          </motion.p>
...
$ pd done`}</CodeBlock>
          <motion.p className="text-sm" style={{ color: 'var(--text-muted)' }}>
            <strong>Claim, Work, Signal, Done.</strong> Every agent follows this lifecycle. Port Daddy handles conflict detection, crash recovery, and the audit trail.
          </motion.p>
        </section>
        ...
        <section className="space-y-3">
          ...
          <motion.ul className="space-y-2" style={{ color: 'var(--text-secondary)' }}>
            <motion.li><a href="/tutorials/fleet" style={{ color: 'var(--brand-primary)' }}>Fleet Agents</a> — automate this pattern with background agents on every commit</motion.li>
            <motion.li><a href="/tutorials/session-phases" style={{ color: 'var(--brand-primary)' }}>Session Phases</a> — track progress through planning → in_progress → testing → done</motion.li>
            <motion.li><a href="/tutorials/pheromone" style={{ color: 'var(--brand-primary)' }}>Pheromone Trails</a> — ambient signals that let agents sense each other without talking</motion.li>
          </motion.ul>
        </section>
