...
const commonEvidenceFlags = [
  ...
  { flag: 'actor_id', description: 'Required agent or reviewer that is recording the evidence.' },
...
const toolPages = {
  doctrine_list: {
    ...
    flags: [
      { flag: 'status', description: 'Optional revision state: candidate, provisional, established, contested, or deprecated.' },
  ...
  record_doctrine_episode: {
    ...
    syntax: 'record_doctrine_episode({ decision_class, summary, historical_action, project_dir, actor_id, citations, alternatives?, cues?, fidelity?, id?, idempotency_key? })',
    flags: [
    ...
    examples: [{
      ...
      code: 'record_doctrine_episode({ decision_class: "integration.merge", summary: "A technically mergeable PR was held for an unresolved bot thread", historical_action: "withheld merge", alternatives: ["merge after reviewing evidence", "request a technical response"], project_dir: "/workspace/app", actor_id: "steward-7", citations: ["pr:case-13", "review:case-13"] })',
  ...
  propose_doctrine_candidate: {
    ...
    syntax: 'propose_doctrine_candidate({ episode_id, decision_class, title, when, prefer, over, because, project_dir, actor_id, citations, unless?, school?, skill_refs?, doctrine_id? })',
    flags: [
  ...
  preregister_doctrine_experiment: {
    ...
    syntax: 'preregister_doctrine_experiment({ candidate_id, hypothesis, primary_outcome, control, treatment, project_dir, actor_id, citations, sham?, preregistered_at?, id?, idempotency_key? })',
    flags: [
    ...
    usagePatterns: [
      ...
      'Keep factual fidelity separate from outcome quality; a prompt-only reconstruction is not a matched control.',
    ],
    examples: [{
      description: 'Preregister a CASE-13 factual test',
      code: 'preregister_doctrine_experiment({ candidate_id: "doctrine-candidate_…", hypothesis: "Independent evidence changes the merge decision more than thread state", primary_outcome: "merge, hold, or request-response decision", control: "unresolved thread without independent evidence", treatment: "unresolved thread with independent evidence", project_dir: "/workspace/app", actor_id: "researcher-2", citations: ["fork:case-13:checkpoint"] })',
      output: '{\n  "experiment": { "experimentId": "doctrine-experiment_…" }\n}',
    }],
  },
  record_doctrine_treatment_run: {
    description: 'Append one preregistered control, treatment, or sham run. Unmatched and prompt-only replays remain useful fidelity evidence, but cannot satisfy the gate for advisory admission.',
    syntax: 'record_doctrine_treatment_run({ experiment_id, arm, action, outcome, fidelity, project_dir, actor_id, citations, notes?, id?, idempotency_key? })',
    flags: [
    ...
    usagePatterns: [
      ...
      'Admission needs both a matched factual control and a matched factual treatment from this experiment.',
    ],
    examples: [{
      description: 'Record a matched treatment run',
      code: 'record_doctrine_treatment_run({ experiment_id: "doctrine-experiment_…", arm: "treatment", action: "requested a technical response", outcome: "independent evidence changed the decision", fidelity: "matched", project_dir: "/workspace/app", actor_id: "researcher-2", citations: ["fork:case-13:treatment-1"] })',
      output: '{\n  "treatmentRun": { "treatmentRunId": "doctrine-run_…", "fidelity": "matched" }\n}',
    }],
  },
  admit_doctrine_candidate: {
    description: 'Admit a candidate as provisional or established advisory guidance only after its own preregistered experiment has both matched factual control and treatment runs. It never authorizes or enforces an action.',
    syntax: 'admit_doctrine_candidate({ candidate_id, experiment_id, reviewer_id, project_dir, actor_id, citations, status?, idempotency_key? })',
    flags: [
      ...
      { flag: 'reviewer_id', description: 'Required reviewer who inspected the cited evidence.' },
      { flag: 'status', description: 'Optional provisional or established advisory status.' },
      { flag: 'idempotency_key', description: 'Optional retry key for the append-only admission call.' },
      ...commonEvidenceFlags,
    ],
    usagePatterns: [
      'Do not call after an unmatched replay, a prompt-only reconstruction, or a test that lacks one of the factual arms.',
      'Treat admission as an advisory packet for retrieval, not a merge gate or an automatic training event.',
    ],
    examples: [{
      description: 'Admit a CASE-13 candidate after factual fidelity is verified',
      code: 'admit_doctrine_candidate({ candidate_id: "doctrine-candidate_…", experiment_id: "doctrine-experiment_…", reviewer_id: "reviewer-case13", status: "provisional", project_dir: "/workspace/app", actor_id: "researcher-2", citations: ["review:case-13:admission"] })',
  ...
  doctrine_orders: {
    ...
    syntax: 'doctrine_orders({ decision_id, decision_class, project_dir, actor_id, citations, limit? })',
    flags: [
      ...
      { flag: 'actor_id', description: 'Required agent that records the receipt.' },
    ...
    examples: [{
      ...
      code: 'doctrine_orders({ decision_id: "merge-482", decision_class: "integration.merge", project_dir: "/workspace/app", actor_id: "steward-7", citations: ["pr:482", "ci:482"] })',
  ...
  record_doctrine_application: {
    ...
    syntax: 'record_doctrine_application({ retrieval_id, doctrine_id, response, decision, project_dir, actor_id, citations, note? })',
    flags: [
    ...
    examples: [{
      ...
      code: 'record_doctrine_application({ retrieval_id: "doctrine-retrieval_…", doctrine_id: "doctrine:integration:independent-evidence", response: "adapt", decision: "Asked for a technical reply, then merged", project_dir: "/workspace/app", actor_id: "steward-7", citations: ["pr:482"] })',
  ...
  record_doctrine_outcome: {
    ...
    syntax: 'record_doctrine_outcome({ application_id, verdict, summary, verified_by, project_dir, actor_id, citations })',
    flags: [
    ...
    examples: [{
      ...
      code: 'record_doctrine_outcome({ application_id: "doctrine-application_…", verdict: "inconclusive", summary: "No regression found, but the thread was not independently reproduced", verified_by: "ci:482:verified", project_dir: "/workspace/app", actor_id: "steward-7", citations: ["ci:482:verified"] })',
  ...
  contest_doctrine: {
    ...
    syntax: 'contest_doctrine({ doctrine_id, reason, project_dir, actor_id, citations, severity?, idempotency_key? })',
    flags: [
    ...
    examples: [{
      ...
      code: 'contest_doctrine({ doctrine_id: "doctrine:integration:independent-evidence", reason: "A policy-bound release required thread closure regardless of technical evidence", severity: "high", project_dir: "/workspace/app", actor_id: "reviewer-2", citations: ["pr:9911", "policy:release-gate"] })',
  ...
  return (
    <CommandPage
      ...
      apiSpec={[
        ...
        { label: 'Evidence rule', value: 'Citations are required for live decisions, applications, and outcomes' },
      ]}
