...
export default function BeginSessionTool() {
  return (
    <CommandPage
      ...
      description="Start a new agent session, register as a coordinated agent, and declare telos. Sessions track work context, claims, notes, salvage, and visibility into ongoing work. Returns session and agent IDs."
      version="3.11.0"
      ...
      flags={[
        ...
        { flag: 'telos', description: 'Purpose contract/tagline for how the agent judges done-ness' },
        { flag: 'files', description: 'Files to claim at session start' },
      ]}
      usagePatterns={[
        'begin_session()',
        'begin_session({ identity: "myapp:api" })',
        'begin_session({ identity: "myapp:frontend", purpose: "Build login page", telos: "Make login trustworthy" })',
      ]}
      examples={[
        {
          description: 'Basic session with identity',
          code: 'begin_session({ identity: "myapp:api", purpose: "Fix auth bug", telos: "Make API auth trustworthy" })',
          output: `{
  "session_id": "abc123-def456",
  "agent_id": "agent-001",
  "identity": "myapp:api",
  "purpose": "Fix auth bug",
  "telosHeadline": "Make API auth trustworthy",
