...
export const PORT_DADDY_VERSION = '3.30.3'
...
export const CLI_REFERENCE_GROUPS: ReferenceGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'pd doctrine <command>', description: 'Record, test, retrieve, apply, and contest cited advisory decision doctrine; never treats one transcript as automatic fleet learning.', flags: ['status', 'candidates', 'show', 'orders', 'record-episode', 'propose', 'preregister', 'run', 'admit', 'application', 'outcome', 'contest', '--input', '--dir', '--decision-class'] },
...
export const SDK_REFERENCE_GROUPS: ReferenceGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'takeoverSession', description: 'Start a successor session from an existing one without deleting its notes.' },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'registerActor', description: 'Mint or resolve this client as a durable actor and return its credential receipt.' },
      { name: 'ensureActorCredential', description: 'Ensure this client holds a durable actor credential, minting one when necessary.' },
  ...
  {
    title: 'Empirically Earned Doctrine',
    description: 'Append cited episodes, preregistered tests, advisory retrieval receipts, agent responses, and verifier-backed outcomes without turning a transcript or skill into automatic policy.',
    href: '/docs/features/doctrine',
    source: 'lib/client.ts doctrine methods, routes/doctrine.ts, lib/doctrine.ts',
    items: [
      { name: 'doctrineStatus', description: 'Read current candidate and evidence counts from the canonical Agent Harbor evidence stream.' },
      { name: 'listDoctrineCandidates', description: 'List candidate or admitted advisory revisions without applying any advice.' },
      { name: 'getDoctrine', description: 'Read one doctrine revision with its episode, experiment, receipts, applications, outcomes, and contest history.' },
      { name: 'recordDoctrineEpisode', description: 'Append a cited historical decision episode.' },
      { name: 'proposeDoctrineCandidate', description: 'Append a falsifiable conditional preference grounded in an episode.' },
      { name: 'preregisterDoctrineExperiment', description: 'Declare the candidate-linked control, treatment, primary outcome, and optional sham before results.' },
      { name: 'recordDoctrineTreatmentRun', description: 'Append a control, treatment, or sham run with its factual-fidelity status.' },
      { name: 'admitDoctrine', description: 'Admit a candidate as advisory only after its own preregistered matched factual control and treatment runs; revisions require successor candidates.' },
      { name: 'retrieveDoctrineOrders', description: 'Retrieve exact structured matches and append a decision-time receipt, including an honest empty receipt.' },
      { name: 'recordDoctrineApplication', description: 'Record whether an agent followed, adapted, or rejected advice it actually received.' },
      { name: 'recordDoctrineOutcome', description: 'Append a later verifier-backed helped, harmed, or inconclusive outcome.' },
      { name: 'contestDoctrine', description: 'Preserve contrary evidence and remove a contested advisory revision from active retrieval without erasing its history.' },
    ],
  },
  {
