...
export const CLI_REFERENCE_GROUPS: ReferenceGroup[] = [
  {
    ...
    items: [
      ...
      { name: 'pd use <target>', description: 'Print the shell exports that point this one shell at a stable, dev, or named daemon. Run it as eval "$(pd use dev)"; it never switches your other shells.' },
      { name: 'pd install-bosun', description: 'Install only the Bosun supervisor, without the rest of the daemon install path. This is what the Homebrew formula calls after install.' },
      { name: 'pd self-update', description: 'Run the unattended freshness self-heal. The hourly LaunchAgent calls this with --tick; you can run it yourself at any time.', flags: ['--tick'] },
      { name: 'pd upgrade', description: 'Compare this binary against the published release feed and report what a Homebrew upgrade would bring, or perform it with --apply.', flags: ['--apply'] },
      { name: 'pd hooks <command>', description: 'Install, inspect, or remove the managed git hook pieces that run the coordination guard at commit time.' },
      { name: 'pd benchmark', description: 'Run the broader benchmark suite, distinct from the daemon latency check in pd bench.' },
      { name: 'pd attest', description: 'Print the honest self-report: the daemon side (integrity, schema, permissions) merged with the checks only the CLI can make, including CLI-to-daemon version match.' },
      { name: 'pd safe <command>', description: 'Audit the host-safety posture read-only with pd safe scan, and apply the opt-in reversible hardening steps the audit recommends.' },
      { name: 'pd backup', description: 'Take a durable snapshot of the port registry database, to the default backend or a backend URI you name.', flags: ['--to'] },
      { name: 'pd restore <snapshot>', description: 'Restore a snapshot to the daemon database path, optionally from another backend or to another destination.', flags: ['--from', '--dest', '--force'] },
      ...
      { name: 'pd batten', description: 'Check a release tarball against its manifest before it sails, so an artifact that must be aboard cannot be missing.' },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'pd takeover <session-id>', description: 'Start a successor session from an existing one without deleting what it wrote, so the notes and claims survive the handover.' },
      { name: 'pd plan <session-id>', description: 'Read or append the checklist a session carries. Appends write a complete plan, bound to the caller rather than to a credential found on the target.' },
      { name: 'pd booty <command>', description: 'Harvest artifacts into the blob store with their provenance: design workups, images, HTML, video and shaders, on any branch or plane.' },
      { name: 'pd transcripts <command>', description: 'Read the conversation record of any fleet ship run: list recent runs, then open one and follow what the agent actually said.', aliases: ['pd transcript'] },
      { name: 'pd harbor-ledger <command>', description: 'Read the Agent Harbor event ledger and the projections built from it.' },
      { name: 'pd commit "<object>"', description: 'Make a durable commitment with the commands that decide whether it succeeded, became impossible, or still motivates work.', flags: ['--success', '--impossible', '--motivation', '--scope', '--strategy'] },
      { name: 'pd obligations', description: 'List what is still owed: open commitments, overdue ones, and what each one is waiting on.' },
    ],
  },
  {
    ...
    items: [
      ...
      { name: 'pd send <agent> "message"', description: 'Send one durable direct message, the short form of pd inbox send. It survives the recipient being offline, which ephemeral pub/sub does not.' },
      { name: 'pd sent', description: 'Read receipts for what you sent: which messages have been read, and when.' },
      { name: 'pd attention', description: 'Read the inbox and every subscribed channel in one call. Run it first in a session, or wire it into the harness SessionStart hook.' },
      { name: 'pd nudge <command>', description: 'Send and answer agent-to-agent coaching signals. Distinct from pd suggest, which is the fleet-health queue an operator reads.' },
      { name: 'pd parley <command>', description: 'Hold a structured argument over one surface: call a parley, propose, critique, revise, agree or refuse, then resolve it as collapsed, escalated or void.', flags: ['--surface', '--reason', '--with', '--performative', '--content', '--status', '--decision', '--harbor', '--as'] },
    ],
  },
  {
    ...
    items: [
      ...
      { name: 'pd roster <command>', description: 'Manage durable named agents rather than live process registrations: list, show, search by expertise, create, promote a session, and attach a sanitized handoff.' },
      { name: 'pd sortie <task>', description: 'Run a one-shot multi-agent mission in an ephemeral harbor against an explicit budget, then tear it down.' },
      { name: 'pd jury-rig <command>', description: 'Inspect or warm the local skill index that fleet ships read, the same index jury_rig in pd-fleet.yml uses.' },
      { name: 'pd harbormaster <command>', description: 'Run the merge-owning actor body: start it in the foreground or under launchd, stop it, and read what it is holding.', aliases: ['pd hm'] },
      { name: 'pd seamanship <command>', description: 'Work the skill registry: list what is installed across roots, search skill descriptions, graft a skill in, and report outcomes.', aliases: ['pd skills'] },
      { name: 'pd whois <query>', description: 'Look a name up in the semantic phonebook and get ranked matches for agents or humans, with their similarity scores.', flags: ['--kind', '--fresh', '--limit', '--json'] },
    ],
  },
  {
    ...
    items: [
      ...
      { name: 'pd periscope', description: 'Raise the periscope: one command that answers what the state is and what wants your attention, the sight stage of the operator loop.', aliases: ['pd sight', 'pd scope'] },
      { name: 'pd coast-guard <command>', description: 'See the guard that sandboxes spawned agents away from your secrets: whether confinement is on, what the broker allows, and where the egress cap sits.', aliases: ['pd cg'] },
      { name: 'pd suggest', description: 'Read the pending fleet-health suggestions the Tender has queued for you, then approve or dismiss each one.', flags: ['--json'] },
      { name: 'pd work <command>', description: 'The Work Intent family, the runtime launch primitive the older verbs now feed. pd work probe runs the adapter conformance probe.' },
      { name: 'pd dispatch <command>', description: 'Queue autonomous feature work and follow it: what is queued, what is running, and what has produced a pull request.', aliases: ['pd nightshift'] },
      { name: 'pd review <id>', description: 'Accept or reject a dispatch once its worker has opened a pull request and moved it to review.', flags: ['--accept', '--reject'] },
      { name: 'pd morning', description: 'Read the start-of-day summary of what the overnight dispatches did.' },
      { name: 'pd popper <command>', description: 'Run the autonomous roadmap task puller: read its status and next candidate, and pause or resume it.' },
    ],
  },
]
...
export const SDK_REFERENCE_GROUPS: ReferenceGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'takeoverSession', description: 'Start a successor session from an existing one, keeping its notes.' },
  ...
  },
  {
    ...
    items: [
      { name: 'registerActor', description: 'Mint an actor identity and capture the credential the daemon returns once. Needed by flows that write without calling begin().' },
      { name: 'ensureActorCredential', description: 'Return the credential the client already holds, registering an actor first if it holds none.' },
