...
export const PORT_DADDY_VERSION = '3.28.0'
...
export const CLI_REFERENCE_GROUPS: ReferenceGroup[] = [
  {
    ...
    items: [
      ...
      { name: 'pd account <command>', description: 'Authenticate the CLI through the GitHub device flow and inspect the current account.' },
      ...
      { name: 'pd benchmark [command]', description: 'Run the broader benchmark suite and inspect recorded benchmark results.' },
      ...
      { name: 'pd install-freshness', description: 'Install the unattended freshness check for the stable Homebrew runtime.' },
      ...
      { name: 'pd use <label>', description: 'Print shell exports for a selected named daemon instead of assuming a fixed port.' },
      { name: 'pd self-update', description: 'Run the unattended stable-runtime freshness check.', flags: ['--tick'] },
      { name: 'pd upgrade', description: 'Inspect the published release feed and optionally apply the Homebrew upgrade.', flags: ['--feed', '--apply', '--json'] },
      { name: 'pd hooks <command>', description: 'Install, inspect, or remove project-scoped agent harness hooks.' },
      { name: 'pd periscope', description: 'Summarize live operator state and the next useful cut.', aliases: ['pd sight', 'pd scope'] },
      { name: 'pd coast-guard <command>', description: 'Inspect subprocess confinement, secret scrubbing, and spend-cap posture.', aliases: ['pd cg'] },
      { name: 'pd secret <command>', description: 'Manage provider credentials in the OS-backed secret store.', aliases: ['pd secrets'] },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'pd roster <command>', description: 'Inspect and reconcile the durable agent/session roster.' },
      { name: 'pd attention', description: 'Read unread inbox and subscribed-channel traffic, then mark it seen.' },
      { name: 'pd nudge <target>', description: 'Deliver a durable wake-up or follow-up signal to an agent or session.' },
      ...
      { name: 'pd booty <command>', description: 'Harvest artifacts into the provenance-aware blob store.' },
      { name: 'pd transcripts', description: 'List and inspect durable backend transcripts and collection state.', aliases: ['pd transcript'] },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'pd send <agent> <message>', description: 'Send a durable direct message that survives the recipient being offline.' },
      { name: 'pd sent', description: 'Inspect delivery and read receipts for messages sent by the current agent.' },
      { name: 'pd tuple <command>', description: 'Linda-style tuple space: out, rd/read, in/take, scan, and count.', flags: ['--harbor', '--ttl', '--as', '--limit'] },
      { name: 'pd relay <command>', description: 'Configure and inspect the zero-trust cross-machine relay fabric.', flags: ['url', 'status', 'exchange'] },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'pd work <command>', description: 'Probe backend work-order conformance and related work-intent contracts.' },
      { name: 'pd dispatch <command>', description: 'Queue and run autonomous feature-development work in isolated worktrees.', aliases: ['pd nightshift'] },
      { name: 'pd review <id>', description: 'Accept or reject a queued dispatch result against its review contract.' },
      { name: 'pd morning', description: 'Review the overnight dispatch and operator-attention summary.' },
      { name: 'pd sortie <command>', description: 'Run a bounded multi-agent mission with explicit budget and durable evidence.' },
      ...
      { name: 'pd harbormaster <command>', description: 'Inspect or direct the merge-owning Harbormaster actor.', aliases: ['pd hm'] },
      { name: 'pd harbor-ledger <command>', description: 'Inspect the append-only Agent Harbor event ledger and its projections.' },
      { name: 'pd backend <command>', description: 'Inspect backend routing, readiness, selection, and cost evidence.' },
      { name: 'pd squid <command>', description: 'Manage the Giant Squid harness, adapters, hooks, and conformance checks.' },
      { name: 'pd parley <command>', description: 'Run a structured multi-agent deliberation and retain the competing cases.' },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'pd suggest <command>', description: 'List, approve, or dismiss candidate work from the Tender suggestion queue.' },
      { name: 'pd seamanship <command>', description: 'Search and inspect the shared skill registry and outcome history.', aliases: ['pd skills'] },
      { name: 'pd plan <command>', description: 'Create or inspect a durable execution plan tied to coordination state.' },
      ...
      { name: 'pd backup <command>', description: 'Create and inspect recoverable Port Daddy state backups.' },
      { name: 'pd restore <command>', description: 'Restore a selected backup through the guarded recovery path.' },
      { name: 'pd cut <command>', description: 'Prepare a release candidate from exact source and evidence inputs.' },
      { name: 'pd batten <command>', description: 'Seal release provenance, tag identity, and archive bytes before publication.' },
      { name: 'pd attest', description: 'Audit critical invariants and exit non-zero when any critical check fails.' },
      { name: 'pd safe <command>', description: 'Audit host safety posture and apply only explicit reversible repairs.' },
      { name: 'pd popper <command>', description: 'Pull bounded work from the roadmap into the dispatch queue.' },
      ...
      { name: 'pd whois <query>', description: 'Resolve people, agents, skills, and semantic identities through the shared router.' },
      { name: 'pd embed <command>', description: 'Use the one shared local embedding model and cache for semantic work.' },
      { name: 'pd skill-graft <command>', description: 'Inspect or warm the native skill-graft index used by fleet agents.', aliases: ['pd skillgraft'] },
      { name: 'pd commit <command>', description: 'Create and inspect durable commitments.' },
      { name: 'pd obligations', description: 'Inspect outstanding commitments and obligation-monitor state.' },
...
export const SDK_REFERENCE_GROUPS: ReferenceGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'continueSession', description: 'Create or reattach to a durable linked successor while preserving lineage and receipt evidence.' },
