    ...
    <figure className="grid gap-[var(--space-4)] border-2 border-[var(--border-strong)] bg-[var(--surface-base)] shadow-[var(--shadow-brutal)]">
      <div className="grid gap-[var(--space-5)] p-[var(--space-5)] lg:grid-cols-[minmax(0,0.46fr)_minmax(0,0.54fr)] lg:items-center lg:gap-[var(--space-6)]">
        ...
        {/* The same four shells, as a read-down legend keyed by chapter.
            Each card separates its four facts into distinct zones so nothing
            runs together: a left rail carries the LAYER CODE (e.g. "L0 · L1")
            over the chapter roman numeral; the body carries the title, then
            the "for whom" line, then the build-state pill on its own row. */}
        <ol className="grid gap-[var(--space-2)]">
          {SHELLS.map((shell) => (
            <li
              key={shell.code}
              className="grid grid-cols-[auto,1fr] items-stretch gap-x-[var(--space-3)] border-2 border-[var(--border-default)] bg-[var(--surface-raised)] p-[var(--space-3)]"
            >
              {/* Left rail: layer code (top) + chapter numeral chip (bottom). */}
              <div className="flex w-[3.25rem] flex-col items-center gap-[var(--space-2)] border-r-2 border-[var(--border-default)] pr-[var(--space-3)]">
                <span className="font-mono text-[length:var(--type-meta-size)] font-black uppercase leading-none tracking-[var(--tracking-meta)] text-[var(--brand-primary)]">
                  {shell.code.replace(/ · /g, '·')}
                </span>
                <span
                  aria-hidden="true"
              ...
              {/* Body: title, then a labelled "for" line, then the state pill. */}
              <div className="min-w-0 space-y-[var(--space-1)]">
                ...
                <div className="flex flex-wrap items-center gap-x-[var(--space-3)] gap-y-[var(--space-1)] pt-[var(--space-1)]">
