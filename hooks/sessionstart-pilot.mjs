 ...
 * operator has one switch. Default is 'off' (the compulsion is opt-in per
 * repo; an absent or unreadable config must fail toward silence).
 *
  ...
  for (let i = 0; i < 12; i++) {
    for (const name of ['agent.config.json', '.portdaddy/sitrep.json', '.portdaddy/project.json']) {
      try {
        ...
        /* missing or malformed config file: keep walking, default stays off */
      }
    }
    const parent = dirname(dir);
    if (parent === dir) break;
...
const SITREP_DUTY = [
  ...
  'Status. Any row you write code for MUST link a roadmap item from the moment the row',
  'is created — mint one first with `pd roadmap upsert`.',
].join('\n');
...
function main() {
  ...
  // The SITREP duty rides the same envelope only when the repo opted in — the
  // steering block stays byte-lean for every repo that didn't.
