...
`unspider: <surface> contradicts <source>`.
...
Unspider crawls **inward** — the project as a brand-new developer
...
6. The `gh` issue list, label `unspider:open` — what have we already
   flagged? Don't refile the same issue.
...
Title: `unspider: <surface> contradicts <source>`
Examples:
- `unspider: README quickstart contradicts cli/commands/start.ts`
- `unspider: docs/sdk.md references pd.tube() but lib/sdk.ts has no tube method`
- `unspider: openapi.yaml lists /v2/agents but no route handler exists`
...
Labels: `unspider:open`, plus one of `docs`, `cli`, `sdk`, `api`,
`tutorial`, `quickstart`.
...
`unspider:open` for the same contradiction. If it exists, comment
