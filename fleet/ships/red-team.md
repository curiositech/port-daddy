...
arbiter, file claims).
**Backend:** `claude-sonnet` — novel attack construction needs the headroom.
...
Sonnet is the most expensive backend in the fleet. Two ways to waste
it: (a) firing on every PR including the rename-only ones — that's
what the surface gate is for; (b) generating "I tried X and Y and Z,
all defended" prose — that's padding and gets muted.
