...
class TestCommandLine(unittest.TestCase):
    ...
    runs unchanged. The substitution is undone in tearDown whether or not the
    test passes.
    """
    ...
    def test_the_script_runs_as_a_process_and_propagates_its_exit_code(self):
        # Pins `sys.exit(main())` and the real `--allow` spelling through the
        # actual __main__ path. Runs against the repository's own corpus, which
