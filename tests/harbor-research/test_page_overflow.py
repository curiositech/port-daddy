...
class PageGeometryMatchesTheBookTests(unittest.TestCase):
    ...
    def geometry(self):
        ...
        # The Book is twoside: \geometry names the gutter and the outer trim as
        # `inner`/`outer`, not `left`/`right`, and `textwidth` is given
    ...
    def test_the_text_block_matches(self):
        ...
        # against it rather than deriving it from paperwidth minus the
        # margins: for a twoside book, paperwidth = inner + textwidth + outer,
        # where `outer` is itself the margin column plus the trim white, and
