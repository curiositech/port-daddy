...
test('generator rejects a citation with no chapter-local bibliography entry', {
  ...
  try {
    injectAfterDocumentStart(root, 'whitepaper/source/legible-swarm.tex', '\\cite{purserMissingCitation}');
    const result = runGenerator(root);

    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /citation purserMissingCitation has no local bibliography entry/u);
    assert.match(result.stderr, /whitepaper\/source\/legible-swarm\.tex/u);
  } finally {
    cleanupFixture(root);
  }
});
