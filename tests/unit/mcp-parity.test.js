...
const MCP_EXEMPT_FEATURES = new Set([
  ...
  'suggest',        // Operator approval queue. An MCP-driving agent must not approve its own suggestion or trigger its own fleet run; list/approve/dismiss stay on operator CLI and native surfaces.
  'seamanship',     // Local skill catalog/read/sync CLI. MCP clients already receive skills from their runtime and use pd_discover for progressive capability disclosure; duplicating local filesystem sync as an MCP action would widen authority without adding a distinct capability.
  'fleet_registry', // Tender writes health and recommendations; operators read/pause/run ships. Direct MCP writes or one-shot run controls would let an agent grade or launch itself.
]);
