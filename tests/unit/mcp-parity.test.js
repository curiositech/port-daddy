...
const MCP_EXEMPT_FEATURES = new Set([
  ...
  'harbor-ledger',  // Agent Harbor read models (routes/agent-harbor.ts, wave3/routes): roster/detail/files/timeline/costs/receipts/compliance projections consumed by the Agent Launch Board and `pd doctor` (binder ch18, ADR-0095). Read-only operator telemetry, same posture as transcripts; MCP wrapper deferred until an agent-driving use case exists.
