...
const LEGACY_GEMINI_START = '<!-- windags-skills:start -->';
const LEGACY_GEMINI_END = '<!-- windags-skills:end -->';
...
const ALLOWED_CATALOG_EXTENSIONS = new Set([
  '.md', '.txt', '.json', '.jsonc', '.yaml', '.yml', '.toml', '.html',
  '.css', '.svg', '.png', '.jpg', '.jpeg', '.webp', '.gif', '.pdf',
]);
...
function isLegacyCatalogPath(path: string): boolean {
  ...
  return normalized.includes('/windags/') || normalized.includes('/windags-skills/');
}
...
function transformAgentsGuide(content: string): {
  ...
  const generatedLegacy = content.includes('# AGENTS.md — User-Level Agent Guide')
    && content.includes('## WinDAGs Skill System')
  ...
  if (generatedLegacy) {
    return {
      ...
      removals: 1,
      removedAuthorities: ['generated-user-guide:WinDAGs'],
    };
  }
...
function transformGeminiInstructions(content: string) {
  ...
  return {
    ...
    removedAuthorities: removals > 0 ? ['marker:windags-skills'] : [],
  };
}
...
function transformClaudeInstructions(content: string) {
  ...
  const kept = lines.filter((line) => {
    if (/^\s*(?:[-*+]\s+)?Embrace WinDAGs\b/i.test(line)) {
      removed += 1;
      return false;
    }
    return true;
  ...
  return {
    ...
    removedAuthorities: removed > 0 ? ['instruction-line:Embrace WinDAGs'] : [],
  };
}
...
export function juryRigBootstrapLayout(
  ...
  return {
    ...
    importRoot: resolve(options.importRoot ?? join(pdHome, 'catalogs', 'imported', 'windags')),
...
export function verifyNativeJuryRigRuntime(
  ...
  const version = runText(pdPath, ['--version'], 'installed pd version').match(/\d+\.\d+\.\d+/)?.[0];
  if (!version) throw new Error('installed pd did not report a semantic version');
  const keg = dirname(dirname(pdPath));
  const expectedKegSegment = `${sep}Cellar${sep}port-daddy${sep}${version}${sep}`;
  if (!`${keg}${sep}`.includes(expectedKegSegment)) {
    throw new Error(`native cutover requires the installed Homebrew keg for ${version}; resolved ${pdPath}`);
  }
...
function acquireJuryRigCutoverLock(
  ...
  atomicWrite(join(lockPath, 'owner.json'), JSON.stringify({ transactionId, pid: process.pid }) + '\n', 0o600);
  let released = false;
  return () => {
    if (released) return;
    released = true;
...
function scanJuryRigTransactionTree(
  ...
  function visit(directory: string): void {
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      ...
      const body = readBounded(path, Math.max(MAX_CATALOG_FILE_BYTES, statSync(path).size)).toString('utf8');
