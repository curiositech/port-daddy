 ...
 * closes the economic-enforcement loop: it turns a *repeated, egregious* rent
 * breach into a GRADUATED, PROPORTIONATE bond slash — the cryptoeconomic
 * "graduated punishment" the doctrine grounds in a graduated-trigger
 * equilibrium (`docs/research/north-star/doctrine/game-theory.md §4`).
 *
 * ════════════════════════════════════════════════════════════════════════
 *  SAFETY POSTURE — ADVISORY BY DEFAULT (read this before touching anything)
 * ════════════════════════════════════════════════════════════════════════
 * Slashing moves real money out of a principal's bond into the commons pool.
 * That is an OPERATOR DECISION, not a default. So this loop ships in three
 ...
 *   • 'enforce'  — (operator opt-in) actually call `bonds.slash` with the
 *                  graduated amount against the breaching principal's bond.
 *
 * The mode comes from `PD_RENT_SLASH_MODE`; an unset/unknown value resolves to
 * 'advisory'. There is NO path from a rent breach to a debit unless an operator
 * has explicitly set `PD_RENT_SLASH_MODE=enforce`. (See `resolveRentSlashMode`.)
 *
 * ════════════════════════════════════════════════════════════════════════
 ...
 * committer. We slash the Anchor/principal identity (the bond is keyed on the
 * semantic identity the spawn escrowed under), not a re-rollable co-located
 * agent id (ADR-0014/0022 Sybil defense). The resolution + the actual debit
 * live in `rent-slash-enforcer.ts`; THIS file is PURE — it computes amounts and
