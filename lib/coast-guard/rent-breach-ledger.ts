 ...
 *
 * ADVISORY-ONLY LANDING (2026-06-26): this ledger ships alongside the pure
 * `rent-slash.ts` policy as the safe foundation. It records and reads breach
 * state; it moves no money. The enforcer + HTTP routes that would TRIGGER
 * recordBreach/cure from the network are quarantined until they ride on
 * ADR-0087 (#500) daemon-signed verdicts — see `docs/adr/0050-coast-guard.md`
 * §"Phase 7 — advisory-only landing" for why an open, sessionId-authenticated
 * cure/breach surface is unsafe (grief + Goodhart).
 *
