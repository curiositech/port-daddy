...
export function createConductor(deps: ConductorDeps) {
   ...
   * Powers the operator console's Conductor pane (ADR-0060): the CLI renders
   * one lineage at a time via `tree(rootId)`, but the console needs to show
   * every active subtree at once without first knowing a rootId. The pane groups
   * the flat list by `rootId` and indents by `depth` to reconstruct the trees.
   */
