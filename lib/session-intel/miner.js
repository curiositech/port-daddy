...
//   3. handoff          (hit)  — a handoff/continuation note, then a later claim
...
//                               a short window with no bridging coordination note.
...
const HANDOFF_TYPES = new Set(['handoff', 'continuation']);
  ...
  for (const [file, fileClaims] of claimsByFile) {
    ...
    for (let i = 0; i < fileClaims.length; i++) {
      for (let j = i + 1; j < fileClaims.length; j++) {
        ...
        // A handoff/continuation note in either session during the window = coordinated.
        ...
        out.push({
          ...
          observation:
            ...
            (coordinated ? ' (a handoff/continuation note bridged them — coordinated)' : ' with no handoff bridging them'),
          excerpt: {
            source: 'claim-pair',
            filePath: file,
            claimA: claimRef(s1),
...
function detectHandoffs(out, { notes, claims, byId, claimsByFile }) {
  ...
  for (const n of handoffNotes) {
    ...
    if (!pickup && n.type !== 'continuation') continue;
    // A continuation note followed by the successor's claims also counts.
    if (!pickup && n.type === 'continuation') {
    ...
    out.push({
      ...
      observation:
        n.type === 'continuation'
          ? `A continuation note in ${shortId(n.sessionId)} was followed by real successor claims — a clean recovery.`
...
function detectDuplicateWork(out, { claimsByFile, byId, notesBySession }) {
  for (const [file, fileClaims] of claimsByFile) {
    ...
    for (let i = 0; i + 1 < ordered.length; i++) {
      ...
      // No handoff/continuation note bridging the two claims (no coordination).
      const bridged =
