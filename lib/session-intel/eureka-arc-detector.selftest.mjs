...
import * as D from './eureka-arc-detector.mjs';
...
// === 6c) Every harness-artifact class is filtered, not counted as an arc =====
// Each fixture: 2 results of one harness-artifact class (real machine strings),
// then a genuine success on the same command. Because the artifacts never ran
// against the real problem, there is NO fail→fail→success arc to report.
{
  const CLASSES = {
    'edit-policy-read-first': '<tool_use_error>File has not been read yet. Read it first before writing to it.</tool_use_error>',
    'edit-policy-string-not-found': 'String to replace not found in file.',
    'edit-policy-modified-since-read': '<tool_use_error>File has been modified since read, either by the user or by a linter. Read it again.</tool_use_error>',
    'guard-isolation': 'Isolation guard: this Agent/Task dispatch can write files but is not isolated. Parallel unsafe.',
    'guard-coordination': 'Exit code 1\n│\n■  Coordination Guard: ENFORCE found 1 issue(s)\n  - No active Port Daddy session',
    'guard-main-worktree': 'Exit code 1\nERROR: Port Daddy sessions refuse the main Git worktree by default.',
    // Model prefix is irrelevant to the sentinel — use a placeholder, never a
    // real model ID (no-hardcoded-model-ids guard scans .selftest.js too).
    'model-unavailable': '<the-model> is temporarily unavailable, so auto mode cannot determine the safety of Bash right now.',
    'permission-denied': 'Permission for this action was denied by the Claude Code auto mode classifier. Reason: [Cron].',
    'permission-rejected': "The user doesn't want to proceed with this tool use. The tool use was rejected.",
    'tool-input-error': '<tool_use_error>InputValidationError: Read failed due to the following issue:\nThe parameter is invalid.</tool_use_error>',
  };
  let allFiltered = true;
  for (const [name, sentinel] of Object.entries(CLASSES)) {
    const lines = [
      assistantToolUse('h1', 'Bash', { command: 'run-thing' }),
      userToolResult('h1', true, sentinel),
      assistantToolUse('h2', 'Bash', { command: 'run-thing' }),
      userToolResult('h2', true, sentinel),
      assistantToolUse('h3', 'Bash', { command: 'run-thing' }),
      userToolResult('h3', false, 'done'),
    ];
    const arcs = D.detectArcsFromText(lines.join('\n'), { minFailures: 2 });
    if (arcs.length !== 0) { allFiltered = false; console.error(`  ✗ class not filtered: ${name} → ${arcs.length} arc(s)`); }
    if (D.classifyHarnessArtifact(sentinel) === null) { allFiltered = false; console.error(`  ✗ class not classified: ${name}`); }
  }
  ok(allFiltered, 'harness classes: all artifact classes filtered AND classified');
}

// A GENUINE failure (Exit code 1 + traceback) is NOT a harness artifact and
// still yields an arc — the filter must not swallow real fail→fix breakthroughs.
{
  const realFail = 'Exit code 1\nTraceback (most recent call last):\n  File "build.py", line 95\nKeyError: 42';
  ok(D.classifyHarnessArtifact(realFail) === null, 'genuine failure: real traceback is NOT classified as a harness artifact');
  const lines = [
    assistantToolUse('r1', 'Bash', { command: 'python3 build.py' }),
    userToolResult('r1', true, realFail),
    assistantToolUse('r2', 'Bash', { command: 'python3 build.py' }),
    userToolResult('r2', true, realFail),
    assistantToolUse('r3', 'Bash', { command: 'python3 build.py' }),
    userToolResult('r3', false, 'built ok'),
  ];
  const arcs = D.detectArcsFromText(lines.join('\n'), { minFailures: 2 });
  ok(arcs.length === 1 && arcs[0].failCount === 2, 'genuine failure: real fail→fail→success still detected');
}

// A guard sentinel embedded in an "Exit code 1" body is filtered even though it
// carries a non-zero exit — the sentinel wins over the raw exit code.
{
  ok(D.classifyHarnessArtifact('Exit code 1\n■  Coordination Guard: ENFORCE found 1 issue(s)') === 'guard-block',
    'guard-in-exit: coordination-guard body classified despite Exit code 1');
}
