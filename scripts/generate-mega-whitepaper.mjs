 ...
 * preamble macros (\pdcontentsbegin, \pdcontentspart, \pdcontentschapter,
 * \pdconcordance* and \pdcontentsend), which dispatch on \pdedition; this
 * stays semantic. Every part hue emitted here is the part's own `color` field
 * out of whitepaper/textbook.json — the preamble never chooses one.
 *
 * \pdcontentsbegin comes FIRST, before the section head, because in the
 * technical edition it lays the drawing sheet under the page and shortens the
 * text block to clear the sheet's title block; both have to be in force
 * before any of the index is set.
 */
function renderContents(textbook) {
  ...
  lines.push(
    ...
    // booktabs only, and no vertical rules: the columns are separated by the
    // space between them. The rules are named rather than written out so the
    // preamble can give them a part hue (and the technical edition its ochre
    // and burnt orange) instead of black.
    '\\begin{pdconcordance}',
    '\\pdconcordancetoprule',
    '\\pdconcordancehead{First edition}{This edition}{Chapter}\\\\',
    '\\pdconcordancemidrule',
    ...concordance.map(
      (chapter) => `${chapter.formerNumeral} & ${chapter.number} & \\pdchapref{${chapter.prefix}}{${texText(chapter.title)}}\\\\`,
    ),
    '\\pdconcordancebottomrule',
    '\\end{pdconcordance}',
    '\\pdcontentsend',
    '',
  );
