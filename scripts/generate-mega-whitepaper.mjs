...
/**
 * mega-volume-contents.tex — the Book's two-page argument map. Styling lives
 * in the Book's preamble macros; this renderer stays semantic and deliberately
 * carries no historical-numbering concordance page.
 */
function renderContents(textbook) {
  ...
  const lines = [
    ...
    '\\pdcontentsspreadbegin',
    '\\phantomsection',
    '\\addcontentsline{toc}{section}{The argument, chapter by chapter}',
  ];
  for (const [partIndex, part] of parts.entries()) {
    if (partIndex === 2) lines.push('\\pdcontentsspreadbreak');
