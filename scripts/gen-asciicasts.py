...
def cast_quickstart():
    ...
    c.type_command('pd begin "Building the photo upload API" --identity photoapp:api')
    output = run_cmd('pd begin "Building the photo upload API" --identity photoapp:api')
...
def cast_coordination():
    ...
    c.type_command('pd begin "REST API for user auth" --identity myapp:api --agent backend-dev')
    output = run_cmd('pd begin "REST API for user auth" --identity myapp:api --agent backend-dev')
    c.show_output(output)
    c.pause(0.8)
    ...
    c.type_command('pd begin "React login page" --identity myapp:web --agent frontend-dev')
    output = run_cmd('pd begin "React login page" --identity myapp:web --agent frontend-dev')
    c.show_output(output)
    c.pause(0.8)
