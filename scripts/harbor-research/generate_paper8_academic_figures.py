...
# -------------------------------------------------------------------------
def make_fig0():
    fig = plt.figure(figsize=(13.0, 9.4), dpi=300)
    ...
    fig.suptitle("The AI Engineer's Field Guide: Cellular Sheaves & Cohomology in Multi-Agent Swarms",
                 fontsize=21, fontweight='bold', color=SLATE_DARK)
    ...
    ax_a.set_title(r"(a) What is a Stalk $\mathcal{F}(v)$? (An Agent's Operational State)",
                   fontsize=16.5, fontweight='bold', color=SLATE_DARK, pad=10)
    ax_a.set_xlim(-0.04, 1.04)
    ax_a.set_ylim(-0.20, 1.05)
    ax_a.axis('off')

    # Central Agent Circle (strictly circular via physical inch calibration)
    draw_circle(ax_a, 0.14, 0.52, 0.55, BG_LIGHT, HARBOR_BLUE, lw=1.8, zorder=2)
    ax_a.text(0.14, 0.56, "AgentNode", ha='center', va='center', fontsize=14.5, fontweight='bold', color=HARBOR_BLUE, zorder=5)
    ax_a.text(0.14, 0.46, r"($v \in V$)", ha='center', va='center', fontsize=13.0, color=SLATE_MID, zorder=5)

    # Exploded Stalk Vector Box with generous width and padding
    rect_stalk = patches.Rectangle((0.30, 0.12), 0.68, 0.80, facecolor=BG_WHITE, edgecolor=LINE_GRAY, lw=1.0)
    ax_a.add_patch(rect_stalk)
    ax_a.text(0.64, 0.84, r"Stalk Vector $x_v \in \mathbb{R}^D$ (Local State)", ha='center', va='center',
              fontsize=15.5, fontweight='bold', color=HARBOR_BLUE)
    ...
    for f_title, f_desc in stalk_fields:
        ax_a.text(0.33, y_pos, f_title, fontsize=13.5, fontweight='bold', color=SLATE_DARK)
        ax_a.text(0.33, y_pos - 0.055, f_desc, fontsize=12.0, color=SLATE_MID, fontstyle='italic')
        y_pos -= 0.14

    # Connecting arrow
    ax_a.annotate('', xy=(0.29, 0.52), xytext=(0.22, 0.52),
                  arrowprops=dict(arrowstyle="-|>", color=HARBOR_BLUE, lw=1.6, mutation_scale=14))
    ax_a.text(0.255, 0.57, r"$\mathcal{F}(v)$", ha='center', va='bottom', fontsize=14.5, color=HARBOR_BLUE, fontweight='bold')

    card_a = patches.Rectangle((0.02, -0.18), 0.96, 0.25, facecolor=BG_LIGHT, edgecolor=LINE_GRAY, lw=0.9)
    ax_a.add_patch(card_a)
    ax_a.text(0.50, -0.02, r"$\mathbf{Why\ AI\ Engineers\ Care:}\ \text{Automated Proofs vs Log Archaeology}$",
              ha='center', va='center', fontsize=13.5, fontweight='bold', color=SLATE_DARK)
    ax_a.text(0.50, -0.11, "Abstracting agent state into numeric stalks allows instant linear-algebraic verification\nwithout parsing hundreds of thousands of LLM chat tokens.",
              ha='center', va='center', fontsize=12.0, color=SLATE_MID)
    ...
    stages = [
        ...
        ("2-Cochains $C^2$", "3-Way Joins", r"Review triads $\tau$", 0.70, 0.28, SHIP_RED)
    ]
    for title, role, desc, sx, sw, col in stages:
        box = patches.Rectangle((sx, 0.38), sw, 0.54, facecolor=BG_LIGHT, edgecolor=LINE_GRAY, lw=1.0)
        ax_b.add_patch(box)
        ax_b.text(sx + sw/2, 0.82, title, ha='center', va='center', fontsize=14.5, fontweight='bold', color=col)
        ax_b.text(sx + sw/2, 0.65, role, ha='center', va='center', fontsize=14.0, fontweight='bold', color=SLATE_DARK)
        ax_b.text(sx + sw/2, 0.49, desc, ha='center', va='center', fontsize=12.5, color=SLATE_MID)

    ax_b.annotate('', xy=(0.35, 0.65), xytext=(0.31, 0.65),
                  arrowprops=dict(arrowstyle="-|>", color=SLATE_MID, lw=1.6, mutation_scale=14))
    ax_b.text(0.33, 0.72, r"$\delta_0$", ha='center', va='bottom', fontsize=16.5, fontweight='bold', color=SLATE_DARK)

    ax_b.annotate('', xy=(0.69, 0.65), xytext=(0.65, 0.65),
                  arrowprops=dict(arrowstyle="-|>", color=SLATE_MID, lw=1.6, mutation_scale=14))
    ax_b.text(0.67, 0.72, r"$\delta_1$", ha='center', va='bottom', fontsize=16.5, fontweight='bold', color=SLATE_DARK)

    rect_id = patches.Rectangle((0.02, -0.18), 0.96, 0.50, facecolor=BG_WHITE, edgecolor=LINE_GRAY, lw=1.0)
    ax_b.add_patch(rect_id)
    ax_b.text(0.50, 0.22, r"$\mathbf{Fundamental\ Simplicial\ Identity:}\ \delta_1 \circ \delta_0 = 0$",
              ha='center', va='center', fontsize=14.5, fontweight='bold', color=SLATE_DARK)
    ax_b.text(0.50, 0.11, "(The curl of a gradient is identically zero across any 3-agent join)",
              ha='center', va='center', fontsize=12.5, color=SLATE_MID, fontstyle='italic')
    ax_b.text(0.50, -0.01, r"$\mathbf{Why\ AI\ Engineers\ Care:}\ \text{Zero False-Alarm Guarantee}$",
              ha='center', va='center', fontsize=13.5, fontweight='bold', color=SLATE_DARK)
    ax_b.text(0.50, -0.10, "Legitimate asynchronous agent progress never produces spurious review-cycle alerts,\neliminating developer alert fatigue in high-throughput swarms.",
              ha='center', va='center', fontsize=12.0, color=SLATE_MID)
    ...
    ax_c.text(0.24, 0.94, "Open Delegation Tree\n(LangChain / AutoGen)", ha='center', va='center', fontsize=13.5, fontweight='bold', color=SLATE_DARK)
    ...
    for u, v in [(0, 1), (0, 2), (1, 3), (2, 4)]:
        ...
        ax_c.annotate('', xy=(x2, y2), xytext=(x1, y1),
                      arrowprops=dict(arrowstyle="-|>", color=SLATE_MID, lw=1.4, shrinkA=10, shrinkB=10, mutation_scale=12))

    for n, (x, y) in tree_nodes.items():
        fc = SHIP_RED if n == 1 else HARBOR_BLUE
        draw_circle(ax_c, x, y, 0.22, fc, SLATE_DARK, lw=1.0, zorder=4)
        ax_c.text(x, y, f"v{n}", ha='center', va='center', fontsize=13.0, color=BG_WHITE, fontweight='bold', zorder=5)

    card_c1 = patches.Rectangle((0.02, -0.18), 0.44, 0.25, facecolor=BG_LIGHT, edgecolor=LINE_GRAY, lw=0.8)
    ax_c.add_patch(card_c1)
    ax_c.text(0.24, -0.02, r"$\ker(\delta_0^T) = \{0\}$ (Silent!)", ha='center', va='center', fontsize=13.5, fontweight='bold', color=SHIP_RED)
    ax_c.text(0.24, -0.11, "Acyclic trees swallow lies\nas benign network delays", ha='center', va='center', fontsize=12.0, color=SLATE_MID)

    # Sub-panel 2: Cycle (right)
    ax_c.text(0.76, 0.94, "Closed Review Cycle\n(Port Daddy / Harbor)", ha='center', va='center', fontsize=13.5, fontweight='bold', color=SLATE_DARK)
    ...
    for u, v, lie in [(0, 1, False), (1, 2, True), (2, 3, False), (3, 0, False)]:
        ...
        ax_c.annotate('', xy=(x2, y2), xytext=(x1, y1),
                      arrowprops=dict(arrowstyle="-|>", color=col, lw=1.8 if lie else 1.4, shrinkA=10, shrinkB=10, mutation_scale=12))

    for n, (x, y) in cycle_nodes.items():
        fc = SHIP_RED if n == 1 else HARBOR_BLUE
        draw_circle(ax_c, x, y, 0.22, fc, SLATE_DARK, lw=1.0, zorder=4)
        ax_c.text(x, y, f"u{n}", ha='center', va='center', fontsize=13.0, color=BG_WHITE, fontweight='bold', zorder=5)

    arc = patches.Arc((0.76, 0.47), 0.22, 0.22, angle=0, theta1=20, theta2=310, color=SHIP_RED, lw=1.4, ls='--')
    ax_c.add_patch(arc)
    ax_c.text(0.76, 0.47, r"$\rho \neq 0$", ha='center', va='center', fontsize=13.5, color=SHIP_RED, fontweight='bold')

    card_c2 = patches.Rectangle((0.54, -0.18), 0.44, 0.25, facecolor=BG_LIGHT, edgecolor=LINE_GRAY, lw=0.8)
    ax_c.add_patch(card_c2)
    ax_c.text(0.76, -0.02, r"$\ker(\delta_0^T) \neq \{0\}$ (Trapped!)", ha='center', va='center', fontsize=13.5, fontweight='bold', color=HARBOR_BLUE)
    ax_c.text(0.76, -0.11, "Cycle traps hallucination\nwith certified residual $r > 0$", ha='center', va='center', fontsize=12.0, color=SLATE_MID)
    ...
    ax_d.text(0.24, 0.94, "Gauge Skew\n(Benign Turn Lag)", ha='center', va='center', fontsize=13.5, fontweight='bold', color=SEA_GREEN)
    ...
    for i, (x, y) in enumerate(g_pts):
        draw_circle(ax_d, x, y, 0.24, BG_WHITE, SEA_GREEN, lw=1.5, zorder=4)
        ax_d.text(x, y, f"t{i}", ha='center', va='center', fontsize=13.0, color=SLATE_DARK, fontweight='bold', zorder=5)

    ax_d.text(0.24, 0.42, r"$\delta_1 g = 0$" + "\nCurl-free", ha='center', va='center', fontsize=13.5, color=SEA_GREEN, fontweight='bold')
    
    card_d1 = patches.Rectangle((0.02, -0.18), 0.44, 0.25, facecolor=BG_LIGHT, edgecolor=LINE_GRAY, lw=0.8)
    ax_d.add_patch(card_d1)
    ax_d.text(0.24, -0.02, "Action: Do Nothing!", ha='center', va='center', fontsize=13.5, fontweight='bold', color=SEA_GREEN)
    ax_d.text(0.24, -0.11, "Benign step latency;\nself-resolves on next turn", ha='center', va='center', fontsize=12.0, color=SLATE_MID)

    # Right: Triadic Curl (Hallucination Bug)
    ax_d.text(0.76, 0.94, "Triadic Curl\n(Review Bug)", ha='center', va='center', fontsize=13.5, fontweight='bold', color=SHIP_RED)
    c_pts = [(0.62, 0.26), (0.90, 0.26), (0.76, 0.70)]
    poly = plt.Polygon(c_pts, facecolor=BG_LIGHT, edgecolor=SHIP_RED, lw=1.5, zorder=2)
    ax_d.add_patch(poly)

    arc_d = patches.Arc((0.76, 0.43), 0.20, 0.20, angle=0, theta1=20, theta2=310, color=SHIP_RED, lw=1.4, ls='--')
    ax_d.add_patch(arc_d)
    ax_d.annotate('', xy=(0.86, 0.47), xytext=(0.86, 0.42), arrowprops=dict(arrowstyle="-|>", color=SHIP_RED, lw=1.4))

    role_lbls = ["Coder", "Critic", "Mgr"]
    for i, (x, y) in enumerate(c_pts):
        draw_circle(ax_d, x, y, 0.24, BG_WHITE, SHIP_RED, lw=1.5, zorder=4)
        ax_d.text(x, y, role_lbls[i], ha='center', va='center', fontsize=12.5, color=SLATE_DARK, fontweight='bold', zorder=5)

    ax_d.text(0.76, 0.43, r"$\delta_1 g \neq 0$" + "\nParadox!", ha='center', va='center', fontsize=13.5, color=SHIP_RED, fontweight='bold')

    card_d2 = patches.Rectangle((0.54, -0.18), 0.44, 0.25, facecolor=BG_LIGHT, edgecolor=LINE_GRAY, lw=0.8)
    ax_d.add_patch(card_d2)
    ax_d.text(0.76, -0.02, "Action: Re-prompt Triad!", ha='center', va='center', fontsize=13.5, fontweight='bold', color=SHIP_RED)
    ax_d.text(0.76, -0.11, "Review contract violated;\nre-prompt dissent loop", ha='center', va='center', fontsize=12.0, color=SLATE_MID)
...
# -------------------------------------------------------------------------
def make_fig1():
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(11.0, 5.0), dpi=300)
    fig.patch.set_facecolor(BG_WHITE)

    # Panel A: Open Tree
    ax1.set_facecolor(BG_WHITE)
    ax1.set_title("(a) Acyclic Gossip Tree ($r = 0$, Provably Silent)", pad=14, fontsize=16.5, fontweight='bold', color=SLATE_DARK)
    ...
    for u, v, dx, dy in tree_edges:
        ...
        ax1.annotate('', xy=(x2, y2), xytext=(x1, y1),
                     arrowprops=dict(arrowstyle="-|>", color=SLATE_MID, lw=1.5, mutation_scale=14, shrinkA=12, shrinkB=12))
        mx, my = (x1 + x2)/2, (y1 + y2)/2
        ax1.text(mx + dx, my + dy, r"$g_e = x_v - x_u$", fontsize=13.0, color=SLATE_MID, fontstyle='italic', ha='center')

    for node, (x, y) in pos_tree.items():
        fc = SHIP_RED if node == 1 else HARBOR_BLUE
        circle = plt.Circle((x, y), 0.058, facecolor=fc, edgecolor=SLATE_DARK, lw=1.2, zorder=4)
        ax1.add_patch(circle)
        label = r"$v_1^*$" if node == 1 else f"$v_{node}$"
        ax1.text(x, y, label, ha='center', va='center', fontsize=14.0, color=BG_WHITE, fontweight='bold', zorder=5)
    ...
    ax1.text(0.5, -0.06, 
             ...
             ha='center', va='top', fontsize=13.0, color=SLATE_DARK,
             bbox=dict(boxstyle="square,pad=0.5", facecolor=BG_LIGHT, edgecolor=LINE_GRAY, lw=1.0))
    ...
    ax2.set_title("(b) Cycle Topology ($r > 0$, Certified Lower Bound)", pad=14, fontsize=16.5, fontweight='bold', color=SLATE_DARK)
    ...
    for u, v, lbl, is_lie in cycle_edges:
        ...
        lw = 2.0 if is_lie else 1.5
        ax2.annotate('', xy=(x2, y2), xytext=(x1, y1),
                     arrowprops=dict(arrowstyle="-|>", color=col, lw=lw, mutation_scale=14, shrinkA=12, shrinkB=12))
        mx, my = (x1 + x2)/2, (y1 + y2)/2
        dx, dy = (0, 0.06) if y1 == y2 else (0.06, 0)
        ax2.text(mx + dx, my + dy, lbl, ha='center', va='center', fontsize=13.5,
                 color=SHIP_RED if is_lie else SLATE_DARK, fontweight='bold' if is_lie else 'normal')

    arc = patches.Arc((0.5, 0.485), 0.24, 0.24, angle=0, theta1=20, theta2=310, color=SHIP_RED, lw=1.5, ls='--')
    ax2.add_patch(arc)
    ax2.annotate('', xy=(0.61, 0.53), xytext=(0.62, 0.49),
                 arrowprops=dict(arrowstyle="-|>", color=SHIP_RED, lw=1.5, mutation_scale=10))
    ax2.text(0.5, 0.485, r"$\rho \in \ker(B^T)$" + "\n" + r"$\|\rho\|_2 = 1.500$",
             ha='center', va='center', fontsize=13.5, color=SHIP_RED, fontweight='bold')
    ...
    for node, (x, y) in pos_cycle.items():
        ...
        label = r"$v_1^*$" if node == 1 else f"$v_{node}$"
        ax2.text(x, y, label, ha='center', va='center', fontsize=14.0, color=BG_WHITE, fontweight='bold', zorder=5)
...
def make_fig2():
    ...
    ax.set_title("The Rosetta Stone: Mapping Multi-Agent Swarms to Cellular Sheaves", 
                 pad=14, fontsize=18.5, fontweight='bold', color=SLATE_DARK)
    ...
    for title, x, w, items in cols:
        ...
        ax.add_patch(rect)
        ax.text(x + w/2, 0.86, title, ha='center', va='center', fontsize=15.0, fontweight='bold', color=HARBOR_BLUE)
        body = "\n".join(items)
        ax.text(x + 0.015, 0.62, body, ha='left', va='center', fontsize=13.0, color=SLATE_DARK, linespacing=1.35)

    ax.annotate('', xy=(0.355, 0.68), xytext=(0.325, 0.68),
                arrowprops=dict(arrowstyle="-|>", lw=1.6, color=SLATE_MID, mutation_scale=14))
    ax.text(0.34, 0.73, r"$\delta_0$", ha='center', va='center', fontsize=17.5, fontweight='bold', color=SLATE_DARK)

    ax.annotate('', xy=(0.675, 0.68), xytext=(0.645, 0.68),
                arrowprops=dict(arrowstyle="-|>", lw=1.6, color=SLATE_MID, mutation_scale=14))
    ax.text(0.66, 0.73, r"$\delta_1$", ha='center', va='center', fontsize=17.5, fontweight='bold', color=SLATE_DARK)

    rect_bottom = patches.Rectangle((0.04, 0.05), 0.92, 0.33, facecolor=BG_WHITE, edgecolor=LINE_GRAY, lw=1.0)
    ...
    ax.text(0.50, 0.25, 
            ...
            ha='center', va='center', fontsize=14.5, color=SLATE_DARK)
    ...
    ax.text(0.50, 0.12,
            ...
            ha='center', va='center', fontsize=13.0, color=SLATE_MID)
...
# -------------------------------------------------------------------------
def make_fig3():
    fig = plt.figure(figsize=(12.5, 5.2), dpi=300)
    fig.patch.set_facecolor(BG_WHITE)
    ...
    fig.suptitle("Simplicial Hodge Decomposition: Categorizing Swarm Disagreements for AI Supervisors",
                 fontsize=19.5, fontweight='bold', color=SLATE_DARK, y=0.96)
    ...
    ax1.set_title(r"1. Gauge Gradient $\delta_0 x$" + "\n" + r"$\mathbf{Benign\ Turn\ Skew}$" + "\n" + r"($\delta_1(\delta_0 x) \equiv 0$, Curl-Free)", 
                  fontsize=14.5, fontweight='bold', color=SEA_GREEN, pad=8)
    ...
    for i, (x, y) in enumerate(tri_pts):
        ax1.add_patch(plt.Circle((x, y), 0.080, facecolor=BG_WHITE, edgecolor=SEA_GREEN, lw=1.5, zorder=4))
        ax1.text(x, y, f"v{i}", ha='center', va='center', fontsize=13.5, color=SLATE_DARK, fontweight='bold', zorder=5)

    ax1.text(0.5, 0.36, "Turn skew:\n$x_v - x_u$\n" + r"$\mathbf{\delta_1 g = 0}$", ha='center', va='center', fontsize=13.0, color=SEA_GREEN)
    ax1.text(0.5, -0.10, "Action: Ignore.\nSelf-resolves naturally.", ha='center', va='center', fontsize=12.5, color=SLATE_MID,
    ...
    ax2.set_title(r"2. Harmonic Cavity $h \in \mathcal{H}^1$" + "\n" + r"$\mathbf{Network\ Split\ /\ Partition}$" + "\n" + r"($\delta_1 h = 0$, $\delta_0^* h = 0$)", 
                  fontsize=14.5, fontweight='bold', color=HARBOR_BLUE, pad=8)
    ...
    hole = plt.Circle((0.50, 0.47), 0.16, facecolor=BG_LIGHT, edgecolor=HARBOR_BLUE, lw=1.0, ls='--')
    ax2.add_patch(hole)
    ax2.text(0.50, 0.47, "Macro Void\n(Cross-Harbor\nLag)", ha='center', va='center', fontsize=12.5, color=HARBOR_BLUE)

    for i, (x, y) in enumerate(square_pts):
        ax2.add_patch(plt.Circle((x, y), 0.080, facecolor=BG_WHITE, edgecolor=HARBOR_BLUE, lw=1.5, zorder=4))
        ax2.text(x, y, f"u{i}", ha='center', va='center', fontsize=13.5, color=SLATE_DARK, fontweight='bold', zorder=5)

    ax2.text(0.5, -0.10, "Action: Reconnect relay.\nGlobal state drifting.", ha='center', va='center', fontsize=12.5, color=SLATE_MID,
             bbox=dict(boxstyle="square,pad=0.4", facecolor=BG_LIGHT, edgecolor=LINE_GRAY, lw=0.8))

    # 3. Triadic Curl delta_1^* psi
    ax3.set_title(r"3. Triadic Curl $\delta_1^* \psi$" + "\n" + r"$\mathbf{Review\ Hallucination}$" + "\n" + r"($\delta_1 g \neq 0$, Divergence-Free)", 
                  fontsize=14.5, fontweight='bold', color=SHIP_RED, pad=8)
    
    poly = plt.Polygon(tri_pts, facecolor=BG_LIGHT, edgecolor=SHIP_RED, lw=1.5, zorder=2)
    ax3.add_patch(poly)
    arc = patches.Arc((0.50, 0.42), 0.22, 0.22, angle=0, theta1=20, theta2=320, color=SHIP_RED, lw=1.4, ls='--')
    ax3.add_patch(arc)
    ax3.annotate('', xy=(0.60, 0.46), xytext=(0.61, 0.42), arrowprops=dict(arrowstyle="-|>", color=SHIP_RED, lw=1.4))
    
    ax3.text(0.50, 0.32, "Broken 3-Way\nReview Join\n" + r"$(\mathbf{\delta_1 g \neq 0})$", ha='center', va='center', fontsize=13.0, color=SHIP_RED)

    role_labels = ["Coder", "Critic", "Mgr"]
    for i, (x, y) in enumerate(tri_pts):
        ax3.add_patch(plt.Circle((x, y), 0.080, facecolor=BG_WHITE, edgecolor=SHIP_RED, lw=1.5, zorder=4))
        ax3.text(x, y, role_labels[i], ha='center', va='center', fontsize=12.5, color=SLATE_DARK, fontweight='bold', zorder=5)

    ax3.text(0.5, -0.10, "Action: Re-prompt triad!\nPrompt critique loop.", ha='center', va='center', fontsize=12.5, color=SHIP_RED,
             bbox=dict(boxstyle="square,pad=0.4", facecolor=BG_LIGHT, edgecolor=LINE_GRAY, lw=0.8))
    ...
    ax4.set_title(r"$\mathbf{Swarm\ Legibility\ \mathcal{L}(g)}$" + "\n" + r"$\frac{\|h\|^2}{\|h\|^2 + \|\delta_1^*\psi\|^2}$",
                  fontsize=15.0, fontweight='bold', color=SLATE_DARK, pad=10)
    
    categories = ['Review Bug\n(Micro)', 'Network Split\n(Macro)']
    values = [0.00, 1.00]
    bars = ax4.bar(categories, values, width=0.45, color=[SHIP_RED, HARBOR_BLUE], edgecolor=SLATE_DARK, lw=1.0)
    ax4.set_ylabel(r"$\mathcal{L}(g) \in [0, 1]$", fontsize=13.5)
    ax4.set_ylim(0, 1.25)
    ax4.grid(axis='y', linestyle=':', alpha=0.35)
    ax4.text(0, 0.06, "0.00", ha='center', va='bottom', fontsize=14.0, fontweight='bold', color=SHIP_RED)
    ax4.text(1, 1.04, "1.00", ha='center', va='bottom', fontsize=14.0, fontweight='bold', color=HARBOR_BLUE)
...
# -------------------------------------------------------------------------
def make_fig4():
    fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(12.0, 4.6), dpi=300)
    fig.patch.set_facecolor(BG_WHITE)

    # Panel 1: Residual Flow Graph
    ax1.set_facecolor(BG_WHITE)
    ax1.set_aspect('equal')
    ax1.set_title(r"(a) Residual Flow $\rho = \Pi_K g_K$", fontsize=16.0, fontweight='bold', color=SLATE_DARK, pad=10)
    ...
    for (u, v), en in edge_energies.items():
        ...
        ax1.text(mx, my, f"{en:.2f}", fontsize=13.0, color=SHIP_RED if en > 1.5 else SLATE_MID,
                 bbox=dict(boxstyle="square,pad=0.2", facecolor=BG_WHITE, edgecolor=LINE_GRAY, lw=0.7))

    for node, (x, y) in pos.items():
        fc = SHIP_RED if node == 2 else HARBOR_BLUE
        ax1.add_patch(plt.Circle((x, y), 0.080, facecolor=fc, edgecolor=SLATE_DARK, lw=1.2, zorder=4))
        ax1.text(x, y, f"$v_{node}$", ha='center', va='center', fontsize=14.5, color=BG_WHITE, fontweight='bold', zorder=5)

    ax1.text(0.5, -0.06, r"Initial $r = 2.683$; Node $v_2$ equivocating", ha='center', va='top', fontsize=13.5, color=SLATE_DARK)
    ax1.set_xlim(0.05, 0.95)
    ax1.set_ylim(-0.14, 0.95)
    ax1.axis('off')

    # Panel 2: Ratio Controller Plot
    ax2.set_facecolor(BG_WHITE)
    ax2.set_title(r"(b) Controller: $\arg\max \frac{E(e)}{w(e)}$", fontsize=16.0, fontweight='bold', color=SLATE_DARK, pad=10)
    ...
    ax2.set_ylabel(r"Ratio $E(e)/w(e)$", fontsize=14.0)
    ax2.grid(axis='y', linestyle=':', alpha=0.35)
    ax2.text(4, ratios[4] + 0.07, r"Optimal Cut $e^*$" + "\n" + r"(Ratio=1.85)", ha='center', va='bottom', fontsize=13.0, color=HARBOR_BLUE, fontweight='bold')
    ax2.set_ylim(0, 2.4)

    # Panel 3: Monotonic Decay Curve
    ax3.set_facecolor(BG_WHITE)
    ax3.set_title(r"(c) Residual Collapse: $r(t) \to 0$", fontsize=16.0, fontweight='bold', color=SLATE_DARK, pad=10)
    rounds = [0, 1, 2]
    residuals = [2.683, 1.528, 0.000]
    
    ax3.plot(rounds, residuals, marker='s', lw=2.0, color=HARBOR_BLUE, markersize=8)
    for r_idx, val in zip(rounds, residuals):
        dy = 0.20 if val > 0 else 0.20
        dx = -0.12 if val == 0 else 0.0
        ax3.text(r_idx + dx, val + dy, f"$r={val:.3f}$", ha='center', va='bottom', fontsize=13.5, color=HARBOR_BLUE, fontweight='bold')
    
    ax3.set_xlabel("Intervention Round", fontsize=14.0)
    ax3.set_ylabel("Completion Residual $r$", fontsize=14.0)
    ax3.set_xticks([0, 1, 2])
    ax3.set_ylim(-0.25, 3.4)
    ax3.grid(True, linestyle=':', alpha=0.35)
    ax3.text(1.10, 0.55, r"Bound: $\leq \beta_1(G)$ rounds" + "\n" + r"$\beta_1(G) = 5 - 4 + 1 = 2$",
             fontsize=13.0, color=SEA_GREEN, fontweight='bold', bbox=dict(boxstyle="square,pad=0.4", facecolor=BG_LIGHT, edgecolor=LINE_GRAY, lw=0.8))
...
# -------------------------------------------------------------------------
def make_fig5():
    fig = plt.figure(figsize=(13.0, 6.8), dpi=300)
    fig.patch.set_facecolor(BG_WHITE)
    ...
    fig.suptitle("16-Agent Enterprise Swarm: Simplicial Hodge Triage of Mixed Failures",
                 fontsize=20.0, fontweight='bold', color=SLATE_DARK)
    ...
    ax_graph.set_title(r"Enterprise Architecture: 4 Clusters, 28 Edges ($\mathcal{L} = 0.342$)", 
                       fontsize=16.0, fontweight='bold', color=SLATE_DARK, pad=10)
    ...
    for name, (bx, by, bw, bh) in team_boxes.items():
        ...
        ax_graph.text(bx + 0.03, by + bh - 0.05, f"Team: {name}", fontsize=14.0, fontweight='bold', color=HARBOR_BLUE, zorder=2)
    ...
    ax_graph.text(0.77, 0.72, r"Triad Bug" + "\n" + r"$\delta_1^* \psi = 3.46$",
                 ha='center', va='center', fontsize=12.5, color=SHIP_RED, fontweight='bold',
                 bbox=dict(boxstyle="square,pad=0.25", facecolor=BG_WHITE, edgecolor=SHIP_RED, lw=0.8), zorder=6)
    ...
    for u, v in inter_edges:
        ...
        lw = 1.8 if is_lag else 1.0
        ls = '--' if is_lag else ':'
        ax_graph.plot([pos[u][0], pos[v][0]], [pos[u][1], pos[v][1]], color=col, lw=lw, ls=ls, zorder=3)

    ax_graph.text(0.50, 0.72, r"Partition Cavity" + "\n" + r"$h = 2.50$",
                 ha='center', va='center', fontsize=12.5, color=HARBOR_BLUE, fontweight='bold',
                 bbox=dict(boxstyle="square,pad=0.25", facecolor=BG_WHITE, edgecolor=HARBOR_BLUE, lw=0.8), zorder=6)
    ...
    for node, (x, y) in pos.items():
        ...
        ax_graph.text(x, y, f"{node}", ha='center', va='center', fontsize=13.0, color=BG_WHITE, fontweight='bold', zorder=5)
    ...
    ax_stats.set_title("Hodge Component Energy Breakdown", fontsize=15.5, fontweight='bold', color=SLATE_DARK, pad=8)
    ...
    bars = ax_stats.barh(components, energies, color=colors, edgecolor=SLATE_DARK, lw=1.0, height=0.50)
    ax_stats.set_xlabel(r"Energy Metric ($\|\cdot\|_2^2$)", fontsize=13.5)
    ...
    for bar, val in zip(bars, energies):
        ax_stats.text(val + 0.35, bar.get_y() + bar.get_height()/2, f"{val:.1f}", 
                     va='center', fontsize=13.5, fontweight='bold', color=SLATE_DARK)
    ...
    card_rect = patches.Rectangle((0.02, 0.03), 0.96, 0.94, facecolor=BG_LIGHT, edgecolor=LINE_GRAY, lw=1.0)
    ax_triage.add_patch(card_rect)

    ax_triage.text(0.06, 0.88, r"$\mathbf{Automated\ Swarm\ Triage\ Report}$", fontsize=15.5, fontweight='bold', color=SLATE_DARK)
    
    ax_triage.text(0.06, 0.74, r"$\bullet\ \mathbf{Swarm\ Legibility\ Ratio:}\ \mathcal{L}(g) = \frac{\|h\|^2}{\|h\|^2 + \|\delta_1^* \psi\|^2} = \mathbf{0.342}$",
                   fontsize=13.5, color=SLATE_DARK)
    ax_triage.text(0.10, 0.63, "(34.2% Macro Partition Cavity vs. 65.8% Micro Review Curl)", fontsize=12.0, color=SLATE_MID, fontstyle='italic')

    ax_triage.text(0.06, 0.49, r"$\mathbf{Prescriptive\ Supervisor\ Interventions:}$", fontsize=14.0, fontweight='bold', color=SLATE_DARK)
    
    ax_triage.text(0.08, 0.35, r"1. $\mathbf{65.8\%\ Micro\ Action:}$ Re-prompt Backend Triad $(v_4, v_5, v_6)$",
                   fontsize=13.0, fontweight='bold', color=SHIP_RED)
    ax_triage.text(0.12, 0.25, "Broken 3-way review join detected. Inject dissent diff into next prompt.",
                   fontsize=12.0, color=SLATE_MID)

    ax_triage.text(0.08, 0.13, r"2. $\mathbf{34.2\%\ Macro\ Action:}$ Sync API Bridge $(v_2, v_4)$ via Harbor Relay",
                   fontsize=13.0, fontweight='bold', color=HARBOR_BLUE)
    ax_triage.text(0.12, 0.04, "Global partition detected across clusters. Trigger outbound SSE sync.",
                   fontsize=12.0, color=SLATE_MID)
