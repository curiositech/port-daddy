...
- **Dispatch — autonomous feature-dev queue** (ADR-0035; `cli/commands/dispatch.ts`, `lib/dispatch/runner.ts`, `lib/dispatch/conductor-adapter.ts`, `lib/dispatch/conductor-lifecycle.ts`). `pd dispatch propose|queue|list|show|run|cancel`. `run` is **dry-run by default**, returning a non-executable semantic plan. `--really-run` requests Conductor execution (default `cli:codex`) in an isolated worktree under `~/coding/tmp/port-daddy-dispatch-<id>`. A configured GitHub App publisher is not yet wired: completed local work is preserved in salvage with a human-attention receipt; no ambient-account push or draft PR is attempted. Per-dispatch `--budget` (default 5 USD, max 25) and `--timeout` (default 3h, max 6h).
