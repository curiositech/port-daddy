...
function spawnIsTerminal(data: Record<string, unknown>): boolean {
  return data.terminal === true || ['completed', 'failed', 'killed', 'over_budget'].includes(String(data.status || ''));
}
...
async function collectSpawn(monitorUrl: string): Promise<Record<string, unknown>> {
  ...
  while (true) {
    try {
      ...
      reconnects = 0;
      if (spawnIsTerminal(data)) return data;
    } catch (error) {
      if (error instanceof SpawnMonitorMissingError) {
...
function printSpawnResult(data: Record<string, unknown>, options: CLIOptions): void {
  const failed = data.success === false || ['failed', 'killed', 'over_budget'].includes(String(data.status || ''));
  ...
  if (data.error) console.error(`  Error: ${data.error as string}`);
...
export async function handleSpawn(
  ...
  if (args[0] === 'kill') {
    ...
    const res: PdFetchResponse = await pdFetch(`/spawn/${encodeURIComponent(agentId)}`, {
      ...
      // Fastify rejects an application/json request with no body before the
      // route runs. Keep the explicit content type and send the empty object.
      body: '{}',
    });
  ...
  const requestedTimeoutMs = options.timeout != null
  ...
  if (!task) {
    ...
    console.error('  --timeout <ms>         Optional hard execution deadline; no default for CLI agents');
  ...
  const res: PdFetchResponse = await pdFetch('/spawn', {
    ...
    // Admission is short. Agent execution belongs to the durable spawn resource,
    // never to this HTTP request's lifetime.
  ...
  if (res.status === 202 && data.monitorUrl && options.detach !== true) {
    if (!isQuiet(options) && !isJson(options)) {
      ui.success(`Accepted ${data.agentId as string}; following durable run (Ctrl-C only detaches the client).`);
    }
    const final = await collectSpawn(String(data.monitorUrl));
    printSpawnResult(final, options);
    return;
  }
...
export async function handleSpawned(
  ...
  if (agentId) {
    ...
    if (options.wait === true || options.follow === true) {
      const final = await collectSpawn(monitorUrl);
      printSpawnResult(final, options);
      return;
    }
    ...
    if (!one.ok) {
      ui.error((data.error as string) || `Failed to inspect ${agentId}`);
      process.exit(1);
    }
    printSpawnResult(data, options);
    return;
