...
function gateWrapperScript(): string {
  return [
    ...
    '      # diagnostics after shell redirections are restored. -o isolates only',
    '      # timing data while hook stderr still reaches the host. Input stays on stdin.',
    '      printf "%s" "$pd_input" | "$pd_time_bin" -p -o "$pd_time_file" "$pd_real_hook" "$@"',
    '      pd_exit=$?',
    '    else',
