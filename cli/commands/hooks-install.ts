...
function gateWrapperScript(): string {
  return [
    ...
    'fi',
    '',
    '# (a) exact daemon generation ready? The heartbeat starts before the',
    '# database-integrity boot gate, so liveness alone must never delegate hooks.',
    'ready_file="$PD_HOME/daemon.ready"',
    'pid_file="$PD_HOME/daemon.pid"',
    ...
    '# (b) daemon alive? Use the Bosun heartbeat; process probes are sandbox-denied.',
