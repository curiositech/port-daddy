 ...
 * spawns no `pd` process. It allows/no-ops (exit 0) unless the daemon is alive
 * and the cwd is inside a pd-enabled project, then execs the real tentacle with
 * stdin/argv intact.
 */
function gateWrapperScript(): string {
  return [
    ...
    '# (a) daemon alive? fast fs-only check (no `pd` spawn on the tool hot path).',
    'pid=$(cat "$PD_HOME/daemon.pid" 2>/dev/null) || exit 0',
...
export function uninstallTarget(
  ...
  try {
    if (target.format === 'codex-toml') {
      writeFileSync(configPath, stripCodexBlock(readFileSync(configPath, 'utf-8')));
