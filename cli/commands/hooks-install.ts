...
function gateWrapperScript(): string {
  return [
    ...
    '      # The nested shell is an interface requirement: dash emits reserved-word',
    '      # `time` diagnostics after group redirections have been restored. Keep',
    '      # buffered hook input on stdin, never in argv.',
    '      printf "%s" "$pd_input" | /bin/sh -c \'time -p "$@" 2>&3\' pd-hook-runtime "$pd_real_hook" "$@" 3>&2 2> "$pd_time_file"',
    '      pd_exit=$?',
    '    else',
    '      /bin/sh -c \'time -p "$@" 2>&3\' pd-hook-runtime "$pd_real_hook" "$@" 3>&2 2> "$pd_time_file"',
    '      pd_exit=$?',
    '    fi',
    '    trap - HUP INT TERM',
    '    pd_health_duration_read',
