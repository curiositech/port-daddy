...
export async function handleDaemon(action: string, options: Record<string, unknown> = {}): Promise<void> {
  ...
  switch (action) {
    ...
    // REMOVED: install-bosun command. The Homebrew service (homebrew.mxcl.port-daddy)
    // is the single production daemon supervisor. Legacy com.portdaddy.bosun artifacts
    // can be cleaned up via `pd doctor --fix` or manual unload.
