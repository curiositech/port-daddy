...
/**
 * Port Daddy - Semantic Port Management Service (Fastify)
 *
 * Phase 4A: Fastify migration. Same API surface as Express server.ts,
 * same module initialization, same lifecycle — different HTTP framework.
 *
 * During transition, this file coexists with server.ts. Once all routes
 * are converted and tests pass, this becomes the new server.ts.
 */
...
function calculateCodeHash(): string {
  ...
  const libFiles: string[] = existsSync(libDir)
    ...
    : [];
  const filesToHash: string[] = ['server-fastify.ts', ...libFiles];
