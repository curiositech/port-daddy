...
// RELAY sink for cost/token/model telemetry (fed from each ship's ShipMetrics;
// written to the relay's fleet_runs row at run end). Second sink alongside the
// daemon emitCloudTelemetry path above — see usage.ts.
...
export async function executeFleet(job: FleetRunJob, env: ExecutorEnv): Promise<void> {
  ...
  for (const ship of cloudShips) {
    ...
    results.push(result);
    // Daemon sink: per-ship cost/failure event (FleetBar / /metrics/cost).
    await emitShipTelemetry(env, job, prCtx, ship, result, metrics, checkRunId, shipStartMs);
    // Relay sink: fold this ship's token totals into the run meter so the
    // fleet_runs row carries cost/tokens/model too (pd-console / /v1/fleet/*).
    usage.add(ship.cfModel, metrics.inputTokens, metrics.outputTokens);
  }
