...
describe('getInstallationTokenCached', () => {
  it('mints once, then serves a cache hit without a second mint', async () => {
    ...
    const t1 = await getInstallationTokenCached('app-1', PEM, 42, kv, async () => {});
    expect(t1).toBe('tok-1');
    expect(state.tokenMints).toBe(1);

    // Second call for the same installation must hit KV, not GitHub.
    const t2 = await getInstallationTokenCached('app-1', PEM, 42, kv, async () => {});
  ...
  it('forceRefresh bypasses the cache and re-mints', async () => {
    const kv = memoryKV();
    await getInstallationTokenCached('app-1', PEM, 42, kv, async () => {});
    expect(state.tokenMints).toBe(1);

    const t = await getInstallationTokenCached('app-1', PEM, 42, kv, async () => {}, true);
    expect(t).toBe('tok-2');
