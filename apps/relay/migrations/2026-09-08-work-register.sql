...
--
--   The REGISTER says who is on it right now. That is these two tables, and
...
-- One row per (operator, repo, slug). The primary key is why two agents cannot
-- both hold one slug: the second INSERT ... ON CONFLICT sees the first.
CREATE TABLE IF NOT EXISTS work_claims (
  ...
  PRIMARY KEY (user_id, repo_full_name, slug)
);
...
CREATE INDEX IF NOT EXISTS idx_work_claims_board
  ON work_claims(user_id, repo_full_name, state, updated_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_work_claims_holder
  ON work_claims(user_id, repo_full_name, agent, heartbeat_at);
...
CREATE INDEX IF NOT EXISTS idx_work_notes_thread
  ON work_notes(user_id, repo_full_name, slug, at DESC);
...
-- snapshot in the repo, read through the signed-in operator's own GitHub
-- token, and every read here is stamped with the ref it came from and the
-- moment it was taken. A stale cache is shown as stale rather than served as
-- current -- the failure this whole design exists to avoid is a projection
-- that reads as truth after its source moved.
