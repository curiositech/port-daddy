-- STAGING BASELINE BACKFILL (ADR-0119)
--
-- The staging database was created after Relay v0 and received only the dated
-- migrations that existed at the time. It never received schema.sql, so the
-- first 2026-08-09 migration failed on the missing identities table and every
-- later migration remained unapplied. This idempotent snapshot is the exact
-- pre-2026-08-09 baseline (git 9e1036a92^), ordered before those migrations.
-- It is safe on the already-bootstrapped production database because every
-- table/index is IF NOT EXISTS and the sole seed is INSERT OR IGNORE.
--
-- Do not update this snapshot as schema.sql evolves. New changes belong in a
-- later forward-only migration; CI executes the full chain against an empty DB.
