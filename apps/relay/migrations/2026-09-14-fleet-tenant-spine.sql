...
-- A GitHub App installation is an account-scoped billing and credential
-- boundary. It therefore belongs to exactly one Port Daddy tenant. This makes
-- the installation-keyed entitlement and reservation tables tenant-safe.
CREATE TABLE fleet_tenant_installations (
...
CREATE INDEX fleet_tenant_installations_account_idx
...
CREATE TRIGGER fleet_tenant_installations_immutable_ids
BEFORE UPDATE OF installation_id, tenant_account_id, github_account_id
ON fleet_tenant_installations
BEGIN
