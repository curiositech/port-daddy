...
import { isFleetAdmin } from './fleet-settings-access.js';
...
export function renderAccountPage(
  user: UserRow,
  opts: { showBillingUpsell?: boolean; interruptions?: InterruptionsBanner | null; fleetAdmin?: boolean } = {},
): string {
...
export async function handleAccountPage(request: Request, env: Env): Promise<Response> {
  ...
  return htmlPage(renderAccountPage(session.user, { showBillingUpsell, interruptions, fleetAdmin: isFleetAdmin(session.user, env) }));
}
