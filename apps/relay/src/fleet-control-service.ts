...
export class FleetControlService extends WorkerEntrypoint<Env> implements FleetControlServiceContract {
  ...
  async admit(expectedRevision?: number, runId?: string) {
    if (!runId) return { status: 'unknown' as const, paused: null, revision: null, reason: 'run-id-required' };
    return fleetControlRequest(this.env.FLEET_CONTROL, '/admit', { expectedRevision, runId });
  }
}
