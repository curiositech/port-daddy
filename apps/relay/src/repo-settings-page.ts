 ...
 * (and launch) setting is the SITREP dial — `sitrep.endOfTurn` — which compels
 * agents to end every turn with the SITREP table (ideas / roadmap claims /
 * assigned work, code rows linked to roadmap items at creation).
 *
 * Honesty (repo law: no Potemkin): the ENFORCEMENT point is each clone's local
 * dial (`agent.config.json` → `sitrep.endOfTurn`, read per turn by the squid
 * tentacles and at session start by the Pilot hook). This screen is the
 * account-of-record for the operator's cross-device intent, and
 * `GET /v1/repo-settings` (pdu_ device token or session cookie) is what a
 * device polls to converge its clones. The page SAYS this — it renders the
 * exact local snippet per repo rather than pretending the server reaches into
 * checkouts.
 *
 ...
 *
 * Why closed: the value lands in a CHECK-constrained column and in injected
 * agent steering — an unknown level must collapse to null (reject the write)
...
/** The local-enforcement snippet the page prints for one repo's dial. */
function localSnippet(level: SitrepLevel): string {
  return `// agent.config.json (repo root) — the dial the squid tentacles read
{
...
function repoCard(row: RepoSettingRow): string {
  ...
  <div class="rc-body">
    <form method="post" action="/account/repos/set">
      ...
      <p class="setting-desc">Compels every agent working this repository to end each turn with the
 ...
 * runtime, matching the account-page test pattern.
 *
...
<div class="shell">
  ...
  <div class="truth">
    ...
    and each clone's squid harness enforces the dial locally from
...
/** GET /account/repos — session-gated; redirects to /login when signed out. */
 ...
 * HTML form. Session-gated; the repo must pass the same GitHub read-ACL gate
 * as /account/runs so an account can only configure repositories its GitHub
 * identity can actually read (and enumeration probes learn nothing).
 */
...
/**
 * GET /v1/repo-settings — the device-facing JSON read path. Auth: a pdu_
 * device token (from `pd account login`) or the browser session cookie —
 * the same dual gate as /auth/whoami. Optional `?repo=owner/name` narrows to
 * one repository. This is what a daemon polls to converge its clones' local
 * dials with the account record.
 */
