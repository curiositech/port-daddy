...
-- (apps/fleet-executor/src/review-coverage.ts; migration
-- 2026-08-05-fleet-review-coverage.sql). Storage foundation only: nothing yet
-- writes to these tables from a route, `merge_group`, or AI execution.
--
-- fleet_review_coverage       — one immutable row per (subject, sha) that
--                                reached a SHIP verdict. `predecessor_sha` is
--                                the exact SHA `sha` was reviewed against, so
--                                rows form a hash chain proving every commit
--                                on the subject was covered — no gaps, no
--                                forks. Rows are NEVER updated or deleted; a
--                                bad submission is rejected by the app layer
--                                (review-coverage.ts: reject non-SHIP,
--                                malformed SHA, self-loop, predecessor
--                                gap/mismatch, conflicting replay) before it
--                                ever reaches SQL. Only an exact re-submit of
--                                an already-recorded (sha, predecessor_sha,
--                                kind) triple is accepted, as a no-op
--                                idempotent replay.
-- fleet_review_coverage_heads — one row per subject caching the current
--                                chain tip (the last accepted sha), so
--                                continuity checks are an O(1) point lookup
