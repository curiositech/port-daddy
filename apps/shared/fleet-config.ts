 ...
 *
 * Consumes the document parsed by each Worker's own YAML dependency and extracts
 * ships whose trigger matches
...
function triggerMatches(trigger: unknown, requested: string): boolean {
  ...
  return triggers.some(
    t => typeof t === 'string' && (requested === '*' || t === requested || t === `${reqEvent}:*`),
  );
}
 ...
 */
export function fleetSquidEventsFromDocument(doc: unknown): boolean {
 ...
 *
 * @param doc Parsed pd-fleet.yml from the trusted default branch.
 * @returns True only when the tenant explicitly opted in with `xo: true`.
 */
export function fleetXoFromDocument(doc: unknown): boolean {
 ...
 */
export function fleetMediatorFromDocument(doc: unknown): FleetMediatorConfig {
